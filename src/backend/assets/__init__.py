...
from typing import Dict, Any, Union, Tuple, Optional  # >=3.10 - Type hints for factory functions and compatibility validation
...
from .default_colormap import (
    ...
    validate_color_scheme,  # Color scheme validation utility
...
def get_default_assets(render_mode: str, enable_optimization: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        color_scheme = create_default_scheme(
            scheme_type=PredefinedScheme.STANDARD,
        ...
        rgb_template_config = TemplateConfig(
            quality_level=TemplateQuality.STANDARD if not enable_optimization else TemplateQuality.FAST,
        ...
        _logger.info(f"Successfully created default assets for {render_mode} mode with optimization: {enable_optimization}")
...
def validate_asset_compatibility(
    ...
    if not isinstance(color_scheme, ColorScheme):
        raise TypeError(f"Expected ColorScheme instance, got {type(color_scheme).__name__}")
    ...
    try:
        ...
        try:
            ...
            if color_scheme_validation:
                ...
                compatibility_report['errors'].append("Color scheme failed internal validation")
                is_compatible = False
        except Exception as e:
        ...
        try:
            if hasattr(render_template, 'validate_performance'):
                ...
                if template_validation:
                    ...
                    compatibility_report['warnings'].append("Template performance validation failed")
            else:
        ...
        if template_type == 'RGBTemplate':
            ...
            compatibility_report['validation_details']['render_mode_match'] = 'rgb_optimized'
            ...
            compatibility_report['validation_details']['render_mode_match'] = 'human_optimized'
            
            # Check if color scheme supports matplotlib rendering
            if hasattr(color_scheme, 'supports_matplotlib') and not color_scheme.supports_matplotlib():
        ...
        try:
            ...
            required_color_methods = ['get_concentration_colormap', 'apply_to_rgb_array']
            ...
            compatibility_report['warnings'].append(f"Sample rendering test failed: {e}")
