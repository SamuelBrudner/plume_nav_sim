...
from typing import Optional, Union, Tuple, Dict, Any, List  # >=3.10 - Type hints and annotations
from dataclasses import dataclass, field  # >=3.10 - Structured configuration with validation
...
PREDEFINED_SCHEMES = ['standard', 'high_contrast', 'colorblind_friendly', 'minimal', 'debug']

# Module exports for external access
__all__ = [
    'ColorScheme', 'PredefinedScheme', 'DEFAULT_COLORMAP',
    'create_default_scheme', 'create_color_scheme', 'validate_color_scheme',
    'normalize_concentration_to_rgb', 'apply_agent_marker', 'apply_source_marker',
    'get_matplotlib_colormap', 'optimize_for_performance'
]
...
class PredefinedScheme(Enum):
    ...
    def is_accessibility_enhanced(self) -> bool:
        ...
        """
        return self in [PredefinedScheme.HIGH_CONTRAST, PredefinedScheme.COLORBLIND_FRIENDLY]
...
class ColorScheme:
    ...
    _cached_colormap: Optional[matplotlib.colors.Colormap] = field(default=None, init=False)
    ...
    def __post_init__(self):
        ...
        try:
            ...
            warnings.warn(f"Colormap '{self.concentration_colormap}' not found, falling back to 'gray'")
    ...
    def _validate_rgb_color(self, color: Tuple[int, int, int], name: str):
        ...
        for i, value in enumerate(color):
            ...
            if not (COLOR_VALUE_MIN <= value <= COLOR_VALUE_MAX):
                raise ValueError(f"{name}[{i}] must be in range [{COLOR_VALUE_MIN}, {COLOR_VALUE_MAX}], got {value}")
    ...
    def get_agent_color(self, format_type: str = 'rgb') -> Union[Tuple[int, int, int], np.ndarray, str]:
        """
        ...
        if format_type == 'rgb':
            ...
            raise ValueError(f"Unsupported format_type '{format_type}'. Use 'rgb', 'normalized', or 'hex'")
    
    def get_source_color(self, format_type: str = 'rgb') -> Union[Tuple[int, int, int], np.ndarray, str]:
        """
    ...
    def get_concentration_colormap(self, use_cache: bool = True, 
                                  normalization_range: Optional[Tuple[float, float]] = None) -> matplotlib.colors.Colormap:
        """
        ...
        try:
            ...
            warnings.warn(f"Colormap '{self.concentration_colormap}' not available, using 'gray'")
        ...
        norm = matplotlib.colors.Normalize(vmin=normalization_range[0], vmax=normalization_range[1])
    ...
    def apply_to_rgb_array(self, concentration_field: np.ndarray, 
        ...
        if concentration_field.ndim != 2:
            raise ValueError(f"concentration_field must be 2D, got {concentration_field.ndim}D")
        
        # Clamp concentration values to valid range
        concentration_field = np.clip(concentration_field, CONCENTRATION_RANGE[0], CONCENTRATION_RANGE[1])
        ...
        rgb_array = apply_source_marker(rgb_array, source_position, 
                                       self.source_color, self.source_marker_size)
        
        # Apply agent marker (3×3 square pattern)
        rgb_array = apply_agent_marker(rgb_array, agent_position, 
                                      self.agent_color, self.agent_marker_size)
        
        return rgb_array
    
    def configure_matplotlib_axes(self, axes: matplotlib.axes.Axes, 
                                 concentration_field: np.ndarray) -> matplotlib.image.AxesImage:
        """
        ...
        im = axes.imshow(concentration_field, 
                        cmap=colormap, 
    ...
    def optimize_for_render_mode(self, render_mode: str, optimization_options: Dict[str, Any] = None):
        """
    ...
    def validate(self, check_accessibility: bool = False, 
                check_performance: bool = False, strict_mode: bool = False) -> bool:
        """
        ...
        try:
            ...
            validation_errors.append(f"Colormap '{self.concentration_colormap}' is not available")
        ...
        if check_accessibility:
            ...
            if agent_contrast < min_contrast:
                validation_errors.append(f"Agent-background contrast {agent_contrast:.2f} below threshold")
            if source_contrast < min_contrast:
                validation_errors.append(f"Source-background contrast {source_contrast:.2f} below threshold")
    ...
    def to_dict(self, include_performance_info: bool = False, 
               include_optimization_state: bool = False) -> Dict[str, Any]:
        """
    ...
    def clone(self, color_overrides: Optional[Dict[str, Any]] = None, 
             preserve_optimizations: bool = True) -> 'ColorScheme':
        """
...
def create_default_scheme(render_mode: Optional[str] = None, 
                         enable_accessibility: bool = False,
                         cache_colormap: bool = True) -> ColorScheme:
    """
...
def create_color_scheme(color_config: Dict[str, Any], 
                       scheme_type: Optional[str] = None,
                       render_mode: Optional[str] = None, 
                       validate_accessibility: bool = False) -> ColorScheme:
    """
    ...
    required_keys = {'agent_color', 'source_color', 'background_color', 'concentration_colormap'}
    ...
    if scheme_type and scheme_type in PREDEFINED_SCHEMES:
        try:
            ...
            warnings.warn(f"Unknown scheme_type '{scheme_type}', using custom config only")
...
def validate_color_scheme(color_scheme: ColorScheme, 
    ...
    if validation_report['warnings']:
        validation_report['recommendations'].append("Consider optimizing for performance mode")
...
def normalize_concentration_to_rgb(concentration_field: np.ndarray, 
    ...
    normalized_field = (concentration_field - value_range[0]) / (value_range[1] - value_range[0])
...
def apply_agent_marker(rgb_array: np.ndarray, 
                      agent_position: Tuple[int, int],
                      marker_color: Tuple[int, int, int] = DEFAULT_AGENT_COLOR,
                      marker_size: Optional[Tuple[int, int]] = None) -> np.ndarray:
    """
    ...
    """
    # Validate RGB array
    if not isinstance(rgb_array, np.ndarray) or rgb_array.ndim != 3 or rgb_array.shape[2] != RGB_CHANNELS:
    ...
    if not (0 <= x < width and 0 <= y < height):
        raise ValueError(f"agent_position {agent_position} is outside array bounds ({width}, {height})")
...
def apply_source_marker(rgb_array: np.ndarray, 
    ...
    x, y = source_position
    if not (0 <= x < width and 0 <= y < height):
        raise ValueError(f"source_position {source_position} is outside array bounds ({width}, {height})")
...
def get_matplotlib_colormap(colormap_name: str, 
                           ...
                           use_cache: bool = True, 
                           validate_backend: bool = False) -> matplotlib.colors.Colormap:
    """
    ...
    if validate_backend:
        try:
            ...
            if backend == 'Agg':
                warnings.warn("Using headless Agg backend, interactive features unavailable")
        except ImportError:
...
def optimize_for_performance(color_scheme: ColorScheme, 
                           render_mode: str,
                           optimization_options: Dict[str, Any] = None) -> ColorScheme:
    """
