...
from typing import Optional, Union, Dict, Any, Tuple, List  # >=3.10 - Type hints for template methods, configuration parameters, and factory functions with enhanced code clarity and IDE support
from dataclasses import dataclass, field  # >=3.10 - Template configuration data structures with automatic validation, immutability, and serialization support
from abc import ABC, abstractmethod  # >=3.10 - Abstract base classes for template interface definitions and ensuring implementation compliance
from enum import Enum  # >=3.10 - Template type enumeration and rendering quality level definitions with type safety
...
from .default_colormap import (
    ColorScheme, PredefinedScheme, create_default_scheme, 
    apply_agent_marker, apply_source_marker, normalize_concentration_to_rgb
)
...
from ..plume_nav_sim.core.constants import (
    PERFORMANCE_TARGET_RGB_RENDER_MS, PERFORMANCE_TARGET_HUMAN_RENDER_MS,
    RGB_DTYPE, MATPLOTLIB_DEFAULT_FIGSIZE, BACKEND_PRIORITY_LIST
)
...
class TemplateConfig:
    ...
    performance_options: Dict[str, Any] = field(default_factory=lambda: {
        'enable_caching': True,
        'vectorized_operations': True,
        'memory_optimization': True
    })
    backend_preferences: Dict[str, Any] = field(default_factory=lambda: {
    ...
    def validate(self, strict_mode: bool = False) -> Tuple[bool, List[str]]:
        ...
        if not (8 <= self.grid_size.width <= 2048):
            validation_errors.append(f"Grid width {self.grid_size.width} outside reasonable range [8, 2048]")
        
        if not (8 <= self.grid_size.height <= 2048):
            validation_errors.append(f"Grid height {self.grid_size.height} outside reasonable range [8, 2048]")
        ...
        if self.performance_options.get('enable_caching', True) and not self.caching_enabled:
            validation_errors.append("Performance caching enabled but template caching disabled")
        
        # Apply strict validation if requested
        if strict_mode:
            # Check memory requirements
            memory_estimate = (self.grid_size.width * self.grid_size.height * 4) / (1024 * 1024)  # MB
            if memory_estimate > 100:
                validation_errors.append(f"Estimated memory usage {memory_estimate:.1f}MB exceeds strict limit")
            ...
            try:
                ...
                if preferred_backend not in available_backends:
                    validation_errors.append(f"Preferred backend '{preferred_backend}' not available")
            except Exception:
    ...
    def optimize_for_system(self, system_info: Dict[str, Any]) -> 'TemplateConfig':
        ...
        if memory_gb > 8:
            optimized.performance_options['memory_optimization'] = False  # Allow higher memory usage
...
class BaseRenderTemplate(ABC):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not is_valid:
                raise RuntimeError(f"Template configuration validation failed: {errors}")
            ...
            try:
                test_result = self._execute_render(test_concentration, test_agent_pos, test_source_pos)
                if test_result is None and self.config.quality_level != TemplateQuality.PREMIUM:
                    warnings.warn("Template functionality test returned None - may indicate rendering issues")
            except Exception as e:
    ...
    def render(
        ...
        try:
            ...
            result = self._execute_render(concentration_field, agent_position, source_position)
            ...
            if hasattr(self, '_fallback_render'):
                try:
                    return self._fallback_render(concentration_field, agent_position, source_position)
                except Exception:
                    pass
    ...
    def optimize(self, optimization_targets: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if current_avg_time > target_time:
            ...
            if not self.config.caching_enabled:
                ...
                self._setup_caching()
                optimization_report['optimizations_applied'].append("Enabled template caching")
            ...
            optimization_report['optimizations_applied'].append("Enabled performance optimizations")
        
        # Memory optimization targets
        memory_target_mb = optimization_targets.get('memory_limit_mb', 50)
        estimated_memory = (self.config.grid_size.width * self.config.grid_size.height * 4) / (1024 * 1024)
    ...
    def get_performance_metrics(self, include_cache_stats: bool = True) -> Dict[str, Any]:
        """
    ...
    def validate_performance(self, test_scenarios: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        """
...
class RGBTemplate(BaseRenderTemplate):
    ...
    def __init__(self, config: TemplateConfig):
        ...
        self._optimization_enabled = config.performance_options.get('vectorized_operations', True)
        self._render_cache = {}
    ...
    def _initialize_template_resources(self) -> None:
        ...
        self._agent_marker_pattern = np.full((*agent_size, 3), agent_color, dtype=RGB_DTYPE)
    ...
    def _apply_concentration_background(self, rgb_array: np.ndarray, concentration_field: np.ndarray) -> None:
        """
    ...
    def _apply_agent_marker(self, rgb_array: np.ndarray, agent_position: Coordinates) -> None:
        """
        ...
        try:
            ...
            if 0 <= agent_y < rgb_array.shape[0] and 0 <= agent_x < rgb_array.shape[1]:
                rgb_array[agent_y, agent_x] = self.config.marker_config.get('agent_color', [255, 0, 0])
    
    def _apply_source_marker(self, rgb_array: np.ndarray, source_position: Coordinates) -> None:
        """
        ...
        try:
            ...
            source_color = self.config.marker_config.get('source_color', [255, 255, 255])
            if 0 <= source_y < rgb_array.shape[0] and 0 <= source_x < rgb_array.shape[1]:
    ...
    def validate_performance(self, test_scenarios: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        if successful_tests:
            ...
            performance_analysis['performance_data'] = {
                ...
                'success_rate': len(successful_tests) / len(test_results),
                'target_compliance_rate': sum(1 for r in successful_tests if r['meets_target']) / len(successful_tests)
            }
            ...
            if not performance_analysis['meets_target']:
                ...
                large_grid_tests = [r for r in successful_tests if '128x128' in r['scenario'] or '256x256' in r['scenario']]
                if large_grid_tests and np.mean([r['avg_render_time_ms'] for r in large_grid_tests]) > rgb_target_ms:
                    recommendations.append("Consider grid size optimization for large environments")
...
class MatplotlibTemplate(BaseRenderTemplate):
    ...
    def __init__(self, config: TemplateConfig):
        ...
        self._animation_interval = config.backend_preferences.get('animation_interval', 0.001)
    ...
    def _initialize_template_resources(self) -> None:
        ...
        figsize = self.config.backend_preferences.get('figsize', MATPLOTLIB_DEFAULT_FIGSIZE)
        ...
        self.agent_marker = self.axes.scatter([], [], c='red', s=100, marker='s', zorder=3)
        self.source_marker = self.axes.scatter([], [], c='white', s=150, marker='+', 
                                             linewidths=3, edgecolors='black', zorder=2)
        
        # Test matplotlib functionality
        try:
            test_data = np.zeros((8, 8))
            test_image = self.axes.imshow(test_data, cmap='gray', origin='lower', 
                                        extent=[0, 8, 0, 8], alpha=0.8, zorder=1)
        ...
        try:
            ...
            if "backend" in str(e).lower() or "display" in str(e).lower():
                ...
                if not fallback_success:
                    raise RuntimeError(f"Matplotlib rendering failed with all backends: {e}")
                else:
                    # Retry render with new backend
                    self._execute_render(concentration_field, agent_position, source_position)
            else:
                raise
    ...
    def _handle_backend_error(self, error: Exception, failed_backend: str) -> bool:
        ...
        remaining_backends = [
            b for b in self._backend_fallback_chain 
    ...
    def validate_performance(self, test_scenarios: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        performance_analysis['backend_analysis']['interactive'] = self.backend_interactive
                ...
                if render_times:
                    ...
                    if not meets_target:
                        ...
                        performance_analysis['target_violations'].append({
                            ...
                            'excess_time_ms': (avg_time * 1000) - matplotlib_target_ms,
                            'backend': self.active_backend
                        })
        ...
        if successful_tests:
            ...
            if not performance_analysis['meets_target']:
                ...
                if self.active_backend in ['TkAgg', 'Qt5Agg'] and not self.backend_interactive:
                    recommendations.append("Switch to Agg backend for non-interactive use")
                
                if np.mean(all_times) > matplotlib_target_ms * 2:
                    recommendations.append("Consider fallback to RGB array mode for critical performance")
...
def create_rgb_template(
    ...
    if memory_estimate_mb > 500:  # Reasonable upper limit
        warnings.warn(f"Large grid size may require {memory_estimate_mb:.1f}MB of memory")
...
def create_matplotlib_template(
    ...
    performance_options = {
        'enable_caching': merged_options.get('caching_enabled', False),  # Less useful for matplotlib
        'vectorized_operations': True,
    ...
    if not template.initialize():
        warnings.warn("Matplotlib template initialization failed - attempting fallback configuration")
        ...
        template = MatplotlibTemplate(config)
        if not template.initialize():
            raise RuntimeError("Matplotlib template initialization failed with all backends")
...
def create_custom_template(
    ...
    if template_type not in supported_types:
        raise ValueError(f"Template type '{template_type}' not supported. Use one of: {supported_types}")
    ...
    if custom_parameters:
        ...
        if template_type == 'rgb':
            rgb_params = ['agent_marker_size', 'source_marker_size', 'performance_optimized']
            invalid_params = [p for p in custom_parameters.keys() if p not in rgb_params and not p.startswith('custom_')]
            if invalid_params:
                warnings.warn(f"Custom parameters {invalid_params} may not apply to RGB templates")
        
        elif template_type == 'matplotlib':
            mpl_params = ['figsize', 'backend_fallback', 'interactive_updates', 'animation_interval']
            invalid_params = [p for p in custom_parameters.keys() if p not in mpl_params and not p.startswith('custom_')]
            if invalid_params:
                warnings.warn(f"Custom parameters {invalid_params} may not apply to matplotlib templates")
    ...
    if template_type == 'rgb':
        ...
        raise NotImplementedError("Hybrid templates not implemented in proof-of-life version")
    else:
    ...
    if validate_performance:
        ...
        if not meets_targets:
            ...
            if recommendations:
                warnings.warn(f"Performance optimization recommendations: {recommendations}")
...
def register_template(
    ...
    if template_name in TEMPLATE_REGISTRY and not allow_override:
        warnings.warn(f"Template '{template_name}' already registered. Use allow_override=True to replace.")
        return False
    
    # Validate template instance completeness and performance
    if not isinstance(template, (RGBTemplate, MatplotlibTemplate)):
        raise TypeError("Template must be an instance of RGBTemplate or MatplotlibTemplate")
    ...
    registry_entry = {
        'template': template,
        'metadata': {
            'description': metadata.get('description', f'Custom {type(template).__name__}'),
    ...
    for template_name, entry in TEMPLATE_REGISTRY.items():
        ...
        if template_type_filter:
            ...
            if template_type_filter not in valid_types:
                warnings.warn(f"Invalid template type filter '{template_type_filter}'. Valid options: {valid_types}")
                continue
            
            if template_type_filter == 'rgb' and not isinstance(template, RGBTemplate):
                continue
            elif template_type_filter == 'matplotlib' and not isinstance(template, MatplotlibTemplate):
                continue
            elif template_type_filter == 'custom' and template_name not in ['rgb_default', 'matplotlib_default']:
        ...
        if quality_filter:
            ...
            if quality_filter not in valid_qualities:
                warnings.warn(f"Invalid quality filter '{quality_filter}'. Valid options: {valid_qualities}")
                continue
        ...
        if include_performance_data:
            ...
            template_info['performance_data'] = {
                'average_render_time_ms': performance_metrics.get('average_render_time', 0.0) * 1000,
            ...
            if cache_stats:
                template_info['performance_data']['cache_hit_rate'] = cache_stats.get('hit_rate', 0.0)
...
def _generate_template_recommendations(templates: Dict[str, Any]) -> List[str]:
    ...
    if not templates:
        recommendations.append("No templates available - consider creating templates with factory functions")
        return recommendations
    
    # Recommend based on template types available
    rgb_templates = [name for name, info in templates.items() if info['template_type'] == 'RGBTemplate']
    matplotlib_templates = [name for name, info in templates.items() if info['template_type'] == 'MatplotlibTemplate']
    
    if rgb_templates:
        most_used_rgb = max(rgb_templates, key=lambda x: templates[x]['usage_count'])
        recommendations.append(f"For programmatic rendering, consider using '{most_used_rgb}' (most popular RGB template)")
    
    if matplotlib_templates:
        most_used_mpl = max(matplotlib_templates, key=lambda x: templates[x]['usage_count'])
        recommendations.append(f"For interactive visualization, consider using '{most_used_mpl}' (most popular matplotlib template)")
    
    # Quality level recommendations
    quality_levels = [info['quality_level'] for info in templates.values()]
    if 'ultra_fast' in quality_levels:
        recommendations.append("ULTRA_FAST quality templates available for maximum performance requirements")
    
    if 'premium' in quality_levels:
        recommendations.append("PREMIUM quality templates available for highest visual quality")
...
def optimize_template(
    ...
    current_render_time = current_metrics.get('average_render_time', 0.0) * 1000  # Convert to ms
    ...
    if optimizations_needed:
        ...
        if not optimized_config.performance_options.get('vectorized_operations', True):
            ...
            optimization_report['optimizations_applied'].append("Enabled vectorized operations")
        ...
        if not optimized_config.performance_options.get('memory_optimization', False):
            ...
            optimization_report['optimizations_applied'].append("Enabled memory optimization")
    
    # Memory optimization
    estimated_memory = (optimized_config.grid_size.width * optimized_config.grid_size.height * 3) / (1024 * 1024)
    if estimated_memory > target_memory_mb:
        # Suggest grid size reduction
        reduction_factor = target_memory_mb / estimated_memory
        suggested_width = int(optimized_config.grid_size.width * np.sqrt(reduction_factor))
        suggested_height = int(optimized_config.grid_size.height * np.sqrt(reduction_factor))
    ...
    try:
        ...
        if current_render_time > 0:
            improvement_percent = ((current_render_time/1000 - optimized_render_time) / (current_render_time/1000)) * 100
            optimization_report['performance_improvements']['render_time_improvement_percent'] = improvement_percent
            optimization_report['performance_improvements']['new_render_time_ms'] = optimized_render_time * 1000
    ...
    if not optimization_report['optimizations_applied']:
        optimization_report['recommendations'].append("Template already well-optimized for current targets")
...
def validate_template_performance(
    ...
    for scenario_name, scenario_config in test_scenarios.items():
        ...
        grid_sizes = scenario_config.get('grid_sizes', [(64, 64), (128, 128)])
        iterations = scenario_config.get('iterations', 10 if isinstance(template, RGBTemplate) else 5)
            ...
            for iteration in range(iterations):
                ...
                try:
                    ...
                    if strict_validation:
                        performance_report['test_results'][f"{scenario_name}_{grid_size[0]}x{grid_size[1]}"] = {
                            'success': False,
                            'error': str(e),
                            'iteration': iteration
                        }
            ...
            if successful_times:
                ...
                if memory_usage:
                    test_result['memory_analysis'] = {
                        ...
                        'memory_efficient': np.mean(memory_usage) < 10  # Reasonable threshold
                    }
        ...
        successful_tests = [t for t in scenario_results['tests'] if t.get('success_rate', 0) > 0.5]
        if successful_tests:
            scenario_results['summary'] = {
                'overall_success_rate': np.mean([t['success_rate'] for t in successful_tests]),
                'average_time_ms': np.mean([t['average_time_ms'] for t in successful_tests]),
                'target_compliance_rate': sum(1 for t in successful_tests if t['meets_target']) / len(successful_tests)
            }
    ...
    if all_test_results:
        successful_results = [r for r in all_test_results if r.get('success_rate', 0) > 0.8]
        
        if successful_results:
            performance_report['performance_analysis'].update({
                'overall_average_time_ms': np.mean([r['average_time_ms'] for r in successful_results]),
                'overall_max_time_ms': np.max([r['max_time_ms'] for r in successful_results]),
                'overall_success_rate': np.mean([r['success_rate'] for r in successful_results]),
                'target_compliance_rate': sum(1 for r in successful_results if r['meets_target']) / len(successful_results),
                'performance_consistency': {
                    'std_deviation_ms': np.std([r['average_time_ms'] for r in successful_results]),
                    'coefficient_of_variation': np.std([r['average_time_ms'] for r in successful_results]) / 
    ...
    if strict_validation:
        ...
        if performance_report['performance_analysis'].get('overall_success_rate', 1.0) < 1.0:
            strict_failures.append("Not all test iterations succeeded")
        
        # Check performance consistency
        consistency = performance_report['performance_analysis'].get('performance_consistency', {})
        ...
        large_grid_tests = [r for r in all_test_results if '128x128' in r['grid_size'] and 'memory_analysis' in r]
        if large_grid_tests:
            avg_memory = np.mean([r['memory_analysis']['average_memory_mb'] for r in large_grid_tests])
            if avg_memory > 25:  # Strict memory limit
                strict_failures.append(f"High memory usage for large grids: {avg_memory:.1f}MB")
    ...
    avg_time = performance_report['performance_analysis'].get('overall_average_time_ms', 0)
    if avg_time > target_ms * 1.5:
        recommendations.append("Significant performance optimization needed - consider ULTRA_FAST quality level")
    ...
    if isinstance(template, RGBTemplate):
        ...
        if grid_area > 128 * 128:
            recommendations.append("Large grid sizes may impact performance - consider optimization")
...
__all__ = [
    ...
    'BaseRenderTemplate', 'RGBTemplate', 'MatplotlibTemplate',
    
    # Configuration and quality enums
    'TemplateConfig', 'TemplateQuality',
    
    # Factory functions
    'create_rgb_template', 'create_matplotlib_template', 'create_custom_template',
    
    # Registry and management functions
    'register_template', 'get_template_registry', 'optimize_template', 
