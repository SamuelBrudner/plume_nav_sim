...
try:
    from .default_config import get_complete_default_config, CompleteConfig
    ...
    class CompleteConfig:
        ...
        def __init__(self, **kwargs):
            ...
            self.source_location = kwargs.get('source_location', DEFAULT_SOURCE_LOCATION)
...
class PresetMetadata:
    ...
    def __post_init__(self):
        ...
        if not self.performance_characteristics:
            object.__setattr__(self, 'performance_characteristics', {
    ...
    def matches_filter(self, filter_criteria: Dict[str, Any]) -> bool:
        ...
        if 'use_cases' in filter_criteria:
            ...
            if not any(uc.lower() in [existing.lower() for existing in self.use_cases] 
    ...
    def estimate_resource_requirements(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'timing_estimates': {
                'initialization_ms': self.performance_characteristics.get('reset_time_ms', 10),
                'step_latency_ms': self.performance_characteristics.get('step_latency_ms', 1),
                'render_time_ms': self.performance_characteristics.get('rendering_time_ms', 5)
            }
        }
...
class ConfigurationRegistry:
    ...
    def _register_builtin_presets(self):
        ...
        standard_metadata = PresetMetadata(
            ...
            use_cases=["algorithm development", "performance comparison", "research studies"],
            technical_specs={
                'grid_size': DEFAULT_GRID_SIZE,
                'source_location': DEFAULT_SOURCE_LOCATION,
                'max_steps': DEFAULT_MAX_STEPS,
        ...
        benchmark_metadata = PresetMetadata(
            ...
            use_cases=["performance benchmarking", "scalability testing", "system validation"],
            technical_specs={
                'grid_size': (256, 256),
                'source_location': (128, 128),
                'max_steps': 2000,
    ...
    def register_preset(self, preset_name: str, config: CompleteConfig, 
                       metadata: PresetMetadata) -> bool:
        """
        ...
        if not config.validate_all():
            raise ValueError(f"Configuration validation failed for preset '{preset_name}'")
    ...
    def get_metadata(self, preset_name: str) -> PresetMetadata:
        ...
        if preset_name not in self._metadata:
            ...
            raise KeyError(f"Metadata for preset '{preset_name}' not found. Available: {available_presets}")
    ...
    def search_presets(self, search_term: str, case_sensitive: bool = False) -> List[str]:
        """
        ...
        for preset_name, metadata in self._metadata.items():
            ...
            desc_to_search = metadata.description if case_sensitive else metadata.description.lower()
        ...
        exact_matches = [name for name in matching_presets if search_term in name.lower()]
    ...
    def validate_registry(self, strict_mode: bool = False) -> Tuple[bool, Dict[str, Any]]:
        """
...
def create_preset_config(preset_name: str, overrides: Optional[Dict[str, Any]] = None, 
    ...
    if validate and not config.validate_all():
        raise ValueError(f"Configuration validation failed for preset '{preset_name}' with overrides")
    
    return config


def create_research_scenario(scenario_name: str, grid_size: Optional[int] = None,
                           plume_complexity: Optional[float] = None,
                           research_params: Optional[Dict[str, Any]] = None) -> CompleteConfig:
    """
    ...
    if scenario_name not in research_scenarios:
        ...
        raise ValueError(f"Unknown research scenario '{scenario_name}'. Available: {available_scenarios}")
...
def create_benchmark_config(benchmark_type: str, target_grid_size: Optional[GridSize] = None,
                          performance_targets: Optional[Dict[str, Any]] = None,
                          enable_profiling: bool = False) -> CompleteConfig:
    """
    ...
    if benchmark_type not in benchmark_configs:
        ...
        raise ValueError(f"Unknown benchmark type '{benchmark_type}'. Available: {available_types}")
    ...
    if not benchmark_config.validate_all():
        raise ValueError(f"Benchmark configuration '{benchmark_type}' failed validation")
    
    return benchmark_config


def create_testing_config(test_type: str, seed: Optional[int] = None,
                         grid_size: Optional[GridSize] = None,
                         fast_mode: bool = False) -> CompleteConfig:
    """
    ...
    if test_type not in test_type_configs:
        ...
        raise ValueError(f"Unknown test type '{test_type}'. Available: {available_types}")
    ...
    if fast_mode:
        ...
        test_params['render_mode'] = RenderMode.RGB_ARRAY  # No human rendering in fast mode
...
def get_available_presets(category_filter: Optional[str] = None, 
                         include_metadata: bool = False) -> Dict[str, PresetMetadata]:
    """
    ...
    for preset_name in preset_names:
        if include_metadata:
            available_presets[preset_name] = ENVIRONMENT_REGISTRY.get_metadata(preset_name)
        else:
...
def validate_preset_name(preset_name: str, suggest_alternatives: bool = False) -> Tuple[bool, str, List[str]]:
    """
    ...
    if suggest_alternatives:
        ...
        for available_preset in available_presets:
            ...
            if preset_name.lower() in available_preset.lower() or available_preset.lower() in preset_name.lower():
        ...
        if not suggestions:
            ...
            if 'test' in preset_name.lower():
                suggestions.extend([p for p in available_presets if 'test' in p.lower()])
            elif 'bench' in preset_name.lower():
                suggestions.extend([p for p in available_presets if 'bench' in p.lower()])
            elif 'research' in preset_name.lower():
                suggestions.extend([p for p in available_presets if 'research' in p.lower()])
...
def create_custom_scenario(scenario_name: str, environment_params: Dict[str, Any],
                          plume_params: Dict[str, Any], 
                          render_params: Optional[Dict[str, Any]] = None,
                          performance_params: Optional[Dict[str, Any]] = None) -> CompleteConfig:
    """
    ...
    if environment_params:
        ...
        if 'grid_size' in environment_params:
            ...
            if isinstance(grid_size, (list, tuple)) and len(grid_size) == 2:
                ...
                raise ValueError("Grid size must be a tuple or list of two positive integers")
        ...
        if 'source_location' in environment_params:
            ...
            if isinstance(source_loc, (list, tuple)) and len(source_loc) == 2:
                ...
                raise ValueError("Source location must be a tuple or list of two coordinates")
