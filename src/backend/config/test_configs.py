...
from dataclasses import dataclass, field  # >=3.10 - Data class decorators for test configuration structures and metadata classes
...
from ..plume_nav_sim.core.types import create_grid_size, create_coordinates, RenderMode
from ..plume_nav_sim.core.constants import (
    DEFAULT_GRID_SIZE, DEFAULT_SOURCE_LOCATION, DEFAULT_PLUME_SIGMA, DEFAULT_GOAL_RADIUS,
    PERFORMANCE_TARGET_STEP_LATENCY_MS, MEMORY_LIMIT_TOTAL_MB, get_testing_constants,
    TESTING_CONSTANTS
)
from ..plume_nav_sim.utils.exceptions import ValidationError, ConfigurationError, ResourceError


# ===== MOCK IMPLEMENTATIONS FOR MISSING default_config.py =====
# These are placeholder implementations for the expected imports that don't exist yet

@dataclass
class CompleteConfig:
    """Mock implementation of CompleteConfig class expected from default_config.py.
    
    This class provides a placeholder for the complete configuration structure until
    the actual default_config.py module is implemented.
    """
    ...
    def clone_with_overrides(self, overrides: Dict[str, Any]) -> 'CompleteConfig':
        ...
        # Apply overrides to configuration parameters
        for key, value in overrides.items():
            if hasattr(new_config, key):
                setattr(new_config, key, value)
            else:
                # Store unknown overrides in advanced_options
                new_config.advanced_options[key] = value
        
        # Update metadata with override information
    ...
    def validate_all(self, strict_mode: bool = False) -> bool:
        ...
        if strict_mode:
            ...
            estimated_memory = (grid_cells * 4) / (1024 * 1024)  # float32 bytes to MB
        ...
        # Raise ConfigurationError if validation failures found
...
class EnvironmentConfig:
    """Mock implementation of EnvironmentConfig class for environment-specific parameters."""
...
class PerformanceConfig:
    """Mock implementation of PerformanceConfig class for performance targets."""
...
def get_complete_default_config() -> CompleteConfig:
    """Mock factory function for creating default complete configuration.
    
    Returns:
        CompleteConfig: Default complete configuration with standard parameters
    """
...
INTEGRATION_TEST_GRID_SIZE = (64, 64)  # Medium grid size for integration testing with realistic parameters
PERFORMANCE_TEST_GRID_SIZE = (128, 128)  # Standard grid size for performance benchmarking and validation
STRESS_TEST_GRID_SIZE = (256, 256)  # Large grid size for stress testing and scalability validation
...
TEST_EPISODE_STEPS_INTEGRATION = 200  # Medium episode length for integration testing scenarios
TEST_EPISODE_STEPS_PERFORMANCE = 1000  # Full episode length for performance benchmark testing
...
TEST_TIMEOUT_MULTIPLIER = 2.0  # Multiplier for test timeout calculations based on complexity
PERFORMANCE_TOLERANCE = 0.1  # Performance tolerance factor for benchmark validation (10%)
...
def create_unit_test_config(grid_size: Optional[Tuple[int, int]] = None, 
                           max_steps: Optional[int] = None, 
                           overrides: Optional[Dict[str, Any]] = None, 
                           validate: bool = True) -> CompleteConfig:
    ...
    config = CompleteConfig(
        ...
        step_latency_target_ms=PERFORMANCE_TARGET_STEP_LATENCY_MS * TEST_TIMEOUT_MULTIPLIER
    )
    ...
    config.advanced_options.update({
        ...
        'performance_tolerance': PERFORMANCE_TOLERANCE * 2  # Relaxed tolerance for unit tests
    })
...
def create_integration_test_config(grid_size: Optional[Tuple[int, int]] = None, 
                                  render_mode: Optional[str] = None, 
                                  overrides: Optional[Dict[str, Any]] = None, 
                                  enable_performance_monitoring: bool = False) -> CompleteConfig:
...
def create_performance_test_config(grid_size: Optional[Tuple[int, int]] = None, 
                                  performance_targets: Optional[Dict[str, float]] = None, 
                                  tolerance: Optional[float] = None, 
                                  strict_timing: bool = False) -> CompleteConfig:
    ...
    test_targets = performance_targets if performance_targets is not None else default_targets
...
def create_reproducibility_test_config(seed: Optional[int] = None, 
                                      num_test_episodes: Optional[int] = None, 
                                      deterministic_overrides: Optional[Dict[str, Any]] = None, 
                                      strict_determinism: bool = False) -> CompleteConfig:
...
def create_edge_case_test_config(edge_case_type: str, 
                                extreme_params: Optional[Dict[str, Any]] = None, 
                                boundary_conditions: Optional[Dict[str, Any]] = None, 
                                enable_error_monitoring: bool = False) -> CompleteConfig:
    ...
    supported_edge_cases = [
        'minimal_grid', 'large_grid_stress', 'extreme_sigma_values', 
        'boundary_conditions', 'memory_limits', 'timeout_stress'
    ]
    ...
    if edge_case_type == 'minimal_grid':
        ...
        base_config = {
            'grid_size': STRESS_TEST_GRID_SIZE,
            'source_location': (STRESS_TEST_GRID_SIZE[0] // 2, STRESS_TEST_GRID_SIZE[1] // 2),
            'max_steps': 500,
            'plume_sigma': 25.0
        }
        ...
        base_config = {
            'grid_size': INTEGRATION_TEST_GRID_SIZE,
            'source_location': (INTEGRATION_TEST_GRID_SIZE[0] // 2, INTEGRATION_TEST_GRID_SIZE[1] // 2),
            'max_steps': 100,
        ...
        base_config = {
            'grid_size': PERFORMANCE_TEST_GRID_SIZE,
            'source_location': (PERFORMANCE_TEST_GRID_SIZE[0] // 2, PERFORMANCE_TEST_GRID_SIZE[1] // 2),
    ...
    config = CompleteConfig(
        ...
        deterministic_mode=True,
        random_seed=REPRODUCIBILITY_SEEDS[3],  # Fixed seed for edge case reproducibility
        memory_limit_mb=MEMORY_LIMIT_TOTAL_MB,
        step_latency_target_ms=PERFORMANCE_TARGET_STEP_LATENCY_MS * 5  # Relaxed timing for edge cases
    )
    ...
    config.advanced_options.update({
        ...
        'safety_limits': True,
        'timeout_multiplier': TEST_TIMEOUT_MULTIPLIER * 3,  # Extended timeout for edge cases
    ...
    config.advanced_options.update({
        'resource_limits': {
            'max_memory_mb': MEMORY_LIMIT_TOTAL_MB * 1.5,  # Allow some overrun for edge case testing
    ...
    try:
        ...
        if edge_case_type == 'memory_limits':
            estimated_memory = (config.grid_size[0] * config.grid_size[1] * 4) / (1024 * 1024)
            if estimated_memory > MEMORY_LIMIT_TOTAL_MB * 2:  # 2x limit is too dangerous
                raise ResourceError(
...
def get_test_config_for_category(test_category: str, 
                                category_overrides: Optional[Dict[str, Any]] = None, 
                                auto_optimize: bool = False, 
                                detect_system_capabilities: bool = False) -> CompleteConfig:
    ...
    supported_categories = ['unit', 'integration', 'performance', 'reproducibility', 'edge_case', 'minimal', 'stress']
...
def validate_test_configuration(test_config: CompleteConfig, 
                               test_category: str, 
                               strict_validation: bool = False, 
                               system_constraints: Optional[Dict[str, Any]] = None) -> Tuple[bool, Dict[str, Any], List[str]]:
    ...
    if test_category in category_requirements:
        ...
        if 'max_grid_size' in requirements:
            if (test_config.grid_size[0] > requirements['max_grid_size'][0] or 
                test_config.grid_size[1] > requirements['max_grid_size'][1]):
                validation_report['issues_found'].append(f"Grid size exceeds {test_category} test limits")
                recovery_suggestions.append(f"Reduce grid size to {requirements['max_grid_size']} or smaller")
                is_valid = False
        
        if 'required_seed' in requirements and requirements['required_seed']:
            if test_config.random_seed is None:
                validation_report['issues_found'].append("Reproducibility tests require fixed seed")
                recovery_suggestions.append("Set random_seed to a fixed integer value")
                is_valid = False
        
        if 'required_monitoring' in requirements and requirements['required_monitoring']:
            if not test_config.enable_performance_monitoring:
                validation_report['warnings'].append("Performance tests should enable monitoring")
                recovery_suggestions.append("Enable performance monitoring for accurate measurements")
    ...
    if system_constraints:
        ...
        if 'max_memory_mb' in system_constraints:
            estimated_memory = (test_config.grid_size[0] * test_config.grid_size[1] * 4) / (1024 * 1024)
            if estimated_memory > system_constraints['max_memory_mb']:
                constraint_issues.append(f"Memory estimate {estimated_memory:.1f}MB exceeds system limit")
    ...
    if strict_validation:
        ...
        if test_category == 'performance':
            if test_config.step_latency_target_ms > PERFORMANCE_TARGET_STEP_LATENCY_MS * 2:
                strict_issues.append("Performance test latency target too relaxed")
                recovery_suggestions.append("Tighten step latency target for performance testing")
    ...
    if (test_config.source_location[0] >= test_config.grid_size[0] or 
        test_config.source_location[1] >= test_config.grid_size[1]):
    ...
    grid_diagonal = ((test_config.grid_size[0]**2 + test_config.grid_size[1]**2) ** 0.5)
    if test_config.plume_sigma > grid_diagonal / 2:
        ...
        validation_report['warnings'].append("Large plume sigma may affect gradient visibility")
    ...
    if not recovery_suggestions and not is_valid:
        recovery_suggestions.append("Review configuration parameters and test category requirements")
    ...
    if strict_validation and is_valid:
        recovery_suggestions.append("Configuration passed strict validation - ready for testing")
    
    # Return comprehensive validation results with actionable feedback
    return is_valid, validation_report, recovery_suggestions


def create_minimal_test_config(seed: Optional[int] = None, 
                              headless_only: bool = False) -> CompleteConfig:
...
def create_stress_test_config(max_grid_size: Optional[Tuple[int, int]] = None, 
                             max_episodes: Optional[int] = None, 
                             enable_resource_monitoring: bool = False, 
                             stress_overrides: Optional[Dict[str, Any]] = None) -> CompleteConfig:
    ...
    test_grid_size = max_grid_size if max_grid_size is not None else STRESS_TEST_GRID_SIZE
    test_source_location = (test_grid_size[0] // 2, test_grid_size[1] // 2)
    ...
    config = CompleteConfig(
        ...
        memory_limit_mb=MEMORY_LIMIT_TOTAL_MB * 2,  # Configure maximum memory usage approaching system limits
        step_latency_target_ms=PERFORMANCE_TARGET_STEP_LATENCY_MS * 5  # Relaxed timing for heavy load
    )
    ...
    config.advanced_options.update({
        ...
        'timeout_multiplier': TEST_TIMEOUT_MULTIPLIER * 4,  # Configure timeout and safety limits for stress test protection
...
class TestConfigMetadata:
    ...
    def _estimate_duration(self) -> float:
        ...
        if 'grid_size' in self.requirements:
            ...
            if isinstance(grid_size, (tuple, list)) and len(grid_size) == 2:
                complexity_factor = (grid_size[0] * grid_size[1]) / (64 * 64)  # Relative to 64x64 base
                base_duration *= min(complexity_factor, 4.0)  # Cap at 4x multiplier
        
        return base_duration
    
    def estimate_execution_time(self, test_config: CompleteConfig, 
                              system_specs: Optional[Dict[str, Any]] = None) -> float:
        ...
        if system_specs:
            ...
            if 'memory_gb' in system_specs:
                memory_factor = max(0.5, min(2.0, system_specs['memory_gb'] / 8.0))  # Normalize to 8GB
                complexity_time /= memory_factor
            
            # CPU factor
            if 'cpu_count' in system_specs:
                cpu_factor = max(0.5, min(2.0, system_specs['cpu_count'] / 4.0))  # Normalize to 4 cores
    ...
    def validate_system_compatibility(self, system_capabilities: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        if 'memory_gb' in system_capabilities and 'min_memory_mb' in self.system_constraints:
            ...
            if available_mb < required_mb:
                ...
                compatibility_report['recommendations'].append("Increase available memory or use smaller grid size")
            else:
        ...
        if 'cpu_count' in system_capabilities and self.category == 'performance':
            if system_capabilities['cpu_count'] < 2:
                compatibility_report['warnings'].append("Performance tests may be unreliable on single-core systems")
                compatibility_report['recommendations'].append("Use multi-core system for performance benchmarks")
            else:
        ...
        if 'has_display' in system_capabilities:
            if not system_capabilities['has_display'] and 'rendering' in self.requirements:
                compatibility_report['warnings'].append("No display detected - using RGB array mode only")
                compatibility_report['recommendations'].append("Enable headless mode or use system with display")
        ...
        for dep in self.dependencies:
            try:
                __import__(dep)
                compatibility_report['capability_checks'][f'dependency_{dep}'] = 'available'
            except ImportError:
                missing_deps.append(dep)
        
        if missing_deps:
            compatibility_report['overall_compatible'] = False
            compatibility_report['constraint_violations'].append(f"Missing dependencies: {', '.join(missing_deps)}")
            compatibility_report['recommendations'].append(f"Install missing dependencies: {', '.join(missing_deps)}")
        
        # Generate detailed compatibility report with specific issues
        if not compatibility_report['overall_compatible']:
            compatibility_report['summary'] = "System incompatible with test requirements"
    ...
    def to_dict(self, include_estimates: bool = False) -> Dict[str, Any]:
        ...
        if include_estimates:
            ...
            metadata_dict['complexity_analysis'] = {
                'category_complexity': self.category in ['performance', 'stress', 'edge_case'],
                'resource_intensive': 'memory' in self.system_constraints or 'cpu' in self.system_constraints,
...
class TestConfigFactory:
    ...
    def __init__(self, default_overrides: Optional[Dict[str, Any]] = None, 
                 auto_optimize: bool = False):
    ...
    def create_for_test_type(self, test_type: str, 
                           type_overrides: Optional[Dict[str, Any]] = None, 
                           validate_result: bool = True) -> CompleteConfig:
    ...
    def create_suite_configs(self, test_categories: List[str], 
                           suite_overrides: Optional[Dict[str, Any]] = None, 
                           optimize_for_speed: bool = False) -> Dict[str, CompleteConfig]:
        ...
        invalid_categories = [cat for cat in test_categories if cat not in self._category_metadata]
        ...
        for category in test_categories:
            ...
            if optimize_for_speed:
                speed_overrides = {
                    ...
                    'grid_size': min(base_config.grid_size, UNIT_TEST_GRID_SIZE),  # Use smaller grids
    ...
    def detect_system_capabilities(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            capabilities['cpu_freq'] = psutil.cpu_freq()._asdict() if psutil.cpu_freq() else {}
        except ImportError:
        ...
        try:
            ...
            capabilities['has_display'] = 'TkAgg' in available_backends or 'Qt5Agg' in available_backends
        ...
        capabilities['performance_baseline'] = {
            ...
            'estimated_performance_tier': 'high' if loop_time < 0.001 else 'medium' if loop_time < 0.01 else 'low'
        }
    ...
    def optimize_for_system(self, base_config: CompleteConfig, 
                          optimization_target: str = 'speed') -> CompleteConfig:
        ...
        if 'memory_gb' in caps:
            ...
            max_grid_cells = int((memory_gb * 1024 * 1024 * 1024 * 0.1) / 4)  # 10% of memory, 4 bytes per cell
        ...
        if 'performance_baseline' in caps:
            perf_tier = caps['performance_baseline'].get('estimated_performance_tier', 'medium')
            if perf_tier == 'low':
                optimizations['step_latency_target_ms'] = PERFORMANCE_TARGET_STEP_LATENCY_MS * 3
            elif perf_tier == 'high':
                optimizations['step_latency_target_ms'] = PERFORMANCE_TARGET_STEP_LATENCY_MS * 0.7
...
__all__ = [
    ...
    'CompleteConfig', 'EnvironmentConfig', 'PerformanceConfig', 'get_complete_default_config',
    
    # Test configuration factory functions
    'create_unit_test_config', 'create_integration_test_config', 'create_performance_test_config',
    'create_reproducibility_test_config', 'create_edge_case_test_config', 'get_test_config_for_category',
    'validate_test_configuration', 'create_minimal_test_config', 'create_stress_test_config',
    
    # Test configuration classes
    'TestConfigFactory', 'TestConfigMetadata',
    
    # Test scenario constants
    'REPRODUCIBILITY_SEEDS', 'UNIT_TEST_SCENARIOS', 'INTEGRATION_TEST_SCENARIOS', 
    'PERFORMANCE_TEST_SCENARIOS', 'EDGE_CASE_TEST_SCENARIOS',
    
    # Test configuration constants
    'UNIT_TEST_GRID_SIZE', 'INTEGRATION_TEST_GRID_SIZE', 'PERFORMANCE_TEST_GRID_SIZE', 
    'STRESS_TEST_GRID_SIZE', 'TEST_EPISODE_STEPS_UNIT', 'TEST_EPISODE_STEPS_INTEGRATION', 
    'TEST_EPISODE_STEPS_PERFORMANCE', 'TEST_SIGMA_SIMPLE', 'TEST_SIGMA_COMPLEX', 
    'TEST_TIMEOUT_MULTIPLIER', 'PERFORMANCE_TOLERANCE'
]