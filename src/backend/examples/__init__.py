...
def list_available_examples(
    category_filter: Optional[str] = None, include_descriptions: bool = True
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for example_name, example_info in AVAILABLE_EXAMPLES.items():
            if example_name in examples_to_include or category_filter is None:
                ...
                example_data["usage_instructions"] = (
                    f"Run with: run_example('{example_name}')"
                )
        ...
        _logger.info(
            f"Listed {len(filtered_examples)} examples with category filter: {category_filter}"
        )
        return result
...
def run_example(
    ...
    try:
        ...
        if example_name not in AVAILABLE_EXAMPLES:
            _logger.error(
                f"Invalid example name '{example_name}'. Available examples: {list(AVAILABLE_EXAMPLES.keys())}"
            )
            return 2
        ...
        try:
            ...
            if config_params:
                ...
                if len(sig.parameters) > 0:
                    ...
                    _logger.warning(
                        f"Example function '{example_name}' does not accept parameters, ignoring config_params"
                    )
...
def get_example_help(example_name: str) -> str:
    ...
    try:
        ...
        help_text += (
            f"Estimated Duration: {example_info['duration_minutes']} minutes\n\n"
        )
        ...
        help_text += (
            f"Verbose output:  run_example('{example_name}', verbose_output=True)\n\n"
        )
        ...
        help_text += (
            f"- Refer to module documentation for '{example_function.__module__}'\n"
        )
...
def discover_examples(
    include_internal_functions: bool = False, validate_signatures: bool = True
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for module_name in example_modules:
            try:
                ...
                if validate_signatures:
                    ...
                    discovery_results["validation_results"][
                        module_name
                    ] = validation_results

            except Exception as import_error:
                _logger.error(
                    f"Failed to import module '{module_name}': {import_error}"
                )
                discovery_results["import_status"][
                    module_name
                ] = f"failed: {str(import_error)}"
        ...
        _logger.info(
            f"Example discovery completed: {discovered_count} functions from {len(example_modules)} modules"
        )
...
def create_example_runner(
    ...
    try:
        ...
        _logger.info(
            f"Example runner created with log_level='{log_level}', capture_output={capture_output}"
        )
    ...
    try:
        ...
        if output_file:
            try:
                ...
                _logger.error(
                    f"Failed to save documentation to {output_file}: {save_error}"
                )
...
class ExampleRunner:
    ...
    def __init__(
        ...
        self.logger.info(
            f"ExampleRunner initialized with {len(self.available_examples)} available examples"
        )
        ...
        try:
            ...
            if example_name not in self.available_examples:
                ...
                execution_result["error"] = (
                    f"Example '{example_name}' not found in registry"
                )
                return execution_result

            # Set up execution context with logging and output capture
            self.logger.info(
                f"Executing example '{example_name}' with timeout_enabled={timeout_enabled}"
            )
            ...
            try:
                exit_code = run_example(
                    example_name, config_params, verbose_output=True
                )
                ...
                if exit_code == 0:
                    ...
                    self.logger.warning(
                        f"Example '{example_name}' completed with exit code {exit_code}"
                    )
                ...
                execution_result["success"] = False
                self.logger.error(
                    f"Example '{example_name}' execution failed: {exec_error}"
                )
            ...
            execution_result["duration_seconds"] = time.time() - execution_start_time
            self.logger.error(
                f"ExampleRunner error for '{example_name}': {runner_error}"
            )
            return execution_result
    ...
    def run_batch_examples(
        ...
        try:
            ...
            if invalid_examples:
                ...
                self.logger.error(
                    f"Batch validation failed - invalid examples: {invalid_examples}"
                )
                return batch_results

            # Set up batch execution context with logging and progress tracking
            self.logger.info(
                f"Starting batch execution of {len(example_names)} examples"
            )
            self.logger.info(
                f"Parallel execution: {parallel_execution}, Stop on failure: {stop_on_failure}"
            )
            ...
            if parallel_execution:
                self.logger.warning(
                    "Parallel execution not implemented in this version - using sequential execution"
                )

            for i, example_name in enumerate(example_names):
                self.logger.info(
                    f"Executing example {i+1}/{len(example_names)}: '{example_name}'"
                )
                ...
                if result.get("success", False):
                    ...
                    if stop_on_failure:
                        self.logger.warning(
                            f"Stopping batch execution due to failure in '{example_name}'"
                        )
            ...
            batch_results["summary_statistics"] = {
                ...
                "success_rate": (
                    successful_executions / total_examples
                    if total_examples > 0
                    else 0.0
                ),
                "total_batch_duration": total_duration,
                "average_execution_time": (
                    total_duration / total_examples if total_examples > 0 else 0.0
                ),
            }
            ...
            self.logger.info(
                f"Batch execution completed - {successful_executions}/{total_examples} successful"
            )
        ...
        try:
            ...
            if not self.execution_history:
                return {
                    ...
                    "total_executions": 0,
                    "recommendations": [
                        "Execute examples to generate performance data"
                    ],
                }
...
_logger.info(
    f"Examples package initialized - Version {EXAMPLES_VERSION}, {len(AVAILABLE_EXAMPLES)} examples available"
)
