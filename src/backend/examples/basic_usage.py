...
NUM_DEMO_STEPS = (
    50  # Number of steps to demonstrate in basic episode for educational timing
)
PERFORMANCE_TARGET_MS = (
    1.0  # Performance target threshold in milliseconds for step execution monitoring
)
_logger = logging.getLogger(
    __name__
)  # Logger instance for example script with hierarchical naming
...
def setup_example_logging(log_level: str = None) -> None:
    ...
    _logger.info(
        f"Basic usage example logging initialized with level: {effective_log_level}"
    )
    _logger.info(
        f"Example configuration - Seed: {EXAMPLE_SEED}, Demo steps: {NUM_DEMO_STEPS}"
    )
...
def demonstrate_environment_registration() -> bool:
    ...
    try:
        ...
        _logger.info(
            "Calling register_env() to register environment with Gymnasium registry..."
        )
        register_env()
        ...
        if ENV_ID in gymnasium.envs.registration.registry:
            ...
            _logger.info(
                f"Environment can now be created using: gymnasium.make('{ENV_ID}')"
            )
            _logger.info(
                "Registration includes entry point and configuration parameters"
            )
            return True
        else:
            _logger.error(
                f"✗ Environment {ENV_ID} not found in registry after registration"
            )
            return False
...
def demonstrate_environment_creation(render_mode: str = None) -> gymnasium.Env:
    ...
    _logger.info(
        f"Creating environment using gym.make('{ENV_ID}', render_mode='{render_mode}')"
    )
    ...
    try:
        ...
        _logger.info(
            "Environment created successfully! Inspecting environment properties:"
        )
        ...
        if hasattr(env, "spec") and env.spec:
            ...
            _logger.info(
                f"  Max episode steps: {getattr(env.spec, 'max_episode_steps', 'Unlimited')}"
            )
...
def demonstrate_basic_episode(
    env: gymnasium.Env, seed: int = None, max_steps: int = None
) -> dict:
    """
    ...
    _logger.info(
        f"Episode parameters - Seed: {effective_seed}, Max steps: {effective_max_steps}"
    )
    ...
    try:
        ...
        _logger.info(
            f"Initial concentration at agent: {observation[0] if len(observation) > 0 else 'N/A'}"
        )
        ...
        for step_num in range(effective_max_steps):
            ...
            action_name = (
                Action(action).name
                if hasattr(Action, "_name2value_")
                and action in Action._name2value_.values()
            ...
            if step_num % 10 == 0 or reward != 0.0 or terminated or truncated:
                _logger.info(
                    f"Step {step_num + 1}: Action={action_name}, Reward={reward:.3f}, "
            ...
            if step_duration_ms > PERFORMANCE_TARGET_MS:
                episode_stats["performance_issues"].append(
                    f"Step {step_num + 1}: {step_duration_ms:.3f}ms"
                )
            ...
            if terminated:
                ...
                _logger.info(
                    f"🎯 Episode terminated successfully at step {step_num + 1}!"
                )
                _logger.info(
                    f"Goal reached with total reward: {episode_stats['total_reward']:.3f}"
                )
                break
            elif truncated:
                episode_stats["episode_completed"] = True
        ...
        episode_stats["total_episode_time_ms"] = (
        ...
        _logger.info(
            f"  Total episode time: {episode_stats['total_episode_time_ms']:.3f}ms"
        )
        _logger.info(
            f"  Average step time: {episode_stats['average_step_time_ms']:.3f}ms"
        )

        # Report performance issues if any detected
        if episode_stats["performance_issues"]:
            _logger.warning(
                f"Performance issues detected in {len(episode_stats['performance_issues'])} steps:"
            )
...
def demonstrate_rendering(env: gymnasium.Env) -> dict:
    ...
    try:
        ...
        try:
            ...
            if env.render_mode != "rgb_array":
                _logger.info(
                    "Creating temporary environment with rgb_array render mode..."
                )
            ...
            if rgb_array is not None:
                ...
                _logger.info(
                    f"  Rendering time: {render_results['rgb_array_time_ms']:.3f}ms"
                )
            else:
                render_results["error_messages"].append(
                    "RGB array render returned None"
                )
        ...
        try:
            ...
            render_results["human_mode_time_ms"] = (
                human_end_time - human_start_time
            ) * 1000
            ...
            _logger.info(
                f"  Rendering time: {render_results['human_mode_time_ms']:.3f}ms"
            )
            _logger.info(
                "  Note: Human mode displays window (may not be visible in headless environment)"
            )
    ...
    _logger.info(
        f"  RGB array support: {'✓' if render_results['rgb_array_supported'] else '✗'}"
    )
    _logger.info(
        f"  Human mode support: {'✓' if render_results['human_mode_supported'] else '✗'}"
    )
    ...
    if render_results["human_mode_supported"]:
        _logger.info(
            f"  Human performance: {render_results['human_mode_time_ms']:.3f}ms"
        )
...
def demonstrate_seeding_reproducibility(
    env: gymnasium.Env, test_seed: int, num_test_steps: int
) -> bool:
    """
    ...
    try:
        ...
        for step_num in range(num_test_steps):
            ...
            if terminated or truncated:
                _logger.info(
                    f"First episode terminated/truncated at step {step_num + 1}"
                )
                break
        ...
        for step_num in range(
            len(first_episode_data) - 1
        ):  # Match first episode length
            action = env.action_space.sample()
            observation, reward, terminated, truncated, info = env.step(action)
            ...
            if terminated or truncated:
                _logger.info(
                    f"Second episode terminated/truncated at step {step_num + 1}"
                )
                break
        ...
        comparison_results = {
            "matching_steps": 0,
            "total_steps_compared": min(
                len(first_episode_data), len(second_episode_data)
            ),
        ...
        for i in range(comparison_results["total_steps_compared"]):
            ...
            obs_match = np.allclose(
                step_data_1["observation"], step_data_2["observation"], rtol=1e-10
            )
            ...
            info_match = True
            if (
                "agent_pos" in step_data_1["info"]
                and "agent_pos" in step_data_2["info"]
            ):
                info_match = (
                    step_data_1["info"]["agent_pos"] == step_data_2["info"]["agent_pos"]
                )
        ...
        success_rate = (
            comparison_results["matching_steps"]
            / comparison_results["total_steps_compared"]
            * 100
        )
        ...
        if reproducibility_passed:
            ...
            _logger.error(
                f"  Observation mismatches: {comparison_results['observation_mismatches']}"
            )
            _logger.error(
                f"  Reward mismatches: {comparison_results['reward_mismatches']}"
            )
...
def demonstrate_error_handling(env: gymnasium.Env) -> None:
    ...
    try:
        ...
        for invalid_action in invalid_actions:
            ...
            try:
                ...
                _logger.warning(
                    f"  Unexpected: Invalid action {invalid_action} was accepted"
                )
                ...
                _logger.info(
                    f"  ✓ Action {invalid_action} rejected with: {type(e).__name__}: {e}"
                )
    ...
    try:
        ...
        try:
            ...
            _logger.info(
                f"  ✓ Closed environment step error handled: {type(e).__name__}"
            )
...
def run_basic_usage_demo() -> int:
    ...
    _logger.info("=" * 80)
    _logger.info(
        "This demonstration showcases essential plume navigation environment functionality"
    )
    _logger.info(
        "including registration, instantiation, episode lifecycle, and API usage patterns."
    )
    _logger.info("")
    ...
    try:
        ...
        if not registration_success:
            _logger.error(
                "❌ Environment registration failed - cannot proceed with demonstration"
            )
        ...
        if env is None:
            _logger.error(
                "❌ Environment creation failed - cannot proceed with demonstration"
            )
            _logger.error(
                "Please verify registration success and dependency installation"
            )
            return 1
        ...
        episode_stats = demonstrate_basic_episode(
            env, seed=EXAMPLE_SEED, max_steps=NUM_DEMO_STEPS
        )

        # Analyze episode performance for educational feedback
        if episode_stats.get("termination_reason") == "error":
            _logger.warning(
                "⚠ Episode execution encountered errors but demonstration continues"
            )
            exit_code = 1
        elif episode_stats.get("performance_issues"):
            _logger.info(
                f"ℹ Performance advisory: {len(episode_stats['performance_issues'])} slow steps detected"
            )
        ...
        if not render_results.get("rgb_array_supported") and not render_results.get(
            "human_mode_supported"
        ):
            _logger.warning(
                "⚠ Neither rendering mode fully functional - this may be expected in headless environments"
            )
        ...
        reproducibility_success = demonstrate_seeding_reproducibility(
            env, test_seed=EXAMPLE_SEED, num_test_steps=10
        )

        if not reproducibility_success:
            _logger.warning(
                "⚠ Reproducibility issues detected - this may affect research reliability"
            )
            exit_code = 1
        ...
        _logger.error(
            "This indicates a serious issue with the environment or dependencies"
        )
        exit_code = 2
...
def main() -> None:
    ...
    try:
        ...
        if exit_status == 0:
            ...
            print(
                "Check the logs above for details on any warnings or non-critical errors."
            )
        else:
