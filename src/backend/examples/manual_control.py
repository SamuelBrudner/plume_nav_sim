...
from ..plume_nav_sim.core.types import Action, Coordinates, GridSize, AgentState, create_coordinates, create_step_info
from ..plume_nav_sim.core.constants import (
    DEFAULT_GRID_SIZE, DEFAULT_SOURCE_LOCATION, DEFAULT_MANUAL_SEED, 
    CONTROL_INSTRUCTIONS, KEYBOARD_MAPPING, CONTROL_UPDATE_DELAY,
    SESSION_TIMEOUT_MINUTES, STATISTICS_DISPLAY_FREQUENCY
)

# Global constants for manual control configuration
CONTROL_INSTRUCTIONS = "MANUAL CONTROL: Use WASD or Arrow Keys to move. Q to quit, R to reset, H for help."
KEYBOARD_MAPPING = {
...
__all__ = [
    "main", "run_manual_control_session", "setup_manual_control", "create_keyboard_handler", 
    "display_control_instructions", "update_session_statistics", "handle_manual_input"
]


def setup_manual_control(seed: int = None, enable_debug_logging: bool = False, check_backend_compatibility: bool = True) -> tuple:
        ...
        if check_backend_compatibility:
            ...
            if current_backend == 'Agg':  # Non-interactive backend
                for backend in interactive_backends:
                    ...
                    logger.warning("No interactive matplotlib backend available - keyboard input may be limited")
...
def create_keyboard_handler(input_queue: queue.Queue, key_mapping: dict, enable_special_commands: bool = True) -> threading.Thread:
...
def display_control_instructions(env: gym.Env, show_environment_info: bool = True, show_advanced_controls: bool = False) -> None:
...
def run_manual_control_session(env: gym.Env, session_timeout_seconds: int, display_statistics: bool = True, 
                              enable_performance_tracking: bool = True) -> dict:
    ...
    try:
        ...
        keyboard_thread = create_keyboard_handler(input_queue, KEYBOARD_MAPPING, enable_special_commands=True)
        ...
        while session_active:
            ...
            try:
                ...
                if input_type == 'action':
                    ...
                    if display_statistics and session_stats['total_actions'] - last_statistics_display >= STATISTICS_DISPLAY_FREQUENCY:
                    ...
                    if input_value == 'quit':
                        ...
                        display_control_instructions(env, show_environment_info=True, show_advanced_controls=True)
        ...
        session_results = generate_session_report(session_stats, enable_performance_tracking)
...
def handle_manual_input(input_key: str, env: gym.Env, session_state: dict, provide_feedback: bool = True) -> dict:
    ...
    try:
        ...
        if normalized_key in KEYBOARD_MAPPING:
            ...
            try:
                ...
                if provide_feedback:
                    if reward > 0:
                        processing_result['feedback_message'] = f"Excellent! Goal reached with reward: {reward}"
                    elif terminated:
                        processing_result['feedback_message'] = "Episode completed!"
                    else:
                        distance = info.get('distance_to_source', 'unknown')
                        processing_result['feedback_message'] = f"Moved {action.name}. Distance to source: {distance}"
        ...
        # Handle special commands (Q=quit, R=reset, H=help) with immediate processing
            ...
            try:
                ...
                processing_result.update({
                    ...
                    'session_updates': {'reset': True, 'observation': observation, 'info': info}
                })
...
def update_session_statistics(session_stats: dict, env_info: dict, display_to_console: bool = True, 
                             include_advanced_metrics: bool = False) -> dict:
    ...
    try:
        ...
        if total_actions > 0 and position_history:
            ...
            if len(position_history) >= 2:
                ...
                straight_distance = np.sqrt((current_pos[0] - start_pos[0])**2 + (current_pos[1] - start_pos[1])**2)
        ...
        if include_advanced_metrics:
            ...
            if 'performance_metrics' in session_stats and session_stats['performance_metrics']:
        ...
        if total_actions > 10:
            if movement_efficiency < 0.3:
                educational_insights.append("Try more direct paths toward higher concentration areas")
        ...
        if goal_reached:
            ...
            if current_distance < 5:
                educational_insights.append("Very close to the source - you're doing great!")
...
def display_session_summary(session_results: dict, include_performance_analysis: bool = True, 
                           include_learning_recommendations: bool = True) -> None:
    ...
    try:
        ...
        if goal_reached:
            ...
            efficiency_rating = "Excellent" if time_to_goal < 60 else "Good" if time_to_goal < 180 else "Needs Improvement"
        ...
        if include_learning_recommendations:
            ...
            if not goal_reached:
                recommendations.append("Focus on following concentration gradients toward brighter areas")
                recommendations.append("Try systematic search patterns (spiral, grid-based exploration)")
                recommendations.append("Use the reset command (R) to practice different approaches")
            ...
            if action_distribution:
                movement_bias = max(action_distribution.values()) / sum(action_distribution.values()) if action_distribution else 0
            ...
            if recommendations:
                ...
                print("   🌟 Excellent performance! Try increasing difficulty or exploring advanced features.")
        ...
        if include_performance_analysis:
            ...
            theoretical_optimal = np.sqrt((64-64)**2 + (64-64)**2) if current_distance != 'unknown' else None
            ...
            if goal_reached and duration > 0:
                ...
                print("   💭 Algorithms like A* or gradient descent might find optimal paths faster")
...
def cleanup_manual_control(env: gym.Env, input_thread: threading.Thread = None, session_data: dict = None, 
                          save_session_data: bool = False) -> bool:
    ...
    try:
        ...
        if input_thread and input_thread.is_alive():
            ...
            if input_thread.is_alive():
                logger.warning("Keyboard input thread still active - it will terminate with main process")
        ...
        if save_session_data and session_data:
            try:
                ...
                logger.info(f"Session summary - Duration: {session_duration:.1f}s, Actions: {total_actions}, Goal: {goal_reached}")
...
def generate_session_report(session_stats: dict, include_performance: bool = True) -> dict:
    ...
    try:
        ...
        report['session_analysis'] = {
            'completion_status': 'completed' if session_stats.get('goal_reached', False) else 'incomplete',
        ...
        if include_performance and 'performance_metrics' in session_stats:
            report['performance_analysis'] = _analyze_performance_metrics(session_stats['performance_metrics'])
...
def _analyze_performance_metrics(performance_metrics: dict) -> dict:
    ...
    if step_times:
        analysis['step_performance'] = {
            ...
            'consistency': np.std(step_times) < 2.0  # Low variance indicates consistent performance
        }
    ...
    if render_times:
        analysis['render_performance'] = {
            ...
            'frame_rate_fps': 1000 / np.mean(render_times) if np.mean(render_times) > 0 else 0
        }
...
def main() -> int:
    ...
    try:
        ...
        display_control_instructions(env, show_environment_info=True, show_advanced_controls=False)
        ...
        if not backend_compatible:
            logger.warning("Limited backend compatibility - some features may not work optimally")
        ...
        if session_data.get('goal_reached'):
            logger.info("Congratulations! Session completed successfully with goal reached!")
