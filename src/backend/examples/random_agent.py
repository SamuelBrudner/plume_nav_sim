...
from plume_nav_sim.registration.register import register_env, ENV_ID  # Environment registration function enabling gym.make() instantiation for random agent demonstration
from plume_nav_sim.core.types import Action  # Action enumeration for random action selection and movement direction analysis
from plume_nav_sim.core.constants import DEFAULT_MAX_STEPS, PERFORMANCE_TARGET_STEP_LATENCY_MS  # Default maximum episode steps for random agent episode configuration and performance target constant for step latency benchmarking and analysis
from plume_nav_sim.utils.exceptions import PlumeNavSimError  # Base exception handling for comprehensive error management in random agent execution
...
ANALYSIS_STATS = ['steps_to_goal', 'success_rate', 'avg_reward', 'step_latency', 'exploration_coverage']
...
def setup_random_agent_logging(log_level: str = None, log_file: str = None) -> None:
    ...
    _logger.info(f"Random agent logging configured - Level: {log_level}, File: {log_file or 'console only'}")
...
def create_random_policy(env: gym.Env, policy_seed: int = None) -> callable:
    ...
    if policy_seed is not None:
        ...
        _logger.debug(f"Random policy created with seed {policy_seed} for reproducible behavior")
    else:
    ...
    if hasattr(env.action_space, 'n'):
        ...
        raise PlumeNavSimError(f"Environment action space {type(env.action_space)} not supported for random policy")
    ...
    def random_policy_function(observation):
        ...
        _logger.debug(f"Random policy selected action {action} from action space [0, {action_space_size})")
        
        # Validate action is within expected range
        if not (0 <= action < action_space_size):
            _logger.warning(f"Generated action {action} outside valid range [0, {action_space_size})")
        
        return action
    
    # Log policy creation completion with action space information
    _logger.info(f"Random policy created for action space size {action_space_size} with reproducible seeding")
    
    # Return configured random policy function ready for agent execution
    return random_policy_function


def execute_random_episode(env: gym.Env, random_policy: callable, seed: int = None, max_steps: int = None, 
                          render_episode: bool = False, track_performance: bool = True) -> dict:
    ...
    _logger.info(f"Starting random agent episode - Seed: {seed}, Max Steps: {max_steps}, Render: {render_episode}")
    ...
    if track_performance:
        episode_stats['performance_tracking'] = {
            ...
            'action_distribution': {i: 0 for i in range(4)}  # Track action selection patterns
        }
    ...
    while step_count < max_steps:
        ...
        # Record step timing
        step_duration = (time.time() - step_start_time) * 1000  # Convert to milliseconds
        if track_performance:
        ...
        if render_episode:
            try:
                ...
                episode_stats['rendering_performance']['render_times'].append(render_duration)
                
                _logger.debug(f"Step {step_count}: Rendered frame in {render_duration:.2f}ms")
                
            except Exception as render_error:
                _logger.warning(f"Rendering error at step {step_count}: {render_error}")
                episode_stats['rendering_performance']['errors'] = episode_stats['rendering_performance'].get('errors', 0) + 1
        ...
        if terminated:
            ...
            episode_stats['end_position'] = info.get('agent_xy', episode_stats['start_position'])
        ...
        if step_count % 100 == 0:
            _logger.debug(f"Episode progress: {step_count}/{max_steps} steps, reward: {episode_stats['total_reward']:.3f}")
    ...
    episode_stats['exploration_coverage_count'] = len(episode_stats['exploration_coverage'])
    episode_stats['exploration_coverage'] = list(episode_stats['exploration_coverage'])  # Convert set to list for JSON serialization
    ...
    if track_performance and episode_stats['step_latencies']:
        ...
        episode_stats['performance_summary'] = {
            ...
            'step_latency_std': statistics.stdev(step_latencies) if len(step_latencies) > 1 else 0.0,
            'steps_exceeding_target': sum(1 for lat in step_latencies if lat > PERFORMANCE_TARGET_STEP_LATENCY_MS)
        }
    
    # Log episode completion with comprehensive statistics and performance analysis
    _logger.info(f"Episode completed - Steps: {step_count}, Reward: {episode_stats['total_reward']:.3f}, "
...
def run_random_agent_analysis(env: gym.Env, num_episodes: int = None, base_seed: int = None, 
                             max_steps_per_episode: int = None, render_episodes: bool = False, 
    ...
    _logger.info(f"Starting random agent analysis - Episodes: {num_episodes}, Base seed: {base_seed}, "
    ...
    for episode_idx in range(num_episodes):
        ...
        _logger.info(f"Executing episode {episode_idx + 1}/{num_episodes} with seed {episode_seed}")
        ...
        try:
            ...
            _logger.error(f"Episode {episode_idx + 1} failed with error: {episode_error}")
    ...
    if analysis_results['episode_results']:
        ...
        exploration_coverages = [ep['exploration_coverage_count'] for ep in episode_data]
        analysis_results['exploration_analysis'] = {
            ...
            'coverage_efficiency': statistics.mean(exploration_coverages) / max_steps_per_episode
        }
        ...
        if detailed_analysis:
            ...
            if all_step_latencies:
                analysis_results['performance_analysis'] = {
                    ...
                    'step_latency_std': statistics.stdev(all_step_latencies) if len(all_step_latencies) > 1 else 0.0,
                    'steps_exceeding_target': sum(1 for lat in all_step_latencies if lat > PERFORMANCE_TARGET_STEP_LATENCY_MS),
                    'performance_target_compliance': (1 - sum(1 for lat in all_step_latencies if lat > PERFORMANCE_TARGET_STEP_LATENCY_MS) / len(all_step_latencies)) * 100
                }
            
            # Analyze action distribution patterns
            aggregate_action_dist = {i: 0 for i in range(4)}
            for ep in episode_data:
                if 'performance_tracking' in ep and 'action_distribution' in ep['performance_tracking']:
                    for action, count in ep['performance_tracking']['action_distribution'].items():
            ...
            if total_actions > 0:
                ...
                analysis_results['exploration_analysis']['action_distribution'] = action_percentages
    ...
    analysis_results['success_analysis'] = {
        'episodes_reaching_goal': analysis_results['aggregate_statistics'].get('successful_episodes', 0),
        'success_percentage': analysis_results['aggregate_statistics'].get('success_rate', 0.0) * 100,
        'avg_steps_to_success': None,
        'success_efficiency': None
    }
    
    # Calculate success-specific metrics
    successful_episodes = [ep for ep in analysis_results['episode_results'] if ep['goal_reached']]
    if successful_episodes:
        ...
        analysis_results['success_analysis'].update({
            ...
            'success_efficiency': statistics.mean(success_steps) / max_steps_per_episode
        })
    
    # Log analysis completion with key findings and statistical significance information
    _logger.info(f"Random agent analysis completed - {len(analysis_results['episode_results'])} episodes, "
...
def analyze_random_agent_performance(episode_results: list, include_visualization: bool = False, 
                                    compare_to_optimal: bool = False) -> dict:
    ...
    successful_episodes = [ep for ep in episode_results if ep.get('goal_reached', False)]
    total_episodes = len(episode_results)
    success_rate = len(successful_episodes) / total_episodes if total_episodes > 0 else 0.0
    ...
    if successful_episodes:
        ...
        if len(success_steps) > 1:
            performance_analysis['goal_achievement']['steps_to_goal_std'] = statistics.stdev(success_steps)
    ...
    if all_latencies:
        target_violations = sum(1 for lat in all_latencies if lat > PERFORMANCE_TARGET_STEP_LATENCY_MS)
        performance_analysis['step_latency'] = {
            ...
            'target_compliance_rate': (1 - target_violations / len(all_latencies)) * 100
        }
        
        if len(all_latencies) > 1:
            performance_analysis['step_latency']['latency_std'] = statistics.stdev(all_latencies)
    
    # Evaluate exploration efficiency including coverage rates and movement pattern analysis
    coverage_counts = [ep.get('exploration_coverage_count', 0) for ep in episode_results]
    ...
    if rewards:
        ...
        if len(rewards) > 1:
            performance_analysis['reward_analysis']['reward_std'] = statistics.stdev(rewards)
    ...
    if compare_to_optimal:
        ...
        if successful_episodes:
            ...
            avg_steps_to_goal = performance_analysis['goal_achievement'].get('avg_steps_to_goal', 0)
            theoretical_optimal_steps = 64  # Rough estimate for 128x128 grid
            efficiency_ratio = theoretical_optimal_steps / avg_steps_to_goal if avg_steps_to_goal > 0 else 0.0
    ...
    if include_visualization:
        try:
            ...
            if all_latencies:
                axes[0, 1].hist(all_latencies, bins=30, alpha=0.7, color='green', edgecolor='black')
                axes[0, 1].axvline(PERFORMANCE_TARGET_STEP_LATENCY_MS, color='red', linestyle='--', 
            ...
            if successful_episodes:
                success_steps = [ep['completed_steps'] for ep in successful_episodes]
                axes[1, 0].hist(success_steps, bins=15, alpha=0.7, color='orange', edgecolor='black')
            ...
            if coverage_counts:
                axes[1, 1].scatter(episode_steps, coverage_counts, alpha=0.6, color='purple')
            ...
            performance_analysis['visualization'] = {
                ...
                'plot_types': ['cumulative_success_rate', 'step_latency_distribution', 
                              'steps_to_goal_histogram', 'exploration_coverage_scatter'],
                'figure_size': (12, 10)
            }
            
        except Exception as viz_error:
            _logger.warning(f"Visualization generation failed: {viz_error}")
            performance_analysis['visualization'] = {'plots_generated': False, 'error': str(viz_error)}
    ...
    performance_analysis['statistical_summary'] = {
        ...
        'key_metrics': {
            ...
            'avg_step_latency_ms': performance_analysis.get('step_latency', {}).get('avg_latency_ms', 0),
            'exploration_efficiency_pct': performance_analysis.get('exploration_efficiency', {}).get('unique_position_rate', 0)
        }
    }
    ...
    if success_rate < 0.1:
        recommendations.append("Very low success rate suggests need for improved navigation strategies")
    elif success_rate > 0.8:
        recommendations.append("High success rate indicates environment may be too easy for random agent")
    
    # Latency analysis
    if 'step_latency' in performance_analysis:
        compliance_rate = performance_analysis['step_latency'].get('target_compliance_rate', 0)
        if compliance_rate < 95:
            recommendations.append(f"Step latency target compliance at {compliance_rate:.1f}% - consider performance optimization")
    
    # Exploration efficiency analysis
    if 'exploration_efficiency' in performance_analysis:
        efficiency = performance_analysis['exploration_efficiency'].get('unique_position_rate', 0)
        if efficiency < 10:
            recommendations.append("Low exploration efficiency - random agent exhibits significant position revisiting")
    
    performance_analysis['optimization_recommendations'] = recommendations
    
    # Log performance analysis completion
    _logger.info(f"Performance analysis completed for {total_episodes} episodes - "
...
def demonstrate_random_agent_rendering(env: gym.Env, render_mode: str = None, demo_steps: int = None, 
    ...
    if demo_steps is None:
        demo_steps = min(100, DEFAULT_MAX_STEPS_PER_EPISODE // 10)  # 10% of max steps or 100, whichever is smaller
    ...
    _logger.info(f"Starting rendering demonstration - Mode: {render_mode}, Steps: {demo_steps}, "
    ...
    for step in range(demo_steps):
        ...
        try:
            ...
            render_duration = (time.time() - render_start_time) * 1000  # Convert to milliseconds
            render_times.append(render_duration)
            
            if step % 20 == 0:  # Log every 20 steps
                _logger.debug(f"Demonstration step {step + 1}: Rendered in {render_duration:.2f}ms")
            
        except Exception as render_error:
            _logger.warning(f"Rendering error at demonstration step {step + 1}: {render_error}")
        ...
        if terminated or truncated:
            _logger.info(f"Demonstration episode ended at step {step + 1} (terminated: {terminated}, truncated: {truncated})")
            break
    ...
    if render_times:
        demo_results['rendering_performance'] = {
            ...
            'render_time_std': statistics.stdev(render_times) if len(render_times) > 1 else 0.0
        }
    ...
    if save_trajectory:
        ...
        _logger.info(f"Trajectory data saved with {len(demo_results['trajectory_data'])} points")
    
    # Analyze exploration patterns from visual demonstration including coverage and efficiency
    if demo_results['trajectory_data']:
        positions = [point['position'] for point in demo_results['trajectory_data'] if point['position']]
        ...
        demo_results['visualization_analysis'] = {
            ...
            'exploration_efficiency': len(unique_positions) / len(positions) if positions else 0.0,
            'position_revisit_rate': (len(positions) - len(unique_positions)) / len(positions) if positions else 0.0
        }
        
        # Calculate movement patterns
        actions_taken = [point['action'] for point in demo_results['trajectory_data'] if point['action'] is not None]
        if actions_taken:
    ...
    demo_results['demo_statistics'] = {
        ...
        'steps_completed': len(demo_results['trajectory_data']) - 1,  # Subtract initial position
        'render_mode_used': render_mode,
        'rendering_success_rate': (len(render_times) / demo_steps * 100) if demo_steps > 0 else 0.0,
        'avg_step_duration_ms': (demo_duration * 1000) / demo_steps if demo_steps > 0 else 0.0
    }
    
    # Log rendering demonstration completion with performance metrics and visual insights
    _logger.info(f"Rendering demonstration completed - {len(demo_results['trajectory_data'])} trajectory points, "
...
def generate_random_agent_report(analysis_results: dict, output_file: str = None, include_graphs: bool = False, 
    ...
    if include_recommendations:
        ...
        if success_rate < 0.1:
            recommendations.append("• Very low success rate (< 10%) confirms random policy limitations")
            recommendations.append("• Results provide excellent baseline for comparing directed navigation algorithms")
        elif success_rate > 0.5:
            recommendations.append("• Unusually high success rate for random agent suggests environment analysis needed")
        
        # Performance insights
        if performance:
            compliance = performance.get('performance_target_compliance', 0.0)
            if compliance > 95:
                recommendations.append("• Excellent step latency performance meets optimization targets")
            elif compliance < 90:
                recommendations.append("• Step latency performance below target suggests system optimization needed")
        
        # Exploration insights
        if exploration:
            coverage_eff = exploration.get('coverage_efficiency', 0.0)
            if coverage_eff < 0.2:
                recommendations.append("• Low exploration efficiency typical for random agents")
                recommendations.append("• Significant position revisiting indicates need for memory-based strategies")
    ...
    _logger.info(f"Random agent report generated - {len(report_lines)} lines, "
...
def benchmark_random_agent(benchmark_config: dict = None, include_stress_test: bool = False, 
                          validate_targets: bool = True) -> dict:
    ...
    _logger.info(f"Starting random agent benchmarking - Stress test: {include_stress_test}, "
    ...
    try:
        ...
        benchmark_results['baseline_performance'] = {
            ...
            'success_rate': baseline_analysis.get('aggregate_statistics', {}).get('success_rate', 0.0),
            'avg_steps': baseline_analysis.get('aggregate_statistics', {}).get('avg_steps', 0.0),
            'avg_latency_ms': baseline_analysis.get('performance_analysis', {}).get('avg_step_latency_ms', 0.0),
            'performance_compliance': baseline_analysis.get('performance_analysis', {}).get('performance_target_compliance', 0.0)
        }
        ...
        _logger.info(f"Executing scalability testing with episode counts: {scalability_episodes}")
        ...
        for episode_count in scalability_episodes:
            ...
            scalability_result = {
                ...
                'success_rate': scalability_analysis.get('aggregate_statistics', {}).get('success_rate', 0.0),
                'avg_episode_duration': scalability_analysis.get('aggregate_statistics', {}).get('avg_episode_duration', 0.0)
            }
            
            scalability_results.append(scalability_result)
            _logger.info(f"Scalability test {episode_count} episodes: {episode_count / scalability_duration:.1f} eps/sec")
        ...
        if include_stress_test:
            ...
            benchmark_results['stress_test_results'] = {
                ...
                'success_rate': stress_analysis.get('aggregate_statistics', {}).get('success_rate', 0.0),
                'performance_stability': 'stable',  # Based on successful completion
                'resource_usage': 'within_limits'  # Simplified assessment
            }
            
            _logger.info(f"Stress test completed - {stress_episodes} episodes in {stress_duration:.1f}s")
        ...
        if validate_targets:
            ...
            baseline_latency_compliance = benchmark_results['baseline_performance'].get('performance_compliance', 0.0)
            ...
            baseline_success_rate = benchmark_results['baseline_performance'].get('success_rate', 0.0)
            if 0.01 <= baseline_success_rate <= 0.3:  # 1% to 30% reasonable for random agent
                performance_validation['success_rate_reasonable'] = True
            
            # Overall validation
            performance_validation['overall_validation_passed'] = (
                performance_validation['step_latency_target_met'] and 
                performance_validation['success_rate_reasonable']
            )
            
            benchmark_results['performance_validation'] = performance_validation
            
            _logger.info(f"Performance validation - Overall: {performance_validation['overall_validation_passed']}, "
        ...
        for seed in consistency_test_seeds:
            ...
            consistency_results.append({
                'seed': seed,
                'success_rate': consistency_analysis.get('aggregate_statistics', {}).get('success_rate', 0.0),
                'avg_steps': consistency_analysis.get('aggregate_statistics', {}).get('avg_steps', 0.0)
            })
        ...
        benchmark_results['consistency_analysis'] = {
            ...
            'avg_steps_variance': statistics.variance(avg_steps) if len(avg_steps) > 1 else 0.0,
    ...
    if baseline_perf.get('performance_compliance', 0.0) < 95.0:
        optimization_recommendations.append("Consider step execution optimization for improved latency compliance")
    
    # Analyze scalability for recommendations
    scalability = benchmark_results.get('scalability_analysis', {})
    if scalability:
        optimization_recommendations.append("Scalability testing shows consistent performance across episode counts")
    ...
    benchmark_results['total_benchmark_duration'] = time.time() - benchmark_results['benchmark_timestamp']
    
    # Log benchmarking completion with key performance indicators and validation status
    validation_status = benchmark_results.get('performance_validation', {}).get('overall_validation_passed', False)
    _logger.info(f"Random agent benchmarking completed - Duration: {benchmark_results['total_benchmark_duration']:.1f}s, "
...
def run_random_agent_demo(demo_config: dict = None) -> int:
    ...
    _logger.info(f"Demo configuration: Episodes={demo_config.get('num_episodes', DEFAULT_NUM_EPISODES)}, "
    ...
    try:
        ...
        render_mode = 'human' if demo_config.get('render_episodes', False) else 'rgb_array'
        ...
        analysis_results = run_random_agent_analysis(
            ...
            base_seed=DEFAULT_SEED,
            max_steps_per_episode=demo_config.get('max_steps_per_episode', DEFAULT_MAX_STEPS_PER_EPISODE),
        ...
        if demo_config.get('demonstrate_rendering', True):
            ...
            _logger.info(f"Rendering demonstration completed - "
        ...
        if demo_config.get('generate_report', True):
            ...
            report_content = generate_random_agent_report(
                ...
                include_recommendations=demo_config.get('include_recommendations', True)
            )
            ...
            print(f"Total Analysis Duration: {analysis_results.get('analysis_duration', 0.0):.2f}s")
            
            # Performance metrics
            perf_analysis = performance_analysis
            if 'step_latency' in perf_analysis:
                print(f"Average Step Latency: {perf_analysis['step_latency'].get('avg_latency_ms', 0.0):.2f}ms")
                print(f"Performance Compliance: {perf_analysis['step_latency'].get('target_compliance_rate', 0.0):.1f}%")
        ...
        if demo_config.get('include_benchmarking', False):
            ...
            validation_passed = benchmark_results.get('performance_validation', {}).get('overall_validation_passed', False)
    ...
    print("Use these results to evaluate the performance of directed navigation strategies.")
...
def main() -> None:
    ...
    parser.add_argument(
        '--detailed-analysis', action='store_true', default=True,
    ...
    def handle_unexpected_error(exc_type, exc_value, exc_traceback):
        ...
        _logger.error(f"Unexpected error: {exc_type.__name__}: {exc_value}", exc_info=True)
