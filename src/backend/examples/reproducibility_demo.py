...
from plume_nav_sim.utils.exceptions import ValidationError, PlumeNavSimError
...
DEMO_EXPERIMENT_NAMES = ['baseline', 'validation', 'stress_test', 'cross_session', 'statistical']
...
def setup_reproducibility_logging(
    ...
    _logger.debug(f"Log level: {effective_log_level}, Include timestamps: {include_timestamps}")
...
def execute_reproducibility_episode(
    ...
    _logger.debug(f"Executing episode with seed {episode_seed}, max_steps {effective_max_steps}")
    ...
    try:
        ...
        if calculate_checksums:
            episode_data['checksums'] = {
                ...
                'total_reward': hashlib.md5(
                    str(total_reward).encode()
                ).hexdigest()
            }
            
            if track_all_states:
                # Create checksum for observation sequence (convert to list for JSON serialization)
                obs_list = [obs.tolist() for obs in episode_data['observation_sequence']]
                episode_data['checksums']['observation_sequence'] = hashlib.md5(
                    json.dumps(obs_list).encode()
                ).hexdigest()
        
        _logger.debug(f"Episode completed: {step_count} steps, reward {total_reward:.4f}")
...
def compare_episodes(
    ...
    try:
        ...
        metadata_comparison = {
            ...
            'total_steps_match': episode1.get('total_steps') == episode2.get('total_steps'),
            'terminated_match': episode1.get('terminated') == episode2.get('terminated'),
        ...
        comparison_results['sequence_comparisons']['total_rewards_match'] = total_rewards_match
        ...
        if 'observation_sequence' in episode1 and 'observation_sequence' in episode2:
            ...
            comparison_results['sequence_comparisons']['observations_match'] = observations_match
        ...
        if checksums1 and checksums2:
            ...
            comparison_results['sequence_comparisons']['checksums_match'] = checksums_match
        ...
        _logger.debug(f"Episode comparison completed: {comparison_results['validation_status']}")
...
def demonstrate_basic_reproducibility(
    ...
    effective_length = episode_length if episode_length is not None else DEFAULT_EPISODE_LENGTH
    ...
    try:
        ...
        policy = create_deterministic_policy(env, policy_seed=12345, policy_type='random')
        _logger.info(f"Created deterministic {policy.policy_type} policy with seed {policy.seed}")
        ...
        if show_detailed_output:
            _logger.info(f"Episode 1 actions: {episode1['action_sequence'][:20]}{'...' if len(episode1['action_sequence']) > 20 else ''}")
            _logger.info(f"Episode 2 actions: {episode2['action_sequence'][:20]}{'...' if len(episode2['action_sequence']) > 20 else ''}")
            ...
            if 'checksums' in episode1 and 'checksums' in episode2:
                ...
                for key in episode1['checksums']:
                    match = "✓" if episode1['checksums'][key] == episode2.get('checksums', {}).get(key) else "✗"
        ...
        if reproducible:
            _logger.info("✓ SUCCESS: Episodes are identical - reproducibility confirmed!")
            _logger.info(f"  - Action sequences match: {comparison['sequence_comparisons']['actions_match']}")
            _logger.info(f"  - Reward sequences match: {comparison['sequence_comparisons']['rewards_match']}")
            _logger.info(f"  - Total rewards match: {comparison['sequence_comparisons']['total_rewards_match']}")
            
            if 'checksums_match' in comparison['sequence_comparisons']:
                _logger.info(f"  - Data checksums match: {comparison['sequence_comparisons']['checksums_match']}")
        else:
            _logger.error("✗ FAILURE: Episodes differ - reproducibility not achieved!")
            _logger.error(f"  - Validation status: {validation_status}")
            _logger.error(f"  - Number of discrepancies: {len(comparison['discrepancies'])}")
        ...
        if reproducible:
            _logger.info("✓ This environment correctly implements reproducibility standards")
        else:
...
def demonstrate_advanced_reproducibility(
    ...
    _logger.info(f"Testing {len(effective_test_seeds)} seeds with {effective_episodes_per_seed} episodes each")
    _logger.info(f"Using SeedManager: {use_seed_manager}, ReproducibilityTracker: {use_reproducibility_tracker}")
    ...
    try:
        ...
        if use_reproducibility_tracker:
            ...
            _logger.info("Initialized ReproducibilityTracker for comprehensive analysis")
        ...
        for seed in effective_test_seeds:
            ...
            for episode_num in range(effective_episodes_per_seed):
                _logger.debug(f"  Episode {episode_num + 1}/{effective_episodes_per_seed} for seed {seed}")
                
                # Create fresh policy for each episode
                policy = create_deterministic_policy(env, policy_seed=42, policy_type='random')
                ...
                if reproducibility_tracker:
                    reproducibility_tracker.record_episode(
                        ...
                        observation_sequence=episode_data.get('observation_sequence', []),
                        episode_metadata={
            ...
            for i in range(len(episodes)):
                for j in range(i + 1, len(episodes)):
                    ...
                    if comparison['episodes_identical']:
                        ...
                        _logger.warning(f"Reproducibility failure for seed {seed}: episodes {i+1} and {j+1} differ")
            
            # Verify reproducibility using ReproducibilityTracker if enabled
            if reproducibility_tracker and len(episodes) >= 2:
                tracker_verification = reproducibility_tracker.verify_episode_reproducibility(
                    episode_seed=episode_seed,
                    reference_actions=episodes[0]['action_sequence'],
                    reference_observations=episodes[0].get('observation_sequence', [])
                )
                
                seed_results['tracker_verification'] = tracker_verification
                
                if not tracker_verification['reproducible']:
                    _logger.warning(f"ReproducibilityTracker verification failed for seed {seed}")
            ...
            if episodes:
                ...
                perfect_reproducibility = (
                    seed_results['statistics']['total_rewards']['std'] == 0.0 and
                    seed_results['statistics']['total_steps']['std'] == 0.0
                )
        ...
        for i, seed1 in enumerate(effective_test_seeds):
            for seed2 in effective_test_seeds[i + 1:]:
                if (results['seed_results'][seed1]['episodes'] and 
                    results['seed_results'][seed2]['episodes']):
        ...
        success_rate = successful_reproductions / total_comparisons if total_comparisons > 0 else 0.0
        ...
        validation_status = "PASS" if (all_seeds_reproducible and success_rate >= 0.95) else "FAIL"
        ...
        if reproducibility_tracker:
            ...
            _logger.info(f"ReproducibilityTracker processed {tracker_report.get('total_episodes', 0)} episodes")
        ...
        _logger.info(f"Seeds produce different results: {seeds_produce_different_results}")
    ...
    try:
        ...
        for experiment_name in effective_names:
            ...
            for episode_num in range(effective_episodes):
                policy = create_deterministic_policy(env, policy_seed=42, policy_type='cyclic')
            ...
            _logger.info(f"  Episodes reproducible: {'✓' if episodes_reproducible else '✗'}")
        ...
        validation_status = (
            all_experiments_reproducible and 
            all_seed_generation_consistent and 
            all_seeds_unique
        )
...
def validate_cross_session_reproducibility(
    ...
    try:
        ...
        for episode_num in range(effective_num_episodes):
            ...
            policy = create_deterministic_policy(env, policy_seed=999, policy_type='spiral')
        ...
        for i, episode in enumerate(session_episodes):
            ...
            policy_verify = create_deterministic_policy(env_verify, policy_seed=999, policy_type='spiral')
        ...
        results['reproducibility_analysis'] = {
            ...
            'consistent_episodes': sum(1 for check in consistency_checks if check['consistent'])
        }
        ...
        if compare_with_previous and save_session_data:
            ...
            if session_data_dir.exists():
                ...
                previous_sessions = [f for f in previous_sessions if f.stem != f'session_{effective_session_id}']
                
                if previous_sessions:
                    # Load most recent previous session
                    latest_previous = max(previous_sessions, key=lambda f: f.stat().st_mtime)
                    ...
                    try:
                        ...
                        config_match = (
                            previous_data.get('base_seed') == effective_base_seed and
                            len(previous_data.get('episodes', [])) >= 1 and
                            len(session_episodes) >= 1
                        )
                        ...
                        if config_match and previous_data.get('episodes'):
                            ...
                            cross_session_match = (
                                prev_episode.get('total_reward') == curr_episode.get('total_reward') and
                                prev_episode.get('total_steps') == curr_episode.get('total_steps') and
                                prev_episode.get('action_sequence') == curr_episode.get('action_sequence')
                            )
                            
                            results['previous_session_comparison'] = {
                                'previous_session_found': True,
                                'previous_session_id': previous_data.get('session_id'),
                                'previous_timestamp': previous_data.get('session_timestamp'),
                                'configuration_match': config_match,
                                'cross_session_reproducible': cross_session_match
                            }
                            
                            if cross_session_match:
                                _logger.info("✓ Cross-session reproducibility confirmed")
                            else:
        ...
        if 'cross_session_reproducible' in results.get('previous_session_comparison', {}):
            validation_passed = (
                validation_passed and 
                results['previous_session_comparison']['cross_session_reproducible']
            )
        
        results['validation_summary'] = {
            'session_consistency': session_consistency,
            'validation_passed': validation_passed,
            'temporal_stability': 'verified' if session_consistency else 'issues_detected',
            'recommendations': []
        }
        ...
        _logger.info(f"Consistent episodes: {results['reproducibility_analysis']['consistent_episodes']}")
        
        if results['previous_session_comparison'].get('cross_session_reproducible') is not None:
            cross_session_status = results['previous_session_comparison']['cross_session_reproducible']
            _logger.info(f"Cross-session reproducible: {'✓' if cross_session_status else '✗'}")
...
def demonstrate_reproducibility_performance(
    ...
    try:
        ...
        _logger.info(f"Seeding overhead: {np.mean(total_times):.3f}ms (avg), {np.max(total_times):.3f}ms (max)")
        ...
        for test_num in range(min(effective_num_tests, 5)):  # Fewer for episode tests
            ...
            policy_minimal = create_deterministic_policy(env, policy_seed=42, policy_type='random')
        ...
        overhead_percentage = (tracking_overhead / np.mean(episode_times_no_tracking)) * 100
        ...
        _logger.info(f"Reproducibility tracking overhead: {tracking_overhead:.3f}ms ({overhead_percentage:.1f}%)")
        ...
        if include_memory_analysis:
            ...
            _logger.info(f"Memory usage: baseline={baseline_memory:.1f}MB, with tracking={memory_with_tracker:.1f}MB")
        ...
        if validate_performance_targets:
            ...
            tracking_overhead_acceptable = results['reproducibility_overhead']['acceptable']
            
            # Memory target: <50MB additional memory for tracking
            if include_memory_analysis:
                memory_target_met = results['memory_analysis']['total_overhead_mb'] < 50.0
            else:
            ...
            results['performance_validation'] = {
                ...
                'all_targets_met': all([seeding_target_met, tracking_overhead_acceptable, memory_target_met])
            }
        ...
        if results['seeding_performance']['statistics']['mean_ms'] > 1.0:
            recommendations.append("Consider optimizing seed validation for <1ms target")
        
        if results['reproducibility_overhead']['percentage'] > 25.0:
            recommendations.append("Reduce reproducibility tracking overhead")
        
        if include_memory_analysis and results['memory_analysis']['total_overhead_mb'] > 25.0:
            recommendations.append("Optimize memory usage in reproducibility components")
        
        if not recommendations:
            recommendations.append("Performance characteristics are within acceptable ranges")
        
        results['optimization_recommendations'] = recommendations
        
        # Log performance results
        _logger.info("=== REPRODUCIBILITY PERFORMANCE RESULTS ===")
        _logger.info(f"Average seeding time: {results['seeding_performance']['statistics']['mean_ms']:.3f}ms")
    ...
    try:
        ...
        if 'advanced_reproducibility' in test_results:
            ...
            if isinstance(advanced_result, dict):
                status = advanced_result.get('validation_summary', {}).get('overall_status', 'UNKNOWN')
                summary_items.append(f"Advanced reproducibility: {status}")
                
                if 'reproducibility_statistics' in advanced_result:
                    success_rate = advanced_result['reproducibility_statistics'].get('success_rate', 0)
        ...
        if 'cross_session' in test_results:
            ...
            if isinstance(cross_session, dict):
                validation_passed = cross_session.get('validation_summary', {}).get('validation_passed', False)
        ...
        if 'performance' in test_results:
            ...
            if isinstance(performance, dict):
                if 'performance_validation' in performance:
                    targets_met = performance['performance_validation'].get('all_targets_met', False)
        ...
        if 'basic_reproducibility' in test_results:
            ...
            if effective_format == 'markdown':
                ...
                report_lines.append("")
                report_lines.append("This test validates that identical seeds produce identical episodes.")
            elif effective_format == 'html':
                report_lines.append(f"<p><strong>Result:</strong> {result_text}</p>")
                report_lines.append("<p>This test validates that identical seeds produce identical episodes.</p>")
            else:
                report_lines.append(f"Result: {result_text}")
                report_lines.append("This test validates that identical seeds produce identical episodes.")
        ...
        if 'advanced_reproducibility' in test_results:
            ...
            if isinstance(advanced, dict):
                ...
                if 'validation_summary' in advanced:
                    ...
                    reproducibility_achieved = validation.get('reproducibility_achieved', False)
        ...
        for test_name, test_data in test_results.items():
            if isinstance(test_data, dict):
                ...
                elif 'validation_summary' in test_data and 'recommendations' in test_data['validation_summary']:
                    all_recommendations.extend(test_data['validation_summary']['recommendations'])
                elif 'optimization_recommendations' in test_data:
                    all_recommendations.extend(test_data['optimization_recommendations'])
            ...
            for key, value in test_results.items():
                if isinstance(value, dict):
                    ...
                    for sub_key, sub_value in value.items():
                        ...
                        if sub_key in ['observation_sequence', 'state_transitions', 'individual_tests']:
                            sanitized_value[sub_key] = f"<{len(sub_value)} items>" if isinstance(sub_value, list) else "<data_truncated>"
                        else:
        ...
        if include_visualizations:
            ...
            try:
                ...
                if 'performance' in test_results:
                    performance = test_results['performance']
                    if 'seeding_performance' in performance and 'individual_tests' in performance['seeding_performance']:
                        seeding_tests = performance['seeding_performance']['individual_tests']
                        
                        plt.figure(figsize=(10, 6))
                        test_nums = [t['test_num'] for t in seeding_tests]
                        total_times = [t['total_seeding'] for t in seeding_tests]
                        
                        plt.plot(test_nums, total_times, 'o-', label='Total Seeding Time')
                        ...
                        _logger.info(f"Saved seeding performance plot: {fig_dir / 'seeding_performance.png'}")
                ...
                if 'advanced_reproducibility' in test_results:
                    ...
                    if 'seed_results' in advanced:
                        ...
                        reproducible = [seed_results[seed]['reproducible'] for seed in seeds]
                        
                        plt.figure(figsize=(8, 6))
                        success_counts = [1 if r else 0 for r in reproducible]
                        plt.bar(range(len(seeds)), success_counts, 
                        ...
                        for i, (seed, repro) in enumerate(zip(seeds, reproducible)):
                            plt.text(i, 0.1 if repro else 0.9, 
                                   '✓' if repro else '✗', 
                        ...
                        _logger.info(f"Saved reproducibility plot: {fig_dir / 'reproducibility_by_seed.png'}")
...
def run_reproducibility_demo(demo_config: Optional[Dict[str, Any]] = None) -> int:
    ...
    try:
        ...
        _logger.info(f"Configuration: seeds={len(test_seeds)}, episodes_per_seed={episodes_per_seed}")
        ...
        try:
            ...
            _logger.error(f"Advanced reproducibility demonstration failed: {e}")
            all_results['advanced_reproducibility'] = {'validation_summary': {'overall_status': 'ERROR'}}
        ...
        try:
            ...
            validation_passed = string_seed_result.get('reproducibility_validation', {}).get('validation_passed', False)
            if validation_passed:
            ...
            _logger.error(f"String-based seed demonstration failed: {e}")
            all_results['string_seed_reproducibility'] = {'reproducibility_validation': {'validation_passed': False}}
        ...
        try:
            ...
            validation_passed = cross_session_result.get('validation_summary', {}).get('validation_passed', False)
            if validation_passed:
                _logger.info("✓ Cross-session reproducibility validation PASSED")
            else:
            ...
            _logger.error(f"Cross-session validation failed: {e}")
            all_results['cross_session'] = {'validation_summary': {'validation_passed': False}}
        ...
        if validate_performance:
            ...
            try:
                performance_result = demonstrate_reproducibility_performance(
                    ...
                    include_memory_analysis=config.get('include_memory_analysis', False)
                )
                all_results['performance'] = performance_result
                
                if 'performance_validation' in performance_result:
                    targets_met = performance_result['performance_validation'].get('all_targets_met', False)
                    if targets_met:
                ...
                all_results['performance'] = {'performance_validation': {'all_targets_met': False}}
        ...
        all_results['demo_duration'] = all_results['demo_end_time'] - all_results['demo_start_time']
        ...
        results_summary.append(f"Basic reproducibility: {'PASS' if basic_passed else 'FAIL'}")
        
        advanced_status = all_results.get('advanced_reproducibility', {}).get('validation_summary', {}).get('overall_status', 'UNKNOWN')
        results_summary.append(f"Advanced reproducibility: {advanced_status}")
        
        string_seed_passed = all_results.get('string_seed_reproducibility', {}).get('reproducibility_validation', {}).get('validation_passed', False)
        results_summary.append(f"String-based seeds: {'PASS' if string_seed_passed else 'FAIL'}")
        
        cross_session_passed = all_results.get('cross_session', {}).get('validation_summary', {}).get('validation_passed', False)
        results_summary.append(f"Cross-session: {'PASS' if cross_session_passed else 'FAIL'}")
        
        if validate_performance:
            performance_passed = all_results.get('performance', {}).get('performance_validation', {}).get('all_targets_met', False)
            results_summary.append(f"Performance: {'PASS' if performance_passed else 'FAIL'}")
        ...
        if overall_success:
            _logger.info("🎉 OVERALL STATUS: SUCCESS - Reproducibility fully validated!")
            _logger.info("The plume_nav_sim environment demonstrates excellent reproducibility characteristics.")
        else:
            _logger.warning("⚠ OVERALL STATUS: PARTIAL SUCCESS - Some issues detected.")
            _logger.warning("Review detailed results and recommendations for improvements.")
        
        _logger.info(f"Total demonstration time: {all_results['demo_duration']:.2f} seconds")
        ...
        _logger.error("This indicates a serious issue with the environment or demonstration script.")
        return 1
...
and performance characteristics suitable for scientific research workflows.
        ...
        def handle_unexpected_error(exc_type, exc_value, exc_traceback):
            if issubclass(exc_type, KeyboardInterrupt):
                ...
                print("This may indicate an issue with the environment or demonstration script.")
        ...
        if exit_code == 0:
            ...
            print("The plume_nav_sim environment demonstrates excellent reproducibility.")
            ...
            print("  4. Consider running with --log-level DEBUG for detailed diagnostics")
