...
def setup_visualization_logging(
    log_level: str = "INFO", include_performance_logging: bool = True
) -> None:
    """
    ...
    _logger.info(
        f"Logging configured: level={log_level}, performance_monitoring={include_performance_logging}"
    )


def demonstrate_system_capabilities(
    detailed_assessment: bool = True, test_all_backends: bool = False
) -> dict:
    """
    ...
    try:
        ...
        display_available = bool(os.environ.get("DISPLAY")) and not bool(
        ...
        current_platform_support = platform_compatibility.get(
            sys.platform, "limited_support"
        )
        ...
        if not display_available:
            recommendations.append(
                "Running in headless mode - use Agg backend for RGB array generation"
            )

        if matplotlib_caps.get("matplotlib_available", False):
            recommendations.append(
                "Matplotlib available - full visualization capabilities supported"
            )
        else:
            recommendations.append(
                "Matplotlib not available - install matplotlib>=3.9.0"
            )

        if current_platform_support == "community_support":
            recommendations.append(
                "Platform has community support - some features may be limited"
            )

        if not recommendations:
            recommendations.append(
                "System fully compatible with all visualization features"
            )
        ...
        _logger.info(
            f"Display available: {display_available}, Platform support: {current_platform_support}"
        )
        ...
        capabilities_report["recommendations"] = [
            "System assessment failed - check dependencies"
        ]
...
def demonstrate_rgb_array_rendering(
    env: gymnasium.Env,
    num_frames: int = RGB_DEMO_FRAMES,
    analyze_pixels: bool = True,
    ...
    try:
        ...
        _logger.debug(
            f"Environment reset for RGB demo, initial observation shape: {observation.shape}"
        )
        ...
        for frame_idx in range(num_frames):
            ...
            if rgb_array is not None:
                ...
                if (
                    isinstance(rgb_array, np.ndarray)
                    and rgb_array.ndim == 3
                    and rgb_array.shape[2] == 3
                    and rgb_array.dtype == np.uint8
                ):
                    ...
                    if analyze_pixels:
                        ...
                        if len(red_pixels[0]) > 0:
                            agent_y, agent_x = np.mean(red_pixels[0]), np.mean(
                                red_pixels[1]
                            )
                            rgb_results["pixel_analysis"][
                                "agent_positions_detected"
                            ].append(
                                {
                                    "frame": frame_idx,
                    ...
                    if save_frames:
                        try:
                            ...
                            filename = (
                                f"rgb_frame_{frame_idx:03d}_{int(time.time())}.png"
                            )
        ...
        if rgb_results["performance_metrics"]["frame_times_ms"]:
            rgb_results["performance_metrics"]["average_frame_time_ms"] = sum(
                rgb_results["performance_metrics"]["frame_times_ms"]
            ) / len(rgb_results["performance_metrics"]["frame_times_ms"])

        # Count total pixels analyzed
        rgb_results["performance_metrics"]["pixels_analyzed"] = len(
            rgb_results["pixel_analysis"]["agent_positions_detected"]
        ) * (
            128 * 128 if analyze_pixels else 0
        )  # Assuming default grid size

        # Log RGB array demonstration completion with performance summary and pixel analysis results
        _logger.info(
            f"RGB array demonstration completed: {rgb_results['total_frames_generated']} frames"
        )
        _logger.info(
            f"Average render time: {rgb_results['performance_metrics']['average_frame_time_ms']:.2f}ms"
        )
        _logger.info(f"Valid frames: {rgb_results['frame_validation']['valid_frames']}")

        if analyze_pixels:
            agent_detections = len(
                rgb_results["pixel_analysis"]["agent_positions_detected"]
            )
...
def demonstrate_interactive_visualization(
    env: gymnasium.Env,
    interactive_steps: int = INTERACTIVE_STEPS,
    ...
    _logger.info(
        f"Starting interactive visualization demonstration: {interactive_steps} steps"
    )
    ...
    try:
        ...
        if not display_available:
            _logger.warning(
                "No display available - demonstration will run in headless mode"
            )
        ...
        for step_idx in range(interactive_steps):
            ...
            interactive_results["performance_metrics"]["update_times_ms"].append(
                step_time
            )
        ...
        interactive_results["performance_metrics"][
            "total_visualization_time_ms"
        ] = total_viz_time

        if interactive_results["performance_metrics"]["update_times_ms"]:
            avg_update_time = sum(
                interactive_results["performance_metrics"]["update_times_ms"]
            ...
            if avg_update_time > 0:
                interactive_results["performance_metrics"]["frame_rate_fps"] = (
                    1000 / avg_update_time
                )

        # Log interactive visualization completion with performance metrics and user feedback
        _logger.info(
            f"Interactive visualization completed: {interactive_results['total_steps_completed']} steps"
        )
        _logger.info(
            f"Average update time: {interactive_results['performance_metrics']['average_update_time_ms']:.2f}ms"
        )
        _logger.info(
            f"Display updates: {interactive_results['visualization_events']['display_updates']}"
        )
...
def demonstrate_color_scheme_customization(
    env: gymnasium.Env,
    ...
    try:
        ...
        for scheme_idx, scheme_name in enumerate(scheme_names):
            ...
            try:
                ...
                color_results["performance_metrics"]["scheme_switch_times_ms"].append(
                    switch_time
                )
                ...
                render_time = (time.time() - render_start) * 1000
                color_results["performance_metrics"]["render_times_per_scheme_ms"][
                    scheme_name
                ] = render_time
                ...
                if analyze_contrast and rgb_array is not None:
                    ...
                    if len(red_pixels[0]) > 0 and len(background_pixels[0]) > 0:
                        ...
                        agent_brightness = np.mean(
                            rgb_array[red_pixels[0][:10], red_pixels[1][:10]]
                        )
                        bg_brightness = np.mean(
                            rgb_array[
                                background_pixels[0][:10], background_pixels[1][:10]
                            ]
                        )

                        contrast_ratio = (max(agent_brightness, bg_brightness) + 5) / (
                            min(agent_brightness, bg_brightness) + 5
                        )
                        color_results["visual_analysis"]["contrast_ratios"][
                            scheme_name
                        ] = contrast_ratio

                        # Simple accessibility scoring
                        accessibility_score = (
                            min(100, (contrast_ratio / 4.5) * 100)
                            if contrast_ratio > 0
                            else 0
                        )
                        color_results["visual_analysis"]["accessibility_scores"][
                            scheme_name
                        ] = accessibility_score

                # Test matplotlib integration
                colormap_name = getattr(color_scheme, "concentration_colormap", "gray")
                color_results["matplotlib_integration"]["colormaps_tested"].append(
                    colormap_name
                )
                ...
                _logger.info(
                    f"Scheme {scheme_name} tested successfully in {scheme_time:.2f}ms"
                )

            except Exception as e:
                _logger.warning(f"Color scheme {scheme_name} test failed: {e}")
                color_results["schemes_tested"].append(
                    {"name": scheme_name, "error": str(e), "successful": False}
                )
        ...
        if show_comparison and len(color_results["schemes_tested"]) > 1:
            try:
                ...
                successful_schemes = [
                    s
                    for s in color_results["schemes_tested"]
                    if s.get("successful", False)
                ]
                if len(successful_schemes) >= 2:
                    ...
                    color_results["visual_analysis"]["scheme_comparisons"] = [
                        comparison_data
                    ]
        ...
        color_results["performance_metrics"][
        ...
        _logger.info(
            f"Schemes tested successfully: {len([s for s in color_results['schemes_tested'] if s.get('successful')])}"
        )

        if analyze_contrast:
            avg_contrast = (
                np.mean(
                    list(color_results["visual_analysis"]["contrast_ratios"].values())
                )
                if color_results["visual_analysis"]["contrast_ratios"]
                else 0
            )
...
def demonstrate_backend_compatibility(
    backends_to_test: list = None,
    force_headless_test: bool = False,
    ...
    try:
        ...
        for backend_name in backends_to_test:
            ...
            try:
                ...
                if backend_name not in ["Agg", "SVG", "PDF"]:
                    ...
                    backend_test_result["interactive_capable"] = hasattr(
                        test_fig.canvas, "toolbar"
                    )
                ...
                if measure_performance:
                    ...
                    backend_test_result["performance_metrics"][
                        "average_render_time_ms"
                    ] = avg_render_time

                    # Memory usage approximation
                    backend_test_result["performance_metrics"]["memory_efficient"] = (
                        avg_render_time < 50
                    )
                ...
                backend_test_result["error_details"].append(
                    f"Functionality error: {str(e)}"
                )
                _logger.warning(
                    f"Backend {backend_name} failed functionality test: {e}"
                )

            # Calculate test duration
            backend_test_result["test_duration_ms"] = (
            ...
            backend_results["compatibility_matrix"][backend_name] = {
                ...
                "recommended": backend_test_result["functional"]
                and backend_test_result.get("interactive_capable", False),
            }
        ...
        if successful_fallback:
            backend_results["fallback_analysis"][
                "recommended_backend"
            ] = successful_fallback
        ...
        current_platform = sys.platform
        recommended_for_platform = platform_recommendations.get(
            current_platform, ["Agg"]
        )

        # Generate performance comparison and backend analysis
        if measure_performance:
            performance_data = {}
            for backend_name, backend_info in backend_results[
                "backends_tested"
            ].items():
        ...
        functional_backends = [
            name
            for name, info in backend_results["backends_tested"].items()
            if info["functional"]
        ]

        if not functional_backends:
            recommendations.append(
                "No functional backends found - check GUI dependencies"
            )
        elif len(functional_backends) == 1:
            recommendations.append(
                f"Only {functional_backends[0]} available - consider installing additional backends"
            )
        else:
            ...
            if interactive_backends:
                recommendations.append(
                    f"Interactive visualization available with: {', '.join(interactive_backends)}"
                )
        ...
        platform_compatible = [
            b for b in functional_backends if b in recommended_for_platform
        ]
        if platform_compatible:
            recommendations.append(
                f"Platform-optimized backends: {', '.join(platform_compatible)}"
            )
        ...
        _logger.info(
            f"Backend compatibility test completed: {total_functional}/{len(backends_to_test)} functional"
        )
        _logger.info(f"Recommendations: {len(recommendations)} items")
...
def demonstrate_performance_optimization(
    env: gymnasium.Env,
    performance_samples: int = PERFORMANCE_SAMPLES,
    test_memory_usage: bool = False,
    ...
    try:
        ...
        for sample_idx in range(baseline_samples):
            ...
            if bool(os.environ.get("DISPLAY")) and not bool(
                os.environ.get("SSH_CONNECTION")
            ):
            ...
            if test_memory_usage:
                ...
                performance_results["baseline_performance"]["memory_usage_mb"].append(
                    current_memory
                )
        ...
        if optimize_updates:
            ...
            performance_results["optimization_analysis"][
                "optimization_techniques"
            ] = optimization_techniques
        ...
        if (
            performance_results["baseline_performance"]["rgb_render_times_ms"]
            and performance_results["optimized_performance"]["rgb_render_times_ms"]
        ):

            baseline_rgb_avg = np.mean(
                performance_results["baseline_performance"]["rgb_render_times_ms"]
            )
            optimized_rgb_avg = np.mean(
                performance_results["optimized_performance"]["rgb_render_times_ms"]
            )

            rgb_improvement = (
                (baseline_rgb_avg - optimized_rgb_avg) / baseline_rgb_avg
            ) * 100
            performance_results["optimization_analysis"][
                "rgb_improvement_percent"
            ] = rgb_improvement

        if (
            performance_results["baseline_performance"]["human_render_times_ms"]
            and performance_results["optimized_performance"]["human_render_times_ms"]
        ):

            baseline_human_avg = np.mean(
                performance_results["baseline_performance"]["human_render_times_ms"]
            )
            optimized_human_avg = np.mean(
                performance_results["optimized_performance"]["human_render_times_ms"]
            )

            human_improvement = (
                (baseline_human_avg - optimized_human_avg) / baseline_human_avg
            ) * 100
            performance_results["optimization_analysis"][
                "human_improvement_percent"
            ] = human_improvement
        ...
        if test_memory_usage:
            ...
            if (
                performance_results["baseline_performance"]["memory_usage_mb"]
                and performance_results["optimized_performance"]["memory_usage_mb"]
            ):

                baseline_memory_avg = np.mean(
                    performance_results["baseline_performance"]["memory_usage_mb"]
                )
                optimized_memory_avg = np.mean(
                    performance_results["optimized_performance"]["memory_usage_mb"]
                )

                memory_improvement = (
                    (baseline_memory_avg - optimized_memory_avg) / baseline_memory_avg
                ) * 100
                performance_results["optimization_analysis"][
        ...
        if performance_results["optimization_analysis"]["rgb_improvement_percent"] > 10:
            recommendations.append(
                f"RGB rendering optimized by {performance_results['optimization_analysis']['rgb_improvement_percent']:.1f}%"
            )

        if (
            performance_results["optimization_analysis"]["human_improvement_percent"]
            > 10
        ):
            recommendations.append(
                f"Human rendering optimized by {performance_results['optimization_analysis']['human_improvement_percent']:.1f}%"
            )
        ...
        if performance_results["baseline_performance"]["rgb_render_times_ms"]:
            avg_rgb_time = np.mean(
                performance_results["baseline_performance"]["rgb_render_times_ms"]
            )
            if avg_rgb_time > 5:  # 5ms target
                recommendations.append(
                    f"RGB rendering ({avg_rgb_time:.1f}ms) exceeds 5ms target - consider optimization"
                )

        if performance_results["baseline_performance"]["human_render_times_ms"]:
            avg_human_time = np.mean(
                performance_results["baseline_performance"]["human_render_times_ms"]
            )
            if avg_human_time > 50:  # 50ms target
                recommendations.append(
                    f"Human rendering ({avg_human_time:.1f}ms) exceeds 50ms target - consider optimization"
                )
        ...
        if performance_results["optimization_analysis"]["rgb_improvement_percent"] > 0:
            _logger.info(
                f"RGB rendering improvement: {performance_results['optimization_analysis']['rgb_improvement_percent']:.1f}%"
            )
        ...
        if (
            ...
            _logger.info(
                f"Human rendering improvement: {performance_results['optimization_analysis']['human_improvement_percent']:.1f}%"
            )

        _logger.info(
            f"Peak memory usage: {performance_results['resource_monitoring']['peak_memory_mb']:.2f} MB"
        )
...
def demonstrate_advanced_features(
    ...
    demonstrate_custom_markers: bool = False,
) -> dict:
    """
    ...
    try:
        ...
        if not display_available:
            _logger.warning(
                "No display available - some advanced features may be limited"
            )
        ...
        if save_publication_figures:
            ...
            try:
                ...
                formats_to_test = (
                    ["png", "pdf", "svg"] if save_publication_figures else ["png"]
                )
                ...
                for format_name in formats_to_test:
                    ...
                    if rgb_array is not None:
                        ...
                        ax.set_title(
                            "Plume Navigation Visualization",
                            fontsize=14,
                            fontweight="bold",
                        )
                        ...
                        advanced_results["performance_metrics"][
                            "figure_save_times_ms"
                        ].append(save_time)
                        advanced_results["output_files"]["figures_saved"].append(
                            filename
                        )
                        advanced_results["publication_quality"][
                            "formats_tested"
                        ].append(format_name)
                        ...
                        _logger.info(
                            f"Saved {format_name.upper()} figure: {filename} ({save_time:.1f}ms)"
                        )

                    plt.close(fig)

                advanced_results["publication_quality"]["dpi_settings"][
                    "high_quality"
                ] = 300
                advanced_results["publication_quality"]["size_configurations"][
                    "publication"
                ] = (8, 6)
            ...
            try:
                ...
                if trajectory_data:
                    ...
                    def animate_frame(frame_idx):
                        ...
                        ax.set_title(
                            f'Step {frame_data["step"]}, Reward: {frame_data["reward"]:.3f}'
                        )
                    ...
                    try:
                        ...
                        advanced_results["output_files"]["animations_created"].append(
                            animation_filename
                        )
        ...
        if demonstrate_custom_markers:
            ...
            try:
                ...
                for step in range(4):
                    ...
                    advanced_results["advanced_customization"][
                        "custom_markers_created"
                    ] += 1
                ...
                advanced_results["output_files"]["custom_outputs"].append(
                    custom_filename
                )
        ...
        try:
            ...
            advanced_results["advanced_customization"]["integration_examples"].append(
                "multi_panel_layout"
            )

            plt.close(fig)
            _logger.info(
                f"Matplotlib integration example saved: {integration_filename}"
            )
        ...
        advanced_results["export_capabilities"]["supported_formats"] = [
            "PNG",
            "PDF",
            "SVG",
            "GIF",
        ...
        _logger.info(
            f"Advanced features demonstration completed in {total_processing_time:.2f}ms"
        )
        _logger.info(f"Features tested: {len(advanced_results['features_tested'])}")
        _logger.info(
            f"Output files created: {len(advanced_results['output_files']['figures_saved']) + len(advanced_results['output_files']['animations_created']) + len(advanced_results['output_files']['custom_outputs'])}"
        )
...
def run_comprehensive_visualization_demo(
    ...
    try:
        ...
        _logger.info(
            f"Save results: {save_results}, Interactive mode: {interactive_mode}"
        )
        ...
        capabilities = demonstrate_system_capabilities(
            detailed_assessment=True, test_all_backends=True
        )
        demonstration_results["system_info"] = capabilities

        # Check if system is capable of running demonstrations
        if not capabilities.get("matplotlib_capabilities", {}).get(
            "matplotlib_available", False
        ):
            _logger.error(
                "Matplotlib not available - cannot run visualization demonstrations"
            )
            return 1
        ...
        try:
            ...
            if "rgb_array_rendering" in components_to_run:
                ...
                demonstration_results["component_results"][
                    "rgb_array_rendering"
                ] = rgb_results
            ...
            if "interactive_visualization" in components_to_run and interactive_mode:
                ...
                if "error" in interactive_results:
                    _logger.warning(
                        f"Interactive visualization had issues: {interactive_results['error']}"
                    )
                else:
                    _logger.info(
                        "Interactive visualization demonstration completed successfully"
                    )
            ...
            if "color_scheme_customization" in components_to_run:
                ...
                if "error" in color_results:
                    _logger.warning(
                        f"Color scheme demonstration had issues: {color_results['error']}"
                    )
                else:
                    _logger.info(
                        "Color scheme customization demonstration completed successfully"
                    )
            ...
            if "backend_compatibility" in components_to_run:
                ...
                if "error" in backend_results:
                    _logger.warning(
                        f"Backend compatibility had issues: {backend_results['error']}"
                    )
                else:
                    _logger.info(
                        "Backend compatibility demonstration completed successfully"
                    )
            ...
            if "performance_optimization" in components_to_run:
                ...
                if "error" in performance_results:
                    _logger.warning(
                        f"Performance optimization had issues: {performance_results['error']}"
                    )
                else:
                    _logger.info(
            ...
            if "advanced_features" in components_to_run:
                ...
                if "error" in advanced_results:
                    _logger.warning(
                        f"Advanced features had issues: {advanced_results['error']}"
                    )
                else:
                    _logger.info(
        ...
        demonstration_results["overall_performance"][
            "total_demo_time_seconds"
        ] = total_demo_time
        demonstration_results["overall_performance"]["components_run"] = len(
            components_to_run
        )

        successful_components = [
            name
            for name, results in demonstration_results["component_results"].items()
            if "error" not in results
        ]
        demonstration_results["overall_performance"]["successful_components"] = len(
            successful_components
        )
        demonstration_results["overall_performance"]["success_rate"] = (
            len(successful_components) / len(components_to_run) * 100
        ...
        if demonstration_results["overall_performance"]["success_rate"] == 100:
            ...
            recommendations.append(
                "Most demonstrations successful - check warnings for minor issues"
            )
        else:
            recommendations.append(
                "Some demonstrations failed - check system configuration and dependencies"
            )
        ...
        if save_results:
            recommendations.append(
                "Output files saved - check current directory for demonstration results"
            )
        ...
        _logger.info(
            f"Success rate: {demonstration_results['overall_performance']['success_rate']:.1f}%"
        )
