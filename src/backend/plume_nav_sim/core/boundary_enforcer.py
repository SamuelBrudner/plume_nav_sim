...
BOUNDARY_ENFORCEMENT_PERFORMANCE_TARGET_MS = 0.1  # Performance target for boundary operations
...
POSITION_BOUNDS_TOLERANCE = 0  # Tolerance for position bounds checking (strict by default)
...
class BoundaryEnforcementResult:
    ...
    constraint_direction: Tuple[int, int] = field(init=False)  # Direction of constraint application
    enforcement_time: float = field(default=0.0, init=False)  # Time taken for enforcement operation
    constraint_reason: str = field(default="", init=False)    # Detailed reason for constraint application
    ...
    def __post_init__(self):
        ...
        if self.boundary_hit and self.position_modified:
            if dx != 0 and dy != 0:
                object.__setattr__(self, 'constraint_reason', 'Corner boundary constraint applied')
            elif dx != 0:
                object.__setattr__(self, 'constraint_reason', 'X-axis boundary constraint applied')
            elif dy != 0:
                object.__setattr__(self, 'constraint_reason', 'Y-axis boundary constraint applied')
            else:
                object.__setattr__(self, 'constraint_reason', 'Position maintained within bounds')
        else:
            object.__setattr__(self, 'constraint_reason', 'No boundary constraint required')
    ...
    def get_constraint_analysis(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'original_coordinates': (self.original_position.x, self.original_position.y),
...
class MovementConstraint:
    ...
    enable_clamping: bool = DEFAULT_CLAMPING_ENABLED      # Enable position clamping to bounds
    ...
    tolerance: float = field(default=POSITION_BOUNDS_TOLERANCE)  # Bounds checking tolerance
    custom_boundaries: Dict[str, Any] = field(default_factory=dict)  # Custom boundary rules
...
class BoundaryEnforcer:
    ...
    def validate_position(
        ...
        try:
            ...
            if self.constraint_config.strict_validation and is_valid:
                ...
                if (coords.x < tolerance or 
                    coords.x >= self.grid_size.width - tolerance or
                    coords.y < tolerance or 
                    coords.y >= self.grid_size.height - tolerance):
            ...
            if context_info and not is_valid:
                self.logger.debug(f"Position validation failed: {context_info}, position={coords}")
        ...
        try:
            ...
            if is_valid_movement:
                ...
                if self.constraint_config.enable_clamping:
                    ...
                    final_position = clamp_coordinates_to_bounds(proposed_position, self.grid_size)
                    position_modified = (final_position.x != coords.x or final_position.y != coords.y)
                else:
    ...
    def update_grid_size(self, new_grid_size: GridSize) -> None:
        ...
        self.logger.info(
            f"Updating grid size from {old_grid} to {new_grid_size}"
        )
    ...
    def get_boundary_statistics(self) -> Dict[str, Any]:
        ...
        hit_rate = (self.boundary_hits / total_operations) if total_operations > 0 else 0.0
        ...
        if self.enable_caching:
            ...
            stats['cache_statistics'] = {
                ...
                'hit_ratio': cache_hits / total_cache_attempts if total_cache_attempts > 0 else 0.0,
        ...
        if self.performance_metrics['validation_times']:
            ...
            stats['validation_metrics'] = {
                ...
                'validation_efficiency': len(validation_times) / max(total_operations, 1)
            }
    ...
    def validate_constraint_configuration(self, strict_mode: bool = False) -> bool:
        ...
        try:
            ...
            if self.constraint_config.tolerance >= min(self.grid_size.width, self.grid_size.height) / 2:
                if strict_mode:
                    raise ValidationError(
            ...
            if strict_mode:
                ...
                if self.constraint_config.custom_boundaries:
                    for boundary_name, boundary_config in self.constraint_config.custom_boundaries.items():
...
def validate_movement_bounds(
    ...
    try:
        ...
        if (new_x < 0 or new_x >= grid_bounds.width or 
            new_y < 0 or new_y >= grid_bounds.height):
            return False
        ...
        if strict_validation:
            ...
            if (new_x == 0 or new_x == grid_bounds.width - 1 or 
                new_y == 0 or new_y == grid_bounds.height - 1):
...
def calculate_bounded_movement(
    ...
    try:
        ...
        is_within_bounds = (
            0 <= proposed_x < grid_bounds.width and
            0 <= proposed_y < grid_bounds.height
        )
