...
from .constants import MIN_PLUME_SIGMA, MAX_PLUME_SIGMA
...
class PlumeModel:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.source_location, Coordinates):
            raise ValidationError(f"Source location must be a Coordinates instance, got {type(self.source_location).__name__}")
        
        if not (MIN_PLUME_SIGMA <= self.sigma <= MAX_PLUME_SIGMA):
            raise ValidationError(f"Plume sigma {self.sigma} is out of the valid range [{MIN_PLUME_SIGMA}, {MAX_PLUME_SIGMA}]")

        if self.grid_compatibility is not None:
            if not isinstance(self.grid_compatibility, GridSize):
                raise ValidationError(f"Grid compatibility must be a GridSize instance, got {type(self.grid_compatibility).__name__}")
            if not self.grid_compatibility.contains_coordinates(self.source_location):
                raise ValidationError(f"Source location {self.source_location.to_tuple()} is outside grid bounds {self.grid_compatibility.to_tuple()}")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'sigma': self.sigma,
            'grid_compatibility': self.grid_compatibility.to_tuple() if self.grid_compatibility else None
        }
