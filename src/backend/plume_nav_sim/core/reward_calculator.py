...
from typing import Optional, Dict, Any, Tuple, List, Union  # >=3.10 - Type hints for reward calculation methods, parameter specifications, and return value annotations
from dataclasses import dataclass, field  # >=3.10 - Data class utilities for reward calculation configuration and result data structures
...
class RewardCalculatorConfig:
    ...
    distance_calculation_method: str = field(default=DEFAULT_DISTANCE_CALCULATION_METHOD)
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.goal_radius, (int, float)) or self.goal_radius < 0:
            raise ValidationError("goal_radius must be non-negative numeric value", 
        ...
        if not isinstance(self.reward_goal_reached, (int, float)):
            raise ValidationError("reward_goal_reached must be numeric value",
        ...
        if not isinstance(self.reward_default, (int, float)):
            raise ValidationError("reward_default must be numeric value",
        ...
        if not isinstance(self.distance_precision, (int, float)) or self.distance_precision <= 0:
    ...
    def validate(self, strict_mode: bool = False, validation_context: Optional[Dict] = None) -> bool:
        """
        ...
        if self.goal_radius < 0 or self.goal_radius > 1000:
            raise ValidationError("goal_radius must be between 0 and 1000 for practical goal detection",
                                parameter_name="goal_radius",
                                parameter_value=self.goal_radius,
        ...
        if abs(self.reward_goal_reached - self.reward_default) < DISTANCE_PRECISION:
            raise ValidationError("reward_goal_reached must differ from reward_default for learning signal",
        ...
        valid_methods = ['euclidean', 'manhattan', 'chebyshev'] 
        if self.distance_calculation_method not in valid_methods:
            raise ValidationError("distance_calculation_method must be supported calculation method",
        ...
        if self.distance_precision <= 0 or self.distance_precision > 1e-3:
            raise ValidationError("distance_precision must be positive and sufficiently small",
        ...
        if strict_mode:
            ...
            if self.goal_radius > 0 and self.goal_radius < self.distance_precision * 100:
                raise ValidationError("goal_radius too small relative to distance_precision",
            ...
            if self.custom_parameters:
                for param_name, param_value in self.custom_parameters.items():
                    if isinstance(param_value, (int, float)) and not math.isfinite(param_value):
                        raise ValidationError(f"custom parameter {param_name} must be finite",
    ...
    def estimate_performance_impact(self, expected_episode_length: int, 
                                  include_caching_benefits: bool = True) -> Dict[str, Any]:
        """
        ...
        if self.distance_precision < 1e-10:
            precision_overhead_factor = 1.2  # Higher precision requires more computation
        
        return {
            'per_step_calculation_ms': per_step_time_ms * precision_overhead_factor,
            'total_episode_overhead_ms': total_episode_overhead_ms * precision_overhead_factor,
            'caching_benefit_factor': caching_benefit_factor,
            'performance_target_compliance': per_step_time_ms < PERFORMANCE_TARGET_STEP_LATENCY_MS / 2,
            'optimization_recommendations': [
                'Enable caching for repeated calculations' if not self.enable_caching else 'Caching already enabled',
                'Consider reducing precision if performance critical' if self.distance_precision < 1e-12 else 'Precision appropriate',
                'Use euclidean distance for best balance' if self.distance_calculation_method != 'euclidean' else 'Optimal method selected'
            ]
        }
    
    def clone(self, overrides: Optional[Dict] = None, preserve_custom_parameters: bool = True) -> 'RewardCalculatorConfig':
        """
        ...
        cloned_config = RewardCalculatorConfig(
            ...
            custom_parameters=self.custom_parameters.copy() if preserve_custom_parameters else {}
        )
    ...
    def to_dict(self, include_performance_estimates: bool = False) -> Dict[str, Any]:
        ...
        if include_performance_estimates:
            config_dict['performance_estimates'] = self.estimate_performance_impact(1000, True)
...
class RewardResult:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.reward, (int, float)) or not math.isfinite(self.reward):
            raise ValidationError("reward must be finite numeric value",
                                parameter_name="reward",
                                parameter_value=self.reward)
        
        # Validate goal_reached is boolean
        if not isinstance(self.goal_reached, bool):
            raise ValidationError("goal_reached must be boolean value",
                                parameter_name="goal_reached", 
                                parameter_value=self.goal_reached)
        
        # Validate distance_to_goal is non-negative numeric
        if not isinstance(self.distance_to_goal, (int, float)) or self.distance_to_goal < 0:
            raise ValidationError("distance_to_goal must be non-negative numeric value",
                                parameter_name="distance_to_goal",
                                parameter_value=self.distance_to_goal)
    
    def set_performance_metrics(self, calculation_time_ms: float, 
                              calculation_details: Optional[Dict] = None) -> None:
        """
...
class TerminationResult:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.terminated, bool):
            raise ValidationError("terminated must be boolean value",
                                parameter_name="terminated",
                                parameter_value=self.terminated)
        
        if not isinstance(self.truncated, bool):  
            raise ValidationError("truncated must be boolean value",
        ...
        if not self.termination_reason or not isinstance(self.termination_reason, str):
            raise ValidationError("termination_reason must be non-empty string",
                                parameter_name="termination_reason",
                                parameter_value=self.termination_reason)
    
    def set_final_state(self, final_distance: float, final_step_count: int,
                       additional_details: Optional[Dict] = None) -> None:
        """
...
class RewardCalculator:
    ...
    def __init__(self, config: RewardCalculatorConfig, 
        ...
        if not isinstance(config, RewardCalculatorConfig):
            raise ValidationError("config must be RewardCalculatorConfig instance",
        ...
        self.logger.info(f"RewardCalculator initialized with config: goal_radius={config.goal_radius}, "
                        f"reward_goal={config.reward_goal_reached}, reward_default={config.reward_default}")
    
    @monitor_performance('reward_calculation', PERFORMANCE_TARGET_STEP_LATENCY_MS / 2, False)
    def calculate_reward(self, agent_position: Coordinates, source_location: Coordinates,
                        calculation_context: Optional[Dict] = None) -> RewardResult:
        """
        ...
        try:
            ...
            distance = self.get_distance_to_goal(agent_position, source_location, 
            ...
            reward = self.config.reward_goal_reached if goal_reached else self.config.reward_default
            ...
            if self.config.enable_performance_monitoring:
                result.set_performance_metrics(calculation_time_ms, {
                ...
                if self.performance_metrics:
                    self.performance_metrics.record_timing('reward_calculation', calculation_time_ms)
            ...
            if self.logger.isEnabledFor(logging.DEBUG):
                self.logger.debug(f"Reward calculated: reward={reward}, goal_reached={goal_reached}, "
    ...
    def check_termination(self, agent_state: AgentState, source_location: Coordinates,
                         max_steps: int, termination_context: Optional[Dict] = None) -> TerminationResult:
        """
        ...
        try:
            ...
            if not hasattr(agent_state, 'position') or not isinstance(agent_state.position, Coordinates):
                raise ValidationError("agent_state must have valid position coordinates",
            ...
            current_distance = self.get_distance_to_goal(agent_state.position, source_location)
            ...
            if goal_achieved:
                ...
                termination_reason = f"Step limit reached: {agent_state.step_count} >= {max_steps}"
            else:
            ...
            result.set_final_state(current_distance, agent_state.step_count, {
                'goal_radius': self.config.goal_radius,
                'max_steps': max_steps,
            ...
            self.logger.info(f"Termination check: terminated={terminated}, truncated={truncated}, "
    ...
    def update_agent_reward(self, agent_state: AgentState, reward_result: RewardResult,
                           update_goal_status: bool = True) -> None:
        """
        ...
        try:
            ...
            if not hasattr(agent_state, 'add_reward'):
                raise ValidationError("agent_state must have add_reward method",
                                    parameter_name="agent_state",
                                    parameter_value=type(agent_state))
            
            if not isinstance(reward_result, RewardResult):
                raise ValidationError("reward_result must be RewardResult instance",
            ...
            if self.logger.isEnabledFor(logging.DEBUG):
                self.logger.debug(f"Agent reward updated: reward={reward_result.reward}, "
    ...
    def get_distance_to_goal(self, agent_position: Coordinates, goal_position: Coordinates,
                           use_cache: bool = True, high_precision: bool = False) -> float:
        """
        ...
        if self.config.distance_calculation_method == 'euclidean':
            ...
            distance = abs(agent_position.x - goal_position.x) + abs(agent_position.y - goal_position.y)
        elif self.config.distance_calculation_method == 'chebyshev':
            distance = max(abs(agent_position.x - goal_position.x), abs(agent_position.y - goal_position.y))
        else:
    ...
    def get_reward_statistics(self, include_performance_analysis: bool = True,
                            include_cache_statistics: bool = True) -> Dict[str, Any]:
        """
        ...
        stats = {
            ...
            'performance_violations': self.goal_achievement_stats['performance_violations'],
            'configuration': {
                'goal_radius': self.config.goal_radius,
        ...
        if self.calculation_count > 0:
            stats['goal_achievement_rate'] = self.goal_achievement_stats['goals_achieved'] / self.calculation_count
        ...
        if include_cache_statistics and self.config.enable_caching:
            ...
            stats['cache_statistics'] = {
                ...
                'cache_utilization': cache_size / cache_capacity if cache_capacity > 0 else 0.0
            }
        ...
        if include_performance_analysis:
            ...
            if self.calculation_count > 0:
                performance_compliance_rate = 1.0 - (self.goal_achievement_stats['performance_violations'] / self.calculation_count)
    ...
    def validate_calculation_parameters(self, agent_position: Coordinates, source_location: Coordinates,
        ...
        try:
            ...
            distance = abs(agent_position.x - source_location.x) + abs(agent_position.y - source_location.y)
            if distance < self.config.distance_precision:
                self.logger.warning(f"Agent and source very close: distance={distance}, precision={self.config.distance_precision}")
            ...
            if validation_context:
                ...
                if max_distance and distance > max_distance:
                    raise ValidationError(f"Distance {distance} exceeds maximum {max_distance}",
            ...
            raise ValidationError(f"Parameter validation failed: {e}",
                                ...
                                parameter_value=(agent_position, source_location)) from e
    
    def _update_statistics(self, distance: float, goal_reached: bool, calculation_time_ms: float) -> None:
        """
        ...
        self.goal_achievement_stats['average_distance'] = (current_avg * (current_count - 1) + distance) / current_count
...
def create_reward_calculator(config: Optional[RewardCalculatorConfig] = None,
                           enable_performance_monitoring: bool = True,
                           enable_validation: bool = True) -> RewardCalculator:
    """
    ...
    performance_metrics = PerformanceMetrics() if enable_performance_monitoring else None
    ...
    if enable_validation:
        ...
        reward_calculator.validate_calculation_parameters(test_agent_pos, test_source_pos)
    
    return reward_calculator


def validate_reward_config(config: RewardCalculatorConfig, strict_validation: bool = False,
                         validation_context: Optional[Dict] = None) -> Tuple[bool, Dict[str, Any]]:
    """
    ...
    try:
        ...
        if not isinstance(config.goal_radius, (int, float)) or config.goal_radius < 0:
            ...
            validation_report['errors'].append("goal_radius must be non-negative numeric value")
        elif config.goal_radius > 100:
            validation_report['warnings'].append("Large goal_radius may affect learning convergence")
        
        # Validate reward values are proper floats with consistency checking
        if not isinstance(config.reward_goal_reached, (int, float)):
            validation_report['is_valid'] = False
            validation_report['errors'].append("reward_goal_reached must be numeric value")
        ...
        if config.goal_radius > 0 and config.goal_radius < config.distance_precision * 100:
            validation_report['warnings'].append("goal_radius may be too small relative to distance_precision")
        ...
        if not performance_estimate['performance_target_compliance']:
            validation_report['warnings'].append("Configuration may not meet performance targets")
        
        # Apply strict validation rules if strict_validation enabled
        if strict_validation:
            if abs(config.reward_goal_reached - config.reward_default) < DISTANCE_PRECISION:
                validation_report['is_valid'] = False
                validation_report['errors'].append("Reward values too similar - insufficient learning signal")
        
        # Generate optimization recommendations
        validation_report['recommendations'].extend([
            "Enable caching for better performance" if not config.enable_caching else "Caching enabled",
            "Consider euclidean distance for optimal balance" if config.distance_calculation_method != 'euclidean' else "Optimal distance method",
            "Performance monitoring recommended" if not config.enable_performance_monitoring else "Performance monitoring enabled"
        ])
