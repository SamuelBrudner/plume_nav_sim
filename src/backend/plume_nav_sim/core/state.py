...
class AgentState:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.position, Coordinates):
            raise ValidationError(f"Agent position must be Coordinates instance, got {type(self.position).__name__}")
    ...
    def update_position(self, new_position: Coordinates, record_history: bool = True) -> None:
        ...
        if not isinstance(new_position, Coordinates):
            raise ValidationError(f"New position must be Coordinates instance, got {type(new_position).__name__}")
    ...
    def add_reward(self, reward: float, validate_reward: bool = True) -> None:
        ...
        if validate_reward and not isinstance(reward, (int, float)):
            raise ValidationError(f"Reward must be numeric, got {type(reward).__name__}")
    ...
    def calculate_trajectory_length(self) -> float:
        ...
        for i in range(len(self.movement_history) - 1):
            total_distance += self.movement_history[i].distance_to(self.movement_history[i + 1])
...
class EpisodeState:
    ...
    def get_episode_summary(self, include_trajectory_analysis: bool = False, include_performance_metrics: bool = False) -> Dict[str, Any]:
    ...
    def reset(self, new_agent_state: AgentState, preserve_episode_id: bool = False) -> None:
        ...
        if not isinstance(new_agent_state, AgentState):
            raise ValidationError(f"New agent state must be an AgentState instance, got {type(new_agent_state).__name__}")
