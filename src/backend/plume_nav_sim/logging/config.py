...
from enum import (  # >=3.10 - Enumeration definitions for log levels and component types
    Enum,
)
from functools import wraps  # >=3.10 - Decorator utilities for performance monitoring
from pathlib import (  # >=3.10 - Path handling for log directory creation and cross-platform compatibility
    Path,
)
from typing import (  # >=3.10 - Type hints for configuration parameters
    Any,
    Dict,
    List,
    Optional,
...
class LogLevel(Enum):
    ...
    def from_string(cls, level_string: str) -> "LogLevel":
        ...
        try:
            ...
            raise ValueError(
                f"Invalid log level '{level_string}'. Valid levels: {valid_levels}"
            )
...
class LoggingConfig:
    ...
    sensitive_information_patterns: List[str] = field(
        default_factory=lambda: SENSITIVE_PATTERNS.copy()
    )
    component_specific_levels: Dict[ComponentType, LogLevel] = field(
    ...
    def to_dict_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "handlers": {},
            "loggers": {},
            "root": {"level": self.log_level.get_numeric_level(), "handlers": []},
        }
        ...
        if self.console_logging_enabled:
            config["handlers"]["console"] = {
                ...
                "formatter": (
                    "development" if self.log_level == LogLevel.DEBUG else "standard"
                ),
    ...
    def update_component_level(
        self, component_type: ComponentType, new_level: LogLevel
    ) -> bool:
        """
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    duration_ms = (time.perf_counter() - start_time) * 1000
                    self.performance(
                        operation_name, duration_ms, status="error", error=str(e)
                    )
                    raise
...
class LoggerFactory:
    ...
    def __init__(self, config: LoggingConfig, enable_caching: bool = True):
        ...
        self.sensitive_filter = SensitiveInfoFilter(
            config.sensitive_information_patterns
        )
    ...
    def create_logger(
        ...
        with self.factory_lock:
            ...
            effective_level = (
                log_level
                or self.logging_config.component_specific_levels.get(
            ...
            enable_performance = (
                self.logging_config.performance_logging_enabled
    ...
    def get_factory_statistics(self) -> Dict[str, Any]:
        ...
        with self.factory_lock:
            cache_hit_rate = (
                self.cache_hits / max(1, self.creation_count + self.cache_hits)
            ) * 100
            ...
            return {
                ...
                "memory_usage_estimate_kb": (
                    len(self.logger_cache) * 1024
                    + len(self.performance_loggers) * 512  # Rough estimate
...
def configure_logging(
    ...
    with _config_lock:
        try:
            ...
            if not is_valid:
                print(
                    f"Logging configuration validation failed: {errors}",
                    file=sys.stderr,
                )
                return False
            ...
            root_logger.info(
                f"Logging system initialized successfully with level {log_level.value}"
            )
...
def setup_performance_logging(
    timing_threshold_ms: float = PERFORMANCE_TARGET_STEP_LATENCY_MS,
    enable_memory_tracking: bool = False,
    ...
    perf_logger.info(
        f"Performance logging initialized with {timing_threshold_ms}ms threshold"
    )
...
def create_component_logger(
    ...
    logger.debug(
        f"Component logger created for {component_type.value}:{component_name}"
    )
    ...
    try:
        ...
        if strict_validation:
            ...
            if config.log_backup_count > 10:
                validation_errors.append(
                    "High backup count may consume excessive disk space"
                )
...
def get_logging_status(
    ...
    include_logger_hierarchy: bool = False,
) -> Dict[str, Any]:
    """
    ...
    if include_performance_stats and _global_factory:
        ...
        for op_name, data in _global_factory.performance_loggers.items():
            if "logger" in data and hasattr(data["logger"], "performance_data"):
                perf_data[op_name] = {
                    "operation_count": len(
                        data["logger"].performance_data.get(op_name, [])
                    ),
