...
from typing import Optional, Union, Dict, Any, List, Tuple  # >=3.10 - Type hints for method parameters, return types, and optional parameter specifications
...
class FieldGenerationError(ComponentError):
    ...
    def _generate_recovery_suggestion(self) -> str:
        ...
        if 'sigma' in self.generation_params:
            ...
            if sigma <= 0:
                return "Ensure sigma parameter is positive for valid Gaussian calculations"
            elif sigma < GAUSSIAN_PRECISION:
                return f"Increase sigma above {GAUSSIAN_PRECISION} for numerical stability"
    ...
    def get_recovery_suggestions(self) -> List[str]:
        ...
        if self.grid_size:
            ...
            if memory_mb > MEMORY_LIMIT_PLUME_FIELD_MB:
                suggestions.append(f"Reduce grid size to stay within {MEMORY_LIMIT_PLUME_FIELD_MB}MB limit")
        ...
        if self.generation_params:
            if 'sigma' in self.generation_params:
                sigma = self.generation_params['sigma']
                if sigma <= 0:
                    suggestions.append("Set sigma to positive value (recommended: 8.0-16.0)")
                elif sigma > 50:
...
class FieldSamplingError(ComponentError):
    ...
    def _serialize_position(self, position: Optional[CoordinateType]) -> Optional[Dict[str, Any]]:
    ...
    def get_position_analysis(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'sampling_method': self.sampling_method,
            'analysis_timestamp': np.float64(functools.lru_cache.cache_info)  # Mock timestamp
        }
        ...
        analysis['sampling_method_valid'] = self.sampling_method in _INTERPOLATION_METHODS
        
        # Generate position correction recommendations
        recommendations = []
        if not analysis.get('bounds_checkable', False):
            recommendations.append("Convert position to Coordinates object or (x, y) tuple")
        ...
        if not analysis.get('sampling_method_valid', False):
            recommendations.append(f"Use valid sampling method: {_INTERPOLATION_METHODS[0]}")
...
class ConcentrationField:
    ...
    def __init__(self, grid_size: GridSize, enable_caching: bool = True):
        ...
        try:
            validate_grid_size(grid_size, check_memory_limits=True, check_performance_feasibility=True)
            self.grid_size = grid_size
    ...
    def generate_field(
        ...
        if self.is_generated and not force_regeneration and self.field_array is not None:
        ...
        with PerformanceTimer("field_generation", logger=self.logger, auto_log=True) as timer:
            try:
                ...
                array_memory_mb = (distance_squared.nbytes + x_mesh.nbytes + y_mesh.nbytes) / (1024 * 1024)
                ...
                if normalize_field:
                    ...
                    if field_max > GAUSSIAN_PRECISION:
                        ...
                        source_idx_y, source_idx_x = source_location.to_array_index(self.grid_size)
                ...
                if timer.duration_ms is not None:
                    ...
                    if self.last_generation_time_ms > PERFORMANCE_TARGET_PLUME_GENERATION_MS:
                        self.logger.warning(
    ...
    def get_field_array(
        ...
        if validate_field:
            ...
            if not validation_result['is_valid']:
                self.logger.warning(f"Field validation found issues: {validation_result['issues']}")
    ...
    def update_field(
        ...
        try:
            ...
            if new_source_location is not None:
                ...
                if old_location is None or (old_location.x != new_source_location.x or 
                                           old_location.y != new_source_location.y):
                    params_changed = True
    ...
    def validate_field(
        ...
        try:
            ...
            if self.field_array is None:
                ...
                validation_result['issues'].append("Field array is None - field not generated")
            ...
            if nan_count > 0:
                validation_result['is_valid'] = False
                validation_result['issues'].append(f"Field contains {nan_count} NaN values")
            
            if inf_count > 0:
                validation_result['is_valid'] = False
                validation_result['issues'].append(f"Field contains {inf_count} infinite values")
            ...
            if check_memory_usage:
                ...
                if self.enable_caching:
                    cache_memory_estimate = len(self.sampling_cache) * 8  # Rough estimate
            ...
            if len(validation_result['warnings']) == 0 and len(validation_result['issues']) == 0:
                validation_result['recommendations'].append("Field validation passed all checks")
            else:
                if field_memory_mb > _FIELD_MEMORY_WARNING_THRESHOLD:
                    validation_result['recommendations'].append(
                        "Consider reducing grid size to lower memory usage"
                    )
                if self.last_generation_time_ms and self.last_generation_time_ms > PERFORMANCE_TARGET_PLUME_GENERATION_MS:
                    validation_result['recommendations'].append(
            ...
            validation_result['is_valid'] = False
            validation_result['issues'].append(f"Field validation failed with error: {e}")
    ...
    def get_field_statistics(
        ...
        if include_distribution_analysis:
            ...
            stats['distribution_analysis'] = {
                ...
                'concentration_above_50%': float(np.sum(self.field_array > 0.5) / self.field_array.size),
                'concentration_above_10%': float(np.sum(self.field_array > 0.1) / self.field_array.size),
                'zero_concentration_cells': int(np.sum(self.field_array < GAUSSIAN_PRECISION))
            }
            
            # Peak analysis with location
            peak_location = np.unravel_index(np.argmax(self.field_array), self.field_array.shape)
            stats['distribution_analysis']['peak_location'] = {
                'array_indices': peak_location,
                'coordinates': (peak_location[1], peak_location[0]),  # Convert to (x, y)
        ...
        if include_performance_data:
            ...
            if self.last_generation_time_ms is not None:
                stats['performance_data']['performance_comparison'] = {
                    ...
                    'meets_target': self.last_generation_time_ms <= PERFORMANCE_TARGET_PLUME_GENERATION_MS,
                    'performance_ratio': self.last_generation_time_ms / PERFORMANCE_TARGET_PLUME_GENERATION_MS
                }
        ...
        if include_memory_analysis:
            ...
            cache_memory_estimate = len(self.sampling_cache) * 32  # Rough estimate per entry
            ...
            stats['memory_analysis'] = {
                ...
                'memory_efficiency': {
                    ...
                    'usage_percentage': (field_memory_mb / MEMORY_LIMIT_PLUME_FIELD_MB) * 100
                }
            }
        ...
        stats['field_quality'] = {
            'value_range_valid': (
                CONCENTRATION_RANGE[0] <= stats['basic_statistics']['min_value'] <= 
                stats['basic_statistics']['max_value'] <= CONCENTRATION_RANGE[1]
            ),
            'no_invalid_values': {
            ...
            'normalization_check': abs(stats['basic_statistics']['max_value'] - 1.0) < GAUSSIAN_PRECISION
        }
    ...
    def clear_cache(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'previous_statistics': {
                ...
                'hit_rate': (old_hits / (old_hits + old_misses)) if (old_hits + old_misses) > 0 else 0.0
            },
    ...
    def clone(
        ...
        try:
            ...
            clone_grid_size = new_grid_size if new_grid_size is not None else self.grid_size
            cloned_field = ConcentrationField(
                grid_size=clone_grid_size,
...
def create_concentration_field(
    ...
    try:
        ...
        if isinstance(grid_size, (tuple, list)):
            if len(grid_size) != 2:
                raise ValidationError(f"Grid size tuple must have 2 elements, got {len(grid_size)}")
...
def validate_field_parameters(
    ...
    try:
        ...
        try:
            ...
            validation_result['is_valid'] = False
            validation_result['critical_errors'].append(f"Grid size validation failed: {e}")
        ...
        try:
            if isinstance(source_location, Coordinates):
                ...
                source_coords = Coordinates(x=int(source_location[0]), y=int(source_location[1]))
            else:
                raise ValidationError("Source location must be Coordinates or (x, y) tuple")
            ...
            validation_result['is_valid'] = False
            validation_result['critical_errors'].append(f"Source location validation failed: {e}")
        ...
        if check_performance_feasibility and grid_obj:
            ...
            validation_result['performance_analysis'] = {
                ...
                'meets_performance_target': estimated_time_ms <= PERFORMANCE_TARGET_PLUME_GENERATION_MS,
...
def clear_field_cache(
    ...
    try:
        ...
        if cache_filter:
            ...
            keys_to_remove = [
                key for key in _GENERATION_PERFORMANCE_CACHE.keys() 
                if cache_filter in key
            ]
            for key in keys_to_remove:
                del _GENERATION_PERFORMANCE_CACHE[key]
                cleared_entries += 1
            cleanup_report['cleanup_actions'].append(f"Selective cache clear with filter: {cache_filter}")
        else:
        ...
        if force_cleanup:
            ...
            cleanup_report['cleanup_actions'].append(f"Garbage collection: {collected} objects collected")
