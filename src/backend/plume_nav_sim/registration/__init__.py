    ...
    try:
        ...
        if is_registered() and not force_reregister:
            ...
            with _cache_lock:
                ...
                _registration_cache['quick_register_count'] = _registration_cache.get('quick_register_count', 0) + 1
        ...
        if validate_creation:
            try:
                ...
                test_env.close()
                _module_logger.debug(f"Environment creation test successful for {ENV_ID}")
        ...
        with _cache_lock:
            _registration_cache.update({
                ...
                'quick_register_count': _registration_cache.get('quick_register_count', 0) + 1,
                'registration_method': 'quick_register'
            })
        ...
        with _cache_lock:
            _registration_cache.update({
                ...
                'last_error': str(e),
                'quick_register_failures': _registration_cache.get('quick_register_failures', 0) + 1
            })
            ...
            try:
                ...
                if is_registered():
                    _module_logger.info(
                        ...
                        extra={'auto_registration_successful': True, 'environment_id': env_id}
                    )
                    ...
                    error_msg = f"Auto-registration completed but {ENV_ID} still not available"
                ...
                _module_logger.error(
                    error_msg,
                    extra={'auto_registration_error': str(e), 'error_type': type(e).__name__}
                )
    ...
    try:
        ...
        if include_cache_info:
            with _cache_lock:
                ...
                cache_info.update({
                    'total_operations_logged': len([k for k in _registration_cache if k.startswith('last_')]),
                    'error_count': len([k for k in _registration_cache if 'error' in k]),
                    'success_operations': len([k for k in _registration_cache if 'successful' in k or '_result' in k])
                })
        ...
        if validate_creation and registry_status:
            try:
                _module_logger.debug(f"Testing environment creation for status check: {ENV_ID}")
                test_env = gymnasium.make(ENV_ID)
                test_env.close()
                ...
                _module_logger.debug(f"Environment creation test successful for status check")
...
def clear_registration_cache(reset_module_state: bool = False) -> None:
    ...
    try:
        ...
        try:
            ...
            _module_logger.critical("Complete cache clearing failure - unable to log error state")
    ...
    try:
        ...
        with _cache_lock:
            _registration_cache.update({
                ...
                'initialization_duration_ms': (initialization_end - initialization_start) * 1000,
