...
from typing import Union, Optional, Dict, List, Tuple, Any, Callable  # >=3.10 - Type hints for parameters and methods
from dataclasses import dataclass, field  # >=3.10 - Structured color scheme configuration with validation
from enum import Enum  # >=3.10 - Color scheme enumeration and predefined scheme selection
...
from ..core.types import RenderMode, Coordinates, GridSize
...
__all__ = [
    'ColorSchemeManager', 'PredefinedScheme', 'CustomColorScheme', 'ColorPalette', 
    'AccessibilityConfig', 'create_color_scheme', 'get_default_scheme', 
    'create_accessibility_scheme', 'optimize_for_render_mode', 'validate_color_scheme',
    'get_predefined_schemes', 'normalize_concentration_to_rgb', 'apply_agent_marker',
    'apply_source_marker', 'get_matplotlib_colormap', 'convert_rgb_to_matplotlib'
]
...
class CustomColorScheme:
    ...
    agent_marker_size: Tuple[int, int] = field(default_factory=lambda: AGENT_MARKER_SIZE)
    source_marker_size: Tuple[int, int] = field(default_factory=lambda: SOURCE_MARKER_SIZE)
    ...
    _cached_colormap: Optional[matplotlib.colors.Colormap] = field(default=None, repr=False)
    ...
    def __post_init__(self):
        ...
        if not all(isinstance(val, int) and 0 <= val <= 255 for val in self.agent_color):
            raise ValidationError("agent_color values must be integers in range [0,255]")
        ...
        if not all(isinstance(val, int) and 0 <= val <= 255 for val in self.source_color):
            raise ValidationError("source_color values must be integers in range [0,255]")

        # Validate background_color tuple has exactly 3 RGB values in range [0,255]
        if not (isinstance(self.background_color, tuple) and len(self.background_color) == 3):
            raise ValidationError("background_color must be a tuple of 3 RGB values")
        if not all(isinstance(val, int) and 0 <= val <= 255 for val in self.background_color):
            raise ValidationError("background_color values must be integers in range [0,255]")

        # Verify concentration_colormap exists in matplotlib.cm registry
        try:
            cm.get_cmap(self.concentration_colormap)
        except ValueError as e:
            raise ValidationError(f"Invalid concentration_colormap '{self.concentration_colormap}': {str(e)}")
    ...
    def apply_to_rgb_array(self, concentration_field: np.ndarray, 
                          agent_position: Coordinates, source_position: Coordinates) -> np.ndarray:
        """
    ...
    def configure_matplotlib_axes(self, axes: Any, concentration_field: np.ndarray) -> Any:
        """
        ...
        norm = matplotlib.colors.Normalize(vmin=CONCENTRATION_RANGE[0], vmax=CONCENTRATION_RANGE[1])
        image.set_norm(norm)
    ...
    def optimize_for_render_mode(self, target_mode: RenderMode, 
                                optimization_options: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        logger.debug(f"Optimized color scheme for {target_mode} with config: {self.performance_config}")

    def enable_accessibility(self, accessibility_type: Optional[str] = None, 
                           validate_contrast: bool = True) -> None:
        """
        ...
        if accessibility_type in ['high_contrast', 'colorblind_friendly']:
            ...
            self.agent_marker_size = (max(3, self.agent_marker_size[0] + 1), 
                                     max(3, self.agent_marker_size[1] + 1))
            self.source_marker_size = (max(5, self.source_marker_size[0] + 1),
        ...
        if validate_contrast:
            ...
            contrast_ratio = (max(agent_luminance, background_luminance) + 0.05) / (min(agent_luminance, background_luminance) + 0.05)
            
            if contrast_ratio < ACCESSIBILITY_CONTRAST_RATIO_MIN:
                logger.warning(f"Contrast ratio {contrast_ratio:.2f} below minimum {ACCESSIBILITY_CONTRAST_RATIO_MIN}")

        logger.info(f"Enabled accessibility features: {accessibility_type}")

    def validate(self, check_accessibility: bool = False, check_performance: bool = False,
                strict_validation: bool = False) -> bool:
        """
        ...
        try:
            ...
            for color_name, color_value in [
                ...
                if not all(isinstance(val, int) and 0 <= val <= 255 for val in color_value):
                    raise ValidationError(f"{color_name} values must be integers in range [0,255]")

            # Check matplotlib colormap availability and compatibility
            try:
                colormap = cm.get_cmap(self.concentration_colormap)
            except ValueError:
                raise ValidationError(f"Invalid colormap '{self.concentration_colormap}'")
            ...
            if self.agent_color == self.source_color:
                if strict_validation:
                    raise ValidationError("Agent and source colors must be different for visibility")
                else:
                    logger.warning("Agent and source colors are identical - may reduce visibility")
    ...
    def clone(self, color_overrides: Optional[Dict[str, Any]] = None,
             preserve_optimizations: bool = True) -> 'CustomColorScheme':
        """
    ...
    def to_dict(self, include_optimization_info: bool = False,
               sanitize_for_logging: bool = False) -> Dict[str, Any]:
        """
        ...
        if include_optimization_info:
            scheme_dict['optimized_for_mode'] = str(self.optimized_for_mode) if self.optimized_for_mode else None
...
class AccessibilityConfig:
    ...
    def apply_to_scheme(self, color_scheme: CustomColorScheme) -> CustomColorScheme:
        ...
        if self.enhanced_visibility:
            enhanced_scheme.agent_marker_size = (enhanced_scheme.agent_marker_size[0] + 2,
                                                enhanced_scheme.agent_marker_size[1] + 2)
            enhanced_scheme.source_marker_size = (enhanced_scheme.source_marker_size[0] + 2, 
    ...
    def validate_compliance(self, color_scheme: CustomColorScheme) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        if contrast_ratio < self.min_contrast_ratio:
            ...
            compliance_report['recommendations'].append(f"Increase agent-background contrast ratio from {contrast_ratio:.2f} to {self.min_contrast_ratio}")

        # Validate colorblind-friendly color selection if required
        if self.colorblind_friendly:
            if color_scheme.agent_color == AGENT_MARKER_COLOR:  # Standard red
                compliance_report['warnings'].append("Standard red may not be colorblind-friendly")
        ...
        for color_name, color_value in colors.items():
            if isinstance(color_value, str):
                ...
                if not (color_value.startswith('#') and len(color_value) == 7):
                    raise ValidationError(f"Invalid hex color '{color_value}' for '{color_name}'")
            elif isinstance(color_value, (tuple, list)):
                # RGB tuple validation  
                if len(color_value) != 3 or not all(isinstance(v, int) and 0 <= v <= 255 for v in color_value):
                    raise ValidationError(f"Invalid RGB color '{color_value}' for '{color_name}'")
            else:
                raise ValidationError(f"Color '{color_name}' must be hex string or RGB tuple")

        # Initialize accessibility_info with contrast analysis
        self._analyze_accessibility()

    def get_color(self, color_name: str, output_format: str = 'tuple') -> Union[Tuple[int, int, int], str, np.ndarray]:
        ...
        for i, color1 in enumerate(color_names):
            for color2 in color_names[i+1:]:
                try:
                    ...
                    if contrast_ratio < ACCESSIBILITY_CONTRAST_RATIO_MIN:
                        report['recommendations'].append(f"Increase contrast between {color1} and {color2}")
                        report['compliance_status'] = 'needs_improvement'
                except Exception as e:
                    logger.warning(f"Failed to calculate contrast for {color1}-{color2}: {str(e)}")
...
class ColorSchemeManager:
    ...
    def __init__(self, enable_caching: bool = True, auto_optimize: bool = True,
    ...
    def get_scheme(self, scheme_name: Union[str, PredefinedScheme], 
                   render_mode: Optional[RenderMode] = None,
                   use_cache: bool = True) -> CustomColorScheme:
        """
        ...
        if isinstance(scheme_name, PredefinedScheme):
            ...
            try:
                ...
                self.logger.warning(f"Unknown scheme name '{scheme_name}', using default")
    ...
    def create_custom_scheme(self, scheme_name: str, color_config: Dict[str, Any],
                            validate_scheme: bool = True, enable_accessibility: bool = False,
                            register_scheme: bool = True) -> CustomColorScheme:
        """
        ...
        color_scheme = create_color_scheme(sanitized_config, validate_immediately=validate_scheme)
        ...
        if self.optimization_enabled:
            color_scheme.optimize_for_render_mode(RenderMode.RGB_ARRAY)  # Default optimization
            self.performance_metrics['optimization_count'] += 1
        
        # Log scheme creation with configuration details
        self.logger.info(f"Created custom scheme '{scheme_name}' with accessibility: {enable_accessibility}")
        
        return color_scheme

    def validate_scheme(self, color_scheme: CustomColorScheme, check_accessibility: bool = False,
                       check_performance: bool = False, check_matplotlib_integration: bool = True) -> Tuple[bool, Dict[str, Any]]:
        """
    ...
    def optimize_scheme(self, color_scheme: CustomColorScheme, target_mode: RenderMode,
                       optimization_config: Optional[Dict[str, Any]] = None) -> CustomColorScheme:
        """
    ...
    def clear_cache(self, clear_scheme_cache: bool = True, clear_colormap_cache: bool = True,
                   reset_metrics: bool = False) -> Dict[str, Any]:
        """
        ...
        clearing_report['memory_freed_estimate_mb'] = (initial_scheme_count * 0.1 + initial_colormap_count * 0.5)
    ...
    def get_performance_metrics(self, include_cache_stats: bool = True,
                               include_optimization_analysis: bool = True) -> Dict[str, Any]:
        """
        Retrieve comprehensive performance metrics for color scheme operations with timing 
        analysis and optimization recommendations.
        """
        metrics = self.performance_metrics.copy()
        metrics['timestamp'] = logging.getLogger().handlers[0].formatter.formatTime(logging.LogRecord('', 0, '', 0, '', (), None)) if logging.getLogger().handlers else 'unknown'
        ...
        if include_cache_stats:
            ...
            if total_requests > 0:
                ...
                metrics['cache_efficiency'] = 'high' if metrics['cache_hit_ratio'] > 0.7 else 'medium' if metrics['cache_hit_ratio'] > 0.4 else 'low'
        ...
        if include_optimization_analysis:
            ...
            if metrics['optimization_count'] == 0:
                recommendations.append("Enable automatic optimization for better performance")
...
def create_color_scheme(scheme_config: Union[Dict[str, Any], str, PredefinedScheme],
                       optimize_for_mode: Optional[RenderMode] = None,
                       enable_accessibility: bool = False,
                       validate_immediately: bool = True,
                       performance_config: Optional[Dict[str, Any]] = None) -> CustomColorScheme:
    """
    ...
    try:
        ...
        concentration_colormap = config_dict.get('concentration_colormap', DEFAULT_COLORMAP)
...
def get_default_scheme(render_mode: Optional[RenderMode] = None,
                      enable_caching: bool = True,
                      apply_accessibility: bool = False) -> CustomColorScheme:
    """
...
def create_accessibility_scheme(accessibility_type: str, 
                               optimize_for_mode: Optional[RenderMode] = None,
                               custom_colors: Optional[Dict[str, Any]] = None,
                               validate_contrast: bool = True) -> CustomColorScheme:
    """
    ...
    if accessibility_type not in supported_types:
        raise ValidationError(f"Unsupported accessibility_type '{accessibility_type}'. Supported: {supported_types}")
    ...
    if validate_contrast:
        is_compliant, compliance_report = accessibility_config.validate_compliance(accessibility_scheme)
        if not is_compliant:
...
def optimize_for_render_mode(color_scheme: CustomColorScheme, target_mode: RenderMode,
                            optimization_options: Optional[Dict[str, Any]] = None,
                            enable_caching: bool = True,
                            validate_performance: bool = True) -> CustomColorScheme:
    """
    ...
    if enable_caching:
        ...
        colormap = get_matplotlib_colormap(
            optimized_scheme.concentration_colormap,
            enable_caching=True
        )
    ...
    if validate_performance:
        ...
        if target_latency > 100.0:  # Warn about potentially slow targets
            logger.warning(f"High performance target: {target_latency}ms may impact user experience")
    
    logger.debug(f"Optimized color scheme for {target_mode} with config: {optimization_config}")
    return optimized_scheme


def validate_color_scheme(color_scheme: Union[CustomColorScheme, ColorSchemeManager, Dict[str, Any]],
    ...
    try:
        ...
        if isinstance(color_scheme, CustomColorScheme):
            ...
            raise ValidationError(f"Unsupported color_scheme type: {type(color_scheme)}")
        ...
        for color_name in ['agent_color', 'source_color', 'background_color']:
            ...
            if not (isinstance(color_value, tuple) and len(color_value) == 3):
                validation_report['errors'].append(f"{color_name} must be a tuple of 3 values")
                validation_report['is_valid'] = False
            elif not all(isinstance(val, int) and 0 <= val <= 255 for val in color_value):
                validation_report['errors'].append(f"{color_name} values must be integers in range [0,255]")
                validation_report['is_valid'] = False
        ...
        if validate_matplotlib_integration:
            try:
                ...
                validation_report['validation_details']['matplotlib_colormap'] = 'available'
            except ValueError as e:
                validation_report['errors'].append(f"Invalid matplotlib colormap: {str(e)}")
                validation_report['is_valid'] = False
        
        # Validate accessibility compliance including contrast ratios
        if check_accessibility:
            if hasattr(scheme_obj, 'accessibility_enabled') and scheme_obj.accessibility_enabled:
                # Simplified contrast check
                agent_luminance = sum(scheme_obj.agent_color) / (3 * 255)
                background_luminance = sum(scheme_obj.background_color) / (3 * 255)
                contrast_ratio = (max(agent_luminance, background_luminance) + 0.05) / (min(agent_luminance, background_luminance) + 0.05)
                
                validation_report['validation_details']['contrast_ratio'] = contrast_ratio
                if contrast_ratio < ACCESSIBILITY_CONTRAST_RATIO_MIN:
                    validation_report['warnings'].append(f"Low contrast ratio: {contrast_ratio:.2f}")
        ...
        if check_performance:
            ...
            if target_latency > PERFORMANCE_TARGET_RGB_RENDER_MS * 5:  # 5x slower than target
                validation_report['warnings'].append(f"High performance target may impact user experience: {target_latency}ms")
        
        # Check for color conflicts between markers and background
        if scheme_obj.agent_color == scheme_obj.source_color:
            if strict_validation:
                validation_report['errors'].append("Agent and source colors must be different")
                validation_report['is_valid'] = False
            else:
                validation_report['warnings'].append("Identical agent and source colors may reduce visibility")
        ...
        if strict_validation:
            ...
            if color_distance(scheme_obj.agent_color, scheme_obj.background_color) < 50:
                validation_report['errors'].append("Agent color too similar to background")
                validation_report['is_valid'] = False
        
        # Generate recommendations based on validation results
        if validation_report['warnings']:
            validation_report['recommendations'].append("Review warning messages for potential improvements")
        if not check_accessibility:
            validation_report['recommendations'].append("Consider enabling accessibility validation")
...
def get_predefined_schemes(include_descriptions: bool = True,
                          include_performance_info: bool = True,
                          filter_by_mode: Optional[RenderMode] = None) -> Dict[str, Any]:
    """
    ...
    for scheme in PredefinedScheme:
        ...
        if include_descriptions:
            if scheme == PredefinedScheme.STANDARD:
                scheme_info['description'] = "Standard plume navigation colors with red agent and white source markers"
                scheme_info['use_cases'] = ["General visualization", "Scientific analysis", "Training environments"]
            elif scheme == PredefinedScheme.HIGH_CONTRAST:
                scheme_info['description'] = "High contrast colors for enhanced visibility and accessibility"
                scheme_info['use_cases'] = ["Accessibility", "Low light conditions", "Presentation displays"]
            elif scheme == PredefinedScheme.COLORBLIND_FRIENDLY:
                scheme_info['description'] = "Colorblind-friendly palette using orange and blue tones"
                scheme_info['use_cases'] = ["Inclusive design", "Color vision deficiency", "Public presentations"]
...
def normalize_concentration_to_rgb(concentration_field: np.ndarray,
                                  colormap_name: Optional[str] = None,
                                  use_caching: bool = True,
                                  normalization_range: Optional[Tuple[float, float]] = None,
                                  validate_output: bool = True) -> np.ndarray:
    """
    ...
    normalized_field = (concentration_clamped - norm_range[0]) / (norm_range[1] - norm_range[0])
    ...
    if validate_output:
        if rgb_array.shape[:2] != concentration_field.shape:
            raise RenderingError("RGB array shape mismatch with input concentration field")
        if rgb_array.dtype != RGB_DTYPE:
            raise RenderingError(f"RGB array dtype {rgb_array.dtype} != expected {RGB_DTYPE}")
...
def apply_agent_marker(rgb_array: np.ndarray, agent_position: Coordinates,
                      marker_color: Optional[Tuple[int, int, int]] = None,
                      marker_size: Optional[Tuple[int, int]] = None,
                      check_boundaries: bool = True) -> np.ndarray:
    """
...
def apply_source_marker(rgb_array: np.ndarray, source_position: Coordinates,
                       marker_color: Optional[Tuple[int, int, int]] = None,
                       marker_size: Optional[Tuple[int, int]] = None,
                       check_boundaries: bool = True) -> np.ndarray:
    """
...
def get_matplotlib_colormap(colormap_name: str,
                           normalization_range: Optional[Tuple[float, float]] = None,
                           validate_backend: bool = True,
                           enable_caching: bool = True) -> matplotlib.colors.Colormap:
    """
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Colormap '{colormap_name}' not found, falling back to '{DEFAULT_COLORMAP}'")
        ...
        if norm_range != (0.0, 1.0):
            ...
            colormap = colormap._segmentdata if hasattr(colormap, '_segmentdata') else colormap
        
        logger.debug(f"Retrieved colormap '{colormap_name}' with normalization {norm_range}")
        return colormap
...
def convert_rgb_to_matplotlib(rgb_color: Union[Tuple[int, int, int], List[int], np.ndarray],
                             normalize_to_unit: bool = True,
                             validate_range: bool = True,
                             output_format: str = 'tuple') -> Union[Tuple[float, float, float], np.ndarray, str]:
    """
    ...
    if validate_range:
        if not all(isinstance(val, (int, float)) and 0 <= val <= 255 for val in rgb_values):
    ...
    if output_format == 'tuple':
        ...
        raise ValidationError(f"Invalid output_format '{output_format}'. Supported: tuple, array, hex")


# Initialize module-level caching and performance monitoring
logger.info(f"Color scheme module initialized with caching: {PERFORMANCE_OPTIMIZATION_ENABLED}")