...
from typing import Dict, List, Optional, Tuple, Union  # >=3.10 - Type hints for matplotlib renderer type safety
...
DEFAULT_DPI = 100  # Standard DPI for matplotlib figure resolution and display compatibility
INTERACTIVE_UPDATE_INTERVAL = 0.001  # Minimal interactive pause for matplotlib display refresh (1ms)
BACKEND_SELECTION_TIMEOUT = 10.0  # Maximum time for backend selection and configuration (10 seconds)
FIGURE_CLEANUP_TIMEOUT = 5.0  # Maximum time for figure cleanup operations (5 seconds)
COLORMAP_CACHE_SIZE = 20  # Maximum number of cached colormaps for performance optimization
MARKER_UPDATE_BATCH_SIZE = 100  # Batch size for efficient marker updates in large-scale visualizations
...
class MatplotlibBackendManager:
    ...
    def __init__(self, backend_preferences: Optional[List[str]] = None, 
                 enable_fallback: bool = True, backend_options: dict = None):
        """
        ...
        self.logger.debug(f"MatplotlibBackendManager initialized: preferences={self.backend_preferences}, "
                         f"fallback_enabled={enable_fallback}, headless={self.headless_mode}")
    
    def select_backend(self, force_reselection: bool = False, validate_functionality: bool = True) -> str:
        """
        ...
        for backend_name in self.backend_preferences:
            try:
                ...
                if self.headless_mode and backend_name not in ['Agg']:
                    if not self.backend_options.get('force_interactive_in_headless', False):
                        self.logger.debug(f"Skipping interactive backend {backend_name} in headless mode")
                        continue
                ...
                if self._test_backend_availability(backend_name):
                    ...
                    if validate_functionality:
                        if self._test_backend_functionality(backend_name):
                            ...
                            self.logger.warning(f"Backend {backend_name} available but functionality test failed")
                    else:
                        selected_backend = backend_name
                        break
    ...
    def get_backend_capabilities(self, backend_name: Optional[str] = None, 
        ...
        try:
            ...
            if self.backend_options.get('measure_performance', False):
                capabilities['performance_characteristics'] = self._measure_backend_performance(effective_backend)
            
            # Cache capabilities for performance optimization
            self.configuration_cache[cache_key] = capabilities
            
        except Exception as e:
            self.logger.error(f"Capability analysis failed for {effective_backend}: {e}")
            capabilities['error'] = str(e)
        
        return capabilities
    
    def configure_backend(self, backend_name: str, configuration_options: dict = None) -> bool:
        """
    ...
    def _measure_backend_performance(self, backend_name: str) -> Dict[str, float]:
        ...
        try:
            ...
            self.logger.warning(f"Performance measurement failed for {backend_name}: {e}")
...
class InteractiveUpdateManager:
    ...
    def update_concentration_field(self, concentration_field: np.ndarray, 
                                 color_scheme: CustomColorScheme, force_update: bool = False) -> bool:
        """
        ...
        try:
            ...
            if not force_update and self.update_cache.get('concentration_field_id') == field_id:
            ...
            if self.heatmap is None:
                ...
                self.heatmap = self.axes.imshow(
                    ...
                    vmin=0.0, vmax=1.0  # Normalize concentration range
                )
            else:
    ...
    def update_agent_marker(self, agent_position: Coordinates, color_scheme: CustomColorScheme, 
                          animate_transition: bool = False) -> bool:
        """
        ...
        try:
            ...
            if self.update_cache.get('agent_position') == position_key and not animate_transition:
            ...
            if self.agent_marker is None:
                ...
                self.agent_marker = self.axes.scatter(
                    ...
                    c=[agent_color], s=100, marker='s',  # Square marker
    ...
    def update_source_marker(self, source_position: Coordinates, color_scheme: CustomColorScheme) -> bool:
        """
        ...
        try:
            ...
            if self.source_marker is None:
                ...
                self.source_marker = self.axes.scatter(
                    ...
                    c=[source_color], s=200, marker='+',  # Cross marker
                    linewidth=3, 
    ...
    def refresh_display(self, force_refresh: bool = False, measure_performance: bool = False) -> float:
        """
    ...
    def batch_update(self, context: RenderContext, color_scheme: CustomColorScheme, 
                    optimize_updates: bool = True) -> Dict[str, any]:
        """
        ...
        try:
            ...
            field_updated = self.update_concentration_field(
                context.concentration_field, color_scheme, 
                force_update=not optimize_updates
            )
            performance_report['component_timings']['concentration_field'] = (time.time() - field_start) * 1000
            if field_updated:
            ...
            performance_report['component_timings']['agent_marker'] = (time.time() - agent_start) * 1000
            if agent_updated:
                performance_report['total_changes'] += 1
            ...
            performance_report['component_timings']['source_marker'] = (time.time() - source_start) * 1000
            if source_updated:
                performance_report['total_changes'] += 1
            ...
            refresh_duration = self.refresh_display(
                force_refresh=not optimize_updates, 
                measure_performance=True
            )
            performance_report['component_timings']['display_refresh'] = refresh_duration
            ...
            performance_report['update_efficiency'] = performance_report['total_changes'] / max(1, total_duration / 10)
...
class MatplotlibRenderer(BaseRenderer):
    ...
    def __init__(self, grid_size: GridSize, color_scheme_name: Optional[str] = None,
                 backend_preferences: Optional[List[str]] = None, renderer_options: dict = None):
        """
        ...
        self.current_color_scheme = self.color_manager.get_scheme(color_scheme_name or 'standard')
    ...
    def _initialize_renderer_resources(self) -> None:
        ...
        try:
            ...
            selected_backend = self.backend_manager.select_backend(validate_functionality=True)
            self.backend_manager.configure_backend(
                selected_backend,
                {
                    'figure_size': self.renderer_options.get('figure_size', DEFAULT_FIGURE_SIZE),
            ...
            update_interval = self.renderer_options.get('update_interval', INTERACTIVE_UPDATE_INTERVAL)
            ...
            self.logger.info(f"Matplotlib renderer initialized with backend: {selected_backend}")
    ...
    def _render_human(self, context: RenderContext) -> None:
        ...
        try:
            ...
            if total_duration > PERFORMANCE_TARGET_HUMAN_RENDER_MS:
                self.logger.warning(f"Render duration {total_duration:.2f}ms exceeds target "
    ...
    def set_color_scheme(self, color_scheme: Union[str, CustomColorScheme], force_update: bool = False) -> bool:
        """
        ...
        try:
            ...
            optimized_scheme = self.color_manager.optimize_scheme(resolved_scheme, RenderMode.HUMAN)
            
            # Update current color scheme configuration
            self.current_color_scheme = optimized_scheme
            
            # Force visualization update if requested and figure exists
            if force_update and self.figure is not None and self.update_manager is not None:
            ...
            self.logger.info(f"Color scheme updated: {getattr(resolved_scheme, 'name', 'custom')}")
            return True
    ...
    def get_figure(self, create_if_needed: bool = True) -> Optional[matplotlib.figure.Figure]:
        """
    ...
    def save_figure(self, filename: str, format: Optional[str] = None, save_options: dict = None) -> bool:
        """
    ...
    def configure_interactive_mode(self, enable_interactive: bool, update_interval: Optional[float] = None,
                                 interactive_options: dict = None) -> bool:
        """
    ...
    def get_performance_metrics(self, include_backend_info: bool = True, 
                              include_update_stats: bool = True) -> Dict[str, any]:
        """
        ...
        if include_backend_info:
            try:
                ...
                metrics['backend_info'] = {
                    ...
                    'display_support': backend_capabilities.get('display_support', False),
                    'interactive_features': backend_capabilities.get('interactive_features', False),
                    'platform_support': backend_capabilities.get('platform_support', {})
                }
            except Exception as e:
        ...
        recommendations = []
        if self.performance_cache.get('last_render', {}).get('duration_ms', 0) > PERFORMANCE_TARGET_HUMAN_RENDER_MS:
            recommendations.append("Consider reducing update frequency or grid size for better performance")
...
def create_matplotlib_renderer(grid_size: GridSize, color_scheme_name: Optional[str] = None,
                             backend_preferences: Optional[List[str]] = None, 
                             renderer_options: dict = None) -> MatplotlibRenderer:
    """
    ...
    try:
        ...
        capabilities = detect_matplotlib_capabilities(test_backends=True, check_display_availability=True)
        ...
        if not capabilities.get('display_available', True):
            effective_preferences = [backend for backend in effective_preferences 
                                   if backend == 'Agg']  # Only headless backend for no display
        ...
        renderer.initialize(validate_immediately=True, enable_performance_monitoring=True)
        
        # Test renderer functionality with basic operations
        test_context = RenderContext(
            concentration_field=np.zeros((grid_size.height, grid_size.width), dtype=np.float32),
            agent_position=Coordinates(x=0, y=0),
            source_position=Coordinates(x=grid_size.width//2, y=grid_size.height//2),
            grid_size=grid_size
        )
        ...
        logger.info(f"MatplotlibRenderer created successfully with backend: "
...
def detect_matplotlib_capabilities(test_backends: bool = False, check_display_availability: bool = True,
                                 assess_performance: bool = False) -> Dict[str, any]:
    """
    ...
    try:
        ...
        if assess_performance:
            ...
            try:
                ...
                performance_metrics['performance_acceptable'] = operation_time < 100  # 100ms threshold
        ...
        if not capabilities['matplotlib_available']:
            recommendations.append("Install matplotlib>=3.9.0 for visualization support")
        
        if not capabilities.get('display_available', True):
            recommendations.append("Running in headless mode - only Agg backend available")
        
        backend_count = sum(capabilities.get('backend_availability', {}).values())
        if backend_count == 0:
            recommendations.append("No matplotlib backends available - check GUI dependencies")
        elif backend_count == 1:
            recommendations.append("Limited backend options - consider installing additional GUI toolkits")
        
        if not recommendations:
            recommendations.append("System fully compatible with matplotlib visualization")
...
def configure_matplotlib_backend(backend_preferences: Optional[List[str]] = None,
                               allow_headless_fallback: bool = True,
                               configuration_options: dict = None) -> Tuple[str, Dict[str, any]]:
    """
...
def validate_matplotlib_integration(backend_name: str, color_scheme: Optional[CustomColorScheme] = None,
                                  test_rendering_operations: bool = True, 
                                  validate_performance: bool = True) -> Tuple[bool, Dict[str, any]]:
    """
    ...
    try:
        ...
        try:
            ...
            if color_scheme:
                try:
                    ...
                    validation_report['error_details'].append(f"Colormap test failed: {e}")
                    is_valid = False
            ...
            if test_rendering_operations:
                ...
                validation_report['performance_metrics']['render_duration_ms'] = render_duration
                ...
                if validate_performance:
                    ...
                    validation_report['test_results']['performance_compliance'] = performance_acceptable
                    validation_report['performance_metrics']['target_ms'] = target_ms
                    validation_report['performance_metrics']['within_target'] = performance_acceptable
            ...
            validation_report['error_details'].append(f"Backend functionality test failed: {e}")
            is_valid = False
        ...
        if not validation_report['test_results'].get('backend_functionality', True):
            recommendations.append(f"Backend {backend_name} not functional - try alternative backend")
        
        if not validation_report['test_results'].get('performance_compliance', True):
            recommendations.append("Performance below target - consider optimization or simpler visualization")
        
        if not validation_report['test_results'].get('colormap_integration', True):
            recommendations.append("Colormap integration issues - check color scheme compatibility")
