...
from typing import Optional, Union, Dict, Any, Tuple  # >=3.10 - Type hints for RGB array types and method annotations
...
from .color_schemes import (
    ColorSchemeManager, CustomColorScheme, normalize_concentration_to_rgb,
    apply_agent_marker, apply_source_marker
)
from ..core.types import RenderMode, Coordinates, GridSize
from ..core.constants import (
    PERFORMANCE_TARGET_RGB_RENDER_MS, RGB_DTYPE, FIELD_DTYPE,
    AGENT_MARKER_SIZE, SOURCE_MARKER_SIZE, PIXEL_VALUE_MIN, PIXEL_VALUE_MAX
)
from ..utils.exceptions import ValidationError, RenderingError, ComponentError
...
class NumpyRGBRenderer(BaseRenderer):
    ...
    def __init__(self, grid_size: GridSize, color_scheme_name: Optional[str] = None, 
                 renderer_options: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        self.color_manager = ColorSchemeManager(enable_caching=True, optimize_for_mode='rgb_array')
        ...
        self.performance_config = {
            'enable_caching': renderer_options.get('enable_caching', True) if renderer_options else True,
    ...
    def generate_optimized_array(self, context: RenderContext, reuse_buffer: bool = True, 
        ...
        if reuse_buffer and self._array_buffer is not None:
            ...
            output_array = np.zeros((self.grid_size.height, self.grid_size.width, 3), dtype=RGB_DTYPE)
        ...
        for dy in range(-1, 2):
            for dx in range(-1, 2):
                ...
                if 0 <= y_pos < self.grid_size.height and 0 <= x_pos < self.grid_size.width:
        ...
        for dx in range(-2, 3):
            x_pos = source_x + dx
            if 0 <= source_y < self.grid_size.height and 0 <= x_pos < self.grid_size.width:
                output_array[source_y, x_pos] = source_color
        # Vertical line
        for dy in range(-2, 3):
            y_pos = source_y + dy
            if 0 <= y_pos < self.grid_size.height and 0 <= source_x < self.grid_size.width:
    ...
    def update_color_scheme(self, color_scheme: Union[str, CustomColorScheme], 
                           optimize_for_rgb: bool = True, clear_cache: bool = True) -> None:
        """
        ...
        if hasattr(self.current_color_scheme, 'performance_impact'):
            performance_impact = getattr(self.current_color_scheme, 'performance_impact', 1.0)
            self._performance_baseline['target_ms'] = PERFORMANCE_TARGET_RGB_RENDER_MS * performance_impact
    
    def get_generation_statistics(self, reset_stats_after_retrieval: bool = False, 
                                 include_memory_analysis: bool = False) -> Dict[str, Any]:
        """
        ...
        if include_memory_analysis:
            buffer_size_mb = self._array_buffer.nbytes / (1024 * 1024) if self._array_buffer is not None else 0
        ...
        if self.generation_stats['average_time_ms'] > 0:
            performance_ratio = self.generation_stats['average_time_ms'] / PERFORMANCE_TARGET_RGB_RENDER_MS
            stats['performance_analysis'] = {
                'target_ms': PERFORMANCE_TARGET_RGB_RENDER_MS,
                'performance_ratio': performance_ratio,
                'meets_target': performance_ratio <= 1.0,
                'performance_grade': 'excellent' if performance_ratio <= 0.8 else 
                                   'good' if performance_ratio <= 1.0 else 
                                   'needs_optimization'
            }
        ...
        if self.generation_stats.get('cache_hits', 0) == 0 and self.caching_enabled:
            recommendations.append("Cache is not being utilized - check caching configuration")
    ...
    def benchmark_performance(self, test_context: RenderContext, num_iterations: int = 100, 
                             include_memory_profiling: bool = False) -> Dict[str, Any]:
        """
        ...
        if include_memory_profiling:
            ...
            benchmark_results['memory_profiling'] = {
                ...
                'buffer_memory_mb': self._array_buffer.nbytes / (1024 * 1024) if self._array_buffer else 0,
                'estimated_per_array_mb': (self.grid_size.height * self.grid_size.width * 3) / (1024 * 1024)
            }
        ...
        benchmark_results['performance_validation'] = {
            ...
            'performance_grade': 'excellent' if average_time <= target_performance * 0.8 else
                               'good' if average_time <= target_performance else
                               'needs_optimization'
        }
        ...
        if average_time > target_performance:
            ...
            optimization_suggestions.append("Consider skipping validation in production")
        
        if np.std(timing_array) > average_time * 0.2:  # High variability
            optimization_suggestions.append("High timing variability - check system load")
        
        benchmark_results['optimization_opportunities'] = optimization_suggestions
        
        return benchmark_results
    
    def validate_output_quality(self, rgb_array: np.ndarray, reference_context: RenderContext, 
                               strict_quality_check: bool = False) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        if rgb_array.shape != expected_shape:
            ...
            quality_report['quality_issues'].append(f"Shape mismatch: {rgb_array.shape} vs {expected_shape}")
        ...
        if rgb_array.dtype != RGB_DTYPE:
            ...
            quality_report['quality_issues'].append(f"Dtype mismatch: {rgb_array.dtype} vs {RGB_DTYPE}")
            quality_report['overall_quality'] = False
        else:
        ...
        if np.any(rgb_array < PIXEL_VALUE_MIN) or np.any(rgb_array > PIXEL_VALUE_MAX):
            ...
            quality_report['quality_issues'].append(f"Pixel values outside [{PIXEL_VALUE_MIN}, {PIXEL_VALUE_MAX}]")
            quality_report['overall_quality'] = False
        else:
        ...
        if (0 <= agent_pos.y < self.grid_size.height and 0 <= agent_pos.x < self.grid_size.width):
        ...
        if not agent_marker_found:
            quality_report['quality_issues'].append("Agent marker not found at expected position")
        ...
        if (0 <= source_pos.y < self.grid_size.height and 0 <= source_pos.x < self.grid_size.width):
        ...
        if not source_marker_found:
            quality_report['quality_issues'].append("Source marker not found at expected position")
            if not strict_quality_check:
                quality_report['overall_quality'] = False
        ...
        if np.any(background_mask):
            ...
            quality_report['concentration_representation']['background_accuracy'] = bg_accuracy
            if bg_accuracy < 0.95:  # 95% accuracy threshold
                quality_report['quality_issues'].append(f"Background color accuracy {bg_accuracy:.2%} below threshold")
                quality_report['overall_quality'] = False
        ...
        if strict_quality_check:
            ...
            agent_region = slice(max(0, agent_pos.y-1), min(self.grid_size.height, agent_pos.y+2)), \
                          slice(max(0, agent_pos.x-1), min(self.grid_size.width, agent_pos.x+2))
            source_region = slice(max(0, source_pos.y-2), min(self.grid_size.height, source_pos.y+3)), \
                           slice(max(0, source_pos.x-2), min(self.grid_size.width, source_pos.x+3))
            ...
            if np.any(mask):
                ...
                pixel_accuracy = np.mean(np.all(field_pixels == expected_pixels, axis=1))
                quality_report['concentration_representation']['pixel_accuracy'] = pixel_accuracy
                
                if pixel_accuracy < 0.99:  # Strict 99% accuracy
                    quality_report['quality_issues'].append(f"Strict pixel accuracy {pixel_accuracy:.2%} below threshold")
        
        # Generate comprehensive quality report with findings and improvement recommendations
        if quality_report['overall_quality']:
            quality_report['recommendations'].append("RGB array meets quality standards")
        else:
...
def create_rgb_renderer(grid_size: GridSize, color_scheme_name: Optional[str] = None, 
                       enable_caching: bool = True, optimize_for_performance: bool = True, 
                       renderer_options: Optional[Dict[str, Any]] = None) -> NumpyRGBRenderer:
    """
    ...
    if not hasattr(grid_size, 'is_performance_feasible') or not grid_size.is_performance_feasible():
        raise ValidationError(
...
def generate_rgb_array_fast(concentration_field: np.ndarray, agent_position: Coordinates, 
                           source_position: Coordinates, color_scheme: Optional[CustomColorScheme] = None, 
                           validate_inputs: bool = True) -> np.ndarray:
    """
    ...
    if validate_inputs:
        if not isinstance(concentration_field, np.ndarray) or concentration_field.ndim != 2:
            raise ValidationError(
                ...
                parameter_name="concentration_field",
                invalid_value=concentration_field.shape if hasattr(concentration_field, 'shape') else None,
    ...
    rgb_array = np.stack([grayscale_values, grayscale_values, grayscale_values], axis=-1)
...
def validate_rgb_array_output(rgb_array: np.ndarray, expected_grid_size: GridSize, 
                             expected_agent_position: Coordinates, expected_source_position: Coordinates, 
    ...
    if rgb_array.shape != expected_shape:
        ...
        validation_report['recommendations'].append(f"Fix array shape: expected {expected_shape}, got {rgb_array.shape}")
    ...
    if rgb_array.dtype != RGB_DTYPE:
        ...
        validation_report['recommendations'].append(f"Convert array to {RGB_DTYPE}, currently {rgb_array.dtype}")
        validation_report['overall_valid'] = False
    else:
    ...
    if np.any(rgb_array < PIXEL_VALUE_MIN) or np.any(rgb_array > PIXEL_VALUE_MAX):
        ...
        validation_report['recommendations'].append(f"Pixel values must be in range [{PIXEL_VALUE_MIN}, {PIXEL_VALUE_MAX}]")
        validation_report['overall_valid'] = False
    else:
    ...
    agent_marker_found = False
    if (0 <= expected_agent_position.y < expected_grid_size.height and 
        0 <= expected_agent_position.x < expected_grid_size.width):
        ...
        if center_pixel[0] > center_pixel[1] and center_pixel[0] > center_pixel[2]:  # Red dominant
            agent_marker_found = True
    
    validation_report['marker_validation']['agent_marker_present'] = agent_marker_found
    if not agent_marker_found:
        validation_report['recommendations'].append("Agent marker not found at expected position")
        validation_report['overall_valid'] = False
    
    # Verify source marker presence at expected_source_position with cross pattern validation
    source_marker_found = False
    if (0 <= expected_source_position.y < expected_grid_size.height and 
        0 <= expected_source_position.x < expected_grid_size.width):
    ...
    validation_report['marker_validation']['source_marker_present'] = source_marker_found
    if not source_marker_found:
        validation_report['recommendations'].append("Source marker not found at expected position")
        validation_report['overall_valid'] = False
    ...
    if validation_report['overall_valid']:
        validation_report['recommendations'].insert(0, "RGB array validation passed - ready for use")
    else:
        validation_report['recommendations'].insert(0, "RGB array validation failed - address issues before use")
    
    return validation_report['overall_valid'], validation_report


def clear_rgb_cache(clear_array_cache: bool = True, clear_performance_stats: bool = True, 
                   force_gc: bool = False) -> Dict[str, Any]:
    """
    ...
    if clear_array_cache:
        ...
        cleanup_report['memory_freed_estimate_mb'] = cache_size_before * 0.5  # Estimate 0.5MB per entry
    ...
    cleanup_report['optimization_impact'] = 'Cache cleared - expect initial performance overhead until cache rebuilds'
    
    return cleanup_report


def get_rgb_performance_stats(include_cache_analysis: bool = True, include_memory_usage: bool = True, 
                             include_optimization_suggestions: bool = True) -> Dict[str, Any]:
    """
    ...
    if include_cache_analysis:
        performance_stats['cache_analysis'] = {
            ...
            'cache_utilization': len(_RGB_ARRAY_CACHE) / _MAX_CACHE_SIZE if _MAX_CACHE_SIZE > 0 else 0
        }
    ...
    if include_optimization_suggestions:
        ...
        if 'timing_analysis' in performance_stats and performance_stats['timing_analysis']:
            avg_time = performance_stats['timing_analysis'].get('average_time_ms', 0)
            if avg_time > _PERFORMANCE_BASELINE_MS:
                suggestions.append("Average generation time exceeds target - consider buffer reuse optimization")
        
        # Check cache utilization
        if include_cache_analysis:
            cache_util = performance_stats['cache_analysis']['cache_utilization']
            if cache_util < 0.5:
                suggestions.append("Low cache utilization - consider increasing cache size or improving cache key strategy")
            elif cache_util > 0.9:
                suggestions.append("High cache utilization - consider increasing max cache size")
        
        # Memory optimization suggestions
        if include_memory_usage:
            if performance_stats['memory_analysis']['estimated_cache_memory_mb'] > 20:
                suggestions.append("High memory usage - consider periodic cache clearing or size limits")
    ...
    performance_stats['performance_validation'] = {
        'target_met': performance_stats.get('timing_analysis', {}).get('meets_target', False),
        'baseline_ms': _PERFORMANCE_BASELINE_MS,
        'performance_grade': 'excellent' if performance_stats.get('timing_analysis', {}).get('average_time_ms', float('inf')) <= _PERFORMANCE_BASELINE_MS * 0.8 else
                           'good' if performance_stats.get('timing_analysis', {}).get('average_time_ms', float('inf')) <= _PERFORMANCE_BASELINE_MS else
                           'needs_optimization'
    }
