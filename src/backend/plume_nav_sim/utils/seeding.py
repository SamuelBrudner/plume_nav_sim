...
from typing import Optional, Union, Dict, List, Tuple, Any, Callable  # >=3.10 - Type hints for comprehensive type safety
...
_HASH_ALGORITHM = 'sha256'  # Default cryptographic hash algorithm for deterministic seed generation
_DEFAULT_ENCODING = 'utf-8'  # Default string encoding for consistent hash generation
_REPRODUCIBILITY_TOLERANCE = 1e-10  # Default floating-point tolerance for reproducibility validation
...
def validate_seed(seed: Any, strict_mode: bool = False) -> Tuple[bool, Optional[int], str]:
    ...
    try:
        ...
        if not (SEED_MIN_VALUE <= seed <= SEED_MAX_VALUE):
            return (False, None, f"Seed {seed} out of range [{SEED_MIN_VALUE}, {SEED_MAX_VALUE}]")
        ...
        if seed > 2**31 - 1:  # Standard 32-bit signed integer limit
            warnings.warn(f"Seed {seed} may cause integer overflow in some systems", UserWarning)
...
def create_seeded_rng(seed: Optional[int] = None, validate_input: bool = True) -> Tuple[numpy.random.Generator, int]:
    ...
    try:
        ...
        if validate_input and seed is not None:
            is_valid, normalized_seed, error_message = validate_seed(seed, strict_mode=False)
...
def generate_deterministic_seed(seed_string: str, hash_algorithm: str = _HASH_ALGORITHM, encoding: str = _DEFAULT_ENCODING) -> int:
    ...
    try:
        ...
        if hash_algorithm not in hashlib.algorithms_available:
            available_algorithms = ', '.join(sorted(list(hashlib.algorithms_available)[:5]))  # Show first 5
            raise ValidationError(
        ...
        _logger.debug(f"Generated deterministic seed {deterministic_seed} from string '{seed_string[:50]}...'")
...
def verify_reproducibility(rng1: numpy.random.Generator, rng2: numpy.random.Generator, 
                          sequence_length: int = 1000, tolerance: float = _REPRODUCIBILITY_TOLERANCE) -> Dict[str, Any]:
    ...
    try:
        ...
        _logger.debug(f"Generating sequences of length {sequence_length} for reproducibility verification")
        ...
        sequences_match = numpy.allclose(sequence1, sequence2, atol=tolerance, rtol=tolerance)
        ...
        if not sequences_match:
            ...
            discrepancy_indices = discrepancies.tolist()[:10]  # Limit to first 10 discrepancies
        ...
        if sequences_match:
            ...
            reproducibility_report['recommendation'] = 'Sequences are reproducible within tolerance'
        else:
            reproducibility_report['status'] = 'FAIL' 
            if mean_absolute_error > tolerance * 1000:
                reproducibility_report['recommendation'] = 'Large discrepancies detected - check RNG initialization'
            else:
                reproducibility_report['recommendation'] = 'Minor discrepancies - consider adjusting tolerance'
        
        # Log reproducibility verification results
        _logger.debug(f"Reproducibility verification: {reproducibility_report['status']} "
...
def get_random_seed(use_system_entropy: bool = True, fallback_method: Optional[int] = None) -> int:
    ...
    try:
        ...
        if use_system_entropy:
            try:
                ...
                random_seed = int.from_bytes(entropy_bytes, byteorder='big', signed=False)
                
                # Apply modulo operation to ensure result is within valid seed range
                random_seed = random_seed % (SEED_MAX_VALUE + 1)
                
                _logger.debug(f"Generated random seed {random_seed} from system entropy")
                return random_seed
                
            except (OSError, NotImplementedError) as e:
                _logger.warning(f"System entropy not available: {e}, falling back to alternative method")
        ...
        if fallback_method == 1 or use_system_entropy is False:
            ...
            try:
                ...
                combined_entropy = (microseconds * 1000 + process_entropy) ^ thread_entropy
                
                # Ensure result is within valid seed range
                fallback_seed = combined_entropy % (SEED_MAX_VALUE + 1)
                
                _logger.debug(f"Generated fallback seed {fallback_seed} from time and process entropy")
...
def save_seed_state(rng: numpy.random.Generator, file_path: Union[str, pathlib.Path], 
                   metadata: Optional[Dict[str, Any]] = None, create_backup: bool = False) -> bool:
    ...
    try:
        ...
        state_data = {
            ...
            'rng_state': {
                ...
                'state': rng_state['state'].tolist() if hasattr(rng_state['state'], 'tolist') else rng_state['state']
            },
        ...
        if metadata:
            ...
            for key, value in metadata.items():
                if isinstance(key, str) and not any(sensitive in key.lower() for sensitive in ['password', 'token', 'key', 'secret']):
        ...
        _logger.debug(f"Successfully saved seed state to {file_path} ({file_size} bytes)")
        return True
...
def load_seed_state(file_path: Union[str, pathlib.Path], validate_state: bool = True, 
                   strict_version_check: bool = False) -> Tuple[numpy.random.Generator, Dict[str, Any]]:
...
class SeedManager:
    ...
    def __init__(self, default_seed: Optional[int] = None, enable_validation: bool = True, thread_safe: bool = False):
        ...
        if default_seed is not None:
            is_valid, normalized_seed, error_message = validate_seed(default_seed, strict_mode=True)
            if not is_valid:
                raise ValidationError(
        ...
        self.logger.debug(f"Initialized SeedManager (default_seed={self.default_seed}, "
                         f"validation={self.enable_validation}, thread_safe={self.thread_safe})")
    
    def seed(self, seed: Optional[int] = None, context_id: Optional[str] = None) -> Tuple[numpy.random.Generator, int]:
        ...
        try:
            ...
            try:
                ...
                if self.enable_validation and effective_seed is not None:
                    is_valid, validated_seed, error_message = validate_seed(effective_seed, strict_mode=True)
                    if not is_valid:
                        raise ValidationError(
                ...
                np_random, seed_used = create_seeded_rng(effective_seed, validate_input=False)
                ...
                if len(self.seed_history) > 1000:
                    self.seed_history = self.seed_history[-500:]  # Keep last 500 entries
                
                # Log seeding operation with context for debugging and reproducibility tracking
                self.logger.debug(f"Created seeded generator: context_id={context_id}, seed={seed_used}")
    ...
    def generate_episode_seed(self, base_seed: int, episode_number: int, 
        ...
        try:
            ...
            is_valid, validated_base_seed, error_message = validate_seed(base_seed, strict_mode=True)
            if not is_valid:
                raise ValidationError(
            ...
            combined_seed = (validated_base_seed + context_seed + episode_number * 1009) % (SEED_MAX_VALUE + 1)
            
            # Apply modulo operation to ensure result is within valid seed range
            episode_seed = combined_seed % (SEED_MAX_VALUE + 1)
            
            # Log episode seed generation with full context for reproducibility tracking
            self.logger.debug(f"Generated episode seed {episode_seed} for episode {episode_number} "
    ...
    def validate_reproducibility(self, test_seed: int, num_tests: int = 10, 
                               tolerance: float = _REPRODUCIBILITY_TOLERANCE) -> Dict[str, Any]:
        ...
        try:
            ...
            is_valid, validated_seed, error_message = validate_seed(test_seed, strict_mode=True)
            if not is_valid:
                raise ValidationError(
            ...
            self.logger.debug(f"Starting reproducibility validation with {num_tests} tests using seed {validated_seed}")
            ...
            for test_idx in range(num_tests):
                try:
                    ...
                    test_results.append({
                        ...
                        'mean_absolute_error': verification_result['statistical_analysis']['mean_absolute_error'],
                        'max_deviation': verification_result['statistical_analysis']['max_deviation'],
                        'reproducibility_score': verification_result['reproducibility_score']
                    })
            ...
            successful_tests = [r for r in test_results if r.get('sequences_match', False)]
            if successful_tests:
                mean_errors = [r['mean_absolute_error'] for r in successful_tests]
                max_deviations = [r['max_deviation'] for r in successful_tests]
                reproducibility_scores = [r['reproducibility_score'] for r in successful_tests]
            ...
            failed_tests = [r for r in test_results if not r.get('sequences_match', False)]
            failure_analysis = {
            ...
            if failed_tests:
                error_messages = [r.get('error', 'Unknown error') for r in failed_tests if 'error' in r]
                if error_messages:
                    from collections import Counter
                    error_counts = Counter(error_messages)
                    failure_analysis['common_errors'] = list(error_counts.most_common(3))
            ...
            if success_rate < 0.95:
                recommendations.append("Success rate below 95% - investigate RNG initialization")
            if aggregate_statistics['mean_of_mean_errors'] > tolerance * 10:
                recommendations.append("High mean errors detected - consider system-level issues")
            if len(failure_analysis['common_errors']) > 0:
                recommendations.append("Common error patterns detected - review error analysis")
            
            if success_rate >= 0.99:
                recommendations.append("Excellent reproducibility - suitable for scientific research")
            elif success_rate >= 0.95:
                recommendations.append("Good reproducibility - acceptable for most applications")
            
            validation_report['recommendations'] = recommendations
            
            # Log validation results for scientific documentation and debugging
            self.logger.info(f"Reproducibility validation completed: {validation_report['overall_status']} "
            ...
            try:
                ...
                for context_id, generator_info in self.active_generators.items():
                    ...
                    summary_info = {
                        ...
                        'age_seconds': current_time - generator_info['creation_timestamp'],
                        'access_count': generator_info['access_count'],
                        'last_access_timestamp': generator_info['last_access_timestamp'],
                        'seconds_since_last_access': current_time - generator_info['last_access_timestamp']
                    }
                ...
                active_generators_report = {
                    ...
                    'memory_usage_estimate': len(generator_summary) * 1024  # Rough estimate in bytes
                }
    ...
    def reset(self, preserve_default_seed: bool = True) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"SeedManager reset: cleared {num_generators} generators, "
...
class ReproducibilityTracker:
    ...
    def __init__(self, default_tolerance: float = _REPRODUCIBILITY_TOLERANCE, 
                 strict_validation: bool = False, session_id: Optional[str] = None):
        ...
        self.logger.debug(f"Initialized ReproducibilityTracker: session={self.session_id}, "
                         f"tolerance={self.default_tolerance}, strict={self.strict_validation}")
    
    def record_episode(self, episode_seed: int, action_sequence: List[Any], 
                      observation_sequence: List[Any], episode_metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            is_valid, validated_seed, error_message = validate_seed(episode_seed, strict_mode=self.strict_validation)
            if not is_valid:
                raise ValidationError(
            ...
            if self.strict_validation:
                expected_obs_length = len(action_sequence) + 1  # Initial obs + one per action
            ...
            observation_checksum = self._calculate_sequence_checksum(observation_sequence)
            ...
            if episode_metadata:
                if isinstance(episode_metadata, dict):
                    ...
                    for key, value in episode_metadata.items():
                        if isinstance(key, str) and not any(sensitive in key.lower() 
                                                          for sensitive in ['password', 'token', 'key', 'secret']):
                            sanitized_metadata[key] = value
                    episode_record['metadata'] = sanitized_metadata
                else:
                    self.logger.warning(f"Episode metadata is not a dictionary, skipping: {type(episode_metadata)}")
            ...
            self.logger.debug(f"Recorded episode {episode_record_id}: seed={validated_seed}, "
    ...
    def verify_episode_reproducibility(self, episode_record_id: str, new_action_sequence: List[Any],
                                     new_observation_sequence: List[Any], 
                                     custom_tolerance: Optional[float] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(new_action_sequence, list) or not isinstance(new_observation_sequence, list):
                raise ValidationError(
            ...
            length_match = {
                ...
                'observations_match': len(original_observations) == len(new_observation_sequence),
                'original_lengths': {'actions': len(original_actions), 'observations': len(original_observations)},
                'new_lengths': {'actions': len(new_action_sequence), 'observations': len(new_observation_sequence)}
            }
            
            if not (length_match['actions_match'] and length_match['observations_match']):
                ...
                self.test_results[verification_result['verification_id']] = verification_result
                
                return verification_result
            
            # Determine tolerance for comparison
            tolerance = custom_tolerance if custom_tolerance is not None else self.default_tolerance
            
            # Perform element-wise comparison of action sequences for exact equality
            action_comparison = self._compare_sequences(original_actions, new_action_sequence, tolerance)
            
            # Compare observation sequences using tolerance-based floating point comparison
            observation_comparison = self._compare_sequences(original_observations, new_observation_sequence, tolerance)
            
            # Calculate statistical measures including mean absolute error and maximum deviation
            overall_match = action_comparison['sequences_match'] and observation_comparison['sequences_match']
            ...
            if not overall_match:
                discrepancy_analysis = {
                    'action_discrepancies': action_comparison.get('discrepancy_indices', []),
                    'observation_discrepancies': observation_comparison.get('discrepancy_indices', []),
                    'total_discrepancies': len(action_comparison.get('discrepancy_indices', [])) + 
                ...
                if discrepancy_analysis['largest_discrepancy'] > tolerance * 1000:
                    recommendations.append("Large discrepancies suggest fundamental reproducibility issue")
                if len(discrepancy_analysis['action_discrepancies']) > 0:
                    recommendations.append("Action sequence mismatch - check RNG seeding for policy")
                if len(discrepancy_analysis['observation_discrepancies']) > 0:
                    recommendations.append("Observation sequence mismatch - check environment seeding")
            ...
            if overall_match:
                verification_result['status_message'] = "Episode successfully reproduced"
            else:
                verification_result['status_message'] = "Episode reproducibility verification failed"
            
            # Store verification results in test_results dictionary for cumulative analysis
            self.test_results[verification_result['verification_id']] = verification_result
            
            # Log verification results
            self.logger.debug(f"Episode verification {verification_result['verification_id']}: "
    ...
    def _compare_sequences(self, seq1: List[Any], seq2: List[Any], tolerance: float) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if not comparison_result['sequences_match']:
                    ...
                    comparison_result['discrepancy_indices'] = numpy.where(discrepancy_mask)[0].tolist()
    ...
    def generate_reproducibility_report(self, report_format: Optional[str] = 'dict', 
                                      include_detailed_analysis: bool = True,
                                      custom_sections: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_verifications > 0:
                successful_verifications = [v for v in self.test_results.values() 
                ...
                if successful_verifications:
                    scores = [v.get('reproducibility_score', 0.0) for v in successful_verifications]
                    statistical_summary = {
            ...
            report = {
                ...
                'summary_statistics': {
                    ...
                    'successful_verifications': len([v for v in self.test_results.values() 
                                                   if v.get('sequences_match', False)]),
                    'failed_verifications': len([v for v in self.test_results.values() 
                ...
                'reproducibility_assessment': {
                    ...
                    'confidence_level': self._calculate_confidence_level(total_verifications, success_rate),
                    'recommendations': self._generate_reproducibility_recommendations(success_rate, statistical_summary)
                }
            }
            
            # Generate detailed failure analysis if include_detailed_analysis is True
            if include_detailed_analysis:
                failed_tests = [v for v in self.test_results.values() if not v.get('sequences_match', False)]
                ...
                if failed_tests:
                    ...
                    for test in failed_tests:
                        if 'discrepancy_analysis' in test:
                            discrepancy = test['discrepancy_analysis'].get('largest_discrepancy', 0.0)
                            discrepancies.append({
                                'verification_id': test.get('verification_id', 'unknown'),
                                'discrepancy_magnitude': discrepancy
                            })
                    
                    # Sort by discrepancy magnitude and take top 5
                    discrepancies.sort(key=lambda x: x['discrepancy_magnitude'], reverse=True)
    ...
    def _generate_reproducibility_recommendations(self, success_rate: float, 
                                                statistical_summary: Dict[str, Any]) -> List[str]:
        """Generate recommendations based on reproducibility analysis."""
        recommendations = []
        
        if success_rate < 0.95:
            recommendations.append("Success rate below 95% - investigate seeding and initialization procedures")
        
        if statistical_summary and statistical_summary.get('std_reproducibility_score', 0) > 0.1:
            recommendations.append("High variability in reproducibility scores - check for non-deterministic components")
        
        if success_rate >= 0.99:
            recommendations.append("Excellent reproducibility achieved - suitable for scientific publication")
        
        if len(self.test_results) < 50:
            recommendations.append("Consider running more verification tests for statistical significance")
    ...
    def _analyze_episode_lengths(self) -> Dict[str, Any]:
        ...
        action_lengths = [record['sequence_lengths']['actions'] for record in self.episode_records.values()]
        observation_lengths = [record['sequence_lengths']['observations'] for record in self.episode_records.values()]
    ...
    def _analyze_temporal_patterns(self) -> Dict[str, Any]:
        ...
        recording_times = [record['recording_timestamp'] for record in self.episode_records.values()]
        verification_times = [result['verification_timestamp'] for result in self.test_results.values()]
        
        analysis = {
            'recording_time_span': max(recording_times) - min(recording_times) if recording_times else 0,
        ...
        if verification_times:
            analysis.update({
                'verification_time_span': max(verification_times) - min(verification_times),
    ...
    def _format_markdown_report(self, report: Dict[str, Any]) -> str:
        ...
        markdown += f"- Total Verifications: {summary['total_verifications_performed']}\n"
    ...
    def export_reproducibility_data(self, export_path: Union[str, pathlib.Path], 
                                  export_format: str = 'json', include_raw_data: bool = True,
        ...
        try:
            ...
            if export_format == 'json':
                ...
                with open(export_path, 'w', newline='', encoding='utf-8') as f:
                    ...
                    # Write header
                    writer.writerow([
                        'verification_id', 'episode_record_id', 'match_status',
                        'sequences_match', 'reproducibility_score', 'verification_timestamp'
                    ])
            ...
            self.logger.info(f"Successfully exported reproducibility data to {export_path} "
