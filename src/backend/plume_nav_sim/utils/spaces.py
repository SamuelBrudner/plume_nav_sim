...
from typing import Dict, List, Optional, Union, Any, Tuple, Type  # >=3.10 - Type hints for space factory functions, validation methods, and parameter specifications
...
# Module exports
__all__ = [
    'create_action_space', 'create_observation_space', 'validate_action', 
    'validate_observation', 'validate_action_space', 'validate_observation_space', 
    'SpaceConfig', 'SpaceValidator', 'get_space_info', 'sample_valid_action', 
    'sample_valid_observation', 'check_space_compatibility', 'optimize_space_operations'
]
...
def validate_action(action: ActionType, action_space: Optional[gymnasium.spaces.Discrete] = None,
                   strict_mode: bool = False, log_validation: bool = False) -> int:
    """
    ...
    try:
        ...
        if strict_mode:
            ...
            if validated_action not in [ACTION_UP, ACTION_RIGHT, ACTION_DOWN, ACTION_LEFT]:
                raise ValidationError(
                    f"Strict mode: action {validated_action} not in standard cardinal directions",
                    context={'standard_actions': [ACTION_UP, ACTION_RIGHT, ACTION_DOWN, ACTION_LEFT]}
                )
...
def validate_observation(observation: ObservationType, 
                        observation_space: Optional[gymnasium.spaces.Box] = None,
                        check_range: bool = True, strict_mode: bool = False,
                        log_validation: bool = False) -> np.ndarray:
    """
    ...
    try:
            ...
            if obs_min < min_val or obs_max > max_val:
                raise ValidationError(
                    ...
                    context={
                        ...
                        'out_of_bounds_count': np.sum((observation < min_val) | (observation > max_val))
                    }
                )
        ...
        logger.error(
            ...
            extra={
                'observation_type': type(observation).__name__ if observation is not None else None,
                'error_type': type(e).__name__
            }
        )
        raise ValidationError(
...
def validate_action_space(action_space: gymnasium.spaces.Discrete, check_metadata: bool = False,
        ...
        if action_space.n != ACTION_SPACE_SIZE:
            if strict_validation:
                raise ValidationError(
                    ...
                    context={'space_size': action_space.n, 'expected_size': ACTION_SPACE_SIZE}
                )
            else:
                warnings.warn(
            ...
            for _ in range(10):  # Test multiple samples
                ...
                if not isinstance(sample, (int, np.integer)):
                    raise ValidationError(
                        ...
                        context={'sample_type': type(sample).__name__, 'sample_value': sample}
                    )
                
                if sample < 0 or sample >= action_space.n:
                    raise ValidationError(
                        f"Action space sample {sample} out of valid range [0, {action_space.n-1}]",
                        context={'sample': sample, 'valid_range': (0, action_space.n-1)}
                    )
        except Exception as e:
            raise ValidationError(
            ...
            for invalid_action in invalid_actions:
                if action_space.contains(invalid_action):
                    raise ValidationError(
                        ...
                        context={'invalid_action': invalid_action, 'space_size': action_space.n}
                    )
        except ValidationError:
            raise
        except Exception as e:
                ...
                if missing_keys:
                    if strict_validation:
                        raise ValidationError(
                            ...
                            context={'metadata': metadata, 'missing_keys': missing_keys}
                        )
                    else:
                        warnings.warn(
            ...
            if action_value < action_space.n:
                if not action_space.contains(action_value):
                    raise ValidationError(
                        ...
                        context={'action_enum': action_enum.name, 'action_value': action_value}
                    )
...
def validate_observation_space(observation_space: gymnasium.spaces.Box, check_bounds: bool = True,
                             check_dtype: bool = True, strict_validation: bool = False) -> bool:
    """
    ...
    try:
        ...
        try:
            for _ in range(10):  # Test multiple samples
                ...
                if check_bounds:
                    if np.any(sample < observation_space.low) or np.any(sample > observation_space.high):
                        raise ValidationError(
                            f"Sample values outside space bounds",
                            context={
                                'sample_range': (float(sample.min()), float(sample.max())),
                                'space_bounds': (observation_space.low.min(), observation_space.high.max())
                            }
                        )
        except ValidationError:
            raise
        ...
        try:
            ...
            warnings.warn(f"Could not test concentration compatibility: {e}", UserWarning)
        ...
        logger.error(
            ...
            extra={
                'space_type': type(observation_space).__name__ if observation_space else None,
                'error_type': type(e).__name__
            }
        )
        raise ValidationError(
...
def get_space_info(space: Union[gymnasium.spaces.Space], include_samples: bool = False,
                  include_metadata: bool = False, include_performance_info: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if include_metadata:
            ...
            if isinstance(space, gymnasium.spaces.Discrete) and space.n == ACTION_SPACE_SIZE:
            ...
            elif isinstance(space, gymnasium.spaces.Box) and space.shape == DEFAULT_OBSERVATION_SHAPE:
                info['plume_nav_compatible'] = True
        ...
        return {
            'error': str(e),
            'space_type': type(space).__name__ if space else None
        }


def sample_valid_action(action_space: Optional[gymnasium.spaces.Discrete] = None,
                       excluded_actions: Optional[List[int]] = None,
                       validate_sample: bool = True) -> int:
    """
    ...
    try:
        ...
        raise ValidationError(
            ...
            context={
                ...
                'available_actions': [i for i in range(action_space.n) if i not in excluded_set]
            }
        )
        ...
        raise ValidationError(
            f"Action sampling failed: {e}",
            context={'original_error': str(e)}
        )


def sample_valid_observation(observation_space: Optional[gymnasium.spaces.Box] = None,
                           concentration_range: Optional[Tuple[float, float]] = None,
                           validate_sample: bool = True) -> np.ndarray:
    """
    ...
    try:
        ...
        if concentration_range is not None:
            if not isinstance(concentration_range, tuple) or len(concentration_range) != 2:
                raise ValidationError(
        ...
        if validate_sample:
            validated_observation = validate_observation(
                observation_sample, 
                observation_space,
                check_range=concentration_range is None  # Skip range check if we manually constrained
            )
        ...
        raise ValidationError(
            f"Observation sampling failed: {e}",
            context={'original_error': str(e)}
        )


def check_space_compatibility(action_space: gymnasium.spaces.Discrete,
                            observation_space: gymnasium.spaces.Box,
                            strict_checking: bool = False,
                            compatibility_requirements: Optional[Dict] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            compatibility_report['issues'].append(f"Action space validation failed: {e}")
        
        try:
            validate_observation_space(observation_space, strict_validation=strict_checking)
        except ValidationError as e:
            compatibility_report['compatible'] = False
            compatibility_report['issues'].append(f"Observation space validation failed: {e}")
        ...
        if strict_checking:
            ...
            expected_low, expected_high = CONCENTRATION_RANGE
            if not (np.allclose(observation_space.low, expected_low) and 
                   np.allclose(observation_space.high, expected_high)):
                compatibility_report['compatible'] = False
                compatibility_report['issues'].append(
        ...
        if compatibility_report['compatible']:
            compatibility_report['recommendations'].append("Spaces are compatible for RL environment use")
        else:
            compatibility_report['recommendations'].append("Fix identified issues before using spaces in environment")
        
        if compatibility_report['warnings']:
            compatibility_report['recommendations'].append("Consider addressing warnings for optimal performance")
...
def optimize_space_operations(space_config: Dict, enable_caching: bool = True,
                            profile_operations: bool = False,
                            optimization_settings: Optional[Dict] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if profile_operations:
            ...
            if 'action_creation_ms' in optimization_results.get('performance_baseline', {}):
                if optimization_results['performance_baseline']['action_creation_ms'] > 10.0:
                    bottlenecks.append('Action space creation is slow (>10ms)')
                    
            if 'observation_creation_ms' in optimization_results.get('performance_baseline', {}):
                if optimization_results['performance_baseline']['observation_creation_ms'] > 10.0:
        ...
        if optimization_settings:
            for setting_name, setting_value in optimization_settings.items():
                if setting_name == 'disable_validation':
                    if setting_value:
                        optimization_results['optimization_applied'].append('Validation disabled for performance')
                    ...
                    if setting_value:
                        optimization_results['optimization_applied'].append('Fast dtype conversion enabled')
                        
                elif setting_name == 'memory_optimization':
                    if setting_value:
                        optimization_results['optimization_applied'].append('Memory optimization enabled')
                        optimization_results['recommendations'].append('Monitor memory usage with optimization')
        ...
        if profile_operations and enable_caching:
            ...
            if 'performance_baseline' in optimization_results:
                ...
                if 'action_creation_ms' in baseline and baseline['action_creation_ms'] > 0:
                    action_improvement = baseline['action_creation_ms'] / cached_action_time
                    optimized_performance['action_speed_improvement'] = f"{action_improvement:.2f}x"
                    
                if 'observation_creation_ms' in baseline and baseline['observation_creation_ms'] > 0:
                    obs_improvement = baseline['observation_creation_ms'] / cached_obs_time
                    optimized_performance['observation_speed_improvement'] = f"{obs_improvement:.2f}x"
        ...
        if space_config.get('memory_constrained'):
            general_recommendations.append("Reduce cache sizes in memory-constrained environments")
        ...
        logger.info(
            ...
            extra={
                'optimizations_applied': len(optimization_results['optimization_applied']),
...
class SpaceConfig:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.observation_shape, tuple) or len(self.observation_shape) == 0:
            raise ValidationError(
        ...
        if (not isinstance(self.concentration_bounds, tuple) or 
            len(self.concentration_bounds) != 2 or
            self.concentration_bounds[0] >= self.concentration_bounds[1]):
            raise ValidationError(
    ...
    def validate_config(self, strict_validation: bool = False) -> bool:
        ...
        try:
            ...
            if not all(isinstance(dim, int) and dim > 0 for dim in self.observation_shape):
                raise ValidationError(
    ...
    def estimate_performance(self, include_memory_usage: bool = True,
                           include_timing_estimates: bool = True) -> Dict[str, Any]:
        """
        ...
        if include_timing_estimates:
            ...
            performance_report['timing_estimates'] = {
                ...
                'total_space_creation_ms': base_action_creation + base_observation_creation
            }
        ...
        if self.enable_validation:
            recommendations.append("Validation enabled - consider disabling in performance-critical paths")
        
        if self.enable_caching:
            recommendations.append("Caching enabled - will improve performance for repeated operations")
        ...
        if include_memory_usage and 'memory_usage' in performance_report:
            ...
            if memory_mb > 1.0:
                recommendations.append(f"High memory usage ({memory_mb:.2f} MB/obs) - consider smaller dtype")
        
        performance_report['recommendations'] = recommendations
        
        return performance_report
    
    def to_dict(self, include_metadata: bool = True, include_optimization_settings: bool = True) -> Dict[str, Any]:
        """
...
class SpaceValidator:
    ...
    def validate_space(self, space: gymnasium.spaces.Space, space_type: str,
        ...
        try:
            ...
            if not hasattr(space, 'sample') or not callable(space.sample):
                ...
                validation_result['errors'].append("Space missing required sample() method")
            
            if not hasattr(space, 'contains') or not callable(space.contains):
                validation_result['valid'] = False
                validation_result['errors'].append("Space missing required contains() method")
            ...
            if space_type == 'action':
                if isinstance(space, gymnasium.spaces.Discrete):
                    try:
                        ...
                        validation_result['valid'] = False
                        validation_result['errors'].append(f"Action space validation failed: {e}")
                else:
                    validation_result['valid'] = False
                    validation_result['errors'].append(f"Expected Discrete action space, got {type(space)}")
                    
            elif space_type == 'observation':
                if isinstance(space, gymnasium.spaces.Box):
                    try:
                        validate_observation_space(space, strict_validation=self.strict_mode)
                    except ValidationError as e:
                        validation_result['valid'] = False
                        validation_result['errors'].append(f"Observation space validation failed: {e}")
                else:
                    validation_result['valid'] = False
                    validation_result['errors'].append(f"Expected Box observation space, got {type(space)}")
                ...
                if req_name == 'required_methods':
                    for method_name in req_value:
                        if not hasattr(space, method_name):
                            validation_result['warnings'].append(f"Space missing recommended method: {method_name}")
            ...
            if self.strict_mode:
                ...
                if hasattr(space, 'seed'):
                    try:
                        space.seed(42)
                        validation_result['recommendations'].append("Space seeding tested successfully")
                    except Exception as e:
                        validation_result['warnings'].append(f"Space seeding failed: {e}")
            ...
            if validation_result['valid']:
                validation_result['recommendations'].append("Space passed all validation checks")
            else:
                validation_result['recommendations'].append("Fix validation errors before using space")
    ...
    def validate_action_space(self, action_space: gymnasium.spaces.Discrete,
                            check_navigation_compatibility: bool = True) -> Dict[str, Any]:
        """
        ...
        if check_navigation_compatibility and result['valid']:
            if action_space.n == ACTION_SPACE_SIZE:
                result['recommendations'].append("Action space compatible with cardinal direction navigation")
            else:
                result['warnings'].append(f"Action space size {action_space.n} may not be optimal for navigation")
        
        return result
    
    def validate_observation_space(self, observation_space: gymnasium.spaces.Box,
                                 check_concentration_compatibility: bool = True) -> Dict[str, Any]:
        """
        ...
        result = self.validate_space(observation_space, 'observation', validation_requirements)
        
        # Additional observation space specific validation
        if check_concentration_compatibility and result['valid']:
            bounds_match = (np.allclose(observation_space.low, CONCENTRATION_RANGE[0]) and 
                          np.allclose(observation_space.high, CONCENTRATION_RANGE[1]))
            
            if bounds_match:
                result['recommendations'].append("Observation space compatible with concentration values")
            else:
                result['warnings'].append("Observation space bounds may not match concentration range")
    ...
    def clear_cache(self) -> Dict[str, Any]:
        ...
        if self.validation_cache:
            ...
            cache_report['memory_freed_estimate_mb'] = cache_report['cache_entries_cleared'] * 0.001  # 1KB per entry estimate
            ...
            self.logger.info(f"Cleared validation cache: {cache_report['cache_entries_cleared']} entries")
        
        return cache_report
    
    def get_validation_stats(self, include_cache_stats: bool = True,
                           include_performance_analysis: bool = True) -> Dict[str, Any]:
        """
        ...
        if include_cache_stats and self.enable_caching:
            total_cache_requests = self.validation_stats['cache_hits'] + self.validation_stats['cache_misses']
        ...
        if include_performance_analysis:
            ...
            if self.validation_stats['total_validations'] > 0:
                perf_stats['average_validation_time_ms'] = round(
                    perf_stats['total_validation_time_ms'] / self.validation_stats['total_validations'], 3
                )
    ...
    def _test_space_performance(self, space: gymnasium.spaces.Space, 
                              performance_requirements: Dict, validation_result: Dict):
        """
