...
class ValidationContext:
    ...
    validation_id: str = dataclasses.field(
    ...
    def merge_context(
        self, context_data: Dict[str, Any], sanitize_sensitive_data: bool = True
    ) -> None:
        """
    ...
    def generate_summary(
        ...
        if include_details and (error_count > 0 or warning_count > 0):
            ...
            for warning in self.warnings:
                param_info = (
                    f" ({warning['parameter']})" if "parameter" in warning else ""
                )
...
class ParameterValidator:
    ...
    def validate_parameter(  # noqa: C901
        ...
        cache_key = self._generate_cache_key(
            parameter_name, parameter_value, validation_constraints
        )
        ...
        if result.is_valid:
            self.logger.debug(
                f"Parameter {parameter_name} validated successfully in {validation_time_ms:.3f}ms"
            )
        else:
            self.logger.warning(
                f"Parameter {parameter_name} validation failed in {validation_time_ms:.3f}ms"
            )
    ...
    def batch_validate(
        ...
        for param_name, param_value in parameters.items():
            ...
            result = self.validate_parameter(
                param_name, param_value, constraints, context
            )
    ...
    def get_validation_stats(
        ...
        if stats["total_validations"] > 0:
            stats["average_validation_time_ms"] = (
                stats["total_timing_ms"] / stats["total_validations"]
            )
    ...
    def _apply_validation_rules(
        ...
        for rule_name, rule_info in self.rule_engine.items():
            try:
                rule_result = rule_info["function"](
                    parameter_value, validation_constraints
                )
                if not rule_result:
                    result.add_error(
...
def validate_action_parameter(  # noqa: C901
    ...
    try:
        ...
        if isinstance(action, Action) and allow_enum_types:
            ...
            try:
                ...
                raise ValidationError(
                    ...
                    parameter_constraints={
                        "expected_types": ["int", "numpy.integer", "Action enum"]
                    },
                )
        else:
            raise ValidationError(
        ...
        if (
            ...
            or validated_action < 0
            or validated_action >= ACTION_SPACE_SIZE
        ):
        ...
        if validation_time > PERFORMANCE_TARGET_STEP_LATENCY_MS:
            logger.warning(
                f"Action validation took {validation_time:.3f}ms (target: {PERFORMANCE_TARGET_STEP_LATENCY_MS}ms)"
            )
...
def validate_observation_parameter(  # noqa: C901
    ...
    try:
        ...
        if strict_dtype_checking:
            if observation.dtype != np.float32:
                if observation.dtype in [np.float64, np.int32, np.int64]:
                    ...
                    logger.debug(
                        f"Converted observation dtype from {observation.dtype} to float32"
                    )
                else:
                    raise ValidationError(
        ...
        if not observation.flags.c_contiguous:
            logger.debug(
                "Observation array is not C-contiguous, performance may be impacted"
            )
    ...
    try:
        ...
        if isinstance(coordinates, Coordinates):
            ...
            raise ValidationError(
                ...
                parameter_constraints={
                    "expected_types": ["Coordinates", "tuple", "list"]
                },
            )
        ...
        try:
            ...
            if (
                abs(x - x_int) > NUMERIC_PRECISION_TOLERANCE
                or abs(y - y_int) > NUMERIC_PRECISION_TOLERANCE
            ):
                if strict_integer_conversion:
                    raise ValidationError(
                        f"Precision loss in coordinate conversion: ({x}, {y}) -> ({x_int}, {y_int})",
                        parameter_constraints={
                            "precision_tolerance": NUMERIC_PRECISION_TOLERANCE
                        },
                    )
                else:
                    logger.debug(
                        f"Rounded coordinates from ({x}, {y}) to ({x_int}, {y_int})"
                    )
    ...
    try:
        ...
        if validate_performance:
            ...
            if estimated_step_time > PERFORMANCE_TARGET_STEP_LATENCY_MS:
                logger.warning(
                    f"Estimated step time ({estimated_step_time:.3f}ms) may exceed target ({PERFORMANCE_TARGET_STEP_LATENCY_MS}ms)"
                )
    ...
    try:
        ...
        if grid_size is not None:
            if not source_coords.is_within_bounds(grid_size):
                raise ValidationError(
                    ...
                    parameter_constraints={
                        "grid_bounds": (grid_size.width, grid_size.height)
                    },
                )
        ...
        if check_mathematical_consistency and grid_size is not None:
            ...
            if sigma > grid_diagonal:
                logger.warning(
                    f"Large sigma ({sigma}) relative to grid diagonal ({grid_diagonal:.1f})"
                )
            elif sigma < grid_diagonal / 20:
                logger.warning(
                    f"Small sigma ({sigma}) relative to grid diagonal ({grid_diagonal:.1f}) may create sharp concentration gradients"
                )
        ...
        if check_backend_availability and validated_mode == RenderMode.HUMAN:
            try:
                ...
                if current_backend == "Agg":
                    logger.warning(
                        "Agg backend detected - interactive rendering may not be available"
                    )
        ...
        if validate_display_capability and validated_mode == RenderMode.HUMAN:
            try:
                ...
                if sys.platform != "win32" and "DISPLAY" not in os.environ:
                    logger.warning(
                        "No DISPLAY environment variable - interactive rendering may fail"
                    )

                # Check if running in headless environment
                if os.environ.get("CI") or os.environ.get("GITHUB_ACTIONS"):
                    logger.warning(
                        "Headless environment detected - human rendering may not work"
                    )
    ...
    try:
        ...
        if isinstance(seed, (int, np.integer)):
            ...
            raise ValidationError(
                ...
                parameter_constraints={
                    "expected_types": ["int", "numpy.integer", "None"]
                },
            )
        ...
        if validated_seed == 0:
            logger.debug(
                "Using seed value 0 - ensure this is intended for reproducibility"
            )
        elif validated_seed < 0:
            logger.debug(
                f"Using negative seed {validated_seed} - behavior may vary across platforms"
            )
        ...
        try:
            ...
            logger.debug(
                f"Seed {validated_seed} compatibility test passed (generated: {test_value})"
            )
        except Exception as e:
            raise ValidationError(
    ...
    try:
        ...
        if check_system_capabilities:
            ...
            try:
                ...
                if (
                    memory_limit and memory_limit > available_memory * 0.8
                ):  # Leave 20% buffer
                    logger.warning(
                        f"Memory requirement {memory_limit}MB may exceed available memory {available_memory:.0f}MB"
                    )
        ...
        if validate_timing_targets:
            ...
            try:
                ...
                if step_latency and benchmark_time > step_latency:
                    logger.warning(
                        f"Benchmark operation took {benchmark_time:.4f}ms, target is {step_latency}ms"
                    )
        ...
        if step_latency and step_latency < 1.0:
            recommendations.append(
                "Consider caching for sub-millisecond latency targets"
            )

        if memory_limit and memory_limit > 1000:
            recommendations.append(
                "Large memory allocation may benefit from memory mapping"
            )
...
def sanitize_parameters(  # noqa: C901
    ...
    if context is None:
        context = create_validation_context(
            operation_name="sanitize_parameters", component_name="security"
        )
    ...
    try:
        ...
        if grid_size and source_location:
            ...
            if grid_width and grid_height:
                if hasattr(source_location, "x") and hasattr(source_location, "y"):
                    ...
                    isinstance(source_location, (tuple, list))
                ...
                if src_x is not None and src_y is not None:
                    ...
                    if strict_consistency_checking:
                        ...
                        if (
                            src_x < margin
                            or src_x > grid_width - margin
                            or src_y < margin
                            or src_y > grid_height - margin
                        ):
                            issues.append(
                                {
            ...
            for rule in consistency_rules:
                try:
                    ...
                    if rule == "performance_vs_accuracy":
                        if (
                            grid_size
                            and isinstance(grid_size, (tuple, list))
                            and len(grid_size) == 2
                        ):
        ...
        if strict_consistency_checking:
            ...
            if grid_size and isinstance(grid_size, (tuple, list)):
                estimated_memory = (grid_size[0] * grid_size[1] * 4) / (
                    1024 * 1024
                )  # Rough MB estimate
...
def validate_with_context(  # noqa: C901
    ...
    # Initialize result
    result = ValidationResult(
        is_valid=True, operation_name=context.operation_name, context=context
    )
    ...
    try:
        ...
        if cache_results:
            ...
            if cache_key in VALIDATION_PERFORMANCE_CACHE:
                ...
                logger.debug(
                    f"Using cached validation result for {validation_function.__name__}"
                )
        ...
        result.add_error(
            str(e), recovery_suggestion="Check parameter values and constraints"
        )
    ...
    if result.is_valid:
        ...
        logger.warning(
            f"Validation {context.operation_name} failed with {len(result.errors)} errors"
        )
...
def get_validation_summary(  # noqa: C901
    ...
    if include_performance_analysis:
        ...
        if performance_metrics:
            ...
            execution_times = [
                m.get("execution_time_ms", 0)
                for m in performance_metrics
                if "execution_time_ms" in m
            ]
            if execution_times:
    ...
    if include_optimization_suggestions:
        ...
        if success_rate < 0.5:
            system_recommendations.append(
                "Low success rate - review validation rules and parameter constraints"
            )
        if total_errors > total_validations * 2:
            system_recommendations.append(
                "High error density - consider parameter validation early in pipeline"
            )
    ...
    summary["common_error_patterns"] = sorted(
...
def optimize_validation_performance(  # noqa: C901
    ...
    try:
        ...
        if (
            "execution_path_optimization"
            in optimization_results["optimizations_applied"]
        ):
        ...
        optimization_results["performance_improvements"] = {
            ...
            "estimated_cache_hit_rate": (
                "70-85%" if enable_aggressive_caching else "40-60%"
            ),
        ...
        if not enable_aggressive_caching:
            recommendations.append(
                "Enable aggressive caching for repeated validation operations"
            )

        if validation_config.get("validation_depth") == "comprehensive":
            recommendations.append(
                "Consider reducing validation depth for performance-critical paths"
            )
...
def validate_resource_constraints(
    ...
    if context is None:
        context = create_validation_context(
            "validate_resource_constraints", "resources"
        )
