...
CACHE_TYPES = {
    'python': {'patterns': ['**/__pycache__', '**/*.pyc', '**/*.pyo'], 'description': 'Python bytecode cache files'},
    'pytest': {'patterns': ['**/.pytest_cache'], 'description': 'pytest testing cache'},
    'logging': {'patterns': ['**/logs/*.log*', '**/plume_nav_sim*.log*'], 'description': 'Log files and logging cache'},
    'matplotlib': {'patterns': ['**/.matplotlib'], 'description': 'Matplotlib rendering cache'},
    'performance': {'patterns': ['**/performance_*.json', '**/benchmarks/*.tmp'], 'description': 'Performance measurement baselines and temporary files'},
    'temp': {'patterns': ['**/tmp_*', '**/.tmp', '**/temp_*'], 'description': 'Temporary development files'}
}
...
def main() -> int:
    ...
    try:
        ...
        if args.cache_types:
            ...
            if invalid_types:
                logger.error(f"Invalid cache types: {invalid_types}. Valid types: {list(CACHE_TYPES.keys())}")
                return 1
        ...
        selected_types = args.cache_types if args.cache_types else list(CACHE_TYPES.keys())
        ...
        logger.info(f"Cache cleanup completed: {cleanup_stats['total_files_removed']} files, "
        ...
        if cleanup_stats['warnings']:
            logger.warning(f"Cleanup completed with {len(cleanup_stats['warnings'])} warnings")
            return 2
...
def clean_python_cache(root_dir: Path, dry_run: bool = False, verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for pycache_dir in pycache_dirs:
            try:
                size, file_count = calculate_directory_size(pycache_dir, follow_symlinks=False)
                total_size += size
            except Exception as e:
            ...
            try:
                ...
                if not dry_run:
                    ...
                    if pycache_dir.name == '__pycache__':
                        ...
                        stats['warnings'].append(f"Skipped non-__pycache__ directory: {pycache_dir}")
                else:
                    stats['directories_removed'] += 1
        ...
        logger.info(f"Python cache cleanup completed: {stats['directories_removed']} directories, "
                   f"{stats['files_removed']} files, {stats['space_reclaimed_mb']:.2f} MB")
        
        if stats['errors'] and verbose:
            logger.warning(f"Encountered {len(stats['errors'])} errors during Python cache cleanup")
...
def clean_pytest_cache(root_dir: Path, preserve_config: bool = True, 
    ...
    try:
        ...
        if preserve_config:
            ...
            if verbose and config_files:
                logger.info(f"Found {len(config_files)} pytest configuration files to preserve")
        
        # Calculate size of cache directories for space reporting
        total_size = 0
        for cache_dir in pytest_cache_dirs:
            try:
                size, file_count = calculate_directory_size(cache_dir, follow_symlinks=False)
                total_size += size
            except Exception as e:
            ...
            try:
                ...
                if not dry_run:
                    ...
                    if cache_dir.name == '.pytest_cache':
                        ...
                        stats['warnings'].append(f"Skipped non-pytest cache directory: {cache_dir}")
                else:
                    stats['cache_directories_removed'] += 1
        ...
        for pattern in temp_patterns:
            for temp_file in root_dir.rglob(pattern):
                try:
                    ...
                    stats['errors'].append(f"Permission denied removing {temp_file}: {e}")
                except FileNotFoundError:
        ...
        logger.info(f"Pytest cache cleanup completed: {stats['cache_directories_removed']} directories, "
...
def clean_logging_cache(root_dir: Path, preserve_recent_logs: bool = True, 
                       dry_run: bool = False, verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            if not dry_run:
                ...
                if verbose:
                    logger.info(f"Cleared {factory_cleared} LoggerFactory cached instances")
        except Exception as e:
        ...
        for log_dir in log_directories:
            if log_dir.exists():
                ...
                log_patterns = ['*.log', '*.log.*', f'{PACKAGE_NAME}*.log*', 'performance*.log*']
        ...
        if verbose:
            logger.info(f"Found {len(log_files)} log files totaling {total_log_size / (1024 * 1024):.2f} MB")
        ...
        performance_patterns = ['**/performance_*.json', '**/timing_*.json', '**/benchmarks/*.tmp']
        for pattern in performance_patterns:
            for perf_file in root_dir.rglob(pattern[3:]):  # Remove **/ prefix
                try:
                    ...
                    stats['errors'].append(f"Permission denied removing {perf_file}: {e}")
                except FileNotFoundError:
                    pass
                except Exception as e:
        ...
        logger.info(f"Logging cache cleanup completed: {stats['logger_instances_cleared']} logger instances, "
...
def clean_matplotlib_cache(root_dir: Path, preserve_font_cache: bool = True, 
                           dry_run: bool = False, verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if verbose:
            logger.info(f"Found {len(matplotlib_cache_dirs)} matplotlib cache directories")
        ...
        for cache_dir in matplotlib_cache_dirs:
            try:
                ...
                if not preserve_font_cache:
                    font_patterns = ['*.ttf', '*.otf', 'fontList.json', 'fontlist-*.json']
                    for pattern in font_patterns:
                        font_cache_files.extend(cache_dir.rglob(pattern))
                
            except Exception as e:
                stats['errors'].append(f"Error processing matplotlib cache {cache_dir}: {e}")
            ...
            try:
                ...
                if preserve_font_cache:
                    ...
                    for item in cache_dir.rglob('*'):
                        if item.is_file() and not any(item.name.endswith(ext) for ext in ['.ttf', '.otf']) and 'font' not in item.name.lower():
                            try:
                                if not dry_run:
                                    item.unlink()
                    ...
                    stats['font_cache_preserved'] += len(list(cache_dir.rglob('*font*')))
                else:
            ...
            if temp_dir.exists():
                for pattern in temp_patterns:
                    for temp_file in temp_dir.glob(pattern):
                        try:
                            if verbose:
                                logger.debug(f"Removing matplotlib temp file: {temp_file}")
        ...
        logger.info(f"Matplotlib cache cleanup completed: {stats['cache_directories_removed']} directories, "
...
def clean_performance_cache(root_dir: Path, preserve_baselines: bool = False, 
                           dry_run: bool = False, verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        performance_patterns = {
            'baselines': ['**/performance_*.json', '**/baseline_*.json', '**/perf_*.json'],
            'benchmarks': ['**/benchmarks/**/*.tmp', '**/benchmarks/**/*.temp', '**/benchmark_*.json'],
        ...
        if verbose:
            ...
            logger.info(f"Found {total_files} performance files totaling {total_size / (1024 * 1024):.2f} MB")
        ...
        for category, files in performance_files.items():
            for perf_file in files:
                try:
                    ...
                    stats['errors'].append(f"Error checking performance file {perf_file}: {e}")
        ...
        for baseline_file in files_to_remove['baselines']:
            try:
                ...
                stats['errors'].append(f"Permission denied removing {baseline_file}: {e}")
            except FileNotFoundError:
                pass
            except Exception as e:
        ...
        for benchmark_file in files_to_remove['benchmarks']:
            try:
                ...
                stats['errors'].append(f"Permission denied removing {benchmark_file}: {e}")
            except FileNotFoundError:
                pass
            except Exception as e:
        ...
        logger.info(f"Performance cache cleanup completed: {stats['baseline_files_removed']} baselines, "
...
def clean_temporary_files(root_dir: Path, max_age_days: int = 7, 
                         dry_run: bool = False, verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        logger.debug(f"Starting temporary file cleanup in {root_dir} (max age: {max_age_days} days)")
            ...
            try:
                ...
                if item_mtime < age_threshold:
                    ...
                    if temp_item.is_file():
                        ...
                        size, _ = calculate_directory_size(temp_item, follow_symlinks=False)
                        total_size += size
                else:
                    stats['files_preserved_by_age'] += 1
                    if verbose:
                        age_days = (current_time - item_mtime) / (24 * 3600)
                        logger.debug(f"Preserving recent temp item: {temp_item} (age: {age_days:.1f} days)")
                
            except Exception as e:
                stats['errors'].append(f"Error checking temp item {temp_item}: {e}")
        
        if verbose:
            logger.info(f"Will remove {len(items_to_remove)} items older than {max_age_days} days, "
                ...
                if not dry_run:
                    if temp_item.is_file():
                        try:
                            ...
                            stats['errors'].append(f"Error removing file {temp_item}: {e}")
                    
                    elif temp_item.is_dir():
                        try:
                            dir_size, _ = calculate_directory_size(temp_item, follow_symlinks=False)
                            shutil.rmtree(temp_item)
                            removed_size += dir_size
                            stats['temp_directories_removed'] += 1
                        except Exception as e:
                            stats['errors'].append(f"Error removing directory {temp_item}: {e}")
                else:
                    # Dry run counting
                    if temp_item.is_file():
                        stats['temp_files_removed'] += 1
        ...
        if not dry_run:
            for temp_item in reversed(items_to_remove):  # Process in reverse to handle nested directories
        ...
        logger.info(f"Temporary file cleanup completed: {stats['temp_files_removed']} files, "
...
def create_backup(source_path: Path, backup_type: str, backup_dir: Path) -> Optional[Path]:
    """
    ...
    try:
        ...
        try:
            if source_path.is_file():
                ...
                final_backup_path = Path(shutil.make_archive(archive_path, 'zip', source_path))
            
            # Verify backup archive integrity and completeness
            if final_backup_path.exists():
                backup_size = final_backup_path.stat().st_size
                if backup_size > 0:
                    logger.info(f"Backup created successfully: {final_backup_path} ({format_size(backup_size)})")
...
def restore_backup(backup_path: Path, restore_location: Path, verify_integrity: bool = True) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            if restore_location.exists():
                ...
                logger.error(f"Restoration failed - target not found: {restore_location}")
                return False
...
def calculate_directory_size(directory_path: Path, follow_symlinks: bool = False) -> Tuple[int, int]:
    """
    ...
    try:
        ...
        if follow_symlinks:
            ...
            for dirpath, dirnames, filenames in os.walk(directory_path, followlinks=True):
...
def generate_cleanup_report(cleanup_stats: Dict[str, Any], verbose: bool = False, 
    ...
    report_lines.append(f"Total Files {'to be ' if dry_run else ''}Removed: {total_files:,}")
    report_lines.append(f"Total Directories {'to be ' if dry_run else ''}Removed: {total_dirs:,}")
    report_lines.append(f"Total Space {'to be ' if dry_run else ''}Reclaimed: {format_size(int(total_space_mb * 1024 * 1024))}")
    report_lines.append("")
    ...
    if 'cache_type_results' in cleanup_stats:
        ...
        for cache_type, results in cleanup_stats['cache_type_results'].items():
            if results:
                ...
                report_lines.append(f"  Description: {CACHE_TYPES.get(cache_type, {}).get('description', 'N/A')}")
                
                # Extract relevant statistics based on cache type
                if 'files_removed' in results:
                    report_lines.append(f"  Files {'to be ' if dry_run else ''}removed: {results['files_removed']:,}")
                if 'directories_removed' in results:
                    report_lines.append(f"  Directories {'to be ' if dry_run else ''}removed: {results['directories_removed']:,}")
                if 'cache_directories_removed' in results:
                    report_lines.append(f"  Cache directories {'to be ' if dry_run else ''}removed: {results['cache_directories_removed']:,}")
                if 'space_reclaimed_mb' in results:
                    space_size = int(results['space_reclaimed_mb'] * 1024 * 1024)
                    report_lines.append(f"  Space {'to be ' if dry_run else ''}reclaimed: {format_size(space_size)}")
                
                # Add type-specific details if verbose
                if verbose:
                    for key, value in results.items():
                        if key not in ['files_removed', 'directories_removed', 'cache_directories_removed', 'space_reclaimed_mb', 'errors', 'warnings']:
    ...
    if total_space_mb > 100:
        report_lines.append("  💡 Large amount of cache cleared - consider more frequent cleanups")
    elif total_space_mb < 1:
        report_lines.append("  💡 Minimal cache accumulation - current cleanup frequency is appropriate")
    ...
    if 'backup_files' in cleanup_stats and cleanup_stats['backup_files']:
        report_lines.append(f"  💾 {len(cleanup_stats['backup_files'])} backup files created for rollback capability")
    
    report_lines.append("  📅 Regular cache cleanup helps maintain optimal development performance")
    ...
    if total_files > 0 or total_dirs > 0:
        efficiency = "High" if total_space_mb > 10 else "Medium" if total_space_mb > 1 else "Low"
        report_lines.append(f"  Cleanup Efficiency: {efficiency}")
        report_lines.append(f"  Cache Accumulation: {'Heavy' if total_space_mb > 50 else 'Moderate' if total_space_mb > 10 else 'Light'}")
    else:
...
def validate_cleanup_safety(target_path: Path, patterns: List[str], 
                           strict_mode: bool = False) -> Tuple[bool, List[str]]:
    """
    ...
    try:
        ...
        for excluded_pattern in EXCLUDED_PATTERNS:
            if target_path.match(excluded_pattern):
                warnings.append(f"Target path matches excluded pattern: {excluded_pattern}")
                is_safe = False
        
        # Validate patterns don't match critical system or project files
        critical_files = [
            'pyproject.toml', 'setup.py', 'requirements.txt', 'Pipfile', 
            'setup.cfg', 'tox.ini', '.gitignore', 'README.md', 'LICENSE'
        ]
                ...
                if target_path.glob(critical_file):
                    ...
                    for potential_match in target_path.rglob(pattern):
                        if potential_match.name in critical_files:
                            warnings.append(f"Pattern '{pattern}' may match critical file: {potential_match}")
                            if strict_mode:
                                is_safe = False
        ...
        if target_path.exists():
            ...
            for lock_file in target_path.rglob('*.lock'):
                warnings.append(f"Found lock file indicating active process: {lock_file}")
                if strict_mode:
                    is_safe = False
            
            for pid_file in target_path.rglob('*.pid'):
                warnings.append(f"Found PID file indicating running process: {pid_file}")
                if strict_mode:
                    is_safe = False
        ...
        if config_files_found:
            ...
            if config_count > 10:  # Many config files might indicate important directory
                warnings.append("Large number of config files suggests important directory")
                if strict_mode:
                    is_safe = False
        ...
        if strict_mode:
            ...
            if target_path == Path.home():
                warnings.append("Target path is user home directory - extremely dangerous")
                is_safe = False
            
            if target_path == Path('/'):
                warnings.append("Target path is root directory - system destroying operation")
                is_safe = False
        ...
        if not warnings:
            ...
            logger.info(f"Safety validation found {len(warnings)} potential issues for {target_path}")
...
class CacheCleanupManager:
    ...
    def __init__(self, root_directory: Path, cleanup_config: Dict[str, Any], 
        ...
        if not is_safe and not cleanup_config.get('force', False):
            raise ValueError(f"Cleanup operation not safe for {self.root_directory}. Use --force to override.")
        
        self.logger.info(f"CacheCleanupManager initialized for {self.root_directory}")
    
    def execute_cleanup(self, cache_types: List[str], force_cleanup: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for cache_type in cache_types:
                try:
                    ...
                    cache_result = self.cleanup_cache_type(
                        ...
                        type_config=self.cleanup_configuration
                    )
                    ...
                    if 'files_removed' in cache_result:
                        cleanup_results['total_files_removed'] += cache_result['files_removed']
                    if 'directories_removed' in cache_result:
                        cleanup_results['total_directories_removed'] += cache_result['directories_removed']
                    if 'cache_directories_removed' in cache_result:
                        cleanup_results['total_directories_removed'] += cache_result['cache_directories_removed']
                    if 'space_reclaimed_mb' in cache_result:
                        cleanup_results['total_space_reclaimed_mb'] += cache_result['space_reclaimed_mb']
                    ...
                    self.logger.debug(f"Completed cache type {cache_type}: "
            ...
            self.cleanup_statistics.update({
                ...
                'total_directories_removed': cleanup_results['total_directories_removed'],
                'total_space_reclaimed_mb': cleanup_results['total_space_reclaimed_mb']
            })
            ...
            cleanup_results['duration_seconds'] = cleanup_results['end_time'] - cleanup_results['start_time']
            
            self.logger.info(f"Cleanup execution completed in {cleanup_results['duration_seconds']:.2f} seconds")
    ...
    def cleanup_cache_type(self, cache_type: str, type_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            if cache_type == 'python':
                ...
                raise ValueError(f"No cleanup function defined for cache type: {cache_type}")
    ...
    def create_cleanup_backup(self, cache_paths: List[Path], backup_name: str) -> Optional[Path]:
        """
                    ...
                    for i, cache_path in enumerate(cache_paths):
                        if cache_path.exists():
                            ...
                            if cache_path.is_file():
                                ...
                                shutil.copytree(cache_path, dest_path, ignore_errors=True)
    ...
    def rollback_cleanup(self, backup_id: str, verify_restoration: bool = True) -> bool:
        ...
        try:
            ...
            if restoration_success:
                self.logger.info(f"Rollback completed successfully to {restore_location}")
    ...
    def get_cleanup_summary(self, include_details: bool = False) -> Dict[str, Any]:
        ...
        if self.cleanup_warnings:
            summary['warnings'] = self.cleanup_warnings[:10]  # Limit to first 10 warnings
        ...
        if total_space > 100:
            recommendations.append("Consider more frequent cache cleanup to prevent large accumulations")
        elif total_space > 50:
            recommendations.append("Current cleanup frequency appears appropriate")
        else:
            recommendations.append("Cache accumulation is minimal - cleanup frequency can be reduced")
        
        if self.cleanup_statistics.get('operations_failed', 0) > 0:
            recommendations.append("Review failed operations and check file permissions")
        
        if not self.backup_enabled:
            recommendations.append("Consider enabling backups for safer cleanup operations")
        ...
        if include_details:
            summary['detailed_operations'] = {
                ...
                'performance_metrics': {
                    'operations_completed': self.cleanup_statistics.get('operations_completed', 0),
                    'operations_failed': self.cleanup_statistics.get('operations_failed', 0),
                    'success_rate': (
                        self.cleanup_statistics.get('operations_completed', 0) / 
                        max(1, self.cleanup_statistics.get('operations_completed', 0) + 
    ...
    def cleanup_old_backups(self, max_age_days: int = MAX_BACKUP_AGE_DAYS) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for backup_file in backup_dir.glob('*.zip'):
                try:
                    ...
                    backup_cleanup_stats['errors'].append(f"Error checking backup file {backup_file}: {e}")
            ...
            for backup_info in backups_to_remove:
                try:
                    ...
                    manifest_path = backup_path.with_name(backup_path.stem + '_manifest.json')
            ...
            backup_cleanup_stats['space_reclaimed_mb'] = total_size_removed / (1024 * 1024)
            
            # Log backup cleanup progress with file counts and space reclaimed
            self.logger.info(f"Old backup cleanup completed: {backup_cleanup_stats['old_backups_removed']} backups removed, "
