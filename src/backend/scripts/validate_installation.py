...
from typing import Optional, List, Dict, Any, Tuple  # >=3.10 - Type hints for function parameters, return types, and comprehensive validation result structures
    ...
    try:
        ...
        try:
            ...
            system_adequate, system_info = check_system_resources(
                check_performance_capability=True,
            ...
            dependency_results = check_dependencies(
                ...
                test_integration_compatibility=True
            )
            _validation_results['dependencies'] = dependency_results
            
            # Check if all required dependencies are available
            required_deps = ['gymnasium', 'numpy', 'matplotlib']
            missing_required = [dep for dep in required_deps if not dependency_results.get(dep, False)]
            ...
            if parsed_args.get('performance', False) and validation_success:
                ...
                if step_time > PERFORMANCE_TARGET_STEP_LATENCY_MS * 2:
                    ...
                    if not QUIET_MODE:
                        print(f"⚠️ Performance warning: Step time {step_time:.2f}ms exceeds target")
                elif VERBOSE_OUTPUT:
                    print(f"✅ Performance acceptable: {step_time:.2f}ms average step time")
        ...
        _validation_results['duration_seconds'] = _validation_results['end_time'] - _validation_results['start_time']
        
        if validation_success:
            _validation_results['overall_status'] = 'success' if validation_warnings == 0 else 'success_with_warnings'
        else:
        ...
        if validation_success:
            return VALIDATION_SUCCESS_EXIT_CODE if validation_warnings == 0 else VALIDATION_WARNING_EXIT_CODE
        else:
            return VALIDATION_FAILURE_EXIT_CODE
...
def check_python_version(strict_checking: bool = False) -> bool:
    ...
    try:
        ...
        if major < required_major or (major == required_major and minor < required_minor):
            if not QUIET_MODE:
                print(f"❌ Python {major}.{minor} is below minimum requirement {required_major}.{required_minor}")
            return False
        ...
        required_modules = ['typing', 'dataclasses', 'asyncio', 'functools', 'itertools', 'collections']
        missing_modules = []
        ...
        if missing_modules:
            if not QUIET_MODE:
                print(f"❌ Missing required standard library modules: {missing_modules}")
            return False
...
def check_system_resources(check_performance_capability: bool = False,
                          estimate_resource_usage: bool = False) -> Tuple[bool, Dict[str, Any]]:
    """
    ...
    try:
        ...
        if PSUTIL_AVAILABLE and psutil:
            try:
                ...
                if VERBOSE_OUTPUT:
                    print(f"   Memory: {available_mb:.0f}MB available / {total_mb:.0f}MB total ({memory.percent:.1f}% used)")
            ...
            resource_info['resource_warnings'].append("psutil not available - cannot check memory usage")
        
        # Validate CPU information and core count for performance capability assessment
        cpu_count = os.cpu_count()
        if cpu_count is None or cpu_count < 2:
            resource_info['resource_warnings'].append("CPU core count appears low or unavailable")
        ...
        if MATPLOTLIB_AVAILABLE and matplotlib:
            try:
                ...
                resource_info['resource_warnings'].append(f"Matplotlib backend issue: {e}")
        ...
        if system_platform != 'Windows' and not display_env:
            resource_info['resource_warnings'].append("No DISPLAY environment variable - GUI rendering may not work")
        ...
        if estimate_resource_usage:
            try:
                if CONSTANTS_AVAILABLE:
                    ...
                    estimated_mb = (grid_cells * 4 + 1024) / (1024 * 1024)  # Add 1KB overhead
                    resource_info['estimated_memory_usage_mb'] = estimated_mb
                    
                    if VERBOSE_OUTPUT:
                        print(f"   Estimated memory usage: {estimated_mb:.1f}MB for {width}x{height} grid")
        ...
        if not any(arch in architecture for arch in supported_archs):
            resource_info['resource_warnings'].append(f"Unknown architecture: {architecture}")
        ...
        critical_issues = [w for w in resource_info['resource_warnings'] 
...
def check_package_installation(check_installation_path: bool = False,
                              verify_package_integrity: bool = False) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            if CONSTANTS_AVAILABLE and version != PACKAGE_VERSION:
                if not QUIET_MODE:
                    print(f"⚠️ Version mismatch: imported {version}, expected {PACKAGE_VERSION}")
        ...
        if check_installation_path:
            try:
                ...
                if 'site-packages' not in package_dir and 'dist-packages' not in package_dir:
                    if VERBOSE_OUTPUT:
        ...
        try:
            ...
            if VERBOSE_OUTPUT:
                ...
                print(f"   Environment ID: {package_info.get('environment_id', 'unknown')}")
        ...
        try:
            ...
            if CONSTANTS_AVAILABLE:
                required_constants = [
                    'PACKAGE_VERSION', 'DEFAULT_GRID_SIZE', 'DEFAULT_SOURCE_LOCATION',
...
def check_dependencies(check_optional_dependencies: bool = False,
                      test_integration_compatibility: bool = False) -> Dict[str, bool]:
    """
    ...
    try:
        ...
        if dependency_status['gymnasium']:
            try:
                ...
                observation_space = Box(low=0.0, high=1.0, shape=(1,), dtype=np.float32 if NUMPY_AVAILABLE else float)
        ...
        if check_optional_dependencies:
            if PSUTIL_AVAILABLE:
                try:
                    ...
                    if VERBOSE_OUTPUT:
                        print(f"   ✅ psutil functional (Memory: {memory_info.percent:.1f}% used)")
        ...
        if test_integration_compatibility and all([dependency_status['gymnasium'], dependency_status['numpy']]):
            try:
        ...
        if VERBOSE_OUTPUT:
            ...
            available_deps = len([k for k, v in dependency_status.items() if v and k != 'psutil'])
...
def validate_environment_functionality(test_extended_functionality: bool = False,
                                     num_test_steps: int = BASIC_PERFORMANCE_TEST_STEPS) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            if not hasattr(action_space, 'n') or action_space.n != 4:
                if not QUIET_MODE:
                    print(f"❌ Invalid action space: expected Discrete(4), got {action_space}")
                return False
            
            if not hasattr(observation_space, 'shape') or observation_space.shape != (1,):
                if not QUIET_MODE:
                    print(f"❌ Invalid observation space: expected Box(shape=(1,)), got {observation_space}")
                return False
        ...
        try:
            ...
            if VERBOSE_OUTPUT:
                print(f"   ✅ Reset successful: obs={obs[0]:.4f}, info keys={list(info.keys())}")
        ...
        try:
            ...
            while step_count < max_steps:
                ...
                for action in range(4):
                    ...
                    if not isinstance(obs, np.ndarray) or obs.shape != (1,) or obs.dtype != np.float32:
                        if not QUIET_MODE:
                            print(f"❌ Step observation invalid: {type(obs)}, {obs.shape if hasattr(obs, 'shape') else 'no shape'}")
                        return False
                    ...
                    if not isinstance(terminated, bool) or not isinstance(truncated, bool):
                        if not QUIET_MODE:
                            print(f"❌ Step termination flags not boolean: {type(terminated)}, {type(truncated)}")
                        return False
        ...
        try:
            ...
            try:
                ...
                if VERBOSE_OUTPUT:
                    print(f"   ⚠️ Human rendering failed (may be expected in headless): {e}")
            ...
            try:
                ...
                for episode in range(3):
                    ...
                    if VERBOSE_OUTPUT and episode == 0:
                        print(f"     Episode {episode}: {episode_steps} steps, final reward: {reward}")
...
def validate_registration_system(test_custom_parameters: bool = False,
                               cleanup_registration: bool = True) -> bool:
    """
    ...
    try:
        ...
        try:
            if not is_registered(ENV_ID):
                if not QUIET_MODE:
                    print(f"❌ Environment not found in registry after registration: {ENV_ID}")
                return False
        ...
        try:
            ...
            if not hasattr(env, 'reset') or not hasattr(env, 'step'):
                if not QUIET_MODE:
                    print(f"❌ gym.make() created invalid environment: missing reset/step methods")
                return False
        ...
        if test_custom_parameters:
            try:
                ...
                if custom_id != custom_env_id:
                    if not QUIET_MODE:
                        print(f"❌ Custom registration failed: expected {custom_env_id}, got {custom_id}")
                    return False
                ...
                if VERBOSE_OUTPUT:
                    print(f"   ✅ Custom parameter registration successful: {custom_env_id}")
        ...
        try:
            ...
            if hasattr(gym.envs, 'registry') and hasattr(gym.envs.registry, 'env_specs'):
                env_spec = gym.envs.registry.env_specs.get(ENV_ID)
                
                if env_spec is None:
                    if not QUIET_MODE:
                        print(f"❌ Environment not found in Gymnasium registry: {ENV_ID}")
                    return False
...
def run_basic_performance_validation(test_duration_steps: int = BASIC_PERFORMANCE_TEST_STEPS,
                                   test_rendering_performance: bool = False,
                                   monitor_memory_usage: bool = False) -> Dict[str, float]:
    """
    ...
    try:
        if not PLUME_NAV_SIM_AVAILABLE:
            performance_metrics['performance_warnings'].append("Package not available for testing")
        ...
        reset_time = (time.perf_counter() - reset_start) * 1000  # Convert to milliseconds
        ...
        if step_times:
            performance_metrics['average_step_time_ms'] = sum(step_times) / len(step_times)
            performance_metrics['min_step_time_ms'] = min(step_times)
            performance_metrics['max_step_time_ms'] = max(step_times)
            performance_metrics['steps_per_second'] = (test_duration_steps / total_time) * 1000
            
            if VERBOSE_OUTPUT:
                print(f"     Average step time: {performance_metrics['average_step_time_ms']:.3f}ms")
                print(f"     Step time range: {performance_metrics['min_step_time_ms']:.3f} - {performance_metrics['max_step_time_ms']:.3f}ms")
                print(f"     Steps per second: {performance_metrics['steps_per_second']:.1f}")
        ...
        if test_rendering_performance:
            try:
                ...
                if render_times:
                    ...
                    if VERBOSE_OUTPUT:
                        print(f"     Average render time: {avg_render_time:.3f}ms ({render_frames} frames)")
                    
                    # Check against rendering performance target
                    if CONSTANTS_AVAILABLE and hasattr('PERFORMANCE_TARGET_RGB_RENDER_MS', 'replace'):
                        # Fallback target if constant not available
                        render_target = 5.0  # 5ms target
                    else:
                        render_target = getattr(constants, 'PERFORMANCE_TARGET_RGB_RENDER_MS', 5.0) if CONSTANTS_AVAILABLE else 5.0
                ...
                performance_metrics['performance_warnings'].append(f"Rendering test failed: {e}")
        ...
        if monitor_memory_usage and PSUTIL_AVAILABLE:
            try:
                ...
                if VERBOSE_OUTPUT:
                    print(f"     Memory usage: {final_memory:.1f}MB (growth: {performance_metrics['memory_growth_mb']:.1f}MB)")
                ...
                performance_metrics['performance_warnings'].append(f"Memory monitoring failed: {e}")
        ...
        try:
            ...
            if monitor_memory_usage and PSUTIL_AVAILABLE:
                try:
                    ...
                    performance_metrics['memory_freed_mb'] = final_memory - cleanup_memory
                    
                    if VERBOSE_OUTPUT:
                        print(f"     Memory after cleanup: {cleanup_memory:.1f}MB (freed: {performance_metrics['memory_freed_mb']:.1f}MB)")
                except Exception:
                    pass
                    
        except Exception as e:
            performance_metrics['performance_warnings'].append(f"Cleanup test failed: {e}")
        ...
        if VERBOSE_OUTPUT:
            ...
            if warning_count == 0:
                ...
                print(f"   ⚠️ Performance validation completed with {warning_count} warnings")
        ...
        performance_metrics['performance_warnings'].append(f"Performance validation failed: {e}")
        if not QUIET_MODE:
            print(f"❌ Performance validation error: {e}")
        return performance_metrics


def generate_validation_report(include_system_details: bool = False,
    ...
    try:
        ...
        report_lines.append(f"Report generated: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(end_time))}")
        ...
        report_lines.append(f"{python_icon} Python Version Compatibility: {'PASS' if python_result else 'FAIL'}")
        ...
        report_lines.append(f"{system_icon} System Resources: {'ADEQUATE' if system_adequate else 'INADEQUATE'}")
        
        # Package installation check
        package_result = _validation_results.get('package_installation', False)
        package_icon = "✅" if package_result else "❌"
        report_lines.append(f"{package_icon} Package Installation: {'VALID' if package_result else 'INVALID'}")
        ...
        deps_icon = "✅" if deps_available == len(required_deps) else "❌" if deps_available == 0 else "⚠️"
        report_lines.append(f"{deps_icon} Dependencies: {deps_available}/{len(required_deps)} available")
        ...
        report_lines.append(f"{env_icon} Environment Functionality: {'VALIDATED' if env_result else 'FAILED'}")
        
        # Registration system check
        reg_result = _validation_results.get('registration_system', False)
        reg_icon = "✅" if reg_result else "❌"
        report_lines.append(f"{reg_icon} Registration System: {'FUNCTIONAL' if reg_result else 'FAILED'}")
        ...
        if perf_metrics and isinstance(perf_metrics, dict):
            ...
            if avg_step_time > 0:
                report_lines.append(f"{perf_icon} Performance: {avg_step_time:.2f}ms avg step time")
                if warnings_count > 0:
        ...
        if include_troubleshooting_guide:
            ...
            failed_components = []
            if not _validation_results.get('python_version'):
                failed_components.append("Python version incompatible - upgrade to Python 3.10+")
            
            if not _validation_results.get('system_resources', {}).get('adequate'):
                failed_components.append("Insufficient system resources - check memory availability")
            
            if not _validation_results.get('package_installation'):
                failed_components.append("Package installation issues - reinstall with 'pip install -e .'")
            
            deps_result = _validation_results.get('dependencies', {})
            missing_deps = [dep for dep in ['gymnasium', 'numpy', 'matplotlib'] if not deps_result.get(dep)]
            if missing_deps:
                failed_components.append(f"Missing dependencies: {', '.join(missing_deps)} - install with pip")
            
            if not _validation_results.get('environment_functionality'):
                failed_components.append("Environment functionality issues - check package integrity")
            
            if not _validation_results.get('registration_system'):
                failed_components.append("Registration system problems - verify Gymnasium installation")
            ...
            if failed_components:
                ...
                report_lines.append("• Review error messages above for specific guidance")
                report_lines.append("• Ensure all dependencies are installed: pip install gymnasium>=0.29.0 numpy>=2.1.0 matplotlib>=3.9.0")
                report_lines.append("• Reinstall package: pip uninstall plume-nav-sim && pip install -e .")
                report_lines.append("• Check Python version: python --version (requires 3.10+)")
                report_lines.append("• For headless systems, set appropriate matplotlib backend")
            else:
        ...
        if overall_status == 'success':
            report_lines.append("✅ Validation completed successfully. Package is ready for use.")
        elif overall_status == 'success_with_warnings':
            report_lines.append("⚠️ Validation completed with warnings. Basic functionality is available.")
        else:
            report_lines.append("❌ Validation failed. Please address the issues above before using the package.")
...
def display_troubleshooting_guide(failed_validations: List[str],
                                show_platform_specific_guidance: bool = False,
                                include_advanced_troubleshooting: bool = False) -> None:
    """
    ...
    if 'system_resources' in failed_validations:
        ...
        print("   • Check available memory: free -h (Linux) or Activity Monitor (macOS)")
    ...
    if 'package_installation' in failed_validations:
        ...
        print("   • Uninstall and reinstall: pip uninstall plume-nav-sim && pip install -e .")
    ...
    dependency_issues = [item for item in failed_validations if item in ['gymnasium', 'numpy', 'matplotlib']]
    if dependency_issues:
        print("❌ DEPENDENCY INSTALLATION ISSUES:")
        print(f"   Problem: Missing required dependencies: {', '.join(dependency_issues)}")
        print("   Solutions:")
        print("   • Install dependencies: pip install gymnasium>=0.29.0 numpy>=2.1.0 matplotlib>=3.9.0")
    ...
    if 'environment_functionality' in failed_validations:
        ...
        print("   • Verify package integrity: python -c \"import plume_nav_sim; print('OK')\"")
    ...
    if show_platform_specific_guidance:
        ...
        if current_platform == "Linux":
            ...
            print("   • Install system dependencies: sudo apt-get install python3-dev python3-tk")
            ...
            print("   • Install additional packages: sudo apt-get install python3-matplotlib")
    ...
    if include_advanced_troubleshooting:
        ...
        print("   • Check package structure: python -c \"import plume_nav_sim; print(plume_nav_sim.__file__)\"")
        print("   • Validate constants: python -c \"from plume_nav_sim.core.constants import *\"")
        print("   • Test minimal environment:")
        print("     python -c \"from plume_nav_sim import create_plume_search_env; env = create_plume_search_env(); print('OK')\"")
        print()
    ...
    print("   • Matplotlib installation: https://matplotlib.org/stable/users/installing/")
    print("   • Python environment management: https://docs.python.org/3/tutorial/venv.html")
    print()
...
Examples:
  ...
  python validate_installation.py --help
        ...
        if args_dict['verbose'] and args_dict['quiet']:
            print("⚠️ Warning: --verbose and --quiet are mutually exclusive, using --verbose")
            args_dict['quiet'] = False
        
        if args_dict['quick'] and args_dict['performance']:
            print("⚠️ Warning: --quick and --performance are contradictory, using --performance")
