...
class TestBaseEnvironmentInitialization:
    ...
    def test_successful_initialization(self):
        ...
        assert hasattr(mock_env, "config")
        assert mock_env.config == config
    ...
    def test_configuration_validation(self):
        ...
        # Test invalid configurations raise appropriate ValidationError or ConfigurationError exceptions
        ...
        try:
            ...
            assert "goal_radius" in str(e).lower() or "negative" in str(e).lower()
...
class TestBaseEnvironmentAPICompliance:
    ...
    def test_step_api_compliance(self):
        ...
        with patch.object(mock_env, "_check_terminated", return_value=True):
            ...
            # Note: BaseEnvironment doesn't enforce state machine (no error on step after termination)
            # Only ComponentBasedEnvironment has strict state enforcement
            # Test that step still returns valid 5-tuple
            try:
                obs2, reward2, term2, trunc2, info2 = mock_env.step(0)
                assert len((obs2, reward2, term2, trunc2, info2)) == 5
            except (StateError, RuntimeError):
                # Some implementations may enforce this
                pass
    ...
    def test_render_api_compliance(self):
        ...
        with pytest.raises((ValidationError, ValueError, RenderingError)):
...
class TestBaseEnvironmentErrorHandling:
    ...
    def test_validation_error_handling(self):
        ...
        # Test invalid action parameter raises ValidationError or StateError (which wraps it)
        invalid_actions = [-1, 4, 10, "invalid", None, 3.5]
        for invalid_action in invalid_actions:
            with pytest.raises((ValidationError, StateError)) as exc_info:
                mock_env.step(invalid_action)

            error = exc_info.value
            # Check for validation error attributes (may be wrapped in StateError)
            error_str = str(error).lower()
            assert "action" in error_str or "out of bounds" in error_str
        ...
        # Check invalid render mode raises ValidationError or RenderingError (which wraps it)
        with pytest.raises((ValidationError, RenderingError)) as exc_info:
...
class TestBaseEnvironmentIntegration:
    ...
    def test_validation_integration(self):
        ...
        with patch.object(
            ...
            error = exc_info.value
            # Check error contains action information (may be wrapped in StateError)
            error_str = str(error).lower()
            assert "action" in error_str or "out of bounds" in error_str
