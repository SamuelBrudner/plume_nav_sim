...
def _make_injected_env() -> PlumeEnv:
    ...
    return PlumeEnv(
        ...
        reward_fn=SparseGoalReward(goal_position=goal_location, goal_radius=2.0),
        wind_field=None,
    )


def test_env_state_typed_dict_required_keys_match_contract() -> None:
    assert EnvState.__required_keys__ == EXPECTED_ENV_STATE_KEYS


def test_plume_env_build_env_state_dict_matches_typed_structure() -> None:
    env = _make_injected_env()
    env.reset(seed=7)
    ...
    try:
        ...
        assert env_state["grid_size"] == env._grid_size
