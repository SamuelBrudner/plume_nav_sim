...
BOUNDARY_TEST_POSITIONS = [(0, 0), (31, 31), (15, 0), (15, 31), (0, 15), (31, 15), (16, 16)]
...
def create_test_boundary_enforcer(
    ...
    assert enforcer.constraint_config.enable_clamping == constraint_config.enable_clamping
...
def generate_boundary_test_cases(grid_size: GridSize, include_invalid_positions: bool = False):
    """
    ...
    corners = [
        (0, 0), (0, grid_size.height - 1),
        (grid_size.width - 1, 0), (grid_size.width - 1, grid_size.height - 1)
    ]
    ...
    for position in [center] + corners[:2] + edges[:4]:
        for action in ALL_VALID_ACTIONS:
            ...
            within_bounds = (0 <= new_x < grid_size.width and 0 <= new_y < grid_size.height)
...
def validate_boundary_enforcement_consistency(
    ...
    movement_within_bounds = (
        0 <= expected_x < grid_bounds.width and
        0 <= expected_y < grid_bounds.height
    )
    ...
    if (enforcement_result.final_position.x < 0 or 
        enforcement_result.final_position.x >= grid_bounds.width or
        enforcement_result.final_position.y < 0 or 
        enforcement_result.final_position.y >= grid_bounds.height):
        return False
...
def create_movement_constraint_variations(include_edge_cases: bool = False):
    ...
    variations.append(MovementConstraint(
        enable_clamping=True,
        strict_validation=False,
        log_boundary_violations=False,
        tolerance=0.0
...
def verify_boundary_statistics_accuracy(
    ...
    expected_boundary_hits = expected_stats.get('expected_boundary_hits', manual_boundary_hits)
    ...
    expected_operations = expected_stats.get('expected_operations', len(test_operations))
...
class TestBoundaryEnforcerInitialization:
    ...
    def test_boundary_enforcer_invalid_initialization(self):
        ...
        with pytest.raises((ValidationError, TypeError, AttributeError)):
            BoundaryEnforcer(
                grid_size=TEST_GRID_SIZE,
                constraint_config="invalid_constraint"
            )
    ...
    def test_boundary_enforcer_constraint_configuration_validation(self):
        ...
        with pytest.raises((ValidationError, Warning)):
            enforcer = BoundaryEnforcer(
                grid_size=TEST_GRID_SIZE,
                constraint_config=invalid_config
            )
...
class TestPositionValidation:
    ...
    def test_validate_position_performance(self):
        ...
        test_positions = [TEST_COORDINATES_CENTER, TEST_COORDINATES_CORNER_ORIGIN, TEST_COORDINATES_EDGE_TOP]
        timing_results = []
        ...
        assert percentile_99 < BOUNDARY_ENFORCEMENT_PERFORMANCE_TARGET_MS * 2  # Allow some tolerance
...
class TestMovementValidation:
    ...
    def test_is_movement_valid_within_bounds(self):
        ...
        interior_positions = [
            Coordinates(10, 10), Coordinates(20, 20), Coordinates(5, 25)
        ]
    ...
    def test_movement_validation_performance(self):
        ...
        # Assert average validation time is well below performance target
        average_time = np.mean(timing_results)
        assert average_time < BOUNDARY_ENFORCEMENT_PERFORMANCE_TARGET_MS / 2  # Should be faster than full enforcement
...
class TestBoundaryEnforcement:
    ...
    def test_enforce_movement_bounds_valid_movement(self):
        ...
        for action in ALL_VALID_ACTIONS:
            ...
            assert validate_boundary_enforcement_consistency(
    ...
    def test_boundary_enforcement_result_analysis(self):
        ...
        valid_result = enforcer.enforce_movement_bounds(TEST_COORDINATES_CENTER, Action.UP)
    ...
    def test_boundary_enforcement_performance_monitoring(self):
        ...
        if 'performance_metrics' in stats:
            avg_time = stats['performance_metrics'].get('average_enforcement_time_ms', 0)
            assert avg_time < BOUNDARY_ENFORCEMENT_PERFORMANCE_TARGET_MS * 2  # Allow some tolerance
...
class TestStandaloneFunctions:
    ...
    def test_validate_movement_bounds_function(self):
        ...
        assert validate_movement_bounds(TEST_COORDINATES_CENTER, Action.UP, TEST_GRID_SIZE) == True
        assert validate_movement_bounds(TEST_COORDINATES_CENTER, Action.RIGHT, TEST_GRID_SIZE) == True
        ...
        assert validate_movement_bounds(TEST_COORDINATES_EDGE_TOP, Action.UP, TEST_GRID_SIZE) == False
        assert validate_movement_bounds(TEST_COORDINATES_EDGE_RIGHT, Action.RIGHT, TEST_GRID_SIZE) == False
    ...
    def test_enforce_position_bounds_function(self):
        ...
        result_pos, modified = enforce_position_bounds(TEST_COORDINATES_CENTER, TEST_GRID_SIZE)
        ...
        clamped_pos, was_modified = enforce_position_bounds(out_of_bounds, TEST_GRID_SIZE)
    ...
    def test_is_position_within_bounds_function(self):
        ...
        assert is_position_within_bounds(TEST_COORDINATES_CENTER, TEST_GRID_SIZE) == True
        assert is_position_within_bounds(TEST_COORDINATES_CORNER_ORIGIN, TEST_GRID_SIZE) == True
        assert is_position_within_bounds(TEST_COORDINATES_CORNER_OPPOSITE, TEST_GRID_SIZE) == True
        ...
        assert avg_time < BOUNDARY_ENFORCEMENT_PERFORMANCE_TARGET_MS / 10  # Should be very fast
    ...
    def test_calculate_bounded_movement_function(self):
        ...
        clamp_pos, clamp_success, clamp_hit = calculate_bounded_movement(
            TEST_COORDINATES_EDGE_TOP, Action.UP, TEST_GRID_SIZE, allow_boundary_clamping=True
        )
        
        no_clamp_pos, no_clamp_success, no_clamp_hit = calculate_bounded_movement(
            TEST_COORDINATES_EDGE_TOP, Action.UP, TEST_GRID_SIZE, allow_boundary_clamping=False
        )
        ...
        assert no_clamp_pos == TEST_COORDINATES_EDGE_TOP  # Should stay at original position
...
class TestMovementConstraintConfiguration:
    ...
    def test_movement_constraint_cloning(self):
        ...
        overridden_clone = original.clone(overrides={
            'strict_validation': True,
            'tolerance': 2.0
        })
...
class TestBoundaryStatistics:
    ...
    def test_boundary_statistics_collection(self):
        ...
        test_operations = [
            {'position': TEST_COORDINATES_CENTER, 'action': Action.UP, 'expect_hit': False},
            {'position': TEST_COORDINATES_EDGE_TOP, 'action': Action.UP, 'expect_hit': True},
            {'position': TEST_COORDINATES_EDGE_RIGHT, 'action': Action.RIGHT, 'expect_hit': True},
            {'position': TEST_COORDINATES_CENTER, 'action': Action.DOWN, 'expect_hit': False}
        ]
        
        expected_hits = 0
        for operation in test_operations:
            result = enforcer.enforce_movement_bounds(operation['position'], operation['action'])
    ...
    def test_performance_statistics_accuracy(self):
        ...
        positions = [TEST_COORDINATES_CENTER, TEST_COORDINATES_EDGE_TOP, TEST_COORDINATES_CORNER_ORIGIN]
...
class TestErrorHandling:
    ...
    def test_validation_error_handling(self):
        ...
        for invalid_action in invalid_actions:
            with pytest.raises((ValidationError, TypeError, AttributeError)):
                enforcer.enforce_movement_bounds(TEST_COORDINATES_CENTER, invalid_action)
...
class TestPerformanceBenchmarks:
    ...
    def test_boundary_enforcement_latency_benchmark(self):
        ...
        assert mean_latency < BOUNDARY_ENFORCEMENT_PERFORMANCE_TARGET_MS, \
        ...
        assert percentile_99 < BOUNDARY_ENFORCEMENT_PERFORMANCE_TARGET_MS * 2, \
    ...
    def test_position_validation_performance_benchmark(self):
        ...
        for position in test_positions:
            for _ in range(iterations_per_position):
                start_time = time.perf_counter()
                try:
                    result = enforcer.validate_position(position, raise_on_invalid=False)
                except:
        ...
        assert avg_validation_time < BOUNDARY_ENFORCEMENT_PERFORMANCE_TARGET_MS / 2, \
    ...
    def test_scaling_performance_analysis(self):
        ...
        for grid_desc, avg_time in performance_results.items():
            assert avg_time < BOUNDARY_ENFORCEMENT_PERFORMANCE_TARGET_MS * 3, \
...
class TestIntegrationAndCompatibility:
    ...
    def test_grid_size_update_integration(self):
        ...
        assert enforcer.validate_position(large_position, raise_on_invalid=False) == True
    ...
    def test_constraint_configuration_integration(self):
        ...
        constraint_variations = create_movement_constraint_variations(include_edge_cases=True)
        ...
        for constraint in constraint_variations:
            ...
            result = enforcer.enforce_movement_bounds(boundary_position, boundary_action)
...
class TestReproducibilityAndDeterminism:
    ...
    def test_deterministic_boundary_enforcement(self):
        ...
        enforcer1 = create_test_boundary_enforcer(grid_size=TEST_GRID_SIZE, enable_caching=False)
        enforcer2 = create_test_boundary_enforcer(grid_size=TEST_GRID_SIZE, enable_caching=False)
    ...
    def test_consistent_validation_results(self):
        ...
        enforcers = [
            create_test_boundary_enforcer(enable_caching=False) 
            for _ in range(3)
        ]
    ...
    def test_reproducible_performance_metrics(self):
        ...
        mean_variance = np.std(mean_times) / np.mean(mean_times)  # Coefficient of variation
        
        # Performance should be relatively consistent (within 50% coefficient of variation)
        assert mean_variance < 0.5, f"Performance variance too high: {mean_variance:.3f}"
