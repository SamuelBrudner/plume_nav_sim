...
from typing import Any, Dict, List, Tuple, Union  # >=3.10 - Type validation and type hint verification for constant types
...
from plume_nav_sim.core.constants import (
    ...
    # Data type constants
    FIELD_DTYPE, OBSERVATION_DTYPE,
    
    # Plume model constants
    ...
    # Factory functions
    get_default_environment_constants, get_plume_model_constants,
...
from plume_nav_sim.utils.exceptions import ValidationError, ConfigurationError
...
class TestPackageMetadataConstants:
    ...
    def test_package_metadata_constants(self):
        ...
        assert PACKAGE_NAME == EXPECTED_PACKAGE_NAME, f"Expected package name '{EXPECTED_PACKAGE_NAME}', got '{PACKAGE_NAME}'"
        assert isinstance(PACKAGE_NAME, str), f"PACKAGE_NAME should be string, got {type(PACKAGE_NAME)}"
        ...
        assert isinstance(PACKAGE_VERSION, str), f"PACKAGE_VERSION should be string, got {type(PACKAGE_VERSION)}"
        ...
        assert version_match is not None, f"PACKAGE_VERSION '{PACKAGE_VERSION}' does not match semantic versioning pattern"
        
        # Validate ENVIRONMENT_ID follows Gymnasium versioning convention
        assert isinstance(ENVIRONMENT_ID, str), f"ENVIRONMENT_ID should be string, got {type(ENVIRONMENT_ID)}"
        ...
        assert env_id_match is not None, f"ENVIRONMENT_ID '{ENVIRONMENT_ID}' does not match Gymnasium pattern"
        
        # Test that package constants are immutable and hashable types
        assert isinstance(PACKAGE_NAME, str), "PACKAGE_NAME should be immutable string type"
        assert isinstance(PACKAGE_VERSION, str), "PACKAGE_VERSION should be immutable string type"
        assert isinstance(ENVIRONMENT_ID, str), "ENVIRONMENT_ID should be immutable string type"
...
class TestEnvironmentDefaultConstants:
    ...
    def test_environment_default_constants(self):
        ...
        assert DEFAULT_GRID_SIZE == EXPECTED_GRID_SIZE_DEFAULT, f"Expected grid size {EXPECTED_GRID_SIZE_DEFAULT}, got {DEFAULT_GRID_SIZE}"
        assert isinstance(DEFAULT_GRID_SIZE, tuple), f"DEFAULT_GRID_SIZE should be tuple, got {type(DEFAULT_GRID_SIZE)}"
        assert len(DEFAULT_GRID_SIZE) == 2, f"DEFAULT_GRID_SIZE should have 2 elements, got {len(DEFAULT_GRID_SIZE)}"
        assert all(isinstance(x, int) for x in DEFAULT_GRID_SIZE), "DEFAULT_GRID_SIZE elements should be integers"
        assert all(x > 0 for x in DEFAULT_GRID_SIZE), "DEFAULT_GRID_SIZE elements should be positive"
        assert all(x <= MAX_REASONABLE_GRID_SIZE for x in DEFAULT_GRID_SIZE), "DEFAULT_GRID_SIZE elements should be within reasonable limits"
        
        # Assert DEFAULT_SOURCE_LOCATION equals (64, 64) and is tuple of two integers
        assert DEFAULT_SOURCE_LOCATION == EXPECTED_SOURCE_LOCATION_DEFAULT, f"Expected source location {EXPECTED_SOURCE_LOCATION_DEFAULT}, got {DEFAULT_SOURCE_LOCATION}"
        assert isinstance(DEFAULT_SOURCE_LOCATION, tuple), f"DEFAULT_SOURCE_LOCATION should be tuple, got {type(DEFAULT_SOURCE_LOCATION)}"
        assert len(DEFAULT_SOURCE_LOCATION) == 2, f"DEFAULT_SOURCE_LOCATION should have 2 elements, got {len(DEFAULT_SOURCE_LOCATION)}"
        assert all(isinstance(x, int) for x in DEFAULT_SOURCE_LOCATION), "DEFAULT_SOURCE_LOCATION elements should be integers"
        assert all(x >= 0 for x in DEFAULT_SOURCE_LOCATION), "DEFAULT_SOURCE_LOCATION elements should be non-negative"
        
        # Validate DEFAULT_SOURCE_LOCATION is within DEFAULT_GRID_SIZE boundaries
        assert DEFAULT_SOURCE_LOCATION[0] < DEFAULT_GRID_SIZE[0], "Source X coordinate should be within grid width"
        assert DEFAULT_SOURCE_LOCATION[1] < DEFAULT_GRID_SIZE[1], "Source Y coordinate should be within grid height"
        
        # Assert DEFAULT_PLUME_SIGMA is positive float within reasonable range
        assert isinstance(DEFAULT_PLUME_SIGMA, (int, float)), f"DEFAULT_PLUME_SIGMA should be numeric, got {type(DEFAULT_PLUME_SIGMA)}"
        assert DEFAULT_PLUME_SIGMA > 0, "DEFAULT_PLUME_SIGMA should be positive"
        assert MIN_REASONABLE_SIGMA <= DEFAULT_PLUME_SIGMA <= MAX_REASONABLE_SIGMA, f"DEFAULT_PLUME_SIGMA {DEFAULT_PLUME_SIGMA} should be within reasonable range [{MIN_REASONABLE_SIGMA}, {MAX_REASONABLE_SIGMA}]"
        ...
        assert abs(DEFAULT_SOURCE_LOCATION[0] - grid_center_x) <= 1, "Default source should be near grid center X"
        assert abs(DEFAULT_SOURCE_LOCATION[1] - grid_center_y) <= 1, "Default source should be near grid center Y"
...
class TestActionSpaceConstants:
    ...
    def test_action_space_constants(self):
        ...
        assert expected_actions == EXPECTED_ACTION_RANGE, f"Action constants should be {EXPECTED_ACTION_RANGE}, got {expected_actions}"
        
        # Validate all action constants are unique integers in range [0,3]
        actions = [ACTION_UP, ACTION_RIGHT, ACTION_DOWN, ACTION_LEFT]
        assert all(isinstance(action, int) for action in actions), "All action constants should be integers"
        assert len(set(actions)) == len(actions), "All action constants should be unique"
        assert all(0 <= action <= 3 for action in actions), "All action constants should be in range [0,3]"
        
        # Assert MOVEMENT_VECTORS dictionary has keys matching action constants
        assert isinstance(MOVEMENT_VECTORS, dict), f"MOVEMENT_VECTORS should be dict, got {type(MOVEMENT_VECTORS)}"
        expected_keys = set(EXPECTED_ACTION_RANGE)
        actual_keys = set(MOVEMENT_VECTORS.keys())
        assert actual_keys == expected_keys, f"MOVEMENT_VECTORS keys should be {expected_keys}, got {actual_keys}"
        
        # Validate movement vectors are tuples of two integers (dx, dy)
        for action, vector in MOVEMENT_VECTORS.items():
            assert isinstance(vector, tuple), f"Movement vector for action {action} should be tuple, got {type(vector)}"
            assert len(vector) == 2, f"Movement vector for action {action} should have 2 elements, got {len(vector)}"
            assert all(isinstance(x, int) for x in vector), f"Movement vector for action {action} should contain integers"
            assert all(-1 <= x <= 1 for x in vector), f"Movement vector for action {action} should be unit vector"
        
        # Test movement vector consistency: UP=(0,1), RIGHT=(1,0), DOWN=(0,-1), LEFT=(-1,0)
        assert MOVEMENT_VECTORS[ACTION_UP] == (0, 1), f"UP movement should be (0,1), got {MOVEMENT_VECTORS[ACTION_UP]}"
        assert MOVEMENT_VECTORS[ACTION_RIGHT] == (1, 0), f"RIGHT movement should be (1,0), got {MOVEMENT_VECTORS[ACTION_RIGHT]}"
        assert MOVEMENT_VECTORS[ACTION_DOWN] == (0, -1), f"DOWN movement should be (0,-1), got {MOVEMENT_VECTORS[ACTION_DOWN]}"
        assert MOVEMENT_VECTORS[ACTION_LEFT] == (-1, 0), f"LEFT movement should be (-1,0), got {MOVEMENT_VECTORS[ACTION_LEFT]}"
        ...
        assert ACTION_RIGHT == Action.RIGHT.value, f"ACTION_RIGHT should match Action.RIGHT.value"
        assert ACTION_DOWN == Action.DOWN.value, f"ACTION_DOWN should match Action.DOWN.value"
        assert ACTION_LEFT == Action.LEFT.value, f"ACTION_LEFT should match Action.LEFT.value"
...
class TestRewardSystemConstants:
    ...
    def test_reward_system_constants(self):
        ...
        assert isinstance(REWARD_GOAL_REACHED, (int, float)), f"REWARD_GOAL_REACHED should be numeric, got {type(REWARD_GOAL_REACHED)}"
        assert abs(REWARD_GOAL_REACHED - 1.0) < TOLERANCE_FLOAT_COMPARISON, f"REWARD_GOAL_REACHED should be 1.0, got {REWARD_GOAL_REACHED}"
        ...
        assert isinstance(REWARD_DEFAULT, (int, float)), f"REWARD_DEFAULT should be numeric, got {type(REWARD_DEFAULT)}"
        assert abs(REWARD_DEFAULT - 0.0) < TOLERANCE_FLOAT_COMPARISON, f"REWARD_DEFAULT should be 0.0, got {REWARD_DEFAULT}"
        
        # Test that goal reward is greater than default reward for proper incentive
        assert REWARD_GOAL_REACHED > REWARD_DEFAULT, "Goal reward should be greater than default reward for proper incentive structure"
        
        # Verify reward values are within reasonable range for RL training
        assert -10.0 <= REWARD_DEFAULT <= 10.0, "Default reward should be within reasonable RL range"
        assert -10.0 <= REWARD_GOAL_REACHED <= 10.0, "Goal reward should be within reasonable RL range"
        
        # Assert reward constants support mathematical operations without overflow
        reward_sum = REWARD_GOAL_REACHED + REWARD_DEFAULT
        assert isinstance(reward_sum, (int, float)), "Reward constants should support addition"
        reward_product = REWARD_GOAL_REACHED * 1000  # Test scaling
        assert isinstance(reward_product, (int, float)), "Reward constants should support scaling"
...
class TestDataTypeConstants:
    ...
    def test_data_type_constants(self):
        ...
        assert FIELD_DTYPE == np.float32, f"FIELD_DTYPE should be numpy.float32, got {FIELD_DTYPE}"
        assert isinstance(FIELD_DTYPE, type), f"FIELD_DTYPE should be type, got {type(FIELD_DTYPE)}"
        
        # Assert OBSERVATION_DTYPE is numpy.float32 for Gymnasium observation compliance
        assert OBSERVATION_DTYPE == np.float32, f"OBSERVATION_DTYPE should be numpy.float32, got {OBSERVATION_DTYPE}"
        assert isinstance(OBSERVATION_DTYPE, type), f"OBSERVATION_DTYPE should be type, got {type(OBSERVATION_DTYPE)}"
        ...
        assert obs_dtype_obj is not None, "OBSERVATION_DTYPE should be valid NumPy dtype"
        ...
        assert test_obs.dtype == OBSERVATION_DTYPE, "Observation array should maintain specified dtype"
        ...
        assert np.finfo(FIELD_DTYPE).max > 1.0, "Field dtype should support concentration values > 1.0"
        assert np.finfo(OBSERVATION_DTYPE).max > 1.0, "Observation dtype should support observation values > 1.0"
...
class TestPlumeModelConstants:
    ...
    def test_plume_model_constants(self):
        ...
        assert isinstance(MIN_PLUME_SIGMA, (int, float)), f"MIN_PLUME_SIGMA should be numeric, got {type(MIN_PLUME_SIGMA)}"
        assert isinstance(MAX_PLUME_SIGMA, (int, float)), f"MAX_PLUME_SIGMA should be numeric, got {type(MAX_PLUME_SIGMA)}"
        assert MIN_PLUME_SIGMA > 0, "MIN_PLUME_SIGMA should be positive"
        assert MAX_PLUME_SIGMA > 0, "MAX_PLUME_SIGMA should be positive"
        assert MIN_PLUME_SIGMA < MAX_PLUME_SIGMA, "MIN_PLUME_SIGMA should be less than MAX_PLUME_SIGMA"
        
        # Validate DEFAULT_PLUME_SIGMA is within MIN/MAX range with proper consistency
        assert MIN_PLUME_SIGMA <= DEFAULT_PLUME_SIGMA <= MAX_PLUME_SIGMA, f"DEFAULT_PLUME_SIGMA {DEFAULT_PLUME_SIGMA} should be within range [{MIN_PLUME_SIGMA}, {MAX_PLUME_SIGMA}]"
        ...
        assert 0.0 <= test_gaussian_min <= 1.0, "Min sigma should produce valid Gaussian values"
        
        # Test that max sigma produces reasonable plume extent
        max_extent = 3 * MAX_PLUME_SIGMA  # 3-sigma rule
        assert max_extent < MAX_REASONABLE_GRID_SIZE, "Max sigma should not produce unreasonably large plume extent"
...
class TestRenderingConstants:
    ...
    def test_rendering_constants(self):
        ...
        assert SUPPORTED_RENDER_MODES == EXPECTED_RENDER_MODES, f"Expected render modes {EXPECTED_RENDER_MODES}, got {SUPPORTED_RENDER_MODES}"
        assert isinstance(SUPPORTED_RENDER_MODES, list), f"SUPPORTED_RENDER_MODES should be list, got {type(SUPPORTED_RENDER_MODES)}"
        assert len(SUPPORTED_RENDER_MODES) == 2, f"Should support exactly 2 render modes, got {len(SUPPORTED_RENDER_MODES)}"
        assert all(isinstance(mode, str) for mode in SUPPORTED_RENDER_MODES), "All render modes should be strings"
        
        # Validate render modes match RenderMode enumeration values
        assert 'rgb_array' in SUPPORTED_RENDER_MODES, "rgb_array mode should be supported"
        assert 'human' in SUPPORTED_RENDER_MODES, "human mode should be supported"
        assert SUPPORTED_RENDER_MODES[0] == RenderMode.RGB_ARRAY.value, "First render mode should match RenderMode.RGB_ARRAY"
        assert SUPPORTED_RENDER_MODES[1] == RenderMode.HUMAN.value, "Second render mode should match RenderMode.HUMAN"
        ...
        for mode in SUPPORTED_RENDER_MODES:
            ...
            assert mode.isalnum() or '_' in mode, f"Render mode '{mode}' should be valid identifier"
...
class TestPerformanceTargetConstants:
    ...
    def test_performance_target_constants(self):
        ...
        assert isinstance(PERFORMANCE_TARGET_STEP_LATENCY_MS, (int, float)), f"Step latency target should be numeric, got {type(PERFORMANCE_TARGET_STEP_LATENCY_MS)}"
        assert PERFORMANCE_TARGET_STEP_LATENCY_MS > 0, "Step latency target should be positive"
        assert PERFORMANCE_TARGET_STEP_LATENCY_MS <= 2.0, "Step latency target should be achievable (<= 2ms)"
        
        # Assert MEMORY_LIMIT_TOTAL_MB equals reasonable value for resource constraint validation
        assert isinstance(MEMORY_LIMIT_TOTAL_MB, int), f"Memory limit should be integer, got {type(MEMORY_LIMIT_TOTAL_MB)}"
        assert MEMORY_LIMIT_TOTAL_MB > 0, "Memory limit should be positive"
        assert 10 <= MEMORY_LIMIT_TOTAL_MB <= 500, f"Memory limit {MEMORY_LIMIT_TOTAL_MB}MB should be reasonable [10-500MB]"
        
        # Verify performance targets are achievable and mathematically consistent
        assert PERFORMANCE_TARGET_STEP_LATENCY_MS < 1000, "Step latency should be sub-second"
...
class TestValidationErrorMessages:
    ...
    def test_validation_error_messages(self):
        ...
        assert isinstance(VALIDATION_ERROR_MESSAGES, dict), f"VALIDATION_ERROR_MESSAGES should be dict, got {type(VALIDATION_ERROR_MESSAGES)}"
        assert len(VALIDATION_ERROR_MESSAGES) > 0, "VALIDATION_ERROR_MESSAGES should not be empty"
        
        # Test error messages for common validation cases
        expected_error_keys = ['invalid_action', 'invalid_coordinates', 'invalid_grid_size', 'invalid_sigma']
        for key in expected_error_keys:
            if key in VALIDATION_ERROR_MESSAGES:
                message = VALIDATION_ERROR_MESSAGES[key]
                assert isinstance(message, str), f"Error message for '{key}' should be string"
                assert len(message) > 0, f"Error message for '{key}' should not be empty"
        ...
        for key, message in VALIDATION_ERROR_MESSAGES.items():
            ...
            assert isinstance(message, str), f"Error message should be string, got {type(message)}"
            assert len(key) > 0, f"Error key '{key}' should not be empty"
            assert len(message) > 10, f"Error message for '{key}' should be descriptive (>10 chars)"
        ...
        for key, message in VALIDATION_ERROR_MESSAGES.items():
            ...
            try:
                ...
                if '{' in message and '}' in message:
                    ...
                    assert message.count('{') == message.count('}'), f"Unmatched braces in error message for '{key}'"
...
class TestFactoryFunctions:
    ...
    def test_get_default_environment_constants(self):
        ...
        assert isinstance(result, dict), f"Factory should return dict, got {type(result)}"
        ...
        if 'grid_size' in result:
            ...
            assert isinstance(grid_size, tuple), f"Grid size should be tuple, got {type(grid_size)}"
    ...
    def test_get_plume_model_constants(self):
        ...
        if 'default_sigma' in result and 'sigma_range' in result:
            ...
            if isinstance(sigma_range, tuple) and len(sigma_range) == 2:
                ...
                assert min_sigma <= default_sigma <= max_sigma, "Default sigma should be within sigma range"
    ...
    def test_get_action_space_constants(self):
        ...
        if 'movement_vectors' in result:
            ...
            assert isinstance(movement_vectors, dict), "Movement vectors should be dictionary"
            ...
            for action, vector in movement_vectors.items():
                if isinstance(vector, tuple) and len(vector) == 2:
                    ...
                    assert magnitude_squared <= 2, f"Movement vector {vector} should be unit vector or stationary"
    ...
    def test_get_performance_constants(self):
        ...
        assert len(present_keys) > 0, f"At least one performance key should be present: {performance_keys}"
    ...
    def test_get_validation_constants(self):
        ...
        if 'error_messages' in result:
            ...
            assert isinstance(error_messages, dict), "Error messages should be dictionary"
    ...
    def test_get_testing_constants(self):
        ...
        if 'test_seeds' in result:
            ...
            assert isinstance(test_seeds, (list, tuple)), "Test seeds should be sequence"
            if len(test_seeds) > 0:
                assert all(isinstance(seed, int) for seed in test_seeds), "Test seeds should be integers"
...
class TestConstantConsistencyValidation:
    ...
    def test_validate_constant_consistency_normal(self):
        ...
        assert isinstance(result, tuple), f"Validation should return tuple, got {type(result)}"
        ...
        assert isinstance(is_valid, bool), f"is_valid should be boolean, got {type(is_valid)}"
        assert isinstance(validation_report, dict), f"validation_report should be dict, got {type(validation_report)}"
        
        # Test mathematical consistency: default source location within default grid bounds
        assert DEFAULT_SOURCE_LOCATION[0] < DEFAULT_GRID_SIZE[0], "Source X should be within grid width"
        assert DEFAULT_SOURCE_LOCATION[1] < DEFAULT_GRID_SIZE[1], "Source Y should be within grid height"
        
        # Assert sigma parameter relationships: DEFAULT_PLUME_SIGMA within MIN/MAX range
        assert MIN_PLUME_SIGMA <= DEFAULT_PLUME_SIGMA <= MAX_PLUME_SIGMA, "Default sigma should be within min/max range"
    ...
    def test_validate_constant_consistency_strict(self):
        ...
        assert isinstance(result, tuple), f"Strict validation should return tuple, got {type(result)}"
        assert len(result) == 2, f"Strict validation should return 2-tuple, got {len(result)}"
        
        is_valid, validation_report = result
        
        # Assert strict mode applies additional precision and bounds validation
        assert isinstance(is_valid, bool), f"Strict is_valid should be boolean, got {type(is_valid)}"
        assert isinstance(validation_report, dict), f"Strict validation_report should be dict, got {type(validation_report)}"
        
        # Assert comprehensive validation report includes detailed findings and recommendations
        assert len(validation_report) > 0, "Strict mode should provide detailed validation report"
...
class TestConstantTypesAndImmutability:
    ...
    def test_constant_types_and_immutability(self):
        ...
        for const in string_constants:
            assert isinstance(const, str), f"String constant should be str, got {type(const)}"
        ...
        for const in tuple_constants:
            assert isinstance(const, tuple), f"Tuple constant should be tuple, got {type(const)}"
        ...
        numeric_constants = [DEFAULT_PLUME_SIGMA, REWARD_GOAL_REACHED, REWARD_DEFAULT, 
                           PERFORMANCE_TARGET_STEP_LATENCY_MS, MIN_PLUME_SIGMA, MAX_PLUME_SIGMA]
        for const in numeric_constants:
            assert isinstance(const, (int, float)), f"Numeric constant should be int or float, got {type(const)}"
        ...
        for const in dict_constants:
            assert isinstance(const, dict), f"Dictionary constant should be dict, got {type(const)}"
        ...
        for const in list_constants:
            assert isinstance(const, list), f"List constant should be list, got {type(const)}"
        ...
        for const in dtype_constants:
            assert isinstance(const, type), f"Dtype constant should be type, got {type(const)}"
            # Test that dtype can create valid numpy dtype
            dtype_obj = np.dtype(const)
            assert dtype_obj is not None, "Dtype constant should create valid numpy dtype"
...
class TestConstantRangesAndBounds:
    ...
    def test_constant_ranges_and_bounds(self):
        ...
        assert DEFAULT_GRID_SIZE[0] > 0 and DEFAULT_GRID_SIZE[1] > 0, "Grid dimensions should be positive"
        assert DEFAULT_GRID_SIZE[0] <= MAX_REASONABLE_GRID_SIZE, f"Grid width should be <= {MAX_REASONABLE_GRID_SIZE}"
        assert DEFAULT_GRID_SIZE[1] <= MAX_REASONABLE_GRID_SIZE, f"Grid height should be <= {MAX_REASONABLE_GRID_SIZE}"
        ...
        assert MAX_PLUME_SIGMA > MIN_PLUME_SIGMA, "Max sigma should be greater than min sigma"
        assert MIN_PLUME_SIGMA >= MIN_REASONABLE_SIGMA, f"Min sigma should be >= {MIN_REASONABLE_SIGMA}"
        assert MAX_PLUME_SIGMA <= MAX_REASONABLE_SIGMA, f"Max sigma should be <= {MAX_REASONABLE_SIGMA}"
        
        # Validate reward constants are within reasonable RL training ranges
        assert -100.0 <= REWARD_DEFAULT <= 100.0, "Default reward should be within reasonable range"
        assert -100.0 <= REWARD_GOAL_REACHED <= 100.0, "Goal reward should be within reasonable range"
        
        # Assert performance target constants are achievable with current hardware
        assert 0.01 <= PERFORMANCE_TARGET_STEP_LATENCY_MS <= 1000.0, "Step latency target should be reasonable"
        ...
        actions = [ACTION_UP, ACTION_RIGHT, ACTION_DOWN, ACTION_LEFT]
        assert set(actions) == set(range(4)), "Actions should form complete space [0,1,2,3]"
...
class TestCrossConstantMathematicalRelationships:
    ...
    def test_cross_constant_mathematical_relationships(self):
        ...
        assert abs(DEFAULT_SOURCE_LOCATION[0] - center_x) <= 2, "Source X should be near grid center"
        assert abs(DEFAULT_SOURCE_LOCATION[1] - center_y) <= 2, "Source Y should be near grid center"
        ...
        assert plume_extent < grid_diagonal, "Plume extent should be reasonable for grid size"
        ...
        assert estimated_mb < MEMORY_LIMIT_TOTAL_MB, "Default configuration should fit within memory limits"
        
        # Verify reward system constants provide proper incentive gradients
        reward_difference = REWARD_GOAL_REACHED - REWARD_DEFAULT
        assert reward_difference > 0, "Goal reward should be greater than default reward"
        ...
        assert len(unique_magnitudes) <= 2, "Movement vectors should have consistent magnitudes (0 or 1)"
...
class TestConstantsIntegrationWithTypes:
    ...
    def test_constants_integration_with_types(self):
        ...
        assert ACTION_UP == Action.UP.value, f"ACTION_UP {ACTION_UP} should match Action.UP.value {Action.UP.value}"
        assert ACTION_RIGHT == Action.RIGHT.value, f"ACTION_RIGHT {ACTION_RIGHT} should match Action.RIGHT.value {Action.RIGHT.value}"
        assert ACTION_DOWN == Action.DOWN.value, f"ACTION_DOWN {ACTION_DOWN} should match Action.DOWN.value {Action.DOWN.value}"
        assert ACTION_LEFT == Action.LEFT.value, f"ACTION_LEFT {ACTION_LEFT} should match Action.LEFT.value {Action.LEFT.value}"
        
        # Assert render mode constants match RenderMode enumeration values
        assert 'rgb_array' == RenderMode.RGB_ARRAY.value, f"'rgb_array' should match RenderMode.RGB_ARRAY.value"
        assert 'human' == RenderMode.HUMAN.value, f"'human' should match RenderMode.HUMAN.value"
        
        # Verify all supported render modes match enumeration
        for mode in SUPPORTED_RENDER_MODES:
            render_mode_values = [rm.value for rm in RenderMode]
            assert mode in render_mode_values, f"Render mode '{mode}' should match RenderMode enumeration value"
...
class TestFactoryFunctionConsistency:
    ...
    def test_factory_function_consistency(self):
        ...
        for factory in factories:
            result = factory()
            assert isinstance(result, dict), f"Factory {factory.__name__} should return dict"
            assert len(result) > 0, f"Factory {factory.__name__} should return non-empty dict"
        ...
        assert env_constants1 == env_constants2, "Factory should return consistent results"
        ...
        for key in all_keys:
            if isinstance(key, str):
                ...
                assert key.islower() or '_' in key or key.isupper(), f"Key '{key}' should follow consistent naming convention"
        
        # Assert factory function outputs are JSON serializable for configuration storage
        import json
        for i, result in enumerate(results):
            try:
                json_str = json.dumps(result, default=str)  # Use str for non-serializable types
                assert len(json_str) > 0, f"Factory result {i} should be JSON serializable"
...
class TestConstantEdgeCases:
    ...
    def test_movement_vector_completeness(self):
        ...
        expected_directions = {(0, 1), (1, 0), (0, -1), (-1, 0)}  # UP, RIGHT, DOWN, LEFT
        actual_directions = set(MOVEMENT_VECTORS.values())
        
        assert expected_directions.issubset(actual_directions), f"Movement vectors should include all cardinal directions"
        ...
        for i, vec1 in enumerate(vectors):
            for j, vec2 in enumerate(vectors):
                if i != j:
                    ...
                    assert dot_product in [-1, 0], f"Movement vectors should be orthogonal or opposite"
    ...
    def test_dtype_numpy_compatibility(self):
        ...
        assert field_array.dtype == FIELD_DTYPE, "Field array should maintain correct dtype"
        assert obs_array.dtype == OBSERVATION_DTYPE, "Observation array should maintain correct dtype"
        ...
        assert result.dtype == FIELD_DTYPE, "Arithmetic operations should preserve dtype"
    ...
    def test_error_message_format_strings(self):
        ...
        for key, message in VALIDATION_ERROR_MESSAGES.items():
            ...
            assert open_braces == close_braces, f"Error message '{key}' has unbalanced braces"
            ...
            for pattern in sensitive_patterns:
                assert pattern not in message_lower, f"Error message '{key}' should not contain sensitive information"
    ...
    def test_sigma_range_validation(self, sigma_value):
        ...
        assert 0.0 <= gaussian_value <= 1.0, f"Gaussian value should be in [0,1] for sigma={sigma_value}"
        assert np.isfinite(gaussian_value), f"Gaussian value should be finite for sigma={sigma_value}"
    ...
    def test_performance_target_relationships(self):
        ...
        assert total_time_estimate < 10000, "Total episode time should be reasonable (< 10 seconds)"
        ...
        assert MEMORY_LIMIT_TOTAL_MB >= max_memory_mb * 0.1, "Memory limit should accommodate reasonable grid sizes"
...
class TestConstantPerformance:
    ...
    def test_factory_function_performance(self):
        ...
        for factory in factories:
            ...
            assert elapsed_time < 0.01, f"Factory {factory.__name__} took too long: {elapsed_time:.4f}s"
            assert isinstance(result, dict), f"Factory {factory.__name__} should return dict"
