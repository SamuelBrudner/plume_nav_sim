...
class TestEpisodeManagerConfig:
    ...
    def test_resource_estimation(self):
        ...
        resource_estimate = config.estimate_episode_resources(expected_episode_length=100)
    ...
    def test_config_cloning(self):
        ...
        assert cloned_config.env_config.grid_size == original_config.env_config.grid_size
        assert cloned_config.env_config.source_location == original_config.env_config.source_location
        ...
        assert overridden_config.env_config.grid_size == original_config.env_config.grid_size
...
class TestEpisodeManager:
    ...
    def test_reset_episode(self, episode_manager):
        ...
        required_info_keys = ['agent_xy', 'distance_to_source', 'step_count', 'episode_id']
    ...
    def test_process_step(self, episode_manager):
        ...
        obs, reward, terminated, truncated, info = episode_manager.process_step(Action.RIGHT)
    ...
    def test_episode_termination(self, episode_manager):
        ...
        while not terminated and step_count < max_attempts:
            ...
            obs, reward, terminated, truncated, info = episode_manager.process_step(action)
            step_count += 1
            
            # Check if we reached the goal
            if info['distance_to_source'] <= episode_manager.config.env_config.goal_radius:
        ...
        if not terminated:
            ...
            for step in range(max_steps + 1):
                obs, reward, terminated, truncated, info = episode_manager.process_step(Action.UP)
    ...
    def test_state_consistency_validation(self, episode_manager):
        ...
        for _ in range(5):
            obs, reward, terminated, truncated, info = episode_manager.process_step(Action.DOWN)
            if terminated or truncated:
                break
    ...
    def test_episode_statistics(self, episode_manager):
        ...
        expected_success_rate = terminated_count / len(episode_results) if episode_results else 0.0
    ...
    def test_cleanup(self, episode_manager):
        ...
        assert statistics_after_cleanup.episodes_completed == statistics_before_cleanup.episodes_completed
...
class TestEpisodeManagerReproducibility:
    ...
    def test_identical_episodes_same_seed(self, episode_manager_pair, test_seed):
        ...
        action_sequence = [Action.RIGHT, Action.DOWN, Action.LEFT, Action.UP, Action.RIGHT]
    ...
    def test_different_episodes_different_seeds(self, episode_manager_pair):
        ...
        assert differences_found, "Episodes with different seeds should produce different trajectories"
        ...
        assert (result1.total_steps != result2.total_steps or 
                result1.final_agent_position != result2.final_agent_position)
    ...
    def test_seeding_isolation(self, episode_manager_pair):
        ...
        # Process some steps
        for _ in range(5):
            obs, reward, terminated, truncated, info = manager1.process_step(Action.RIGHT)
            if terminated or truncated:
                break
...
class TestEpisodeManagerPerformance:
    ...
    def test_step_latency_performance(self, performance_episode_manager):
        ...
        for i in range(min(PERFORMANCE_TEST_ITERATIONS, 20)):  # Reduced for test efficiency
            start_time = time.perf_counter()
        ...
        assert percentile_95 < PERFORMANCE_TARGET_STEP_LATENCY_MS * 20  # 20ms for 95th percentile
        
        # Assert no steps exceed extreme outlier threshold
        outlier_threshold = PERFORMANCE_TARGET_STEP_LATENCY_MS * 50  # 50ms outlier threshold
        outliers = [t for t in step_times if t > outlier_threshold]
        assert len(outliers) == 0, f"Found {len(outliers)} outliers exceeding {outlier_threshold}ms"
    ...
    def test_component_coordination_efficiency(self, performance_episode_manager):
        ...
        def timed_state_manager_step(*args, **kwargs):
            ...
            component_times['state_manager'].append((time.perf_counter() - start) * 1000)
            return result
        ...
        consistency_check_time = (time.perf_counter() - consistency_check_time_start) * 1000
    ...
    def test_performance_under_load(self, performance_episode_manager):
        ...
        overall_step_times = [time for episode_times in step_times_per_episode for time in episode_times]
        ...
        episode_avg_times = [np.mean(times) for times in step_times_per_episode if times]
...
class TestEpisodeManagerErrorHandling:
    ...
    def test_component_failure_handling(self, episode_manager):
        ...
        episode_manager.reset_episode(seed=123)
        obs, reward, terminated, truncated, info = episode_manager.process_step(Action.LEFT)
        assert isinstance(obs, np.ndarray)
    ...
    def test_configuration_error_handling(self):
        ...
        try:
            ...
            assert "source" in str(e.message).lower() or "location" in str(e.message).lower()
...
class TestEpisodeResultAndStatistics:
    ...
    def test_episode_result_summary(self):
        ...
        # Set final state
        final_position = create_coordinates(x=TEST_SOURCE_LOCATION[0], y=TEST_SOURCE_LOCATION[1])
        final_agent_state = AgentState(
            position=final_position,
            step_count=30,
            total_reward=1.0,
        ...
        summary_detailed = episode_result.get_summary(include_performance=True, include_components=True)
    ...
    def test_episode_statistics_collection(self):
        ...
        # Successful episode
        success_episode = EpisodeResult("ep1", terminated=True, truncated=False, total_steps=20)
        success_episode.total_reward = 1.0
        episodes.append(success_episode)
        
        # Truncated episode
        truncated_episode = EpisodeResult("ep2", terminated=False, truncated=True, total_steps=100)
        truncated_episode.total_reward = 0.0
        episodes.append(truncated_episode)
        
        # Another successful episode
        success_episode2 = EpisodeResult("ep3", terminated=True, truncated=False, total_steps=35)
    ...
    def test_success_rate_calculation(self):
        ...
        for i in range(3):
            success_ep = EpisodeResult(f"success_{i}", terminated=True, truncated=False, total_steps=20+i)
            success_ep.total_reward = 1.0
            statistics.add_episode_result(success_ep)
            
        for i in range(2):
            truncated_ep = EpisodeResult(f"truncated_{i}", terminated=False, truncated=True, total_steps=100)
        ...
        success_ep = EpisodeResult("all_success", terminated=True, truncated=False, total_steps=15)
        ...
        truncated_ep = EpisodeResult("all_truncated", terminated=False, truncated=True, total_steps=100)
    ...
    def test_performance_summary_analysis(self):
        ...
        for i, (steps, reward, duration) in enumerate(episode_performance_data):
            episode = EpisodeResult(f"perf_ep_{i}", terminated=(reward==1.0), truncated=(reward==0.0), total_steps=steps)
...
class TestFactoryFunctions:
    ...
    def test_create_episode_manager_custom(self):
        ...
        episode_manager = create_episode_manager(
            ...
            seed_manager=custom_seed_manager
        )
    ...
    def test_validate_episode_config_valid(self):
        ...
        is_valid_strict, strict_report = validate_episode_config(valid_config, strict_mode=True)
    ...
    def test_factory_function_integration(self):
        ...
        if is_valid:
            ...
            obs, info = episode_manager.reset_episode(seed=42)
            obs2, reward, terminated, truncated, info2 = episode_manager.process_step(Action.RIGHT)
