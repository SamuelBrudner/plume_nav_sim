...
from plume_nav_sim.utils.exceptions import ValidationError, ComponentError
...
@pytest.mark.unit
@pytest.mark.parametrize("agent_pos,source_pos,goal_radius,expected_reward", [
...
def test_reward_calculation_accuracy(agent_pos, source_pos, goal_radius, expected_reward):
    """
...
def test_goal_detection_boundary_conditions(goal_radius):
    ...
    if goal_radius > 0:
        ...
        boundary_pos = Coordinates(
            int(round(source_pos.x + offset)),
            int(round(source_pos.y + offset))
        )
    ...
    if precision_test_distance != goal_radius:  # Only if precision matters
        ...
        actual_precision_distance = calculate_euclidean_distance(precision_pos, source_pos)
...
def test_termination_logic():
    ...
    goal_termination = calculator.check_termination(goal_agent_state, source_pos, max_steps)
    ...
    limit_termination = calculator.check_termination(limit_agent_state, source_pos, max_steps)
    ...
    both_termination = calculator.check_termination(both_agent_state, source_pos, max_steps)
    ...
    ongoing_termination = calculator.check_termination(ongoing_agent_state, source_pos, max_steps)
...
def test_agent_reward_integration():
    ...
    calculator.update_agent_reward(agent_state, second_reward_result, update_goal_status=False)
...
def test_performance_targets():
    ...
    assert average_time < target_time, f"Average time {average_time:.3f}ms exceeds target {target_time:.3f}ms"
    
    # Ensure 95th percentile execution time meets performance targets
    assert percentile_95 < PERFORMANCE_TARGET_STEP_LATENCY_MS, f"95th percentile {percentile_95:.3f}ms exceeds target {PERFORMANCE_TARGET_STEP_LATENCY_MS:.3f}ms"
    ...
    assert large_coord_average < target_time, "Large coordinate performance degradation detected"
...
def test_caching_efficiency():
    ...
    no_cache_calculator = RewardCalculator(RewardCalculatorConfig(
        goal_radius=1.0,
        reward_goal_reached=1.0,
        reward_default=0.0,
...
def test_reward_statistics_tracking():
    ...
    performance_stats = calculator.get_reward_statistics(include_performance_analysis=True)
...
def test_parameter_validation_comprehensive():
    ...
    boundary_config = RewardCalculatorConfig(
        goal_radius=0.0,  # Exact boundary
        reward_goal_reached=1.0,
        reward_default=0.0
    )
...
def test_configuration_cloning_and_modification():
    ...
    assert cloned_config.distance_calculation_method == original_config.distance_calculation_method
    ...
    assert overridden_config.distance_calculation_method == original_config.distance_calculation_method
...
def test_performance_impact_estimation():
    ...
    for i, config in enumerate(configs):
        ...
        for key in required_keys:
            assert key in impact, f"Missing key '{key}' in performance impact for config {i}"
        ...
        assert impact['per_step_calculation_ms'] * expected_episode_length <= impact['total_episode_overhead_ms'] * 1.2  # Allow some overhead
        ...
        if config.enable_caching:
            ...
            no_caching_impact = config.estimate_performance_impact(
                expected_episode_length,
                include_caching_benefits=False
            )
            
            # With caching benefits should be faster
            assert caching_impact['per_step_calculation_ms'] <= no_caching_impact['per_step_calculation_ms']
        ...
        if config.distance_calculation_method != 'euclidean':
            ...
            assert euclidean_rec, "Should recommend euclidean method for optimal performance"
...
def test_reward_result_data_structure():
    ...
    assert perf_result.calculation_details['performance_warning'] == 'Exceeded target latency'
...
def test_factory_function_comprehensive():
    ...
    assert validated_calculator.validate_calculation_parameters(
        Coordinates(5, 5), Coordinates(10, 10)
    ) is True
    
    # Test with performance metrics disabled
    no_perf_calculator = create_reward_calculator(
        enable_performance_monitoring=False
    )
...
def test_configuration_validation_function():
    ...
    normal_valid, normal_report = validate_reward_config(borderline_config, strict_validation=False)
    strict_valid, strict_report = validate_reward_config(borderline_config, strict_validation=True)
    ...
    context_config = RewardCalculatorConfig(
        goal_radius=50.0,  # Large but valid
        reward_goal_reached=1.0,
        reward_default=0.0
    )
    ...
    _, detailed_report = validate_reward_config(detailed_invalid_config, strict_validation=True)
...
def test_reproducibility_with_seeding():
    ...
    for i, (r1, r2) in enumerate(zip(results1, results2)):
        assert r1.reward == r2.reward, f"Reward mismatch at index {i}: {r1.reward} != {r2.reward}"
        assert r1.goal_reached == r2.goal_reached, f"Goal status mismatch at index {i}"
        assert abs(r1.distance_to_goal - r2.distance_to_goal) < DISTANCE_TOLERANCE, f"Distance mismatch at index {i}"
    ...
    distances = []
    for _ in range(10):
        distance = calculator1.get_distance_to_goal(test_agent, test_source, use_cache=False)
    ...
    for _ in range(5):
        ...
        assert abs(baseline_result.distance_to_goal - repeat_result.distance_to_goal) < DISTANCE_TOLERANCE
    ...
    for i, (r3, r4) in enumerate(zip(cache_results3, cache_results4)):
        ...
        assert r3.goal_reached == r4.goal_reached, f"Cache test goal status mismatch at index {i}"
        assert abs(r3.distance_to_goal - r4.distance_to_goal) < DISTANCE_TOLERANCE, f"Cache test distance mismatch at index {i}"
    ...
    assert abs(stats3['goal_achievement_rate'] - stats4['goal_achievement_rate']) < DISTANCE_TOLERANCE
...
def test_stress_testing_large_scale():
    ...
    assert average_time_per_calc < PERFORMANCE_TARGET_STEP_LATENCY_MS * 2, f"Stress test average time {average_time_per_calc:.3f}ms exceeded threshold"
    
    # Sample timing analysis
    if timing_samples:
        sample_average = np.mean(timing_samples)
        assert sample_average < PERFORMANCE_TARGET_STEP_LATENCY_MS * 1.5, f"Sample timing {sample_average:.3f}ms too slow"
    ...
    assert final_cache_size <= max_cache_size, f"Cache size {final_cache_size} exceeded maximum {max_cache_size}"
    ...
    assert success_rate > 0.95, f"Success rate {success_rate:.3f} too low - system unstable under stress"
    ...
    assert stats['performance_violations'] / stats['total_calculations'] < 0.1, "Too many performance violations"
    ...
    if timing_samples and len(timing_samples) >= 10:
        ...
        assert degradation_ratio < 2.0, f"Performance degraded by factor {degradation_ratio:.2f}"
...
def test_mathematical_edge_cases():
    ...
    try:
        ...
        assert abs(large_result.distance_to_goal - expected_distance) < DISTANCE_TOLERANCE * 10
    ...
    for x_min, x_max in test_ranges:
        for y_min, y_max in test_ranges:
            ...
            assert abs(result.distance_to_goal - expected_distance) < DISTANCE_TOLERANCE * max(1, expected_distance * 1e-10)
            
            # Test symmetry
            symmetric_result = calculator.calculate_reward(coord2, coord1)
            assert abs(result.distance_to_goal - symmetric_result.distance_to_goal) < DISTANCE_TOLERANCE
    ...
    assert abs(high_precision_result - math.sqrt(2)) < DISTANCE_TOLERANCE * 100  # Allow for some numerical error with large numbers
...
def test_integration_with_performance_metrics():
    ...
    test_calculations = [
        (Coordinates(0, 0), Coordinates(0, 0)),    # Goal achieved
        (Coordinates(0, 0), Coordinates(5, 5)),    # Goal not achieved
        (Coordinates(10, 10), Coordinates(12, 12)) # Goal achieved (distance ≈ 2.83 > 2.0) - actually not achieved
    ]
    ...
    calculator_stats = calculator.get_reward_statistics(include_performance_analysis=True)
    ...
    if hasattr(performance_metrics, 'record_resource_usage'):
        ...
        performance_metrics.record_resource_usage('cache_size', len(calculator.distance_cache))
    ...
    method_configs = [
        ('euclidean', RewardCalculatorConfig(goal_radius=2.0, reward_goal_reached=1.0, reward_default=0.0, distance_calculation_method='euclidean')),
        ('manhattan', RewardCalculatorConfig(goal_radius=2.0, reward_goal_reached=1.0, reward_default=0.0, distance_calculation_method='manhattan')),
        ('chebyshev', RewardCalculatorConfig(goal_radius=2.0, reward_goal_reached=1.0, reward_default=0.0, distance_calculation_method='chebyshev'))
    ]
    ...
    for method_name, method_config in method_configs:
        ...
        if result.calculation_details:
            assert result.calculation_details.get('distance_calculation_method') == method_name
        
        # Performance metrics should capture method-specific timing
        method_stats = method_calculator.get_reward_statistics(include_performance_analysis=True)
