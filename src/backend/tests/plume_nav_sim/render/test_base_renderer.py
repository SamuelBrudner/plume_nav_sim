...
def assert_render_output_valid(
    render_output: Union[RGBArray, None],
    ...
    if expected_mode == RenderMode.RGB_ARRAY:
        ...
        assert isinstance(
            render_output, np.ndarray
        ), f"Expected numpy array, got {type(render_output)}"
        ...
        assert (
            render_output.shape == expected_shape
        ), f"Shape mismatch: {render_output.shape} != {expected_shape}"

        # Validate RGB array dtype is RGB_DTYPE (uint8) with values in [0,255] range
        assert (
            render_output.dtype == RGB_DTYPE
        ), f"Dtype mismatch: {render_output.dtype} != {RGB_DTYPE}"
        assert np.all(
            (render_output >= 0) & (render_output <= 255)
        ...
        assert (
            render_output is None
        ), f"HUMAN mode must return None, got {render_output}"
...
def measure_rendering_performance(
    ...
    if rgb_times:
        ...
        performance_stats["target_compliance"]["rgb_target_met"] = (
            np.mean(rgb_times) < PERFORMANCE_TARGET_RGB_RENDER_MS
        )
    ...
    if human_times:
        ...
        performance_stats["target_compliance"]["human_target_met"] = (
            np.mean(human_times) < PERFORMANCE_TARGET_HUMAN_RENDER_MS
        )
    ...
    if not performance_stats["target_compliance"].get("rgb_target_met", True):
        performance_stats["optimization_recommendations"].append(
            "Optimize RGB rendering pipeline"
        )

    if not performance_stats["target_compliance"].get("human_target_met", True):
        performance_stats["optimization_recommendations"].append(
            "Optimize human rendering pipeline"
        )
...
class MockRenderer(BaseRenderer):
    ...
    def __init__(
        ...
        super().__init__(
            grid_size, color_scheme_name or "mock_scheme", renderer_options or {}
        )
    ...
    def _initialize_renderer_resources(self) -> None:
        ...
        if self.simulate_failures:
            raise ComponentError(
                "Simulated initialization failure", component_name="MockRenderer"
            )
    ...
    def _render_rgb_array(self, context: RenderContext) -> RGBArray:
        ...
        if (
            0 <= source_x < self.grid_size.width
            and 0 <= source_y < self.grid_size.height
        ):
            ...
            for dx in range(-2, 3):
                for dy in range(-2, 3):
                    if abs(dx) <= 1 or abs(dy) <= 1:  # Cross pattern
                        ...
                        if (
                            0 <= x < self.grid_size.width
                            and 0 <= y < self.grid_size.height
                        ):
    ...
    def _render_human(self, context: RenderContext) -> None:
        ...
        if self.simulate_failures:
            raise RenderingError(
                "Simulated human rendering failure", render_mode="human"
            )

    def configure_for_testing(
        self,
        enable_rgb_array: bool = True,
...
def performance_test_context():
    ...
    concentration_field = create_test_concentration_field(
        grid_size=grid_size, source_location=source_location, sigma=15.0, add_noise=True
    )
...
@pytest.fixture(
    params=[RenderMode.RGB_ARRAY, RenderMode.HUMAN], ids=["rgb_mode", "human_mode"]
)
...
class TestBaseRenderer:
    ...
    def test_renderer_initialization_success(self, mock_renderer):
        ...
        assert (
            not mock_renderer._initialized
        ), "Renderer should not be initialized initially"
    ...
    def test_render_context_validation_success(
        self, initialized_mock_renderer, valid_context
    ):
        ...
        assert (
            valid_context.grid_size == initialized_mock_renderer.grid_size
        ), "Context grid size should match renderer"

        # Test context validation with strict_validation enabled
        strict_validation = initialized_mock_renderer.validate_context(
            valid_context, strict_validation=True
        )
    ...
    def test_render_rgb_array_mode_success(self, rgb_capable_renderer, valid_context):
        ...
        expected_shape = (
            valid_context.grid_size.height,
            valid_context.grid_size.width,
            3,
        )
        assert (
            render_output.shape == expected_shape
        ), f"RGB array shape should be {expected_shape}"

        # Verify array dtype is RGB_DTYPE (uint8) with values in [0, 255] range
        assert (
            render_output.dtype == RGB_DTYPE
        ), f"RGB array dtype should be {RGB_DTYPE}"
        assert np.all(
            (render_output >= 0) & (render_output <= 255)
        ), "RGB values must be in [0,255] range"
        ...
        agent_y, agent_x = (
            valid_context.agent_position.y,
            valid_context.agent_position.x,
        )
        agent_pixel = render_output[agent_y, agent_x]
        assert (
            agent_pixel[0] > 200 and agent_pixel[1] < 50 and agent_pixel[2] < 50
        ), "Agent marker should be red"

        # Verify source marker (white pixels) at context.source_position
        source_y, source_x = (
            valid_context.source_position.y,
            valid_context.source_position.x,
        )
    ...
    def test_render_human_mode_success(self, human_capable_renderer, valid_context):
        ...
        assert (
            render_time_ms
            < PERFORMANCE_TARGET_HUMAN_RENDER_MS + TEST_PERFORMANCE_TOLERANCE_MS
    ...
    def test_render_unsupported_mode(self, valid_context):
        ...
        limited_renderer = create_mock_renderer(
            grid_size=grid_size, supports_rgb_array=True, supports_human=False
        )
        ...
        human_only_renderer = create_mock_renderer(
            grid_size=grid_size, supports_rgb_array=False, supports_human=True
        )
    ...
    def test_performance_metrics_collection(
        ...
        assert (
            "render_operations" in metrics
        ), "Metrics should contain render operation data"
        assert (
            metrics["render_operations"] >= num_operations
        ), "Should track all render operations"

        # Verify operation_count tracking and frequency analysis
        stats = initialized_mock_renderer.get_test_statistics()
        assert (
            stats["rgb_array_renders"] == num_operations
        ), "Should track RGB render count"
    ...
    def test_performance_target_validation(self, valid_context):
        ...
        slow_renderer = create_mock_renderer(
            ...
            performance_overrides={
                "rgb_render_delay_ms": PERFORMANCE_TARGET_RGB_RENDER_MS
                * 2,  # Deliberately slow
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            performance_stats = measure_rendering_performance(
                slow_renderer, valid_context, num_iterations=3, warm_up_runs=False
            )

            # Assert performance metrics indicate target violations
            assert not performance_stats["target_compliance"].get(
                "rgb_target_met", True
    ...
    def test_resource_cleanup_success(self, mock_renderer):
        ...
        valid_context = create_render_context(
            concentration_field=create_test_concentration_field(
                mock_renderer.grid_size, create_coordinates(TEST_SOURCE_LOCATION)
            ),
            agent_position=create_coordinates(TEST_AGENT_POSITION),
            source_position=create_coordinates(TEST_SOURCE_LOCATION),
    ...
    def test_resource_cleanup_timeout(self):
        ...
        slow_cleanup_renderer = create_mock_renderer(
            grid_size=grid_size,
            performance_overrides={"cleanup_delay_ms": 100},  # 100ms delay
        )
    ...
    def test_supports_render_mode_validation(self, mock_renderer):
        ...
        assert mock_renderer.supports_render_mode(
            RenderMode.RGB_ARRAY
        ), "Should support RGB_ARRAY mode"
        assert mock_renderer.supports_render_mode(
            RenderMode.HUMAN
        ), "Should support HUMAN mode"
        ...
        assert mock_renderer.supports_render_mode(
            RenderMode.RGB_ARRAY
        ), "Should still support RGB_ARRAY"
        assert not mock_renderer.supports_render_mode(
            RenderMode.HUMAN
        ), "Should not support HUMAN mode"
    ...
    def test_error_handling_during_rendering(self, valid_context):
        ...
        assert hasattr(
            error, "render_mode"
        ), "Error should include render mode information"
    ...
    def test_context_cloning_and_overrides(self, valid_context):
        ...
        cloned_context = valid_context.clone_with_overrides(
            agent_position=new_agent_position
        )

        # Assert original context remains unchanged (immutability)
        assert (
            valid_context.agent_position != cloned_context.agent_position
        ), "Original context should be unchanged"
        assert (
            valid_context.agent_position.to_tuple() == TEST_AGENT_POSITION
        ), "Original agent position should be preserved"

        # Verify cloned context has updated agent position
        assert cloned_context.agent_position.to_tuple() == (
            20,
            20,
        ), "Cloned context should have new agent position"
        ...
        assert multi_clone.agent_position.to_tuple() == (
            10,
            15,
        ), "Multi-override should update agent position"
        assert multi_clone.source_position.to_tuple() == (
            25,
            25,
        ), "Multi-override should update source position"
    ...
    def test_rendering_with_different_grid_sizes(self, test_grid_sizes):
        ...
        source_pos = create_coordinates(
            (test_grid_sizes.width // 2, test_grid_sizes.height // 2)
        )
        agent_pos = create_coordinates(
            (test_grid_sizes.width // 4, test_grid_sizes.height // 4)
        )

        concentration_field = create_test_concentration_field(
            grid_size=test_grid_sizes, source_location=source_pos
        )
    ...
    def test_concurrent_rendering_operations(self, valid_context):
        ...
        for i, renderer in enumerate(renderers):
            thread = threading.Thread(
                target=render_worker, args=(renderer, valid_context, i)
            )
        ...
        assert (
            len(errors) == 0
        ), f"No rendering errors should occur in concurrent execution: {errors}"
        ...
        for worker_id, output in results:
            assert_render_output_valid(
                output, RenderMode.RGB_ARRAY, grid_size, check_markers=True
            )
