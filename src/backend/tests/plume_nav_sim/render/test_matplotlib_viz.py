...
TEST_GRID_SIZE = GridSize(
    width=32, height=32
)  # Small grid size for fast test execution
...
def mock_matplotlib_backend():
    ...
    with patch("matplotlib.pyplot.switch_backend") as mock_switch:
        with patch("matplotlib.get_backend") as mock_get_backend:
            ...
            yield {"switch_backend": mock_switch, "get_backend": mock_get_backend}
...
def performance_monitor():
    ...
    """
    performance_data = {"start_times": {}, "durations": {}, "operations": []}
    ...
    def end_timing(operation_name: str) -> float:
        ...
        if operation_name in performance_data["start_times"]:
            duration = (
                time.time() - performance_data["start_times"][operation_name]
            ) * 1000
    ...
    def validate_performance(
        operation_name: str, target_ms: float, tolerance_ms: float = 0.0
    ):
        ...
        if operation_name in performance_data["durations"]:
            ...
            ), f"Operation '{operation_name}' took {actual_duration:.2f}ms, exceeds target {max_allowed:.2f}ms"
            return True
        return False
...
def test_matplotlib_availability():
    ...
    assert (
        major_version > 3 or minor_version >= 9
    ), f"Matplotlib version {matplotlib.__version__} < required 3.9.0"
    ...
    ), "matplotlib.figure.Figure not available"
...
def test_backend_configuration():
    ...
    with patch(
        ...
        side_effect=ImportError("Backend switch failed"),
    ):
...
def test_matplotlib_renderer_initialization(test_color_scheme):
    ...
    renderer = MatplotlibRenderer(
        grid_size=TEST_GRID_SIZE, color_scheme=test_color_scheme
    )
...
def test_matplotlib_renderer_supports_render_mode():
    ...
    if MATPLOTLIB_AVAILABLE:
        assert (
            supports_human == True
        ), "Should support HUMAN mode when matplotlib is available"
    ...
    with patch.object(renderer.backend_manager, "select_backend", return_value=None):
        ...
        assert (
            supports_human_no_backend == False
        ), "Should not support HUMAN mode without available backend"
    ...
    agg_renderer = MatplotlibRenderer(
        grid_size=TEST_GRID_SIZE, backend_preferences=["Agg"]
    )
    assert (
        agg_renderer.supports_render_mode(RenderMode.HUMAN) == True
    ), "Should support HUMAN mode with Agg backend"
...
def test_renderer_resource_initialization():
    ...
    with patch(
        "matplotlib.pyplot.subplots", side_effect=Exception("Figure creation failed")
    ):
...
def test_concentration_field_update():
    ...
    force_result = update_manager.update_concentration_field(
        concentration_field, force_update=True
    )
    ...
    assert (
        vmin >= 0.0 and vmax <= 1.0
    ), f"Colormap limits {vmin}, {vmax} should be in [0,1] range"

    # Test update performance timing and optimization validation
    start_time = time.time()
    for _ in range(10):
        update_manager.update_concentration_field(
            concentration_field, force_update=True
        )
    ...
    assert (
        update_time < 20.0
    ), f"Concentration field update should be <20ms, got {update_time:.2f}ms"
...
def test_agent_marker_update():
    ...
    smooth_result = update_manager.update_agent_marker(
        new_position, color_scheme, animate_transition=True
    )
...
def test_display_refresh_functionality():
    ...
    assert (
        refresh_time < 100.0
    ), f"Display refresh should be <100ms, got {refresh_time:.2f}ms"
...
def test_batch_update_optimization():
    ...
    optimized_update_manager = InteractiveUpdateManager(
        figure=fig, axes=ax, optimization_level="high", change_detection=True
    )

    high_perf_result = optimized_update_manager.batch_update(
...
def test_human_mode_rendering(
    ...
    performance_monitor["validate_performance"](
        "human_render", target_ms, PERFORMANCE_TOLERANCE_MS
    )

    # Test error handling and recovery during rendering operations
    with patch.object(
        renderer._axes, "imshow", side_effect=Exception("Rendering failed")
    ):
        with pytest.raises(RenderingError):
            renderer.render(render_context, RenderMode.HUMAN)
    ...
    assert renderer._figure is None or not hasattr(
        renderer._figure, "canvas"
    ), "Should clean up figure resources"
...
def test_color_scheme_integration(test_render_context):
    ...
    assert all(
        0 <= c <= 255 for c in matplotlib_color
    ...
    assert (
        color_render_time < 100.0
    ), f"Color scheme rendering should be efficient, got {color_render_time:.2f}ms"
...
def test_figure_management():
    ...
    with patch(
        "matplotlib.pyplot.figure", side_effect=Exception("Figure creation failed")
    ):
        error_renderer = MatplotlibRenderer(grid_size=TEST_GRID_SIZE)
        with pytest.raises(RenderingError):
...
def test_figure_saving_functionality():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        png_result = renderer.save_figure(
            png_path, format="png", dpi=150, transparent=False
        )
        ...
        pdf_result = renderer.save_figure(
            ...
            format="pdf",
            metadata={
                "Title": "Plume Navigation Visualization",
                "Creator": "plume_nav_sim",
            },
        )
        ...
        svg_result = renderer.save_figure(svg_path, format="svg", transparent=True)
        ...
        with patch(
            "matplotlib.figure.Figure.savefig", side_effect=Exception("Save failed")
        ):
            with pytest.raises(RenderingError):
...
def test_headless_environment_detection(headless_environment):
    ...
    ), "Should select a backend in headless environment"
    ...
    assert renderer.supports_render_mode(
        RenderMode.HUMAN
    ), "Should support human mode in headless environment"
...
def test_backend_fallback_mechanisms(mock_matplotlib_backend):
    ...
    with patch("matplotlib.pyplot.switch_backend") as mock_switch:
        ...
        with patch(
            ...
            renderer = MatplotlibRenderer(
                grid_size=TEST_GRID_SIZE, backend_preferences=backend_preferences
            )
...
def test_resource_cleanup_comprehensive():
    ...
    assert (
        cleanup_time < MAX_CLEANUP_TIMEOUT_SEC
    ), f"Cleanup should complete within {MAX_CLEANUP_TIMEOUT_SEC}s"
...
def test_error_handling_comprehensive():
    ...
    with patch(
        "matplotlib.axes.Axes.imshow", side_effect=Exception("Matplotlib error")
    ):
        render_context = create_render_context(
            concentration_field=np.random.rand(
                TEST_GRID_SIZE.height, TEST_GRID_SIZE.width
    ...
    for platform in platforms:
        with patch("sys.platform", platform):
            ...
            if platform == "win32":
                ...
                assert (
                    selected_backend is not None
                ), f"Should select backend on {platform}"

                # Full functionality should be available on Linux and macOS
                capabilities = backend_manager.get_backend_capabilities()
                assert isinstance(
                    capabilities, dict
                ), "Should provide capabilities on supported platforms"
            ...
            try:
                ...
                assert isinstance(
                    e, (RenderingError, ValidationError)
...
def test_accessibility_features(test_render_context):
    ...
    contrast_ratio = (max(agent_luminance, bg_luminance) + 0.05) / (
        min(agent_luminance, bg_luminance) + 0.05
    )

    assert (
        contrast_ratio >= 3.0
    ), f"High contrast scheme should have contrast ratio >= 3.0, got {contrast_ratio:.2f}"
    ...
    assert (
        colorblind_scheme.accessibility_enabled == True
    ), "Colorblind scheme should have accessibility enabled"
    assert colorblind_scheme.concentration_colormap in [
        "viridis",
        "plasma",
        "inferno",
    ], "Should use colorblind-friendly colormap"
    ...
    assert colorblind_agent_color != (
        255,
        0,
        0,
    ), "Should not use pure red for colorblind accessibility"
    ...
    assert (
        accessibility_time < PERFORMANCE_TARGET_HUMAN_RENDER_MS * 2
    ), f"Accessibility rendering should be reasonable, got {accessibility_time:.2f}ms"
...
def test_memory_usage_optimization():
    ...
    for grid_size in grid_sizes:
        grid_renderer = MatplotlibRenderer(
            grid_size=grid_size, backend_preferences=["Agg"]
        )
    ...
    optimized_renderer = MatplotlibRenderer(
        grid_size=TEST_GRID_SIZE, backend_preferences=["Agg"], memory_optimization=True
    )
...
def test_capability_detection_function():
    ...
    with patch("os.environ.get", return_value=None):  # No DISPLAY
        ...
        assert (
            headless_capabilities["display_support"] == False
...
def test_matplotlib_integration_validation():
    ...
    if "performance_test" in validation_result:
        ...
        if "render_time_ms" in perf_test:
            ...
            assert (
                target_ratio < 5.0
            ), f"Integration test render time should be reasonable, got {render_time}ms"

    # Test integration validation error handling and reporting
    with patch(
        "matplotlib.pyplot.subplots", side_effect=Exception("Integration test failed")
    ):
...
class TestMatplotlibRenderer:
        ...
        if self.cleanup_required and hasattr(self, "renderer"):
            ...
            try:
                ...
                assert self.renderer._figure is None or not hasattr(
                    self.renderer._figure, "canvas"
                ), "Figure should be cleaned up"
    ...
    def test_renderer_inheritance_compliance(self):
        ...
        ), "Should inherit from BaseRenderer"
        ...
        for method_name in required_methods:
            assert hasattr(
                self.renderer, method_name
            ), f"Should implement {method_name} method"
            assert callable(
                getattr(self.renderer, method_name)
        ...
        ), "Should inherit grid_size from BaseRenderer"
        ...
        for base_method in base_renderer_methods:
            if not base_method.startswith("_") and base_method != "render":
                assert (
                    base_method in renderer_methods
                ), f"Should implement {base_method} from base class"
    ...
    def test_render_mode_support_validation(self):
        ...
        # Mock backend unavailability and verify mode support returns False
        with patch.object(
            self.renderer.backend_manager, "get_current_backend", return_value=None
        ):
            no_backend_support = self.renderer.supports_render_mode(RenderMode.HUMAN)
            assert (
                no_backend_support == False
            ), "Should not support HUMAN mode without backend"
        ...
        assert (
            mode_check_time < 10.0
        ), f"Mode support checking should be fast, got {mode_check_time:.2f}ms"

        # Test mode support with different backend configurations
        assert (
            self.renderer.supports_render_mode(RenderMode.HUMAN) == True
        ), "Should consistently support HUMAN mode"
    ...
    def test_complete_rendering_pipeline(self):
        ...
        concentration_field = np.random.rand(
            self.test_grid_size.height, self.test_grid_size.width
        ).astype(np.float32)
        render_context = create_render_context(
            concentration_field=concentration_field,
            agent_position=TEST_AGENT_POSITION,
        ...
        assert (
            self.renderer._figure is not None
        ), "Should create figure during rendering"
...
class TestMatplotlibBackendManager:
    ...
    def test_backend_priority_selection(self):
        ...
        with patch("matplotlib.pyplot.switch_backend") as _:
            ...
            agg_manager = MatplotlibBackendManager(
                backend_preferences=["NonExistent", "Agg"]
            )
    ...
    def test_backend_capability_analysis(self):
        ...
        with patch("os.environ.get", return_value=None):  # No DISPLAY
            ...
            assert (
                headless_caps["display_available"] == False
            ), "Should detect headless environment"
...
class TestInteractiveUpdateManager:
    ...
    def setup_method(self, method):
        ...
        self.test_figure, self.test_axes = plt.subplots(
        ...
        self.performance_data = {"update_times": [], "operation_counts": {}}

        # Initialize test data for concentration fields and marker positions
        self.test_concentration = np.random.rand(
            TEST_GRID_SIZE.height, TEST_GRID_SIZE.width
        ).astype(np.float32)
        self.update_count = 0
    ...
    def test_concentration_field_updates(self):
        ...
        same_result = self.update_manager.update_concentration_field(
            concentration_field
        )
        ...
        for _ in range(5):
            ...
            self.update_manager.update_concentration_field(
                modified_field, force_update=True
            )
        avg_time = (time.time() - start_time) * 1000 / 5

        assert (
            avg_time < 20.0
        ), f"Concentration updates should be <20ms, got {avg_time:.2f}ms"

        # Test force update functionality and cache bypassing
        force_result = self.update_manager.update_concentration_field(
            concentration_field, force_update=True
        )
    ...
    def test_marker_update_efficiency(self):
        ...
        agent_result = self.update_manager.update_agent_marker(
            TEST_AGENT_POSITION, color_scheme
        )
        ...
        source_result = self.update_manager.update_source_marker(
            TEST_SOURCE_POSITION, color_scheme
        )
        assert source_result == True, "Should successfully update source marker"

        # Test marker position optimization and change detection
        same_agent_result = self.update_manager.update_agent_marker(
            TEST_AGENT_POSITION, color_scheme
        )
