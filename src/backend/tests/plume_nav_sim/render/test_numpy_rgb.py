...
from unittest.mock import Mock, patch, MagicMock, call  # >=3.10 - Mocking utilities for testing error conditions, fallback scenarios, and edge cases
from contextlib import contextmanager  # >=3.10 - Context manager utilities for performance monitoring and resource cleanup testing
...
TEST_CONCENTRATION_VALUES = [0.0, 0.25, 0.5, 0.75, 1.0]  # Range of concentration values for testing
EDGE_CASE_POSITIONS = [(0, 0), (31, 31), (127, 127)]  # Boundary and edge positions for testing
...
def create_test_concentration_field(grid_size: GridSize, pattern_type: str = "gaussian", 
    ...
    if pattern_type == "uniform":
        ...
        sigma = min(grid_size.width, grid_size.height) / 6  # Reasonable sigma for visibility
...
def create_test_render_context(grid_size: Optional[GridSize] = None,
                             ...
                             validate_immediately: bool = True) -> RenderContext:
    """
    ...
    if source_position is None:
        ...
        source_y = min(grid_size.height - 5, max(5, grid_size.height - agent_position.y))
...
def validate_rgb_array_format(rgb_array: np.ndarray, expected_grid_size: GridSize, 
                            strict_validation: bool = False, 
                            check_value_distribution: bool = False) -> Dict[str, Any]:
    """
    ...
    expected_shape = (expected_grid_size.height, expected_grid_size.width, EXPECTED_RGB_SHAPE_CHANNELS)
    if rgb_array.shape == expected_shape:
        validation_results['shape_valid'] = True
    else:
        validation_results['issues'].append(f"Shape mismatch: {rgb_array.shape} vs {expected_shape}")
    ...
    if rgb_array.dtype == RGB_DTYPE:
        ...
        validation_results['issues'].append(f"Dtype mismatch: {rgb_array.dtype} vs {RGB_DTYPE}")
    ...
    if np.all(rgb_array >= PIXEL_VALUE_MIN) and np.all(rgb_array <= PIXEL_VALUE_MAX):
        ...
        validation_results['issues'].append(f"Values outside range [{PIXEL_VALUE_MIN}, {PIXEL_VALUE_MAX}]: [{min_val}, {max_val}]")
    ...
    if rgb_array.ndim == 3 and rgb_array.flags.c_contiguous:
        ...
        validation_results['issues'].append("Array structure invalid or not C-contiguous")
    ...
    if strict_validation:
        ...
        if np.all(rgb_array == 0):
            validation_results['issues'].append("Strict validation: All pixels are black (potential rendering failure)")
        
        # Verify reasonable color distribution
        if len(np.unique(rgb_array)) < 10:
            validation_results['issues'].append("Strict validation: Very limited color palette detected")
    ...
    if validation_results['quality_score'] == 100.0:
        validation_results['recommendations'].append("RGB array passes all format validation tests")
    else:
        validation_results['recommendations'].append("Address format issues before using RGB array")
        if not validation_results['shape_valid']:
            validation_results['recommendations'].append("Fix array shape to match grid dimensions")
...
def validate_marker_presence(rgb_array: np.ndarray, agent_position: Coordinates, 
                           source_position: Coordinates, color_tolerance: int = COLOR_VALIDATION_TOLERANCE,
                           check_marker_shapes: bool = True) -> Dict[str, Any]:
    """
    ...
    if (0 <= agent_position.y < height and 0 <= agent_position.x < width):
        ...
        color_diff = np.abs(actual_agent_color.astype(int) - expected_agent_color.astype(int))
        ...
        for dy in range(-expected_size_h//2, expected_size_h//2 + 1):
            for dx in range(-expected_size_w//2, expected_size_w//2 + 1):
                ...
                if (0 <= y_pos < height and 0 <= x_pos < width):
                    ...
                    color_diff = np.abs(pixel_color.astype(int) - expected_color.astype(int))
    ...
    if (0 <= source_position.y < height and 0 <= source_position.x < width):
        ...
        # Check color match within tolerance
        color_diff = np.abs(actual_source_color.astype(int) - expected_source_color.astype(int))
        if np.all(color_diff <= color_tolerance):
    ...
    marker_distance = ((agent_position.x - source_position.x) ** 2 + 
                      (agent_position.y - source_position.y) ** 2) ** 0.5
    ...
    if marker_distance >= min_safe_distance:
        ...
        marker_validation['issues'].append(f"Markers too close: distance {marker_distance:.1f} < {min_safe_distance}")
    
    # Generate comprehensive validation issues and recommendations
    if not marker_validation['agent_marker_found']:
        marker_validation['issues'].append("Agent marker not found at expected position")
        marker_validation['recommendations'].append("Verify agent marker placement and color")
    
    if not marker_validation['source_marker_found']:
        marker_validation['issues'].append("Source marker not found at expected position")
        marker_validation['recommendations'].append("Verify source marker placement and cross pattern")
    
    if marker_validation['agent_marker_found'] and not marker_validation['agent_size_correct']:
        marker_validation['issues'].append("Agent marker size/shape incorrect")
        marker_validation['recommendations'].append("Fix agent marker 3×3 square pattern")
    
    if marker_validation['source_marker_found'] and not marker_validation['source_size_correct']:
        marker_validation['issues'].append("Source marker cross pattern incorrect")
        marker_validation['recommendations'].append("Fix source marker 5×5 cross pattern")
    
    return marker_validation


def benchmark_rgb_generation_performance(renderer: NumpyRGBRenderer, test_context: RenderContext, 
                                       iterations: int = BENCHMARK_ITERATIONS,
                                       include_memory_profiling: bool = False,
                                       validate_targets: bool = True) -> Dict[str, Any]:
    """
    ...
    for i in range(iterations):
        ...
        try:
            ...
            benchmark_results['error'] = f"Benchmark failed on iteration {i+1}: {str(e)}"
    ...
    if include_memory_profiling:
        ...
        benchmark_results['memory_stats'] = {
            ...
            'within_threshold': abs(final_memory - memory_baseline) <= MEMORY_USAGE_THRESHOLD_MB
        }
    ...
    if validate_targets:
        ...
        benchmark_results['performance_validation'] = {
            ...
            'within_tolerance': mean_time <= (target_time + TEST_PERFORMANCE_TOLERANCE_MS),
            'performance_grade': (
                'excellent' if mean_time <= target_time * 0.8 else
                'good' if mean_time <= target_time else
                'acceptable' if mean_time <= target_time + TEST_PERFORMANCE_TOLERANCE_MS else
                'needs_optimization'
            )
        }
    
    # Calculate performance metrics and optimization recommendations
    performance_variability = np.std(timing_array) / np.mean(timing_array) if np.mean(timing_array) > 0 else 0
    
    benchmark_results['optimization_analysis'] = {
        'performance_consistency': performance_variability < 0.1,  # Low variability is good
        'variability_coefficient': performance_variability,
        'outlier_count': np.sum(timing_array > np.percentile(timing_array, 95)),
        'optimization_potential': max(0, np.mean(timing_array) - target_time) if validate_targets else 0
    }
    
    # Generate performance recommendations
    if validate_targets and not benchmark_results['performance_validation']['meets_target']:
        benchmark_results['recommendations'].append("Performance target not met - consider optimization")
    
    if performance_variability > 0.2:
        benchmark_results['recommendations'].append("High timing variability - check system load")
    
    if include_memory_profiling and benchmark_results['memory_stats']['delta_mb'] > MEMORY_USAGE_THRESHOLD_MB:
        benchmark_results['recommendations'].append("Memory usage increase detected - check for leaks")
    
    return benchmark_results


def create_edge_case_test_data(edge_case_type: str, grid_size: GridSize, 
                             custom_parameters: Optional[Dict[str, Any]] = None) -> Tuple[RenderContext, str, Dict[str, Any]]:
    """
    ...
    if edge_case_type == "boundary_positions":
        ...
        source_pos = create_coordinates(grid_size.width - 1, grid_size.height - 1)  # Bottom-right corner
        ...
        field = np.full((grid_size.height, grid_size.width), 1.0, dtype=FIELD_DTYPE)  # Maximum concentration
        ...
        large_grid = create_grid_size(
            custom_params.get('width', 256), 
            custom_params.get('height', 256)
        )
...
def performance_monitor(operation_name: str = "test_operation"):
    ...
    try:
        ...
        performance_data.update({
            ...
            'within_performance_target': duration_ms <= PERFORMANCE_TARGET_RGB_RENDER_MS
        })
...
def rgb_renderer(request):
    ...
    clear_rgb_cache(clear_array_cache=True, clear_performance_stats=False, force_gc=True)
...
def test_render_context():
    ...
    concentration_field = create_test_concentration_field(grid_size, "gaussian", None, 1.0)
...
def edge_case_test_data():
    ...
    edge_cases = {
        ...
        'large_grid': create_edge_case_test_data('large_grid', grid_size, {'width': 256, 'height': 256}),
...
@pytest.fixture(params=TEST_CONCENTRATION_VALUES, ids=['zero_conc', 'quarter_conc', 'half_conc', 'three_quarter_conc', 'full_conc'])
...
@pytest.fixture(params=EDGE_CASE_POSITIONS, ids=['origin', 'corner_small', 'corner_medium', 'corner_large']) 
...
class TestNumpyRGBRenderer:
    ...
    def test_renderer_initialization(self, test_grid_sizes):
        ...
        assert renderer._array_buffer.shape == (test_grid_sizes.height, test_grid_sizes.width, 3)
    ...
    def test_rgb_array_generation(self, rgb_renderer, test_render_context):
        ...
        assert perf['duration_ms'] <= PERFORMANCE_TARGET_RGB_RENDER_MS + TEST_PERFORMANCE_TOLERANCE_MS
    
    @pytest.mark.performance
    @pytest.mark.slow
    def test_performance_benchmarks(self, rgb_renderer, test_render_context, performance_test_env):
        """
        ...
        assert 'error' not in benchmark_results, f"Benchmark error: {benchmark_results.get('error', 'Unknown')}"
        assert performance_validation['meets_target'] == True, f"Performance target not met: {timing_stats['mean_ms']:.2f}ms > {PERFORMANCE_TARGET_RGB_RENDER_MS}ms"
        
        # Check performance consistency across iterations
        assert timing_stats['std_dev_ms'] < timing_stats['mean_ms'] * 0.3, "High performance variability detected"
        ...
        if 'memory_stats' in benchmark_results:
            ...
            assert memory_stats['within_threshold'] == True, f"Memory usage exceeded threshold: {memory_stats['delta_mb']:.2f}MB"
        
        # Verify optimization analysis results
        optimization_analysis = benchmark_results['optimization_analysis']
        assert optimization_analysis['performance_consistency'] == True, "Performance consistency issues detected"
        
        # Assert performance grade is acceptable
        acceptable_grades = ['excellent', 'good', 'acceptable']
        assert performance_validation['performance_grade'] in acceptable_grades, f"Unacceptable performance grade: {performance_validation['performance_grade']}"
    ...
    def test_color_scheme_integration(self, rgb_renderer, test_render_context):
        ...
        np.testing.assert_array_equal(baseline_rgb, updated_rgb, "Color scheme update should not change output for same scheme")
    
    @pytest.mark.unit
    @pytest.mark.parametrize('position_fixture', EDGE_CASE_POSITIONS, ids=['origin', 'corner_small', 'corner_large'])
    def test_marker_placement_accuracy(self, rgb_renderer, position_fixture):
        ...
        agent_position = create_coordinates(min(x, grid_size.width - 3), min(y, grid_size.height - 3))
        source_position = create_coordinates(
            min(max(5, x + 10), grid_size.width - 6),
            min(max(5, y + 10), grid_size.height - 6)
        )
        
        concentration_field = create_test_concentration_field(grid_size, "uniform", None, 0.3)
        ...
        assert marker_validation['agent_marker_found'] == True, f"Agent marker not found at position ({x}, {y})"
        assert marker_validation['agent_color_match'] == True, f"Agent marker color mismatch at position ({x}, {y})"
        
        # Validate source marker (white 5×5 cross) placement accuracy
        assert marker_validation['source_marker_found'] == True, f"Source marker not found near position ({x}, {y})"
        assert marker_validation['source_color_match'] == True, f"Source marker color mismatch near position ({x}, {y})"
        ...
        if x == 0 or y == 0 or x >= grid_size.width - 3 or y >= grid_size.height - 3:
            ...
            assert format_validation['shape_valid'] == True, "Boundary marker placement caused shape issues"
            assert format_validation['value_range_valid'] == True, "Boundary marker placement caused value range issues"
    
    @pytest.mark.unit
    @pytest.mark.parametrize('concentration', TEST_CONCENTRATION_VALUES, ids=['zero_conc', 'quarter_conc', 'half_conc', 'three_quarter_conc', 'full_conc'])
    def test_concentration_field_visualization(self, rgb_renderer, concentration):
        ...
        grid_size = create_grid_size(64, 64)
        concentration_field = np.full((grid_size.height, grid_size.width), concentration, dtype=FIELD_DTYPE)
        ...
        for y in range(5, 8):  # Top area, away from markers
            for x in range(5, 8):
                if (x, y) != (agent_position.x, agent_position.y) and (x, y) != (source_position.x, source_position.y):
        ...
        for pixel in sample_pixels[:5]:  # Check first 5 sample pixels
            ...
            assert abs(pixel[0] - pixel[1]) <= 2, f"RGB channels not equal for grayscale: {pixel}"
            assert abs(pixel[1] - pixel[2]) <= 2, f"RGB channels not equal for grayscale: {pixel}"
            
            # Check approximate match to expected pixel value (allowing some tolerance)
            avg_pixel_value = np.mean(pixel.astype(float))
            assert abs(avg_pixel_value - expected_pixel_value) <= 10, f"Pixel value {avg_pixel_value} doesn't match expected {expected_pixel_value}"
        ...
        if concentration == 0.0:
            ...
            agent_area = slice(max(0, agent_position.y-1), min(grid_size.height, agent_position.y+2)), \
                        slice(max(0, agent_position.x-1), min(grid_size.width, agent_position.x+2))
            source_area = slice(max(0, source_position.y-2), min(grid_size.height, source_position.y+3)), \
                         slice(max(0, source_position.x-2), min(grid_size.width, source_position.x+3))
            ...
            if len(background_pixels) > 0:
                ...
                assert avg_background <= 20, f"Background not dark enough for zero concentration: {avg_background}"
    ...
    def test_error_handling(self, rgb_renderer):
        ...
        assert any(keyword in error_message.lower() for keyword in ['bounds', 'position', 'grid', 'invalid'])
    
    @pytest.mark.unit
    @pytest.mark.parametrize('grid_size_fixture', TEST_GRID_SIZES, ids=['small_grid', 'medium_grid', 'large_grid'])
    def test_memory_usage(self, grid_size_fixture):
        ...
        assert memory_growth <= MEMORY_USAGE_THRESHOLD_MB, f"Memory growth {memory_growth:.2f}MB exceeds threshold {MEMORY_USAGE_THRESHOLD_MB}MB"
        ...
        assert cleanup_delta <= creation_memory_delta + 2, f"Potential memory leak detected: {cleanup_delta:.2f}MB residual"
        
        # Assert efficient memory usage relative to array size
        expected_array_size_mb = (width * height * 3) / (1024 * 1024)  # RGB array size
        efficiency_ratio = creation_memory_delta / expected_array_size_mb if expected_array_size_mb > 0 else 0
        
        # Memory usage should be reasonable compared to array size (allow for overhead)
        assert efficiency_ratio <= 10, f"Memory efficiency poor: {efficiency_ratio:.2f}x array size"
    
    @pytest.mark.unit
    @pytest.mark.performance
    def test_optimization_features(self, rgb_renderer, test_render_context, performance_test_env):
        """
        ...
        format_validation = validate_rgb_array_format(optimized_array, test_render_context.grid_size)
        assert format_validation['shape_valid'] == True
        assert format_validation['dtype_valid'] == True
        assert format_validation['value_range_valid'] == True
        ...
        difference = np.mean(np.abs(optimized_array.astype(float) - standard_array.astype(float)))
        assert difference <= 5.0, f"Optimization changed output too much: difference {difference}"
        ...
        for _ in range(iterations):
            start_time = time.perf_counter()
            _ = rgb_renderer.generate_optimized_array(test_render_context, reuse_buffer=True, skip_validation=True)
        ...
        assert performance_ratio <= 1.2, f"Optimization made performance worse: {performance_ratio:.2f}x slower"
        
        # Ideally, optimization should be faster
        if performance_ratio < 0.9:
            print(f"Optimization provided {((1 - performance_ratio) * 100):.1f}% performance improvement")
    ...
    def test_integration_with_base_renderer(self, rgb_renderer, test_render_context):
        ...
        assert rgb_renderer.supports_render_mode(RenderMode.HUMAN) == False  # NumpyRGBRenderer is RGB-only
        ...
        assert result.shape == (test_render_context.grid_size.height, test_render_context.grid_size.width, 3)
        ...
        def render_via_base_class(renderer: BaseRenderer, context: RenderContext) -> np.ndarray:
...
class TestRGBUtilityFunctions:
    ...
    def test_generate_rgb_array_fast(self, test_render_context):
        ...
        format_validation = validate_rgb_array_format(fast_rgb, test_render_context.grid_size)
        assert format_validation['shape_valid'] == True
        assert format_validation['dtype_valid'] == True
        assert format_validation['value_range_valid'] == True
        ...
        with pytest.raises(ValidationError):
            ...
            invalid_field = np.ones((10, 20, 3), dtype=FIELD_DTYPE)  # Wrong shape (3D instead of 2D)
            generate_rgb_array_fast(invalid_field, agent_position, source_position, validate_inputs=True)
    ...
    def test_validate_rgb_array_output(self):
        ...
        required_keys = ['format_validation', 'marker_validation', 'quality_metrics', 'performance_indicators']
    ...
    def test_cache_management(self):
        ...
        required_stats_keys = ['global_statistics', 'timing_analysis', 'cache_analysis', 
                              'memory_analysis', 'optimization_suggestions']
        ...
        assert global_stats['performance_baseline_ms'] == PERFORMANCE_TARGET_RGB_RENDER_MS
...
class TestRGBRenderingEdgeCases:
    ...
    def test_boundary_conditions(self, rgb_renderer):
        ...
        for agent_pos, source_pos in boundary_cases:
            concentration_field = create_test_concentration_field(boundary_grid, "uniform", None, 0.4)
            ...
            marker_validation = validate_marker_presence(
                rgb_array, agent_pos, source_pos,
                color_tolerance=COLOR_VALIDATION_TOLERANCE,
            ...
            assert marker_validation['agent_marker_found'] == True, f"Agent marker not visible at boundary {agent_pos.to_tuple()}"
            assert marker_validation['source_marker_found'] == True, f"Source marker not visible at boundary {source_pos.to_tuple()}"
    ...
    def test_extreme_grid_sizes(self):
        ...
        tiny_renderer = create_rgb_renderer(tiny_grid, enable_caching=False)  # Disable caching for small grids
        ...
        assert memory_usage <= 200, f"Large grid memory usage too high: {memory_usage:.2f}MB"
        
        # Performance should still be reasonable (allow more time for large grids)
        large_grid_target = PERFORMANCE_TARGET_RGB_RENDER_MS * 10  # 10x target for 16x larger grid
        assert perf['duration_ms'] <= large_grid_target, f"Large grid rendering too slow: {perf['duration_ms']:.2f}ms"
    ...
    def test_concentration_field_edge_cases(self, rgb_renderer):
        ...
        edge_case_fields = [
            ...
            ("checkerboard", create_test_concentration_field(grid_size, "checkerboard", None, 1.0)),
            ("edge_peaks", create_test_concentration_field(grid_size, "edge_peak", None, 1.0))
        ]
        ...
        for field_name, concentration_field in edge_case_fields:
            ...
            format_validation = validate_rgb_array_format(rgb_array, grid_size)
            assert format_validation['shape_valid'] == True, f"Shape invalid for {field_name} field"
            assert format_validation['dtype_valid'] == True, f"Dtype invalid for {field_name} field"
            assert format_validation['value_range_valid'] == True, f"Value range invalid for {field_name} field"
            
            # Check for NaN or infinite values
            assert np.all(np.isfinite(rgb_array)), f"Non-finite values in {field_name} field rendering"
            
            # Ensure markers are still visible
            marker_validation = validate_marker_presence(
                rgb_array, agent_position, source_position,
                color_tolerance=COLOR_VALIDATION_TOLERANCE * 2  # Allow more tolerance for edge cases
            )
            
            # Markers should be visible even in pathological cases
            assert marker_validation['agent_marker_found'] == True, f"Agent marker not found in {field_name} field"
            assert marker_validation['source_marker_found'] == True, f"Source marker not found in {field_name} field"
            
            # Field-specific validations
            if field_name == "all_zeros":
                # Background should be mostly black
                background_mask = (rgb_array[:, :, 0] < 50) & (rgb_array[:, :, 1] < 50) & (rgb_array[:, :, 2] < 50)
                assert np.sum(background_mask) > (32 * 32 * 0.7), "Background not dark enough for all-zeros field"
            
            elif field_name == "all_ones":
                # Background should be mostly white/bright
                background_mask = (rgb_array[:, :, 0] > 200) & (rgb_array[:, :, 1] > 200) & (rgb_array[:, :, 2] > 200)
                assert np.sum(background_mask) > (32 * 32 * 0.5), "Background not bright enough for all-ones field"
    ...
    def test_resource_constraint_handling(self, rgb_renderer):
        ...
        for i, context in enumerate(contexts):
            ...
            assert format_validation['shape_valid'] == True, f"Rendering {i} failed format validation"
            
            # Check for memory growth patterns
            if start_memory > 0:
                memory_growth = current_memory - start_memory
                assert memory_growth <= MEMORY_USAGE_THRESHOLD_MB * 5, f"Excessive memory growth: {memory_growth:.2f}MB"
        ...
        if start_memory > 0:
            ...
            assert memory_released >= 0, f"Memory not properly released: {memory_released:.2f}MB"
        ...
        recovery_validation = validate_rgb_array_format(recovery_rgb, test_context.grid_size)
        assert recovery_validation['shape_valid'] == True, "Renderer not functional after error recovery"
