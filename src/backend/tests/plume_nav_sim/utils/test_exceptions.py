...
__all__ = [
    ...
    "create_mock_logger",
    "generate_test_exceptions",
    "assert_exception_hierarchy",
    "measure_exception_performance",
    "validate_error_message_security",
]


def create_test_error_context(
    component_name: str,
    operation_name: str,
...
def create_mock_logger(
    logger_name: str = "test_logger", log_level: int = logging.DEBUG
) -> unittest.mock.Mock:
...
def generate_test_exceptions(
    include_context: bool = True,
    include_sensitive_data: bool = False,
    exception_count: int = 5,
) -> List[PlumeNavSimError]:
...
def assert_exception_hierarchy(
    exception_instance: ExceptionType,
    ...
    assert isinstance(
        exception_instance, expected_base_class
    ), f"Exception {exception_instance} does not inherit from {expected_base_class}"

    # Verify exception implements required_methods interface
    for method_name in required_methods:
        assert hasattr(
            exception_instance, method_name
        ), f"Exception {exception_instance} missing required method: {method_name}"
    ...
    if isinstance(exception_instance, PlumeNavSimError):
        assert hasattr(
            exception_instance, "severity"
        ), "PlumeNavSimError should have severity attribute"
        assert hasattr(
            exception_instance, "error_id"
        ), "PlumeNavSimError should have error_id attribute"
        assert hasattr(
            exception_instance, "timestamp"
        ), "PlumeNavSimError should have timestamp attribute"


def measure_exception_performance(
    exception_function: Callable, test_parameters: List[Any], iterations: int = 1000
) -> Dict[str, Any]:
    ...
    performance_analysis = {
        ...
        "meets_error_handling_threshold": mean_time * 1000
        < PERFORMANCE_THRESHOLDS["error_handling_ms"],
        "meets_context_creation_threshold": mean_time * 1000
        < PERFORMANCE_THRESHOLDS["context_creation_ms"],
...
def validate_error_message_security(
    error_message: str, error_context: Dict[str, Any], strict_mode: bool = True
) -> bool:
        ...
        for key, value in error_context.items():
            ...
            for sensitive_key in SENSITIVE_CONTEXT_KEYS:
                if (
                    ...
                    or sensitive_key.lower() in value_str
                ):
...
class TestPlumeNavSimError:
    ...
    def setup_method(self):
        ...
        self.test_context = create_test_error_context(
            "test_component", "test_operation"
        )
        self.base_error = PlumeNavSimError(
            self.test_message, self.test_context, ErrorSeverity.MEDIUM
        )
    ...
    def test_base_error_initialization(self):
        ...
        full_error = PlumeNavSimError(
            "Full error", self.test_context, ErrorSeverity.HIGH
        )
    ...
    def test_get_error_details_method(self):
        ...
        assert (
            details_with_recovery["recovery_suggestion"]
    ...
    def test_format_for_user_method(self):
        ...
        user_message_with_suggestions = self.base_error.format_for_user(
        ...
        user_message_no_suggestions = self.base_error.format_for_user(
        ...
        sensitive_error = PlumeNavSimError(
            "Error with password: test123 and token data"
        )
...
class TestValidationError:
    ...
    def test_validation_error_initialization(self):
        ...
        edge_error = ValidationError(
            "Validation failed", None, {"complex": "value"}, None
        )
    ...
    def test_add_validation_error_method(self):
        ...
        assert (
            ...
            in self.validation_error.recovery_suggestion.lower()
        )
...
class TestStateError:
    ...
    def test_suggest_recovery_action_method(self):
        ...
        env_error = StateError(
            "Environment error", "terminated", "active", "environment"
        )
        ...
        assert (
            "rendering pipeline" in render_recovery.lower()
            or "fallback mode" in render_recovery.lower()
        )

        # Test with various state combinations
        error_state_error = StateError(
            "Component in error state", "error", "ready", "component"
        )
    ...
    def test_add_state_details_method(self):
        ...
        if not state_details["episode_active"]:
            ...
            assert (
                "reset" in recovery_suggestion.lower()
                or "episode" in recovery_suggestion.lower()
            )
        ...
        complex_details = {
            "nested_state": {"sub_component": "active", "sub_details": {"level": 2}},
...
class TestRenderingError:
    ...
    def test_get_fallback_suggestions_method(self):
        ...
        assert (
            len(rgb_fallbacks) >= 0
        )  # May have different fallbacks for rgb_array mode
...
class TestConfigurationError:
    ...
    def test_get_valid_options_method(self):
        ...
        grid_error = ConfigurationError(
            "Invalid grid size", "grid_size", (-1, -1), None
        )
        ...
        source_error = ConfigurationError(
            "Invalid source location", "source_location", (-1, -1), None
        )
        ...
        unknown_error = ConfigurationError(
            "Unknown parameter error", "unknown_param", None, None
        )
        unknown_options = unknown_error.get_valid_options()
        assert isinstance(
            unknown_options, dict
        )  # Should return empty dict or default options
    ...
    def test_add_configuration_context_method(self):
        ...
        if "available_backends" in context:
            ...
            assert (
                self.config_error.valid_options["backends"]
        ...
        if "grid_size" in context:
            recovery = self.config_error.recovery_suggestion
            assert (
                str(context["grid_size"]) in recovery or "grid_size" in recovery.lower()
            )
...
class TestComponentError:
    ...
    def test_diagnose_failure_method(self):
        ...
        self.component_error.set_component_state(
            {"initialized": True, "state": "active"}
        )
    ...
    def test_set_component_state_method(self):
        ...
        assert (
            "Component not properly initialized" in self.component_error.diagnostic_info
        )
...
class TestResourceError:
    ...
    def test_suggest_cleanup_actions_method(self):
        ...
        if self.test_resource_type == "memory":
            memory_actions = [
                ...
                if "memory" in action.lower() or "cache" in action.lower()
            ]
            assert len(memory_actions) > 0
            assert (
                "clear_cache" in cleanup_actions
                or "reduce_grid_size" in cleanup_actions
            )
        ...
        assert any(
            "cleanup" in action.lower()
            and ("temp" in action.lower() or "log" in action.lower())
        ...
        if high_usage_actions:
            assert "immediate_cleanup_required" in high_usage_actions[
                0
            ] or high_usage_actions[0].startswith("clear")
    ...
    def test_set_resource_details_method(self):
        ...
        if (
            "memory_breakdown" in details
            and details["memory_breakdown"]["plume_field"] > 50 * 1024 * 1024
        ):
        ...
        assert isinstance(
            self.resource_error.resource_details["optimization_suggestions"], list
        )
...
class TestIntegrationError:
    ...
    def test_integration_error_initialization(self):
        ...
        assert (
            self.integration_error.version_mismatch is True
    ...
    def test_check_compatibility_method(self):
        ...
        if compatibility_report["version_mismatch"]:
            ...
            assert (
                self.test_required_version.replace(">=", "") in upgrade_rec["command"]
            )
    ...
    def test_set_compatibility_info_method(self):
        ...
        assert (
            self.integration_error.version_mismatch is True
        )  # Should remain True based on version_compatible
        ...
        if "upgrade_path" in info and isinstance(info["upgrade_path"], list):
            ...
            for element in path_elements:
                ...
                assert (
                    "upgrade path" in recovery.lower() or "follow" in recovery.lower()
                )
...
class TestErrorContext:
    ...
    def setup_method(self):
        ...
        self.test_context_data = {
            ...
            "nested_data": {"level1": {"level2": "deep_data"}},
        }
    ...
    def test_add_caller_info_method(self):
        ...
        assert (
            context3.function_name == "<unknown>" or context3.function_name is not None
        )
    ...
    def test_sanitize_method(self):
        ...
        context2.additional_data.update(
            {"custom_secret": "sensitive_value", "normal_data": "normal_value"}
        )
    ...
    def test_to_dict_method(self):
        ...
        assert (
            "function_name" not in minimal_dict or minimal_dict["function_name"] is None
        )
...
class TestErrorHandlingFunctions:
    ...
    def setup_method(self):
        ...
        self.test_error_contexts = {
            "validation": {"parameter_name": "grid_size", "safe_info": "public_data"},
            "sensitive": {
                "password": "secret123",
                "token": "api_token",
    ...
    def test_handle_component_error_function(self):
        ...
        validation_error = ValidationError(
            "Parameter validation failed", "grid_size", (-1, -1)
        )
        result = handle_component_error(
            validation_error, "environment", self.test_error_contexts["validation"]
        )
        ...
        state_error = StateError(
            "Critical state error", "error", "ready", "environment"
        )
        ...
        with pytest.raises(
            TypeError, match="Error parameter must be an Exception instance"
        ):
    ...
    def test_sanitize_error_context_function(self):
        ...
        sanitized_custom = sanitize_error_context(
            context_with_custom, ["custom_secret"]
        )
        ...
        nested_context = {
            "level1": {"password": "nested_secret", "safe": "nested_safe"},
            "normal": "value",
        }
    ...
    def test_format_error_details_function(self):
        ...
        test_error = ComponentError(
            "Component failure", "test_component", "test_operation"
        )
        ...
        details_with_trace = format_error_details(test_error, include_stack_trace=True)
    ...
    def test_log_exception_with_recovery_function(self):
        ...
        # Create exception for testing
        test_exception = StateError(
            "State transition failed", "invalid", "ready", "environment"
        )
        ...
        with pytest.raises(
            TypeError, match="Exception parameter must be an Exception instance"
        ):
            log_exception_with_recovery("not_exception", self.mock_logger)

        with pytest.raises(
            TypeError, match="Logger parameter must be a logging.Logger instance"
        ):
...
class TestExceptionHierarchy:
    ...
    def setup_method(self):
        ...
        self.required_base_methods = [
            "get_error_details",
            "format_for_user",
            "log_error",
            "set_recovery_suggestion",
            "add_context",
        ]
    ...
    def test_common_interface_implementation(self):
        ...
        test_exceptions = generate_test_exceptions(
            include_context=True, exception_count=len(self.exception_classes)
        )
        ...
        for exception in test_exceptions:
            ...
            for method_name in self.required_base_methods:
                ...
                ), f"{exception.__class__.__name__} missing method {method_name}"
    ...
    def test_exception_serialization_consistency(self):
        ...
        for exception in test_exceptions:
            ...
            for field in required_fields:
                assert (
                    field in details
                ), f"Missing required field {field} in {exception.__class__.__name__}"
...
class TestErrorSecurity:
    ...
    def setup_method(self):
        ...
        self.security_contexts = {
            ...
            "nested_sensitive": {
                "user_data": {
                    "credentials": {"password": "nested_secret", "key": "nested_key"},
    ...
    def test_sensitive_information_disclosure_prevention(self):
        ...
        for sensitive_data in self.sensitive_test_data:
            ...
            for term in sensitive_terms:
                if term in sensitive_data.lower():
                    ...
                    assert (
                        "<sanitized>" in user_message
                        or term not in user_message.lower()
                    )
        ...
        context_error.context = create_test_error_context(
            "test_component", "test_operation", include_sensitive_data=True
        )

        user_message = context_error.format_for_user()
        assert validate_error_message_security(
            user_message, context_error.context.to_dict()
        )
        ...
        if "context" in details:
            ...
            for key, value in context_dict.get("additional_data", {}).items():
                if any(
                    ...
                    for sensitive in SENSITIVE_CONTEXT_KEYS
                ):
    ...
    def test_context_sanitization_security(self):
        ...
        sanitized_nested = sanitize_error_context(
            self.security_contexts["nested_sensitive"]
        )
        ...
        sanitized_custom = sanitize_error_context(
            custom_context, ["custom_secret", "internal_data"]
        )
        assert sanitized_custom["custom_secret"] == "<sanitized>"
    ...
    def test_error_message_format_security(self):
        ...
        for message in test_messages:
            ...
            assert validate_error_message_security(
                formatted_message, {}, strict_mode=True
            )
    ...
    def test_logging_security_integration(self):
        ...
        sensitive_context = create_test_error_context(
            "auth_component", "authenticate_user", include_sensitive_data=True
        )
        ...
        assert (
            "secret_api_token" not in comprehensive_log
            or "<sanitized>" in comprehensive_log
        )
...
class TestErrorLogging:
    ...
    def setup_method(self):
        ...
        self.logging_scenarios = [
            (
                ValidationError("Validation failed", "param", "value"),
                ErrorSeverity.MEDIUM,
            ),
            (StateError("State error", "current", "expected"), ErrorSeverity.HIGH),
            (
                PlumeNavSimError(
                    "Critical system error", severity=ErrorSeverity.CRITICAL
                ),
                ErrorSeverity.CRITICAL,
            ),
    ...
    def test_structured_error_logging(self):
        ...
        test_exception = ComponentError(
            "Component integration failed", "plume_model", "calculate_field"
        )

        test_context = create_test_error_context(
            "plume_model", "calculate_field", include_caller_info=True
        )
    ...
    def test_logging_performance_integration(self):
        ...
        log_exception_with_recovery(
            performance_exception, self.mock_logger, include_performance_impact=True
        )
    ...
    def test_logging_configuration_integration(self):
        ...
        test_error = StateError(
            "Configuration test error", "init", "ready", "environment"
        )
        ...
        integration_error = IntegrationError(
            "External system integration failed", "external_service", ">=1.0.0", "0.9.0"
        )
...
class TestErrorPerformance:
    ...
    def setup_method(self):
        ...
        self.performance_scenarios = [
            ...
            (
                "complex_exception",
                lambda: ComponentError(
                    "Complex error", "component", "operation", ValueError("underlying")
                ),
            ),
            (
                "context_creation",
                lambda: create_test_error_context("comp", "op", True, False),
            ),
            (
                "sanitization",
                lambda: sanitize_error_context({"password": "secret", "safe": "data"}),
            ),
        ]
    ...
    def test_exception_creation_performance(self):
        ...
        for exception_class, args in exception_classes:
            ...
            results = measure_exception_performance(
                exception_class, args, self.benchmark_iterations
            )

            # Assert exception creation meets error_handling_ms target
            assert (
                results["mean_ms"] < PERFORMANCE_THRESHOLDS["error_handling_ms"]
            ), f"{exception_class.__name__} creation too slow: {results['mean_ms']:.3f}ms"

            # Check performance consistency
            assert (
                results["std_dev_ms"] < results["mean_ms"]
    ...
    def test_error_context_processing_performance(self):
        ...
        assert (
            context_results["mean_ms"] < PERFORMANCE_THRESHOLDS["context_creation_ms"]
        ...
        sanitization_results = measure_exception_performance(
            sanitize_error_context, [test_context], self.benchmark_iterations
        )

        # Verify sanitization performance meets sanitization_ms target
        assert (
            sanitization_results["mean_ms"] < PERFORMANCE_THRESHOLDS["sanitization_ms"]
        ...
        assert (
            avg_serialization_time < 1.0
        ), f"Context serialization too slow: {avg_serialization_time:.3f}ms"
    ...
    def test_error_handling_function_performance(self):
        ...
        assert (
            handle_results["mean_ms"] < PERFORMANCE_THRESHOLDS["error_handling_ms"]
        ...
        def log_test():
            try:
                log_exception_with_recovery(
                    test_error, mock_logger, context={"test": "context"}
                )
            except Exception:
    ...
    def test_memory_usage_optimization(self):
        ...
        for i in range(1000):
            error = ComponentError(
                f"Memory test error {i}", f"component_{i}", f"operation_{i}"
            )
        ...
        cleanup_stats = after_cleanup_snapshot.compare_to(
            after_creation_snapshot, "lineno"
        )

        # Verify memory usage is reasonable (should be < 50MB for 1000 exceptions)
        peak_memory = sum(
            stat.size for stat in creation_stats[:10]
        )  # Top 10 allocations
        assert (
        ...
        try:
            ...
            large_error.context = create_test_error_context(
                "memory_test", "pressure_test"
            )
...
class TestErrorIntegration:
    ...
    def setup_method(self):
        ...
        self.integration_scenarios = {
            "component_cascade": [
                ComponentError(
                    "Primary component failed", "plume_model", "calculate_field"
                ),
                RenderingError("Secondary rendering failed", "human", "matplotlib"),
                StateError(
                    "State corruption detected", "error", "ready", "environment"
                ),
            ],
            "validation_chain": [
                ValidationError("Grid size invalid", "grid_size", (-1, -1)),
                ConfigurationError(
                    "Configuration incompatible", "render_mode", "invalid"
                ),
                IntegrationError(
                    "Dependency version mismatch", "numpy", ">=2.1.0", "1.24.0"
                ),
            ],
        }
    ...
    def test_component_error_integration(self):
        ...
        primary_error = ComponentError(
            "Plume model calculation failed", "plume_model", "sample_concentration"
        )
        primary_context = create_test_error_context(
    ...
    def test_validation_error_integration(self):
        ...
        grid_error = ValidationError(
            "Grid size must be positive", "grid_size", (-10, -10)
        )
        grid_error.set_parameter_constraints(
            {"min_width": 1, "min_height": 1, "max_width": 1000, "max_height": 1000}
        )

        source_error = ValidationError(
            "Source location outside grid", "source_location", (-1, -1)
        )
        source_error.set_parameter_constraints(
            {"x_range": (0, 128), "y_range": (0, 128)}
        )

        # Test compound validation error handling
        compound_validation = ValidationError(
            "Multiple parameter validation failures", None, None
        )
        compound_validation.add_validation_error(
            "Invalid grid_size: must be positive", "grid_size"
        )
        compound_validation.add_validation_error(
            "Invalid source_location: outside bounds", "source_location"
        )
    ...
    def test_logging_monitoring_integration(self):
        ...
        integration_error = IntegrationError(
            "Critical dependency failure", "gymnasium", ">=0.29.0", "0.28.0"
        )
        ...
        log_exception_with_recovery(
            integration_error, metrics_logger, context=error_metrics
        )
    ...
    def test_end_to_end_error_workflows(self):
        ...
        workflow_scenario = [
            ...
            ConfigurationError(
                "Invalid environment configuration", "grid_size", "invalid"
            ),
            ...
            ComponentError(
                "Environment initialization failed", "environment", "initialize"
            ),
            # 4. Recovery attempt error
            StateError(
                "Recovery attempt failed", "error", "initializing", "environment"
            ),
            # 5. Final fallback error
            ResourceError(
                "Insufficient resources for fallback", "memory", 200.0, 100.0
            ),
        ]
        ...
        for i, error in enumerate(workflow_scenario):
            ...
            recovery_strategy = handle_component_error(
                error, component_name, workflow_context
            )
        ...
        assert (
            workflow_logger.error.call_count + workflow_logger.warning.call_count
            >= len(workflow_scenario)
        )
...
def create_performance_test_suite() -> Dict[str, Callable]:
    ...
    return {
        "exception_creation": lambda: measure_exception_performance(
            PlumeNavSimError, ["test"], 1000
        ),
        "context_processing": lambda: measure_exception_performance(
            create_error_context, ["op"], 1000
        ),
        "sanitization": lambda: measure_exception_performance(
            sanitize_error_context, [{"test": "data"}], 1000
        ),
        "error_handling": lambda: measure_exception_performance(
            handle_component_error, [PlumeNavSimError("test"), "comp"], 500
        ),
    }
