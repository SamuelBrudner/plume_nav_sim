...
from unittest.mock import Mock, patch, MagicMock  # standard library - Mock objects for testing error conditions and external dependencies
from typing import Any, Dict, List, Tuple, Union  # >=3.10 - Type hints for test function parameters and return values
...
from plume_nav_sim.utils.spaces import (
    create_action_space, create_observation_space, validate_action, validate_observation,
    validate_action_space, validate_observation_space, SpaceConfig, SpaceValidator,
    get_space_info, sample_valid_action, sample_valid_observation, check_space_compatibility,
...
from plume_nav_sim.core.constants import (
    ACTION_SPACE_SIZE, CONCENTRATION_RANGE, OBSERVATION_DTYPE
)
...
def test_create_action_space_default():
    ...
    assert action_space.n == ACTION_SPACE_SIZE, \
    ...
    for _ in range(10):
        ...
        assert 0 <= sample < ACTION_SPACE_SIZE, \
    ...
    assert "Discrete" in space_str, f"String representation should mention Discrete: {space_str}"
...
def test_create_action_space_with_parameters():
    ...
    test_spaces = [custom_action_space, validated_space, config_space, unvalidated_space]
    ...
    assert default_space.n == custom_action_space.n, \
    ...
    assert cached_space1.n == cached_space2.n, "Cached spaces should have consistent properties"
...
def test_create_observation_space_default():
    ...
    assert obs_space.dtype == OBSERVATION_DTYPE, \
    ...
    assert np.allclose(obs_space.low, expected_low), \
    ...
    assert np.allclose(obs_space.high, expected_high), \
    ...
    assert obs_space.contains(valid_obs), \
    ...
    for _ in range(10):
        ...
        assert CONCENTRATION_RANGE[0] <= sample.min() <= CONCENTRATION_RANGE[1], \
            f"Sample values should be in concentration range"
        assert CONCENTRATION_RANGE[0] <= sample.max() <= CONCENTRATION_RANGE[1], \
            f"Sample values should be in concentration range"
...
def test_create_observation_space_with_parameters():
    ...
    assert shaped_space.shape == custom_shape, \
    ...
    assert np.allclose(bounded_space.low, custom_bounds[0]), \
        f"Custom low bound not applied: expected {custom_bounds[0]}, got {bounded_space.low}"
    assert np.allclose(bounded_space.high, custom_bounds[1]), \
    ...
    assert dtype_space.dtype == custom_dtype, \
    ...
    assert isinstance(validated_space, gymnasium.spaces.Box), \
        "Validation should not affect space type"
    ...
    test_spaces = [shaped_space, bounded_space, dtype_space, validated_space, config_space]
    for space in test_spaces:
    ...
    assert default_space.shape == shaped_space.shape, \
...
def test_validate_action_valid_inputs(action):
    ...
    assert isinstance(validated_action, int), \
        f"Expected int, got {type(validated_action)}"
    assert 0 <= validated_action <= 3, \
    ...
    if isinstance(action, Action):
        assert validated_action == action.value, \
    ...
    # Test integer values (0, 1, 2, 3) for cardinal directions
        ...
        assert validated_action == int(action), \
    ...
    assert logged_result == validated_action, \
...
def test_validate_action_invalid_inputs(invalid_action):
    ...
    assert "Action" in error_message or "action" in error_message, \
    ...
    if isinstance(invalid_action, (int, float)) and not np.isnan(invalid_action):
        try:
            ...
            if int_val < 0 or int_val >= ACTION_SPACE_SIZE:
                assert "range" in error_message.lower() or "bound" in error_message.lower(), \
    ...
    if not isinstance(invalid_action, (int, float, Action)):
        assert "type" in error_message.lower() or "convert" in error_message.lower(), \
...
def test_validate_observation_valid_inputs():
    ...
    assert np.array_equal(range_validated, valid_obs), \
    ...
    assert np.array_equal(no_range_validated, valid_obs), \
    ...
    assert np.array_equal(strict_validated, normal_validated), \
    ...
    assert np.array_equal(logged_validated, valid_obs), \
        "Logging should not affect validation result"
...
def test_validate_observation_invalid_inputs():
    ...
    for invalid_dtype in invalid_dtypes:
        ...
        try:
            with warnings.catch_warnings():
                ...
                assert validated.dtype == OBSERVATION_DTYPE, \
    ...
    with pytest.raises(ValidationError):
        validate_observation(np.array([2.0], dtype=OBSERVATION_DTYPE), 
                           check_range=True, strict_mode=True)
...
def test_validate_action_space_valid():
    ...
    assert valid_space.contains(sample), \
        "Space sample should be contained in space"
    assert 0 <= sample < valid_space.n, \
        f"Sample {sample} should be in valid range"
    
    # Test navigation compatibility with Action enum values
    for action_enum in Action:
        if action_enum.value < valid_space.n:
            assert valid_space.contains(action_enum.value), \
    ...
    assert valid_space.n == ACTION_SPACE_SIZE, \
...
def test_validate_observation_space_valid():
    ...
    assert valid_space.contains(sample), \
        "Space sample should be contained in space"
    assert sample.shape == valid_space.shape, \
    ...
    for conc_val in TEST_CONCENTRATION_VALUES:
        ...
        assert valid_space.contains(test_obs), \
...
def test_validate_observation_space_invalid():
    ...
    assert "Box" in str(exc_info.value), \
        "Error should mention expected Box space type"
    ...
    with pytest.raises(ValidationError):
        validate_observation_space(wrong_bounds_space, check_bounds=True, strict_validation=True)
    ...
    with pytest.raises(ValidationError):
        validate_observation_space(wrong_dtype_space, check_dtype=True, strict_validation=True)
...
class TestSpaceConfig:
    ...
    def test_config_defaults(self):
        ...
        assert config.concentration_bounds == CONCENTRATION_RANGE, \
        ...
        assert config.observation_dtype == OBSERVATION_DTYPE, \
...
def test_space_config_space_creation():
    ...
    assert action_space.n == config.action_space_size, \
    ...
    assert isinstance(obs_space, gymnasium.spaces.Box), \
        "Created space should be Box"
    assert obs_space.shape == obs_config.observation_shape, \
        "Observation space shape should match configuration"
    assert obs_space.dtype == obs_config.observation_dtype, \
    ...
    assert direct_action_space.n == config_action_space.n, \
...
def test_space_config_serialization():
    ...
    if config.metadata:
        assert 'metadata' in dict_with_metadata, \
    ...
    if config.optimization_settings:
        assert 'optimization_settings' in dict_with_settings, \
            "Dictionary should include optimization settings when requested"
    
    # Verify dictionary contains all configuration parameters
    expected_keys = [
        'action_space_size', 'observation_shape', 'concentration_bounds',
        'observation_dtype', 'enable_validation', 'enable_caching'
    ]
...
class TestSpaceValidator:
    ...
    def test_validator_caching_enabled(self):
        ...
        assert stats['cache_performance']['cache_hits'] > 0, \
            "Should have cache hits after repeated validations"
        
        # Test cache memory usage stays within limits
        cache_size = len(validator.validation_cache) if validator.validation_cache else 0
    ...
    def test_validator_strict_mode(self):
        ...
        assert strict_result['valid'] is True, "Valid space should pass strict validation"
        assert normal_result['valid'] is True, "Valid space should pass normal validation"
...
def test_space_validator_initialization():
    ...
    assert default_validator.strict_mode is False, "Default strict mode should be disabled"
...
def test_space_validator_action_space():
    ...
    nav_result = validator.validate_action_space(
        valid_space, 
        check_navigation_compatibility=True
    )
    ...
    if validator.enable_caching:
        assert stats_after['cache_performance']['cache_hits'] > stats_before['cache_performance']['cache_hits'], \
...
def test_space_validator_observation_space():
    ...
    conc_result = validator.validate_observation_space(
        valid_space,
        check_concentration_compatibility=True
    )
...
def test_space_validator_caching():
    ...
    if cache_stats:
        assert cache_stats['cache_hits'] > 0, "Should have cache hits after repeated calls"
        assert cache_stats['total_requests'] >= cache_stats['cache_hits'], \
    ...
    assert final_stats['validation_counts']['total'] >= CACHE_TEST_SIZE, \
...
def test_space_validator_statistics():
    ...
    assert counts['total'] == counts['successful'] + counts['failed'], \
...
def test_get_space_info():
    ...
    assert action_info['n'] == action_space.n, "Should accurately report space properties"
...
def test_sample_valid_action():
    ...
    assert 0 <= custom_action < 8, f"Custom space sample {custom_action} should be in range [0, 7]"
    
    # Test excluded_actions parameter filters out specified actions
    excluded = [0, 1]
    for _ in range(10):
        action = sample_valid_action(excluded_actions=excluded)
        assert action not in excluded, f"Sample {action} should not be in excluded list {excluded}"
    ...
    action_space = create_action_space()
    for sample in samples:
        assert action_space.contains(sample), f"Sample {sample} should be valid for action space"
...
def test_sample_valid_observation():
    ...
    for _ in range(10):
        ...
        assert isinstance(obs, np.ndarray), f"Sample should be numpy array, got {type(obs)}"
        assert obs.shape == (1,), f"Sample shape should be (1,), got {obs.shape}"
        assert obs.dtype == OBSERVATION_DTYPE, f"Sample dtype should be {OBSERVATION_DTYPE}, got {obs.dtype}"
    
    # Test sampling with custom observation_space parameter
    custom_bounds = (0.2, 0.8)
    custom_space = gymnasium.spaces.Box(
        low=custom_bounds[0], high=custom_bounds[1], 
        shape=(1,), dtype=OBSERVATION_DTYPE
    )
    custom_obs = sample_valid_observation(observation_space=custom_space)
    assert custom_space.contains(custom_obs), "Custom space sample should be contained in space"
    ...
    assert 0.3 <= constrained_obs[0] <= 0.7, \
    ...
    assert isinstance(validated_obs, np.ndarray), "Validated sample should be numpy array"
    ...
    for _ in range(20):
        obs = sample_valid_observation()
        assert CONCENTRATION_RANGE[0] <= obs[0] <= CONCENTRATION_RANGE[1], \
    ...
    obs_space = create_observation_space()
    for sample in samples:
        assert obs_space.contains(sample), f"Sample should be valid for observation space"
    ...
    sampling_time = (time.time() - start_time) * 1000
    assert sampling_time < 200, f"Observation sampling too slow: {sampling_time}ms for 100 samples"
...
def test_check_space_compatibility():
    ...
    assert isinstance(compat_report, dict), "Should return compatibility report dictionary"
    ...
    if not incompatible_report['compatible']:
        assert len(incompatible_report['issues']) > 0, \
            "Incompatible spaces should have issues"
    
    # Verify compatibility report includes optimization recommendations
    assert isinstance(compat_report['recommendations'], list), \
        "Recommendations should be list"
    assert len(compat_report['recommendations']) > 0, \
    ...
    assert compat_time < 500, f"Compatibility checking too slow: {compat_time}ms for 10 checks"
...
def test_optimize_space_operations():
    ...
    assert 'optimization_applied' in optimization_result, "Should list applied optimizations"
    assert 'performance_improvements' in optimization_result, "Should report improvements"
    ...
    cached_result = optimize_space_operations(
        default_config, enable_caching=True
    )
    assert 'caching' in str(cached_result['optimization_applied']).lower() or \
           len(cached_result['optimization_applied']) > 0, \
           "Caching optimization should be reported"
    
    # Test profile_operations=True provides performance analysis
    profiled_result = optimize_space_operations(
        default_config, profile_operations=True
    )
...
def test_performance_benchmarks():
    ...
    assert action_validation_time < 1.0, \
    ...
    assert obs_validation_time < 1.0, \
    ...
    assert max_observed_time <= PERFORMANCE_TOLERANCE_MS, \
...
def test_error_handling_comprehensive():
    ...
    for invalid_input in invalid_inputs:
        ...
        error_message = str(exc_info.value)
        assert len(error_message) > 0, "Error message should not be empty"
        assert not any(sensitive in error_message.lower() 
    ...
    try:
        ...
        assert any(hint in error_message.lower() 
    ...
    assert action_error_pattern == obs_error_pattern, \
...
def test_edge_cases():
    ...
    for boundary_val in boundary_values:
        ...
        assert np.allclose(validated, [boundary_val]), \
            f"Boundary value {boundary_val} should be valid"
        assert obs_space.contains(boundary_obs), \
...
def test_integration_with_gymnasium():
    ...
    for _ in range(10):
        ...
        assert mock_env.action_space.contains(action_sample), \
            "Action sample should be contained in action space"
        
        obs_sample = mock_env.observation_space.sample()
        assert mock_env.observation_space.contains(obs_sample), \
