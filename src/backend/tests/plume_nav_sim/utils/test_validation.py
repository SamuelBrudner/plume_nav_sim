...
class TestValidationFunctions:
    ...
    def test_validate_performance_constraints_valid_constraints(self):
        ...
        valid_constraints = [
            ...
            {
                "max_memory_mb": MEMORY_TEST_LIMIT_MB,
                "max_latency_ms": PERFORMANCE_TARGET_STEP_LATENCY_MS,
            },
        ]
...
class TestSecurityFeatures:
    ...
    def test_sanitize_parameters_sensitive_data_protection(self):
        ...
        for key, value in result.items():
            ...
            if any(
                sensitive in key.lower() for sensitive in ["password", "key", "secret"]
            ):
                assert (
                    str(value) != sensitive_data[key]
...
class TestValidationContextClass:
    ...
    def test_validation_context_merge_context(self):
        ...
        assert (
            context1.additional_context["shared_key"] == "merged"
        )  # Secondary context takes precedence
    ...
    def test_validation_context_thread_safety(self):
        ...
        def worker_thread(thread_id):
            try:
                ...
                context.additional_context[f"thread_{thread_id}_data"] = (
                    f"value_{thread_id}"
                )
...
class TestParameterValidatorClass:
    ...
    def test_parameter_validator_batch_validate(self):
        ...
        parameters = {
            ...
            "grid_size": {
                "value": (64, 64),
                "rules": {"type": "tuple", "min_dims": (16, 16)},
            },
    ...
    def test_parameter_validator_add_validation_rule(self):
        ...
        result = validator.validate_parameter(
            "test_coords", (10, 20), {"type": "coordinates"}
        )
        assert result.is_valid is True

        # Test custom rule failure
        result = validator.validate_parameter(
            "test_coords", (-1, 5), {"type": "coordinates"}
        )
        assert result.is_valid is False
    ...
    def test_parameter_validator_get_validation_stats(self):
        ...
        for i in range(10):
            validator.validate_parameter(
                "action", i % 4, {"type": "int", "range": [0, 3]}
            )
            validator.validate_parameter(
                "seed", i * 100, {"type": "int", "range": [0, 999999]}
            )
    ...
    def test_parameter_validator_caching_performance(self):
        ...
        start_time = time.time()
        result1 = validator.validate_parameter(
            "action", 2, {"type": "int", "range": [0, 3]}
        )
        first_validation_time = time.time() - start_time

        # Perform repeated validation (cache hit)
        start_time = time.time()
        result2 = validator.validate_parameter(
            "action", 2, {"type": "int", "range": [0, 3]}
        )
        ...
        assert (
            cached_validation_time <= first_validation_time
...
class TestEnvironmentConfigValidation:
    ...
    def test_validate_environment_config_complete_valid_config(self):
        ...
        valid_config = {
            "grid_size": (128, 128),
            "source_location": (64, 64),
            "plume_parameters": {"sigma": 12.0, "intensity": 1.0},
            "max_steps": 1000,
    ...
    def test_validate_environment_config_minimal_valid_config(self):
        ...
        minimal_config = {"grid_size": (64, 64), "source_location": (32, 32)}
    ...
    def test_validate_environment_config_invalid_configurations(self):
        ...
        for invalid_config in invalid_configs:
            ...
            error = exc_info.value
            assert (
                "config" in str(error.message).lower()
                or "parameter" in str(error.message).lower()
            )
    ...
    def test_validate_environment_config_cross_parameter_consistency(self):
        ...
        for scenario in CONSISTENCY_TEST_SCENARIOS:
            ...
            config = {
                ...
                "source_location": scenario.get(
                    "source_location",
                    (scenario["grid_size"][0] // 2, scenario["grid_size"][1] // 2),
                ),
                "plume_parameters": {"sigma": scenario["sigma"], "intensity": 1.0},
            }
    ...
    def test_validate_environment_config_resource_constraints(self):
        ...
        large_config = {
            ...
            "plume_parameters": {"sigma": 50.0, "intensity": 1.0},
        }
    ...
    def test_validate_environment_config_edge_cases(self):
        ...
        edge_cases = [
            ...
            # Minimum sigma
            {
                "grid_size": (64, 64),
                "source_location": (32, 32),
                "plume_parameters": {"sigma": MIN_PLUME_SIGMA},
            },
            # Maximum sigma
            {
                "grid_size": (64, 64),
                "source_location": (32, 32),
                "plume_parameters": {"sigma": MAX_PLUME_SIGMA},
            },
        ]
...
class TestConsistencyValidation:
    ...
    def test_check_parameter_consistency_performance_feasibility(self):
        ...
        for test_case in performance_test_cases:
            ...
            if test_case.get("expected_fps", 0) > 100:  # Unrealistic FPS
                assert result.get("consistent", True) is False
                assert any(
                    "performance" in violation.lower()
    ...
    def test_check_parameter_consistency_mathematical_relationships(self):
        ...
        for test_case in math_test_cases:
            ...
            grid_diagonal = np.sqrt(
                test_case["grid_size"][0] ** 2 + test_case["grid_size"][1] ** 2
            )
    ...
    def test_check_parameter_consistency_integration_compatibility(self):
        ...
        for test_case in integration_test_cases:
            ...
            if test_case.get("action_space_size") == 4 and test_case.get(
...
class TestPerformanceValidation:
    ...
    def test_validation_performance_latency(self):
        ...
        for func, args in validation_functions:
            ...
            assert (
                avg_time < VALIDATION_PERFORMANCE_TOLERANCE_MS
            ), f"{func_name} avg time {avg_time:.3f}ms exceeds {VALIDATION_PERFORMANCE_TOLERANCE_MS}ms"
            assert (
                max_time < VALIDATION_PERFORMANCE_TOLERANCE_MS * 5
            ), f"{func_name} max time {max_time:.3f}ms exceeds threshold"
    ...
    def test_batch_validation_performance(self):
        ...
        assert (
            batch_time_ms < BATCH_VALIDATION_SIZE
        )  # Should be less than 1ms per parameter
    ...
    def test_validation_memory_usage(self):
        ...
        assert (
            post_clear_objects <= final_objects
        ), "Cache clearing should not increase object count"
    ...
    def test_validation_optimization_effectiveness(self):
        ...
        assert (
            stats["cache_hits"] > 0
        ), "Caching should provide cache hits for repeated validations"
        ...
        if cache_hit_ratio > 0.5:  # If we got good cache hits
            assert (
                optimized_time <= unoptimized_time * 1.1
            ), "Optimization should not significantly degrade performance"
    ...
    def test_concurrent_validation_performance(self):
        ...
        for worker_id in range(THREAD_SAFETY_TEST_THREADS):
            thread = threading.Thread(
                target=validation_worker, args=(worker_id, iterations_per_worker)
            )
            threads.append(thread)
            thread.start()
        ...
        assert (
            overall_avg_time < VALIDATION_PERFORMANCE_TOLERANCE_MS * 2
        ), f"Concurrent validation too slow: {overall_avg_time:.3f}ms"

        # Throughput should be reasonable
        throughput = total_validations / total_time
        assert (
            throughput > 100
        ), f"Validation throughput too low: {throughput:.1f} validations/second"
...
class TestUtilityFunctions:
    ...
    def test_optimize_validation_performance_recommendations(self):
        ...
        performance_data = {
            "validation_times": [
                ...
                2.1,
                0.8,
            ],  # Various validation times in ms
            "cache_hit_rate": 0.65,
    ...
    def test_optimization_strategies_effectiveness(self):
        ...
        if any(
            "cache" in rec.lower()
            for rec in optimize_recommendations.get("recommendations", [])
        ):
            ...
            for param_type, value in common_validations:
                rules = {
                    "type": "int",
                    "range": [0, 3] if param_type == "action" else [0, 999999],
                }
...
class TestIntegrationWithCoreTypes:
    ...
    def test_coordinates_dataclass_integration(self):
        ...
        for coord_input in coordinate_inputs:
            ...
            result = validate_coordinates(
                coord_input, (grid_size.width, grid_size.height), context=context
            )
    ...
    def test_plume_parameters_dataclass_integration(self):
        ...
        plume_input = {"source_location": (64, 64), "sigma": 12.0, "intensity": 1.0}
    ...
    def test_environment_config_dataclass_integration(self):
        ...
        complete_config = {
            "grid_size": (128, 128),
            "source_location": (64, 64),
            "plume_parameters": {"sigma": 15.0, "intensity": 0.8},
            "max_steps": 1000,
            "render_mode": "rgb_array",
            "seed": 42,
        }
    ...
    def test_type_conversion_consistency(self):
        ...
        for test_case in test_cases:
            ...
            for test_input in test_case["inputs"]:
                ...
                ), f"Expected {expected_type}, got {type(result)} for input {test_input}"
    ...
    def test_cross_type_validation_consistency(self):
        ...
        for coords in valid_coordinates:
            ...
            validated_coords = validate_coordinates(
                coords, grid_size, context=context_coord
            )
...
def performance_test_data():
    ...
    return {
        "actions": VALID_TEST_ACTIONS
        * (PERFORMANCE_TEST_ITERATIONS // len(VALID_TEST_ACTIONS)),
        "coordinates": [
            (i % 64, (i * 2) % 64) for i in range(PERFORMANCE_TEST_ITERATIONS)
        ],
