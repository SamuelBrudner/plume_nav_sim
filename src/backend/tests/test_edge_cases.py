...
INVALID_ACTION_TEST_VALUES = [-1, 4, 5, 100, -999, None, 'invalid', 3.14, float('inf'), float('nan')]
EXTREME_GRID_SIZES = [(1, 1), (0, 0), (-1, -1), (1000, 1000), (sys.maxsize, 1), (1, sys.maxsize)]
BOUNDARY_COORDINATE_CASES = [(-1, -1), (-1, 0), (0, -1), (128, 128), (127, 128), (128, 127), (-999, -999), (999, 999)]
EXTREME_PLUME_PARAMETERS = [0.0, 0.001, 0.1, 1000.0, float('inf'), float('nan'), -1.0, -999.0]
    ...
    def create_extreme_environment(self, edge_case_type: str, custom_params: dict = None) -> PlumeSearchEnv:
        ...
        if edge_case_type == "boundary_violation":
            base_config = {
                ...
                'source_location': (0, 0),  # Corner source for maximum boundary pressure
                'goal_radius': 0,
                'max_steps': 10
            }
            ...
            base_config = {
                'grid_size': (32, 32),
                'source_location': (31, 31),  # Source at boundary for transition edge cases
                'goal_radius': 0,
                'max_steps': 1000
            }
        else:
        ...
        if hasattr(self.performance_tracker, 'start_monitoring'):
            self.performance_tracker.start_monitoring(f"{edge_case_type}_{len(self.test_environments)}")
        
        # Return configured environment ready for edge case testing
        return env
    
    def simulate_error_scenario(self, error_type: str, error_context: dict) -> contextlib.AbstractContextManager:
        """
    ...
    def validate_edge_case_recovery(self, environment: PlumeSearchEnv) -> dict:
        ...
        try:
            if hasattr(environment, 'validate_environment_integrity'):
                recovery_report['environment_state_consistent'] = environment.validate_environment_integrity()
            else:
                # Basic state consistency checks
                recovery_report['environment_state_consistent'] = (
                    hasattr(environment, 'action_space') and
                    hasattr(environment, 'observation_space') and
                    hasattr(environment, '_episode_active')
                )
        except Exception as e:
        ...
        try:
            ...
            if hasattr(self.memory_monitor, 'get_current_usage'):
                recovery_report['memory_status'] = self.memory_monitor.get_current_usage()
        ...
        if hasattr(self.performance_tracker, 'get_metrics'):
            recovery_report['performance_impact'] = self.performance_tracker.get_metrics()
    ...
    def cleanup_edge_case_resources(self) -> dict:
        ...
        for env in self.test_environments:
            try:
                ...
                cleanup_report['cleanup_errors'].append(f"Environment cleanup error: {e}")
        ...
        try:
            ...
            cleanup_report['cleanup_errors'].append(f"Performance tracking cleanup error: {e}")
...
def test_boundary_position_edge_cases(edge_case_test_env, boundary_enforcer=None):
    ...
    corners = [
        ...
        (edge_case_test_env.grid_size[0] - 1, edge_case_test_env.grid_size[1] - 1)  # Top-right corner
    ]
    ...
    for corner_x, corner_y in corners:
        ...
        assert position_valid.is_valid, f"Corner position {corner_coords.to_tuple()} should be valid"
        ...
        assert len(corner_actions) >= 2, f"Corner should have at least 2 valid moves, got {len(corner_actions)}"
        ...
        for action in [Action.UP, Action.RIGHT, Action.DOWN, Action.LEFT]:
            ...
            assert result.final_position.x < edge_case_test_env.grid_size[0], "Position X should be within grid width"
            assert result.final_position.y < edge_case_test_env.grid_size[1], "Position Y should be within grid height"
    
    # Test agent placement at grid edges and validate edge movement constraints
    edge_positions = [
        (0, edge_case_test_env.grid_size[1] // 2),  # Left edge
        (edge_case_test_env.grid_size[0] - 1, edge_case_test_env.grid_size[1] // 2),  # Right edge
        (edge_case_test_env.grid_size[0] // 2, 0),  # Bottom edge
        (edge_case_test_env.grid_size[0] // 2, edge_case_test_env.grid_size[1] - 1)  # Top edge
    ]
    ...
    for edge_x, edge_y in edge_positions:
        ...
        assert edge_validation.is_valid, f"Edge position {edge_coords.to_tuple()} should be valid"
        
        # Test edge movement constraints
        valid_moves = boundary_enforcer.get_valid_moves(edge_coords)
        assert 2 <= len(valid_moves) <= 3, f"Edge position should have 2-3 valid moves, got {len(valid_moves)}"
    ...
    for extreme_x, extreme_y in extreme_positions:
        ...
        if extreme_x < 0 or extreme_y < 0 or extreme_x >= edge_case_test_env.grid_size[0] or extreme_y >= edge_case_test_env.grid_size[1]:
            # Should be invalid for out-of-bounds positions
            assert not validation_result.is_valid, f"Extreme position {extreme_coords.to_tuple()} should be invalid"
        
        # Test coordinate clamping behavior with out-of-bounds positions
        if not validation_result.is_valid:
            # Test all actions to ensure clamping works
            for action in [Action.UP, Action.RIGHT, Action.DOWN, Action.LEFT]:
                clamped_result = boundary_enforcer.enforce_movement_bounds(extreme_coords, action)
                
                # Verify clamped position is within bounds
                assert 0 <= clamped_result.final_position.x < edge_case_test_env.grid_size[0]
                assert 0 <= clamped_result.final_position.y < edge_case_test_env.grid_size[1]
                assert clamped_result.boundary_hit, "Should indicate boundary hit for extreme positions"
...
def test_invalid_action_edge_cases(edge_case_test_env, invalid_action):
    ...
    if isinstance(error, ValidationError):
        ...
        assert 'action' in str(error).lower(), "Error message should mention action parameter"
        
        # Verify error provides context about valid ranges
        error_details = error.get_validation_details() if hasattr(error, 'get_validation_details') else {}
        if error_details:
            assert 'expected_format' in error_details or 'parameter_name' in error_details
    ...
    try:
        ...
        assert test_info.get('agent_xy') == initial_agent_position, "Environment state should remain consistent"
...
def test_extreme_environment_configurations(grid_size, parameter_validator=None):
    ...
    try:
        if grid_size[0] <= 0 or grid_size[1] <= 0 or grid_size[0] > 10000 or grid_size[1] > 10000:
            # Should raise ConfigurationError for invalid grid dimensions
            with pytest.raises((ConfigurationError, ValueError, MemoryError)) as exc_info:
                env = create_plume_search_env(grid_size=grid_size)
                
            # Verify appropriate error type and message
            error = exc_info.value
            if isinstance(error, ConfigurationError):
                valid_options = error.get_valid_options() if hasattr(error, 'get_valid_options') else {}
        ...
        pytest.skip(f"Grid size {grid_size} requires too much memory for test environment")
    
    # Test source location placement outside valid grid boundaries
    if grid_size[0] > 0 and grid_size[1] > 0 and grid_size[0] <= 1000 and grid_size[1] <= 1000:
        invalid_source_locations = [
            (-1, 0), (0, -1), (grid_size[0], grid_size[1] // 2), (grid_size[0] // 2, grid_size[1])
        ]
        ...
        for invalid_source in invalid_source_locations:
            with pytest.raises((ConfigurationError, ValidationError, ValueError)):
                env = create_plume_search_env(
                    grid_size=grid_size, 
                    source_location=invalid_source
                )
    
    # Test plume parameter validation with extreme sigma values
    if grid_size[0] > 0 and grid_size[1] > 0 and max(grid_size) <= 256:  # Reasonable size for parameter testing
            ...
            try:
                ...
                if not param_result.is_valid:
                    ...
                    with pytest.raises((ConfigurationError, ValidationError, ValueError)):
                        env = create_plume_search_env(
                            grid_size=grid_size,
                            plume_params={'sigma': extreme_sigma}
                        )
...
def test_mathematical_precision_edge_cases(edge_case_test_env):
    ...
    assert 0.0 <= obs[0] <= 1.0, "Observation should be in valid concentration range [0,1]"
    ...
    for source_x, source_y in [(0, 0), (31, 31), (0, 31), (31, 0)]:
        ...
        assert 0.0 <= obs[0] <= 1.0, f"Boundary source observation should be valid: {obs[0]}"
        ...
        assert not math.isinf(distance), "Distance calculation should not produce infinity"
    ...
    assert abs(obs1[0] - obs2[0]) < 1e-10, "Identical seeds should produce numerically identical results"
    assert info1.get('agent_xy') == info2.get('agent_xy'), "Agent positions should be identical"
...
def test_rendering_edge_cases_and_failures(edge_case_test_env):
    ...
    try:
        ...
        assert rgb_array.shape == (256, 256, 3), "RGB array should have correct dimensions"
        ...
        assert memory_growth < 100, f"Memory growth {memory_growth} should be reasonable"
    ...
    with unittest.mock.patch('matplotlib.pyplot.subplots') as mock_subplots:
        ...
        try:
            ...
            if fallback_result is not None:
                assert isinstance(fallback_result, np.ndarray), "Fallback should produce array"
    ...
    try:
        ...
        assert test_render is not None, "Environment should function after concurrent rendering"
    ...
    with unittest.mock.patch.dict('os.environ', {}, clear=True):
        ...
        try:
            ...
            if hasattr(e, 'get_fallback_suggestions'):
                suggestions = e.get_fallback_suggestions()
                assert 'rgb_array' in str(suggestions).lower(), "Should suggest rgb_array mode for headless"
...
def test_memory_exhaustion_and_resource_limits(memory_monitor, cleanup_validator):
    ...
    try:
        ...
        if len(memory_readings) >= 2:
            ...
            assert memory_per_env < 50, f"Memory per environment {memory_per_env}MB too high"
        ...
        try:
            ...
            assert recovery_obs is not None, "System should remain stable after memory stress"
            recovery_env.close()
        except Exception as e:
...
def test_concurrent_operations_and_thread_safety(edge_case_test_env):
    ...
    def thread_worker(env, worker_id, operations_count=50):
        ...
        try:
            ...
            for i in range(operations_count):
                ...
                assert len(result) == 5, f"Worker {worker_id}: Invalid step result length"
                
                obs, reward, terminated, truncated, info = result
                assert isinstance(obs, np.ndarray), f"Worker {worker_id}: Invalid observation type"
    ...
    for worker_id in range(num_workers):
        thread = threading.Thread(
            ...
            args=(edge_case_test_env, worker_id, 25)
        )
        threads.append(thread)
        thread.start()
    ...
    for worker_id in range(2):
        thread = threading.Thread(target=render_worker, args=(edge_case_test_env, worker_id))
    ...
    assert final_obs is not None, "Environment should remain functional after concurrent operations"
...
def test_episode_lifecycle_edge_cases(edge_case_test_env):
    ...
    assert len(unique_positions) > 1, "Different seeds should produce different start positions"
    ...
    if isinstance(error, StateError) and hasattr(error, 'suggest_recovery_action'):
        ...
        assert recovery_suggestion is not None, "StateError should provide recovery suggestion"
    ...
    assert truncated or terminated, "Episode should end due to truncation or termination"
...
def test_seeding_and_reproducibility_edge_cases(edge_case_test_env, invalid_seed):
    ...
    elif isinstance(invalid_seed, (int, float)) and not (math.isnan(invalid_seed) or math.isinf(invalid_seed)):
    ...
    if len(seed_results) >= 2:
        ...
        assert len(trajectory1) == len(trajectory2), "Trajectories should have same length"
        ...
        for i, (step1, step2) in enumerate(zip(trajectory1, trajectory2)):
            if i == 0:
                ...
                assert info1['agent_xy'] == info2['agent_xy'], f"Initial positions should match"
            else:
                # Step results
...
def test_performance_degradation_scenarios(performance_tracker, edge_case_test_env, step_count):
    """
    ...
    if step_times:
        ...
        if avg_step_time > target_avg_step_time:
            warnings.warn(f"Average step time {avg_step_time:.4f}s exceeds target {target_avg_step_time}s")
        
        if max_step_time > target_max_step_time:
            warnings.warn(f"Maximum step time {max_step_time:.4f}s exceeds target {target_max_step_time}s")
        ...
        if hasattr(performance_tracker, 'get_metrics'):
            ...
            if 'average_step_time' in metrics:
                ...
                assert abs(monitored_avg - avg_step_time) < 0.01, "Performance monitoring should be accurate"
    ...
    if recovery_steps:
        ...
        if step_times and recovery_avg > avg_step_time * 2:
            warnings.warn(f"Performance degraded after cleanup: {recovery_avg:.4f}s vs {avg_step_time:.4f}s")
    
    # Test system stability during sustained high-frequency operations
    stability_test_start = time.perf_counter()
    stability_steps = 0
    
    while time.perf_counter() - stability_test_start < 1.0:  # 1 second of high-frequency ops
        try:
            action = edge_case_test_env.action_space.sample()
            obs, reward, terminated, truncated, info = edge_case_test_env.step(action)
            stability_steps += 1
    ...
    if hasattr(performance_tracker, 'stop_monitoring'):
        ...
        if final_metrics and 'total_steps' in final_metrics:
            ...
            assert step_difference < expected_steps * 0.1, "Step counting should be reasonably accurate"
...
def test_validation_system_edge_cases(parameter_validator=None):
    ...
    try:
        ...
        if hasattr(parameter_validator, 'batch_validate'):
            ...
            for key, value in list(large_parameter_set.items())[:100]:  # Limit for performance
        ...
        assert cache_duration < 5.0, f"Validation caching took too long: {cache_duration:.2f}s"
    ...
    for malicious_input in malicious_inputs:
        try:
            ...
            if result.is_valid:
                ...
                sanitized_value = result.sanitized_value if hasattr(result, 'sanitized_value') else malicious_input['value']
                
                # Basic sanitization checks
                if isinstance(sanitized_value, str):
                    assert len(sanitized_value) <= 1000, "Sanitized value should respect length limits"
            ...
            for keyword in sensitive_keywords:
                if keyword in error_message:
                    assert '*' in error_message or '[REDACTED]' in error_message, "Sensitive data should be redacted"
    ...
    if hasattr(parameter_validator, 'validate_parameter_consistency'):
        consistency_result = parameter_validator.validate_parameter_consistency(conflicting_params)
    ...
    if high_frequency_count > 0:
        ...
        assert avg_validation_time < 0.001, f"Average validation time {avg_validation_time:.6f}s too slow"
    ...
    for thread_id in range(3):
        thread = threading.Thread(
            target=concurrent_validation,
            args=(parameter_validator, thread_id)
        )
    ...
    assert concurrent_error_count == 0, f"Concurrent validation produced {concurrent_error_count} errors"
...
def test_error_handling_and_recovery_mechanisms():
    ...
    for exception_test in exception_tests:
        ...
        try:
            ...
            error_message = str(e)
            assert test_message in error_message, f"Error message should contain original message"
            ...
            if hasattr(e, 'suggest_recovery_action'):
                ...
                assert recovery_suggestion is not None, f"{exception_class.__name__} should provide recovery suggestion"
                assert isinstance(recovery_suggestion, str), "Recovery suggestion should be string"
                assert len(recovery_suggestion) > 0, "Recovery suggestion should not be empty"
            ...
            if isinstance(e, ValidationError) and hasattr(e, 'get_validation_details'):
                ...
                assert isinstance(details, dict), "Validation details should be dictionary"
            ...
            elif isinstance(e, RenderingError) and hasattr(e, 'get_fallback_suggestions'):
                fallbacks = e.get_fallback_suggestions()
                assert fallbacks is not None, "RenderingError should provide fallback suggestions"
            
            elif isinstance(e, ConfigurationError) and hasattr(e, 'get_valid_options'):
                options = e.get_valid_options()
                assert options is not None, "ConfigurationError should provide valid options"
            
            elif isinstance(e, ResourceError) and hasattr(e, 'suggest_cleanup_actions'):
                cleanup = e.suggest_cleanup_actions()
                assert cleanup is not None, "ResourceError should provide cleanup suggestions"
    ...
    while time.perf_counter() - error_handling_start < 0.1:  # 100ms of error handling
        try:
            raise ValidationError(f"Performance test error {error_count}", parameter_name="test")
    ...
    if error_count > 0:
        ...
        assert avg_error_time < 0.001, f"Error handling too slow: {avg_error_time:.6f}s per error"
    ...
    try:
        ...
        if hasattr(e, 'suggest_cleanup_actions'):
            ...
            assert 'immediate' in cleanup_actions.lower() or 'urgent' in cleanup_actions.lower()
    ...
    def concurrent_error_handler(thread_id):
        ...
        try:
            for i in range(10):
                try:
                    if i % 3 == 0:
                        raise ValidationError(f"Thread {thread_id} validation error {i}")
                    elif i % 3 == 1:
    ...
    assert len(error_types_handled) >= 2, "Should handle multiple error types concurrently"