...
from typing import Any, Dict, List, Optional, Tuple, Union  # >=3.10 - Type hints for test parameter specifications, return type validation, and comprehensive type checking
...
from plume_nav_sim.envs.plume_search_env import PlumeSearchEnv  # Primary environment class under test for comprehensive Gymnasium API compliance validation
from plume_nav_sim.envs.base_env import BaseEnvironment  # Base environment class for inheritance testing and abstract method validation
from plume_nav_sim.registration.register import (
    register_env, unregister_env, is_registered, ENV_ID  # Environment registration functions for gym.make() compatibility testing
)
from plume_nav_sim.core.types import (
    Action, Coordinates  # Action enumeration and coordinate types for validation testing and data creation
)
from plume_nav_sim.core.constants import (
    DEFAULT_GRID_SIZE, DEFAULT_SOURCE_LOCATION, PERFORMANCE_TARGET_STEP_LATENCY_MS  # System constants for environment configuration and performance validation
)
...
INVALID_ACTIONS = [-1, 4, 'invalid', None, 1.5]  # Invalid action values for error testing
...
class TestEnvironmentAPI:
        ...
        if self.performance_data:
            for metric_name, values in self.performance_data.items():
                if values:
                    ...
                    print(f"Performance {metric_name}: avg={avg_value:.3f}, max={max_value:.3f}")
    ...
    def create_test_environment(self, config: Optional[Dict] = None, register_for_cleanup: bool = True) -> PlumeSearchEnv:
        """
        ...
        assert isinstance(effective_config.get('grid_size'), tuple), "Grid size must be tuple"
        assert len(effective_config['grid_size']) == 2, "Grid size must have 2 dimensions"
        assert all(isinstance(d, int) and d > 0 for d in effective_config['grid_size']), "Grid dimensions must be positive integers"
    ...
    def validate_api_response(self, response: Any, method_name: str, expected_format: Dict[str, Any]) -> bool:
        """
        ...
        if method_name == 'reset':
            assert isinstance(response, tuple), f"Reset must return tuple, got {type(response)}"
            assert len(response) == 2, f"Reset must return 2-tuple, got length {len(response)}"
            observation, info = response
            
            # Check each tuple element matches expected type from expected_format
            assert isinstance(observation, np.ndarray), f"Observation must be numpy array, got {type(observation)}"
            assert isinstance(info, dict), f"Info must be dictionary, got {type(info)}"
            
        elif method_name == 'step':
            assert isinstance(response, tuple), f"Step must return tuple, got {type(response)}"
            assert len(response) == 5, f"Step must return 5-tuple, got length {len(response)}"
        ...
        if method_name in ['reset', 'step']:
            ...
            assert observation.shape == (1,), f"Observation shape must be (1,), got {observation.shape}"
            assert observation.dtype == np.float32, f"Observation dtype must be float32, got {observation.dtype}"
            assert 0.0 <= observation[0] <= 1.0, f"Observation value must be in [0,1], got {observation[0]}"
    ...
    def measure_method_performance(self, env: PlumeSearchEnv, method_name: str, 
                                 method_call: callable, iterations: int = 100) -> Dict[str, Any]:
        """
        ...
        if timing_data:
            ...
            if method_name == 'step' and performance_metrics['mean_time_ms'] > PERFORMANCE_TARGET_STEP_LATENCY_MS:
                performance_metrics['target_exceeded'] = True
                performance_metrics['target_ratio'] = performance_metrics['mean_time_ms'] / PERFORMANCE_TARGET_STEP_LATENCY_MS
            else:
                performance_metrics['target_exceeded'] = False
                performance_metrics['target_ratio'] = performance_metrics['mean_time_ms'] / PERFORMANCE_TARGET_STEP_LATENCY_MS if method_name == 'step' else 1.0
    ...
    def validate_environment_state(self, env: PlumeSearchEnv, strict_validation: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(env, '_environment_initialized'):
                validation_report['validation_results']['initialized'] = env._environment_initialized
            else:
                validation_report['warnings'].append('Environment initialization status unknown')
            ...
            if hasattr(env, '_step_count') and hasattr(env, '_episode_count'):
                ...
                if episode_count < 0:
                    validation_report['errors'].append('Negative episode count detected') 
                    validation_report['state_valid'] = False
            ...
            if hasattr(env, 'plume_model') and hasattr(env, 'state_manager'):
                if env.plume_model and env.state_manager:
                    ...
                    if hasattr(env.plume_model, 'grid_size') and hasattr(env.state_manager, 'grid_size'):
                        if env.plume_model.grid_size != env.state_manager.grid_size:
                            validation_report['errors'].append('Grid size mismatch between components')
                            validation_report['state_valid'] = False
            ...
            if strict_validation:
                ...
                if hasattr(env, 'action_space') and hasattr(env, 'observation_space'):
                    ...
                    if not isinstance(env.action_space, gymnasium.spaces.Discrete):
                        validation_report['errors'].append('Action space must be Discrete')
                        validation_report['state_valid'] = False
                    elif env.action_space.n != 4:
                        validation_report['errors'].append('Action space must have 4 actions')
                        validation_report['state_valid'] = False
                    
                    # Validate observation space is Box with correct shape
                    if not isinstance(env.observation_space, gymnasium.spaces.Box):
                        validation_report['errors'].append('Observation space must be Box')
                        validation_report['state_valid'] = False
                    elif env.observation_space.shape != (1,):
                        validation_report['errors'].append('Observation space shape must be (1,)')
                        validation_report['state_valid'] = False
            ...
            if validation_report['state_valid'] and not validation_report['warnings']:
                validation_report['recommendations'].append('Environment state is valid and ready')
            elif validation_report['warnings'] and validation_report['state_valid']:
                validation_report['recommendations'].append('Environment functional with minor warnings')
            else:
                validation_report['recommendations'].append('Address validation errors before use')
...
def test_environment_inheritance():
    ...
    try:
        ...
        assert isinstance(test_env, BaseEnvironment), "PlumeSearchEnv must inherit from BaseEnvironment"
        
        # Verify PlumeSearchEnv inherits from gymnasium.Env for API compliance
        assert isinstance(test_env, gymnasium.Env), "PlumeSearchEnv must inherit from gymnasium.Env"
        ...
        for method_name in abstract_methods:
            ...
            assert hasattr(test_env, method_name), f"Method {method_name} not implemented"
        ...
        for name, method in inspect.getmembers(BaseEnvironment, predicate=inspect.isfunction):
        ...
        for abstract_method in base_abstract_methods:
            ...
            assert not getattr(concrete_method, '__isabstractmethod__', False), \
...
def test_gymnasium_api_compliance():
    ...
    try:
        ...
        assert isinstance(observation, np.ndarray), "Reset observation must be numpy array"
        ...
        assert isinstance(test_env.action_space, gymnasium.spaces.Discrete), "Action space must be Discrete"
        ...
        assert hasattr(test_env, 'observation_space'), "Environment must have observation_space"
        assert isinstance(test_env.observation_space, gymnasium.spaces.Box), "Observation space must be Box"
        assert test_env.observation_space.shape == (1,), "Observation space shape must be (1,)"
        assert test_env.observation_space.dtype == np.float32, "Observation space dtype must be float32"
        assert test_env.observation_space.low[0] == 0.0, "Observation space low must be 0.0"
        assert test_env.observation_space.high[0] == 1.0, "Observation space high must be 1.0"
        ...
        if rgb_result is not None:  # Allow None for unavailable rendering
            assert isinstance(rgb_result, np.ndarray), "RGB render must return numpy array"
        ...
        for method_name in required_methods:
            assert hasattr(test_env, method_name), f"Method {method_name} required for Gymnasium compliance"
            assert callable(getattr(test_env, method_name)), f"Method {method_name} must be callable"
...
def test_action_space_validation(action):
    ...
    try:
        ...
        assert isinstance(reward, (int, float)), "Valid action must return numeric reward"
        
        # Verify action_space.contains() correctly identifies valid and invalid actions
        assert test_env.action_space.contains(action), f"Action space must contain valid action {action}"
        
        # Test action space sampling produces valid actions within expected range
        for _ in range(10):
            sampled_action = test_env.action_space.sample()
            assert test_env.action_space.contains(sampled_action), "Sampled action must be valid"
        ...
        assert isinstance(test_env.action_space, gymnasium.spaces.Discrete), "Action space must be Discrete"
        assert test_env.action_space.n == 4, "Action space must have exactly 4 discrete actions"
    ...
    try:
        ...
        if invalid_action is not None and not isinstance(invalid_action, str):
            try:
                ...
                assert not contains_result, f"Action space should not contain invalid action {invalid_action}"
...
def test_observation_space_validation():
    ...
    try:
        ...
        assert test_env.observation_space.contains(observation), "Observation space must contain valid observation"
        
        # Test invalid observations are rejected
        invalid_obs_negative = np.array([-0.1], dtype=np.float32)
        assert not test_env.observation_space.contains(invalid_obs_negative), "Negative observation should be invalid"
        
        invalid_obs_too_high = np.array([1.1], dtype=np.float32)
        assert not test_env.observation_space.contains(invalid_obs_too_high), "Too high observation should be invalid"
        ...
        for action in [0, 1, 2, 3]:
            ...
            assert isinstance(step_obs, np.ndarray), "Step observation must be numpy array"
            assert step_obs.shape == (1,), "Step observation shape must be (1,)"
            assert step_obs.dtype == np.float32, "Step observation dtype must be float32"
            assert 0.0 <= step_obs[0] <= 1.0, "Step observation must be in valid range"
            assert test_env.observation_space.contains(step_obs), "Step observation must be valid"
        ...
        np.testing.assert_array_equal(obs1, obs2, "Identical seeds should produce identical observations")
        
        # Validate observation space bounds and mathematical properties
        assert isinstance(test_env.observation_space, gymnasium.spaces.Box), "Observation space must be Box"
        assert test_env.observation_space.low[0] == 0.0, "Observation space low bound must be 0.0"
        assert test_env.observation_space.high[0] == 1.0, "Observation space high bound must be 1.0"
        assert test_env.observation_space.bounded_above.all(), "Observation space must be bounded above"
        assert test_env.observation_space.bounded_below.all(), "Observation space must be bounded below"
...
def test_reset_method_functionality():
    ...
    try:
        ...
        assert 0.0 <= observation[0] <= 1.0, "Initial observation must be valid concentration"
        ...
        assert reset_info['episode_count'] > info['episode_count'], "Episode count should increment"
        ...
        for i in range(3):
            ...
            np.testing.assert_array_equal(obs_i, obs1 if i == 0 else obs_i, 
        ...
        assert hasattr(test_env, 'state_manager'), "Environment should have state manager"
...
def test_step_method_functionality():
    ...
    try:
        ...
        for action in VALID_ACTIONS[:2]:  # Test first 2 actions
            ...
            assert info['action_taken'] == action, "Action taken must match input action"
...
def test_seeding_and_reproducibility(seed):
    ...
    try:
        ...
        np.testing.assert_array_equal(obs1, obs2, f"Seed {seed}: identical seeds should produce identical initial observations")
        ...
        for action in action_sequence:
            ...
            np.testing.assert_array_equal(step1[0], step2[0], 
                f"Seed {seed}, action {action}: observations should be identical")
            
            # Compare rewards
            assert step1[1] == step2[1], f"Seed {seed}, action {action}: rewards should be identical"
            
            # Compare termination flags
            assert step1[2] == step2[2], f"Seed {seed}, action {action}: terminated flags should be identical"
            assert step1[3] == step2[3], f"Seed {seed}, action {action}: truncated flags should be identical"
        ...
        assert seed_result1[0] == seed_result2[0] == seed, "Seed values should match input"
        ...
        try:
            ...
            np.testing.assert_array_equal(trajectory1[0][0], step3[0],
...
def test_rendering_system_integration():
    ...
    try:
        ...
        if initial_render is not None and step_render is not None:
            ...
            assert isinstance(step_render, np.ndarray), "Step render must be valid array"
            assert step_render.shape == initial_render.shape, "Render shape should be consistent"
    ...
    try:
        ...
        try:
            ...
            if rgb_from_human_env is not None:
                assert isinstance(rgb_from_human_env, np.ndarray), "Mode switch should work"
        except Exception:
        ...
        assert render_time_ms < 100.0, f"Rendering took too long: {render_time_ms:.1f}ms"
...
def test_environment_registration():
    ...
    try:
        ...
        assert registered_id == ENV_ID, f"Registration should return ENV_ID, got {registered_id}"
        
        # Validate gym.make(ENV_ID) creates functional environment instance
        gym_env = gymnasium.make(ENV_ID)
        
        try:
            assert isinstance(gym_env, PlumeSearchEnv), "gym.make should create PlumeSearchEnv instance"
        ...
        try:
            ...
            assert hasattr(custom_env, 'source_location'), "Custom env should have source_location"
        ...
        assert is_registered(ENV_ID, use_cache=False), "Environment should be registered (no cache)"
        ...
        assert not is_registered(ENV_ID), "Environment should not be registered after unregistration"
        ...
        assert ENV_ID.endswith('-v0'), f"Environment ID should end with -v0, got {ENV_ID}"
    ...
    try:
        ...
        try:
            ...
            for action in VALID_ACTIONS:
                try:
                    ...
                    assert len(result) == 5, "Boundary conditions should still return valid results"
                except Exception as e:
                    # Boundary errors should be handled gracefully
                    assert isinstance(e, (ValueError, AssertionError)), f"Unexpected boundary error type: {type(e)}"
        finally:
            boundary_env.close()
        ...
        try:
            ...
            try:
                ...
                assert len(recovery_result) == 5, "Environment should recover from errors"
        ...
        try:
            ...
            assert any(keyword in error_message for keyword in 
                      ['render', 'mode', 'invalid', 'unsupported']), \
...
def test_performance_requirements():
    ...
    try:
        ...
        assert avg_reset_time < 10.0, f"Average reset time {avg_reset_time:.2f}ms exceeds 10ms target"
        ...
        assert avg_step_time < PERFORMANCE_TARGET_STEP_LATENCY_MS, \
        ...
        assert p95_step_time < PERFORMANCE_TARGET_STEP_LATENCY_MS * 2, \
        ...
        if render_times:
            ...
            assert avg_render_time < 50.0, f"Average render time {avg_render_time:.2f}ms exceeds 50ms target"
        ...
        print(f"\nPerformance Summary:")
        print(f"  Average step time: {avg_step_time:.3f}ms (target: <{PERFORMANCE_TARGET_STEP_LATENCY_MS}ms)")
...
def test_episode_lifecycle_management():
    ...
    try:
        ...
        while True:
            ...
            assert step_info['step_count'] == episode_steps, "Step count should increment correctly"
            ...
            if terminated:
                ...
                assert episode_steps >= test_env.max_steps, "Truncation should occur at max steps"
                break
        ...
        try:
            ...
            for step in range(10):
                ...
                if truncated:
                    assert info['step_count'] >= 5, "Truncation should occur after max_steps"
                    break
                elif terminated:
        ...
        assert new_info['episode_count'] > info['episode_count'], "Episode count should increment"
        ...
        for i in range(len(episode_results) - 1):
            ...
            assert next_ep['episode_count'] > current['episode_count'], "Episode counts should increment"
...
def test_component_integration():
    ...
    try:
        ...
        if hasattr(integration_env, 'plume_model'):
            ...
            assert integration_env.plume_model is not None, "Plume model should be initialized"
            
            # Test concentration sampling works
            obs, _ = integration_env.reset()
            assert isinstance(obs, np.ndarray), "Observation should be sampled from plume model"
        ...
        for action in [0, 1, 2, 3]:
            ...
            assert isinstance(obs, np.ndarray), "State manager should provide valid observations"
            assert isinstance(reward, (int, float)), "Reward calculator should provide numeric rewards"
        ...
        try:
            ...
            if render_result is not None:
                assert isinstance(render_result, np.ndarray), "Rendering should integrate with environment state"
        ...
        try:
            ...
            np.testing.assert_array_equal(obs1, obs2, "Seeding should coordinate across all components")
            ...
            for action in [1, 0, 2, 3]:
                ...
                np.testing.assert_array_equal(result1[0], result2[0], "Action results should be reproducible")
        ...
        assert integration_env.action_space.n == 4, "Action space should match movement system"
        assert integration_env.observation_space.shape == (1,), "Observation space should match plume model output"
