...
from plume_nav_sim.utils.validation import validate_environment_config  # Environment configuration validation
...
ERROR_TEST_SCENARIOS = ['invalid_env_id', 'missing_entry_point', 'invalid_parameters', 'registry_corruption']
...
def setup_module():
    ...
    warnings.filterwarnings('ignore', category=UserWarning, message='.*already registered.*')
...
def create_test_registration_config(config_type: str = 'default', 
                                  overrides: dict = None, 
                                  validate_config: bool = True) -> dict:
    """
...
def validate_gym_make_compatibility(env_id: str, 
                                   make_kwargs: dict = None,
                                   test_basic_functionality: bool = True) -> tuple:
    """
    ...
    try:
        ...
        if isinstance(env, PlumeSearchEnv):
            ...
            validation_report['errors'].append(f"Expected PlumeSearchEnv, got {type(env)}")
        ...
        if hasattr(env, 'action_space') and hasattr(env, 'observation_space'):
            ...
            if env.action_space.n == 4:  # Discrete(4)
                ...
                validation_report['errors'].append(f"Expected Discrete(4) action space, got {env.action_space}")
            
            # Validate observation space
            if env.observation_space.shape == (1,) and env.observation_space.dtype.name == 'float32':
                pass  # Correct observation space
            else:
                validation_report['errors'].append(f"Invalid observation space: {env.observation_space}")
        else:
            validation_report['errors'].append("Missing action_space or observation_space")
            ...
            try:
                ...
                if isinstance(obs, type(env.observation_space.sample())):
                    if isinstance(info, dict) and 'agent_xy' in info:
                        ...
                        validation_report['warnings'].append("Info dictionary missing expected keys")
                else:
                    validation_report['errors'].append("Reset observation type mismatch")
                ...
                if validation_report['basic_functionality']:
                    ...
                    if len(step_result) == 5:
                        ...
                        if not (isinstance(reward, (int, float)) and 
                               isinstance(terminated, bool) and 
                               isinstance(truncated, bool)):
                            validation_report['errors'].append("Step return types incorrect")
                    else:
                        validation_report['errors'].append(f"Expected 5-tuple from step, got {len(step_result)}-tuple")
                        
            except Exception as e:
                validation_report['errors'].append(f"Basic functionality test failed: {str(e)}")
        
        # Determine overall compatibility success
        compatibility_success = (validation_report['gym_make_success'] and 
                               validation_report['type_validation'] and 
                               validation_report['api_compliance'])
...
def measure_registration_performance(registration_operation: callable,
    ...
    if timings:
        ...
        performance_metrics = {
            ...
            'meets_target': all(t < REGISTRATION_PERFORMANCE_TARGET_MS for t in timings),
            'errors': errors
        }
        ...
        performance_metrics.update({
            ...
            'p99_ms': sorted_timings[int(len(sorted_timings)*0.99)] if len(sorted_timings) > 1 else sorted_timings[0]
        })
        ...
        if baseline_memory:
            try:
                ...
                performance_metrics['memory_delta_mb'] = current_memory - baseline_memory
            except:
                pass
...
class TestRegistrationIntegration:
    ...
    def test_registration_gym_make_integration(self):
        ...
        assert not is_registered(TEST_ENV_ID_BASE), "Test environment should not be registered initially"
        ...
        try:
            ...
            assert is_registered(TEST_ENV_ID_BASE), "Environment should be registered after register_env()"
            ...
            assert compatibility_success, f"gym.make() compatibility failed: {report['errors']}"
            assert env is not None, "Environment instance should be returned"
            
            # Test environment functionality
            obs, info = env.reset(seed=42)
            assert isinstance(obs, type(env.observation_space.sample())), "Reset observation type mismatch"
            ...
            assert env.grid_size == config['grid_size'], "Grid size should match configuration"
            assert env.source_location == config['source_location'], "Source location should match configuration"
    ...
    def test_registration_lifecycle_management(self):
        ...
        assert is_registered(TEST_ENV_ID_BASE), "Should be registered after initial registration"
        ...
        assert isinstance(env1, PlumeSearchEnv), "First instance should be PlumeSearchEnv"
        assert isinstance(env2, PlumeSearchEnv), "Second instance should be PlumeSearchEnv"
        ...
        assert info1['agent_xy'] != info2['agent_xy'], "Different seeds should produce different initial positions"
        ...
        assert env3.grid_size == updated_config['grid_size'], "Updated grid size should be applied"
        ...
        assert not is_registered(TEST_ENV_ID_BASE), "Should be unregistered after lifecycle completion"
    ...
    def test_configuration_parameter_integration(self):
        ...
        for config_name, overrides in configurations:
            ...
            try:
                ...
                assert is_registered(test_env_id), f"Configuration {config_name} should be registered"
                ...
                for param_name, param_value in overrides.items():
                    if param_name == 'grid_size':
                        assert env.grid_size == param_value, f"Grid size mismatch for {config_name}"
                    elif param_name == 'source_location':
                        assert env.source_location == param_value, f"Source location mismatch for {config_name}"
                        ...
                        if not terminated:
                            assert truncated, f"Should truncate at max_steps for {config_name}"
                
                env.close()
                
            except Exception as e:
                pytest.fail(f"Configuration parameter integration failed for {config_name}: {str(e)}")
    ...
    def test_error_handling_integration(self):
        ...
        with pytest.raises((ValidationError, ValueError)):
            config = create_test_registration_config(overrides=invalid_config, validate_config=True)
    ...
    def test_performance_integration(self):
        ...
        assert baseline_perf['meets_target'], f"Registration performance {baseline_perf['mean_time_ms']:.2f}ms exceeds target {REGISTRATION_PERFORMANCE_TARGET_MS}ms"
        ...
        successful_tests = sum(1 for _, result in performance_results if result.get('meets_target', False))
        ...
        for test_name, result in performance_results:
            ...
            print(f"  {test_name}: {mean_time}ms (Target: {'✓' if meets_target else '✗'})")
        
        # At least baseline performance should meet targets
        assert baseline_perf['meets_target'], "Baseline registration performance must meet targets"
    ...
    def test_cache_consistency_integration(self):
        ...
        assert info.get('env_id') == test_env_id, "Registration info should match environment ID"
        
        # Test multiple registration/unregistration cycles for cache consistency
        for cycle in range(3):
            # Unregister
            unregister_env(test_env_id)
            assert not is_registered(test_env_id), f"Cycle {cycle}: Should be unregistered"
            assert test_env_id not in registry.env_specs, f"Cycle {cycle}: Should not exist in registry"
            ...
            assert test_env_id in registry.env_specs, f"Cycle {cycle}: Should exist in registry"
        ...
        for env_id in additional_envs:
            assert is_registered(env_id), f"Environment {env_id} should remain registered"
        
        # Test cache invalidation on cleanup
        for env_id in additional_envs:
            unregister_env(env_id)
            assert not is_registered(env_id), f"Environment {env_id} should be unregistered"
...
class TestRegistrationScenarios:
    ...
    def test_multiple_environment_registration_scenarios(self):
        ...
        environments = [
            ...
            ("MultiEnv-Medium-v0", {'grid_size': (64, 64), 'source_location': (32, 32)}),
            ("MultiEnv-Large-v0", {'grid_size': (128, 128), 'source_location': (64, 64)}),
            ("MultiEnv-Custom-v0", {'grid_size': (96, 96), 'source_location': (48, 48), 'max_steps': 500})
        ]
        ...
        for env_id, config_overrides in environments:
            try:
                config = create_test_registration_config('default', overrides=config_overrides)
                register_env(
                    env_id=env_id,
                    entry_point='plume_nav_sim.envs.plume_search_env:PlumeSearchEnv',
                    kwargs=config
        ...
        for env_id, env in env_instances:
            _, config_overrides = next((e for e in environments if e[0] == env_id), (None, {}))
            
            # Verify configuration parameters are correctly applied
            if 'grid_size' in config_overrides:
                assert env.grid_size == config_overrides['grid_size'], f"Grid size mismatch for {env_id}"
            if 'source_location' in config_overrides:
                assert env.source_location == config_overrides['source_location'], f"Source location mismatch for {env_id}"
        ...
        for env_id in registered_envs[1:]:  # Skip already unregistered
            unregister_env(env_id)
            self.test_registrations.discard(env_id)
            assert not is_registered(env_id), f"{env_id} should be unregistered after cleanup"
    ...
    def test_registration_parameter_boundary_scenarios(self):
        ...
        for env_id, boundary_config in boundary_scenarios:
            try:
                ...
                config = create_test_registration_config('default', overrides=boundary_config)
                register_env(
                    env_id=env_id,
                    entry_point='plume_nav_sim.envs.plume_search_env:PlumeSearchEnv',
                    kwargs=config
                ...
                # Validate registration success
                assert is_registered(env_id), f"Boundary scenario {env_id} should register successfully"
                
                # Test environment creation and basic functionality
                env = gymnasium.make(env_id)
                
                # Verify parameter application
                assert env.grid_size == boundary_config['grid_size'], f"Grid size mismatch in {env_id}"
                assert env.source_location == boundary_config['source_location'], f"Source location mismatch in {env_id}"
                
                # Test environment functionality at boundaries
                obs, info = env.reset(seed=42)
                assert obs is not None, f"Reset should work for boundary scenario {env_id}"
                
                # Test at least one step
                action = env.action_space.sample()
                obs, reward, terminated, truncated, info = env.step(action)
                assert obs is not None, f"Step should work for boundary scenario {env_id}"
                
                # Test agent position is within bounds
                agent_x, agent_y = info['agent_xy']
                grid_width, grid_height = boundary_config['grid_size']
                assert 0 <= agent_x < grid_width, f"Agent X position out of bounds in {env_id}"
                assert 0 <= agent_y < grid_height, f"Agent Y position out of bounds in {env_id}"
        ...
        invalid_scenarios = [
            ...
            # Source outside grid
            {"grid_size": (10, 10), "source_location": (10, 10)},  # Exactly at boundary (invalid)
            # Negative max steps
            {"max_steps": -1}
        ]
        ...
        for invalid_config in invalid_scenarios:
            with pytest.raises((ValidationError, ValueError, ConfigurationError)):
                create_test_registration_config(overrides=invalid_config, validate_config=True)
    ...
    def test_registration_error_recovery_scenarios(self):
        ...
        assert is_registered(recovery_env_id), "Valid registrations should remain unaffected"
        ...
        for env_id in stress_env_ids:
            assert is_registered(env_id), f"Stress test environment {env_id} should remain registered"
    ...
    def test_cross_session_registration_scenarios(self):
        ...
        assert not is_registered(session_env_id), "Should be unregistered after session 1 cleanup"
        ...
        assert initial_position == second_position, "Same seed should produce same initial position across sessions"
        ...
        assert initial_position != third_position, "Different seed should produce different position"
        ...
        for env_id in session2_env_ids:
            assert is_registered(env_id), f"Session 2 environment {env_id} should be registered"
        ...
        for env_id in session2_env_ids:
            unregister_env(env_id)
            self.test_registrations.discard(env_id)
            assert not is_registered(env_id), f"Environment {env_id} should be cleaned up"
        
        # Final verification: clean state for next session
        registry = gymnasium.envs.registry
        for env_id in [session_env_id] + session2_env_ids:
            assert env_id not in registry.env_specs, f"Environment {env_id} should not persist in registry"
...
class TestRegistrationPerformance:
    ...
    def test_registration_timing_performance(self):
        ...
        baseline_stats = {
            ...
            'std_dev_ms': statistics.stdev(baseline_times) if len(baseline_times) > 1 else 0
        }
        timing_results.append(('baseline', baseline_stats))
        
        # Validate performance target
        assert baseline_stats['mean_ms'] < REGISTRATION_PERFORMANCE_TARGET_MS, \
        ...
        for test_name, stats in timing_results:
            ...
            print(f"  {test_name}: {mean_time:.2f}ms ± {stats.get('std_dev_ms', 0):.2f}ms {target_met}")
    ...
    def test_registration_memory_performance(self):
        ...
        try:
            ...
            print(f"Average per registration: {total_reg_delta/11:.2f}MB")  # 11 total registrations
            ...
            print(f"Memory cleanup effectiveness: {cleanup_effectiveness:.2f}MB recovered")
            ...
            assert single_reg_delta < max_acceptable_per_registration, \
            ...
            assert potential_leak < max_acceptable_leak, \
    ...
    def test_registration_scalability_performance(self):
        ...
        print(f"{'Scale':<6} {'Reg/env':<8} {'Lookup/env':<10} {'Unreg/env':<10} {'Total':<8}")
        for result in scalability_results:
            ...
            print(f"{scale:<6} {reg_per_env:<8.2f} {lookup_per_env:<10.4f} {unreg_per_env:<10.2f} {total_time:<8.1f}")
        ...
        if len(reg_times_per_env) > 1:
            ...
            assert performance_degradation < max_acceptable_degradation, \
        ...
        assert largest_scale_result['total_time_ms'] < max_acceptable_total_time, \
