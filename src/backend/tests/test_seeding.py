...
from plume_nav_sim.utils.exceptions import ValidationError, StateError
...
TEST_SEEDS = [42, 123, 456, 789, 2023]  # Standard test seed values for reproducibility testing
EDGE_CASE_SEEDS = [0, 1, SEED_MAX_VALUE, SEED_MAX_VALUE - 1]  # Edge case seed values for boundary testing
INVALID_SEEDS = [-1, SEED_MAX_VALUE + 1, 'invalid', None, 3.14]  # Invalid seed values for validation testing
REPRODUCIBILITY_TOLERANCE = 1e-10  # Tolerance for floating point reproducibility comparisons
...
class TestSeedValidation:
    ...
    def test_validate_seed_with_valid_inputs(self, seed, strict_mode):
        ...
        is_valid, normalized_seed, error_message = validate_seed(seed, strict_mode=strict_mode)
        
        # Assert validation returns (True, normalized_seed, '') indicating successful validation
        assert is_valid is True, f"Valid seed {seed} failed validation in strict_mode={strict_mode}"
        assert error_message == '', f"Valid seed {seed} produced error message: {error_message}"
        
        # Verify normalized_seed is correct integer within valid range
        assert isinstance(normalized_seed, int), f"Normalized seed {normalized_seed} is not integer"
        ...
        if seed in EDGE_CASE_SEEDS:
            assert normalized_seed >= SEED_MIN_VALUE, f"Edge case seed {seed} normalized incorrectly"
            assert normalized_seed <= SEED_MAX_VALUE, f"Edge case seed {seed} normalized incorrectly"
        ...
        if seed < 0:
            ...
            assert normalized_seed >= SEED_MIN_VALUE, f"Negative seed {seed} not properly normalized"
    ...
    def test_validate_seed_with_invalid_inputs(self, invalid_seed):
        ...
        assert is_valid is False, f"Invalid seed {invalid_seed} incorrectly passed validation"
        assert normalized_seed is None, f"Invalid seed {invalid_seed} returned non-None normalized value"
        
        # Verify error_message contains descriptive information about validation failure
        assert error_message != '', f"Invalid seed {invalid_seed} did not produce error message"
        assert len(error_message) > 10, f"Error message too short for seed {invalid_seed}: {error_message}"
        
        # Test type validation for non-integer inputs (strings, floats, None)
        if isinstance(invalid_seed, str):
            assert 'type' in error_message.lower() or 'invalid' in error_message.lower(), \
        ...
        if isinstance(invalid_seed, (int, float)) and invalid_seed < SEED_MIN_VALUE:
            assert 'range' in error_message.lower() or 'minimum' in error_message.lower(), \
        ...
        if isinstance(invalid_seed, (int, float)) and invalid_seed > SEED_MAX_VALUE:
            assert 'range' in error_message.lower() or 'maximum' in error_message.lower(), \
        ...
        assert any(word in error_message.lower() for word in ['seed', 'value', 'range', 'type']), \
...
class TestRNGCreation:
    ...
    def test_create_seeded_rng_basic_functionality(self, seed):
        ...
        assert isinstance(result, tuple), f"create_seeded_rng should return tuple, got {type(result)}"
        assert len(result) == 2, f"create_seeded_rng should return 2-tuple, got {len(result)} elements"
        ...
        if seed is not None:
            assert isinstance(seed_used, int), f"seed_used should be int for provided seed, got {type(seed_used)}"
            assert seed_used == seed, f"seed_used {seed_used} should match provided seed {seed}"
        
        # Test None seed case where random seed is generated automatically
        if seed is None:
            assert isinstance(seed_used, int), f"Auto-generated seed should be int, got {type(seed_used)}"
        ...
        assert isinstance(random_value, float), f"Generator should produce float, got {type(random_value)}"
        assert 0.0 <= random_value < 1.0, f"Random value {random_value} outside expected range [0, 1)"
        
        # Validate Gymnasium compatibility with returned generator
        # Test standard methods used by Gymnasium
        assert hasattr(np_random, 'integers'), "Generator missing integers method for Gymnasium"
        assert hasattr(np_random, 'random'), "Generator missing random method for Gymnasium"
        
        # Verify integers method works correctly
        int_value = np_random.integers(0, 10)
        assert isinstance(int_value, (int, np.integer)), f"integers() should return integer type"
        assert 0 <= int_value < 10, f"integers(0, 10) returned {int_value} outside range"
    ...
    def test_seeded_rng_reproducibility(self, test_seed):
        ...
        assert seed_used1 == seed_used2, f"Seeds should match: {seed_used1} != {seed_used2}"
        assert seed_used1 == test_seed, f"Seed should match input: {seed_used1} != {test_seed}"
        ...
        assert np_random1_reset.random() == np_random2_reset.random(), \
...
class TestDeterministicSeedGeneration:
    ...
    @pytest.mark.parametrize('seed_string', ['test_experiment', 'baseline_run', 'condition_A'])
    ...
    def test_generate_deterministic_seed_consistency(self, seed_string, hash_algorithm):
        ...
        assert isinstance(seed1, int), f"Generated seed should be int, got {type(seed1)}"
        ...
        if hash_algorithm == 'sha256':
            ...
            assert SEED_MIN_VALUE <= md5_seed <= SEED_MAX_VALUE, "MD5 seed should be in valid range"
        ...
        if seed_string == 'test_experiment':
            ...
            assert SEED_MIN_VALUE <= empty_seed <= SEED_MAX_VALUE, "Empty string seed should be in range"
            
            # Test with special characters
            special_seed = generate_deterministic_seed('!@#$%^&*()', hash_algorithm=hash_algorithm)
            assert isinstance(special_seed, int), "Special characters should produce valid seed"
            assert SEED_MIN_VALUE <= special_seed <= SEED_MAX_VALUE, "Special char seed should be in range"
        
        # Verify hash collision resistance for similar input strings
        similar_string = seed_string + '_modified'
        similar_seed = generate_deterministic_seed(similar_string, hash_algorithm=hash_algorithm)
        assert similar_seed != seed1, \
...
class TestReproducibilityVerification:
    ...
    def test_verify_reproducibility_function(self, sequence_length, tolerance):
        ...
        required_keys = ['match_status', 'sequence_length', 'tolerance', 'differences_found']
        for key in required_keys:
            assert key in report, f"Report missing required key: {key}"
        
        assert isinstance(report['match_status'], bool), "match_status should be boolean"
        assert report['sequence_length'] == sequence_length, "sequence_length should match input"
        assert abs(report['tolerance'] - tolerance) < 1e-12, "tolerance should match input"
        assert isinstance(report['differences_found'], int), "differences_found should be integer"
        ...
        assert report['differences_found'] == 0, "Identical generators should have 0 differences"
        ...
        different_report = verify_reproducibility(
            np_random1, np_random3, 
            sequence_length=sequence_length,
            tolerance=tolerance
        )
        
        # Should detect differences (usually, unless extremely unlucky with randomness)
        assert isinstance(different_report['match_status'], bool), "Different report should have boolean match_status"
        ...
        if tolerance >= 1e-3:
            ...
            assert different_report['tolerance'] == tolerance, "Tolerance should be preserved"
        ...
        if 'statistical_analysis' in report:
            ...
            if 'mean_difference' in stats:
                assert isinstance(stats['mean_difference'], (int, float)), "mean_difference should be numeric"
            
            if 'max_difference' in stats:
                assert isinstance(stats['max_difference'], (int, float)), "max_difference should be numeric"
...
class TestRandomSeedGeneration:
    ...
    def test_get_random_seed_entropy_sources(self, use_system_entropy):
        ...
        assert seed != seed2 or seed != seed3, \
        ...
        seeds_sample = [get_random_seed(use_system_entropy=use_system_entropy) for _ in range(20)]
        
        # Should have variety in generated seeds (basic randomness check)
        unique_seeds = set(seeds_sample)
        assert len(unique_seeds) >= len(seeds_sample) * 0.8, \
        ...
        assert len(low_seeds) > 0 and len(high_seeds) > 0, \
...
class TestSeedStatePersistence:
    ...
    def test_save_and_load_seed_state(self):
        ...
        try:
            ...
            save_result = save_seed_state(np_random, temp_file_path, metadata={'test_seed': test_seed})
            
            # Assert save operation returns True indicating successful persistence
            assert save_result is True, f"save_seed_state should return True for successful save"
            
            # Verify file was created and contains data
            assert temp_file_path.exists(), f"State file should exist after save: {temp_file_path}"
            ...
            for i in range(5):
                assert loaded_sequence[i] == fresh_sequence[i], \
            ...
            assert loaded_metadata.get('test_seed') == test_seed, \
...
class TestSeedManager:
    ...
    def test_seed_manager_basic_operations(self, default_seed, enable_validation, thread_safe):
        ...
        assert manager.default_seed == default_seed, f"Default seed should be {default_seed}"
        assert manager.enable_validation == enable_validation, f"Validation should be {enable_validation}"
        assert manager.thread_safe == thread_safe, f"Thread safety should be {thread_safe}"
        ...
        for context in test_contexts:
            for seed in [42, 123, None]:
                ...
                assert isinstance(used_seed, int), f"Used seed should be int for {context}, {seed}"
        ...
        for context in test_contexts:
            assert context in active_generators or any(context in str(k) for k in active_generators.keys()), \
        ...
        assert len(after_reset_generators) == 0 or len(after_reset_generators) < initial_count, \
        ...
        if enable_validation:
            ...
            try:
                ...
                assert result is not None, "Valid seed should work even without validation"
        ...
        if thread_safe:
            ...
            assert isinstance(np_random, np.random.Generator), "Thread safe manager should work"
    ...
    def test_seed_manager_episode_seed_generation(self, base_seed, episode_number, experiment_id):
        ...
        assert episode_seed == episode_seed2, \
        ...
        if episode_number < 999:  # Avoid edge case overflow
            ...
            assert different_episode_seed != episode_seed, \
        ...
        if experiment_id is not None:
            ...
            assert different_exp_seed != episode_seed, \
                f"Different experiment IDs should produce different seeds"
        
        # Validate all generated seeds are within valid range
        assert isinstance(episode_seed, int), f"Episode seed should be int, got {type(episode_seed)}"
        ...
        assert episode_seed3 == episode_seed, \
    ...
    def test_seed_manager_reproducibility_validation(self, test_seed, num_tests):
        ...
        required_keys = ['success_rate', 'total_runs', 'failed_runs', 'statistical_analysis']
        for key in required_keys:
        ...
        assert 0.0 <= success_rate <= 1.0, f"Success rate {success_rate} should be in [0, 1]"
        
        total_runs = validation_report['total_runs']
        assert total_runs == num_tests, f"Total runs {total_runs} should match input {num_tests}"
        
        failed_runs = validation_report['failed_runs']
        assert isinstance(failed_runs, int), "Failed runs should be int"
        assert 0 <= failed_runs <= total_runs, f"Failed runs {failed_runs} should be <= total {total_runs}"
        
        # Test failure analysis identifies patterns and root causes
        if failed_runs > 0:
            assert 'failure_analysis' in validation_report, "Should have failure analysis if failures occurred"
        ...
        assert validation_report['seed_tested'] == test_seed, "Tested seed should match input"
        
        # Should have timestamp or execution metadata
        assert any(key in validation_report for key in ['timestamp', 'execution_time', 'created_at']), \
    ...
    def test_seed_manager_thread_safety(self):
        ...
        def thread_worker(thread_id):
            ...
            try:
                for i in range(operations_per_thread):
                    ...
                    thread_results.append(('seed', seed, used_seed, np_random is not None))
                    ...
                    thread_results.append(('get_active', len(active), 0, isinstance(active, dict)))
        ...
        for thread_id, thread_results in results.items():
            assert len(thread_results) == operations_per_thread * 3, \
        ...
        assert isinstance(final_active_generators, dict), "Final generators should be dict"
        
        # Should have many generators registered (some may have been cleaned up)
        expected_min_generators = thread_count * operations_per_thread * 0.5  # At least 50% should remain
        ...
        assert isinstance(post_reset_generators, dict), "Post-reset generators should be dict"
...
class TestReproducibilityTracker:
    ...
    def test_reproducibility_tracker_episode_recording(self, episode_seed, episode_length):
        ...
        reward_sequence = [np_random.random() * 2 - 1 for _ in range(episode_length)]  # Rewards in [-1, 1]
        ...
        required_fields = ['seed', 'action_sequence', 'observation_sequence', 'reward_sequence', 'metadata']
        ...
        assert len(stored_episode['action_sequence']) == episode_length, "Action sequence length should match"
        assert len(stored_episode['observation_sequence']) == episode_length, "Observation sequence length should match"
        assert len(stored_episode['reward_sequence']) == episode_length, "Reward sequence length should match"
        ...
        assert stored_metadata['episode_seed'] == episode_seed, "Metadata seed should match"
        assert stored_metadata['episode_length'] == episode_length, "Metadata length should match"
        
        # Test record retrieval and data integrity validation
        # Sequences should match exactly
        for i in range(episode_length):
            assert stored_episode['action_sequence'][i] == action_sequence[i], \
                f"Action mismatch at index {i}: {stored_episode['action_sequence'][i]} != {action_sequence[i]}"
            assert abs(stored_episode['observation_sequence'][i] - observation_sequence[i]) < 1e-10, \
                f"Observation mismatch at index {i}"
            assert abs(stored_episode['reward_sequence'][i] - reward_sequence[i]) < 1e-10, \
    ...
    def test_reproducibility_tracker_verification(self, episodes_should_match):
        ...
        required_keys = ['episodes_match', 'baseline_id', 'comparison_id', 'discrepancies']
        for key in required_keys:
        ...
        if episodes_should_match:
            ...
            assert len(discrepancies) == 0, f"Matching episodes should have no discrepancies: {discrepancies}"
        else:
            ...
            if not episodes_match:
                discrepancies = verification_result['discrepancies']
                assert len(discrepancies) > 0, "Non-matching episodes should have discrepancies"
        ...
        if 'statistical_analysis' in verification_result:
            ...
            if 'total_comparisons' in stats:
                ...
                assert total_comps == expected_total, f"Total comparisons should be {expected_total}"
    ...
    def test_reproducibility_tracker_reporting(self, report_format, include_detailed_analysis):
        ...
        if report_format == 'json':
            assert isinstance(report, (str, dict)), f"JSON report should be string or dict"
            ...
            required_sections = ['summary', 'episodes_recorded', 'verifications_performed']
            ...
            assert summary['total_episodes'] == len(test_seeds), "Summary should count all episodes"
        
        elif report_format == 'markdown':
            assert isinstance(report, str), f"Markdown report should be string, got {type(report)}"
            assert len(report) > 100, "Markdown report should be substantial"
            
            # Check for markdown formatting
            assert '# ' in report or '## ' in report, "Markdown report should have headers"
            assert '|' in report or '-' in report, "Markdown should have tables or lists"
        
        # Verify detailed analysis inclusion based on parameter setting
        if include_detailed_analysis:
            if report_format == 'json':
                assert 'detailed_analysis' in report_data, "Should include detailed analysis section"
                detailed = report_data['detailed_analysis']
                assert isinstance(detailed, dict), "Detailed analysis should be dict"
            elif report_format == 'markdown':
                assert 'detailed' in report.lower() or 'analysis' in report.lower(), \
        ...
        if report_format == 'json':
            ...
            assert any(word in report.lower() for word in ['episode', 'seed', 'reproducibility']), \
...
class TestEnvironmentIntegration:
    ...
    def test_environment_seeding_integration(self, env_seed):
        ...
        assert isinstance(seed_result, list), f"env.seed should return list, got {type(seed_result)}"
        assert len(seed_result) == 1, f"env.seed should return single-element list"
        assert seed_result[0] == env_seed, f"Returned seed {seed_result[0]} should match input {env_seed}"
        ...
        assert isinstance(initial_position, (list, tuple)), "Agent position should be list or tuple"
        ...
        assert np.array_equal(obs1, obs2), f"Initial observations should match with seed {env_seed}"
        ...
        for i, ((obs1, r1, t1, tr1, info1), (obs2, r2, t2, tr2, info2)) in enumerate(zip(trajectory1, trajectory2)):
            ...
            assert info1['agent_xy'] == info2['agent_xy'], f"Position mismatch at step {i}"
        ...
        assert np.array_equal(obs1, obs3), "Seeding should be consistent across environment instances"
        assert info1['agent_xy'] == info3['agent_xy'], "Start positions should be consistent"
    ...
    def test_cross_session_reproducibility(self, session_seed):
        ...
        session1_trajectory = self._execute_seeded_session(session_seed, session_id="session1")
        ...
        session2_trajectory = self._execute_seeded_session(session_seed, session_id="session2")
        
        # Compare trajectories for exact equality
        assert len(session1_trajectory) == len(session2_trajectory), \
            f"Session trajectories have different lengths: {len(session1_trajectory)} != {len(session2_trajectory)}"
        
        for i, (step1, step2) in enumerate(zip(session1_trajectory, session2_trajectory)):
            ...
            assert np.array_equal(obs1, obs2), f"Observation mismatch at step {i} across sessions"
            assert reward1 == reward2, f"Reward mismatch at step {i}: {reward1} != {reward2}"
            assert terminated1 == terminated2, f"Terminated mismatch at step {i}: {terminated1} != {terminated2}"
            assert truncated1 == truncated2, f"Truncated mismatch at step {i}: {truncated1} != {truncated2}"
            assert info1['agent_xy'] == info2['agent_xy'], \
        ...
        if min_length > 0:
            for i in range(min(10, min_length)):  # Check first 10 steps
                ...
                assert np.array_equal(step1[0], step_config[0]), \
                    f"Config change affected seeded trajectory at step {i}"
                assert step1[4]['agent_xy'] == step_config[4]['agent_xy'], \
    ...
    def _execute_seeded_session(self, seed, session_id, max_steps=1000, grid_size=(32, 32)):
        ...
        try:
            ...
            for i, action in enumerate(deterministic_actions):
                ...
                obs, reward, terminated, truncated, info = env.step(action)
                trajectory.append((obs.copy(), reward, terminated, truncated, info.copy()))
...
class TestPerformance:
    ...
    @pytest.mark.parametrize('operation_type', ['seed_validation', 'rng_creation', 'seed_generation'])
    def test_seeding_performance_benchmarks(self, operation_type):
        ...
        assert avg_time < performance_target_seconds, \
        ...
        if operation_type == 'rng_creation':
            ...
            try:
                ...
                target_step_time = PERFORMANCE_TARGET_STEP_LATENCY_MS / 1000.0  # Convert to seconds
                
                assert avg_step_time < target_step_time, \
        ...
        if avg_time > performance_target_seconds * 0.8:  # If close to limit
            performance_report['recommendations'].append('Consider optimizing for better performance')
        
        if max_time > performance_target_seconds * 3:  # If max is high
            performance_report['recommendations'].append('Investigate occasional performance spikes')
...
class TestErrorHandling:
    ...
    @pytest.mark.parametrize('error_scenario', ['invalid_seed', 'corrupted_state', 'file_not_found', 'permission_error'])
    def test_seeding_error_handling(self, error_scenario):
        ...
        if error_scenario == 'invalid_seed':
            ...
            assert isinstance(error, ValidationError), f"Should raise ValidationError, got {type(error)}"
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as temp_file:
                temp_file.write('{"invalid": "json", "missing_required_fields"}')  # Invalid JSON structure
                temp_file_path = pathlib.Path(temp_file.name)
            
            try:
                # Should raise StateError or related exception
                with pytest.raises((StateError, json.JSONDecodeError, KeyError)) as exc_info:
                    load_seed_state(temp_file_path)
                
                error = exc_info.value
                error_message = str(error)
                assert len(error_message) > 0, "Corrupted state error should have message"
            ...
            error = exc_info.value
            error_message = str(error)
            assert 'exist' in error_message.lower() or 'found' in error_message.lower(), \
            ...
            try:
                restricted_path = pathlib.Path("/root/restricted_seed_state.json")  # Usually restricted on Unix
        ...
        try:
            ...
            assert isinstance(np_random, np.random.Generator), "RNG creation should work after error"
...
class TestEdgeCases:
    ...
    def test_seeding_with_edge_cases(self, edge_case_parameter):
        ...
        assert error_message == '', f"Edge case seed {edge_seed} should not have error: {error_message}"
        
        # Test boundary values at minimum and maximum seed ranges
        if edge_seed == SEED_MIN_VALUE:
            assert normalized_seed == SEED_MIN_VALUE, f"Minimum seed should normalize to itself"
        elif edge_seed == SEED_MAX_VALUE:
            assert normalized_seed == SEED_MAX_VALUE, f"Maximum seed should normalize to itself"
        
        # Verify mathematical operations handle edge cases correctly
        np_random, used_seed = create_seeded_rng(edge_seed)
        assert isinstance(np_random, np.random.Generator), f"RNG creation should work with edge case {edge_seed}"
        assert used_seed == edge_seed, f"Used seed {used_seed} should match edge case {edge_seed}"
        
        # Test that edge cases work with environment integration
        env = PlumeSearchEnv(grid_size=(16, 16))
        try:
            seed_result = env.seed(edge_seed)
            assert seed_result[0] == edge_seed, f"Environment seeding should work with edge case {edge_seed}"
            
            obs, info = env.reset(seed=edge_seed)
            assert obs is not None, f"Environment reset should work with edge case {edge_seed}"
            assert 'agent_xy' in info, f"Environment should provide position info with edge case {edge_seed}"
        ...
        if large_operation_result > SEED_MAX_VALUE:
            ...
            assert SEED_MIN_VALUE <= normalized_large <= SEED_MAX_VALUE, \
        ...
        assert val1 == val2, f"Edge case {edge_seed} should still be reproducible: {val1} != {val2}"
        ...
        np_random_mgr, used_seed_mgr = manager.seed(edge_seed, context_id=f'edge_case_{edge_seed}')
        assert isinstance(np_random_mgr, np.random.Generator), \
...
class TestScientificWorkflowCompliance:
    ...
    def test_seeding_scientific_workflow_compliance(self):
        ...
        assert reproduced_results['seed'] == baseline_results['seed'], "Reproduced seed should match"
        assert reproduced_results['final_position'] == baseline_results['final_position'], \
            "Reproduced final position should match"
        assert reproduced_results['total_steps'] == baseline_results['total_steps'], \
            "Reproduced step count should match"
        assert abs(reproduced_results['total_reward'] - baseline_results['total_reward']) < 1e-10, \
        ...
        for condition, results in experiment_results.items():
            for result in results:
                ...
                assert metadata.get('condition') == condition, "Metadata should include condition"
        
        # Verify compliance with FAIR data principles
        # Findable: Results should be identifiable
        all_episode_ids = [result['episode_id'] for results in experiment_results.values() for result in results]
        assert len(set(all_episode_ids)) == len(all_episode_ids), "All episode IDs should be unique"
        ...
        assert retrieved_episode is not None, "Should be able to retrieve recorded episodes"
        ...
        for condition_results in experiment_results.values():
            for result in condition_results:
                ...
                assert isinstance(result['seed'], int), "Seed should be standard integer"
                assert isinstance(result['total_reward'], (int, float)), "Reward should be numeric"
                assert isinstance(result['final_position'], (list, tuple)), "Position should be sequence"
        ...
        assert isinstance(scientific_report, (str, dict)), "Scientific report should be generated"
        ...
        for element in required_elements:
            if element not in report_data:
                ...
                assert any(element in key for key in report_data.keys()), \
        ...
        for condition, results in experiment_results.items():
            for result in results:
                ...
                assert episode['seed'] == result['seed'], "Episode seed should be traceable"
                
                # Metadata should preserve experimental context
                episode_metadata = episode.get('metadata', {})
                assert episode_metadata.get('condition') == condition, \
                    "Experimental condition should be preserved in metadata"

    def _execute_scientific_run(self, seed, condition, episode_length, reproducibility_tracker):
