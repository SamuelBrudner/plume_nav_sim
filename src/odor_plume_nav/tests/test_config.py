...
class TestHydraConfigurationComposition:
    ...
    def setup_hydra_environment(self, tmp_path):
        ...
        # Create base configuration with odor_plume_nav targets
        base_config = """
...
navigator:
  _target_: odor_plume_nav.core.SingleAgentNavigator
  position: [0.0, 0.0]
  orientation: 0.0
  speed: 0.0
  max_speed: 1.0
...
video_plume:
  _target_: odor_plume_nav.data.VideoPlumeEnvironment
...
system:
  _target_: odor_plume_nav.utils.SystemConfig
  random_seed: 42
  debug_mode: false
  output_dir: "outputs"
"""
        
        # Create environment-specific configuration with odor_plume_nav targets
        config_yaml = """
...
"""
    ...
    def test_basic_configuration_composition(self, setup_hydra_environment):
        ...
        with initialize_config_dir(config_dir=str(config_dir), version_base="1.1"):
            ...
            # Validate odor_plume_nav target references
            assert cfg.navigator._target_ == "odor_plume_nav.core.SingleAgentNavigator"
            assert cfg.video_plume._target_ == "odor_plume_nav.data.VideoPlumeEnvironment"
            assert cfg.system._target_ == "odor_plume_nav.utils.SystemConfig"
    ...
    def test_odor_plume_nav_target_validation(self, setup_hydra_environment):
        """Test that all _target_ fields reference odor_plume_nav modules."""
        config_dir = setup_hydra_environment
        
        with initialize_config_dir(config_dir=str(config_dir), version_base="1.1"):
            cfg = compose(config_name="config")
            
            # Validate all _target_ fields reference odor_plume_nav
            assert cfg.navigator._target_.startswith("odor_plume_nav.")
            assert cfg.video_plume._target_.startswith("odor_plume_nav.")
            assert cfg.system._target_.startswith("odor_plume_nav.")
...
class TestEnvironmentVariableInterpolation:
    ...
    def setup_environment_config(self, tmp_path):
        ...
        # Create configuration with environment variable interpolation and odor_plume_nav targets
        env_config = """
navigator:
  _target_: odor_plume_nav.core.SingleAgentNavigator
...
class TestPydanticSchemaIntegration:
    ...
    def test_pydantic_hydra_integration(self, tmp_path):
        ...
        # Create configuration that should pass Pydantic validation with odor_plume_nav targets
        valid_config = """
navigator:
  _target_: odor_plume_nav.core.SingleAgentNavigator
  position: [0.0, 0.0]
...
class TestConfigurationSecurity:
    ...
    def test_path_traversal_prevention(self, tmp_path):
        ...
        # Create legitimate configuration with odor_plume_nav targets
        safe_config = """
video_plume:
  _target_: odor_plume_nav.data.VideoPlumeEnvironment
...
"""
    ...
    def test_configuration_file_permission_validation(self, tmp_path):
        ...
        # Create configuration file with odor_plume_nav targets
        config_content = """
navigator:
  _target_: odor_plume_nav.core.SingleAgentNavigator
  max_speed: 2.0
"""
    ...
    def test_sensitive_parameter_protection(self, tmp_path):
        ...
        # Configuration with potentially sensitive parameters and odor_plume_nav targets
        sensitive_config = """
navigator:
  _target_: odor_plume_nav.core.SingleAgentNavigator
  max_speed: 2.0
...
database:
  _target_: odor_plume_nav.db.DatabaseConfig
  password: "not_a_real_password"
  
api:
  _target_: odor_plume_nav.api.APIConfig
...
class TestConfigurationPerformance:
    ...
    def test_configuration_loading_performance(self, tmp_path):
        ...
        # Create complex configuration for performance testing with odor_plume_nav targets
        complex_config = """
defaults:
  - _self_
...
multi_agent:
  _target_: odor_plume_nav.core.MultiAgentController
  positions: [[0.0, 0.0], [1.0, 1.0], [2.0, 2.0]]
  orientations: [0.0, 90.0, 180.0]
  speeds: [1.0, 1.2, 0.8]
  max_speeds: [2.0, 2.0, 2.0]
...
"""
    ...
    def test_large_configuration_performance(self, tmp_path):
        ...
        # Generate large configuration with many parameters and odor_plume_nav targets
        ...
        for i in range(50):
            large_config_lines.extend([
                f"navigator_{i}:",
                f"  _target_: odor_plume_nav.core.Navigator{i % 3}",
        ...
        for i in range(30):
            large_config_lines.extend([
                f"video_plume_{i}:",
                f"  _target_: odor_plume_nav.data.VideoPlumeEnvironment{i % 2}",
...
class TestConfigurationEvolutionAndCompatibility:
    ...
    def test_backward_compatibility_with_legacy_config(self, tmp_path):
        ...
        # Legacy configuration format (without Hydra features) with odor_plume_nav targets
        legacy_config = """
...
"""
    ...
    def test_schema_evolution_with_optional_fields(self, tmp_path):
        ...
        # Configuration missing some optional fields with odor_plume_nav targets
        minimal_config = """
navigator:
  _target_: odor_plume_nav.core.SingleAgentNavigator
  max_speed: 2.0
...
"""
    ...
    def test_configuration_migration_patterns(self, tmp_path):
        ...
        def migrate_config(old_cfg):
            """Migrate old configuration format to new format with odor_plume_nav targets."""
            new_cfg = {
                "navigator": {
                    "_target_": "odor_plume_nav.core.SingleAgentNavigator",
        ...
        # Validate migrated config references odor_plume_nav
        assert new_config["navigator"]["_target_"] == "odor_plume_nav.core.SingleAgentNavigator"
        assert new_config["video_plume"]["_target_"] == "odor_plume_nav.data.VideoPlumeEnvironment"
