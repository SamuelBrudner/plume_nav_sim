...
of the technical specification. Integration tests ensure that the unified odor plume
navigation architecture functions cohesively across all system domains including configuration
...
- src/odor_plume_nav/api/navigation.py: Public API surface validation
- src/odor_plume_nav/cli/main.py: Command-line interface integration
- src/odor_plume_nav/core/navigator.py: Core domain logic protocols
- src/odor_plume_nav/data/video_plume.py: Video processing adapters
- src/odor_plume_nav/config/models.py: Configuration validation
- src/odor_plume_nav/db/session.py: Database persistence infrastructure
...
from odor_plume_nav.cli.main import main as cli_main, cli, CLIError
from odor_plume_nav.core.navigator import Navigator, NavigatorProtocol
...
from odor_plume_nav.db.session import (
...
class TestEndToEndSimulationWorkflows(IntegrationTestBase):
    ...
    def test_single_agent_simulation_workflow(self, sample_config, mock_video_capture, mock_video_file):
        ...
        positions, orientations, readings = run_plume_simulation(
            ...
            num_steps=sample_config['simulation']['max_steps'],
            dt=sample_config['simulation']['step_size'],
            record_trajectories=True
        )
        execution_time = time.time() - start_time
        
        # Validate performance requirements (30+ FPS capability)
        expected_max_time = sample_config['simulation']['max_steps'] / 30.0  # 30 FPS minimum
        ...
        expected_steps = sample_config['simulation']['max_steps'] + 1
    ...
    def test_simulation_with_hydra_configuration(self, hydra_config, mock_video_capture, mock_video_file):
        ...
        expected_steps = hydra_config.simulation.max_steps + 1
...
class TestKedroIntegration(IntegrationTestBase):
    ...
    def test_kedro_data_catalog_integration(self, sample_config):
        ...
        assert sim_params['max_steps'] == sample_config['simulation']['max_steps']
...
class TestCLIIntegration(IntegrationTestBase):
    ...
    def test_cli_command_execution_workflow(self, sample_config, mock_video_capture, mock_video_file):
        ...
        assert "Odor Plume Navigation" in result.output
        ...
        try:
            ...
            start_time = time.time()
            with patch('odor_plume_nav.cli.main.get_cli_config') as mock_config:
    ...
    def test_cli_batch_processing_mode(self, sample_config, mock_video_capture, mock_video_file):
        ...
        with patch('odor_plume_nav.cli.main.get_cli_config') as mock_config:
            ...
            with patch('odor_plume_nav.cli.main.create_navigator') as mock_nav, \
                 patch('odor_plume_nav.cli.main.create_video_plume') as mock_plume, \
                 patch('odor_plume_nav.cli.main.run_plume_simulation') as mock_sim:
...
class TestConfigurationSystemIntegration(IntegrationTestBase):
    ...
    def test_hierarchical_configuration_composition(self, sample_config):
        ...
        base_config = {
            ...
            'simulation': {'max_steps': 1000, 'step_size': 0.1}
        }
        ...
        assert merged_config.simulation.max_steps == 1000  # Base value preserved
    ...
    def test_configuration_validation_across_components(self, sample_config):
        ...
        assert sim_config.max_steps == sample_config['simulation']['max_steps']
        ...
        with pytest.raises(ValueError):
            SimulationConfig(max_steps=0)  # Invalid zero steps
...
class TestDatabaseSessionIntegration(IntegrationTestBase):
    ...
    def test_simulation_with_database_integration(self, sample_config, mock_video_capture, mock_video_file):
        ...
        with get_session() as session:
            ...
            if session:
                from odor_plume_nav.db.session import PersistenceHooks
...
class TestVisualizationExportWorkflows(IntegrationTestBase):
    ...
    def test_visualization_export_integration(self, sample_config, mock_video_capture, mock_video_file):
        ...
        try:
            from odor_plume_nav.utils.visualization import visualize_simulation_results
    ...
    def test_animation_export_workflow(self, sample_config, mock_video_capture, mock_video_file):
        ...
        try:
            from odor_plume_nav.utils.visualization import export_animation
...
class TestWorkflowOrchestrationIntegration(IntegrationTestBase):
    ...
    def test_dvc_pipeline_integration_patterns(self, sample_config):
        ...
        assert loaded_params['simulation']['max_steps'] == sample_config['simulation']['max_steps']
