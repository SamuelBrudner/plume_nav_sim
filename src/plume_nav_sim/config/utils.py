...
def create_default_config() -> Dict[str, Any]:
    ...
    """
    nav = NavigatorConfig(mode="auto").model_dump(mode="json")
    nav["mode"] = "auto"
    return {
        "navigator": nav,
        "video_plume": VideoPlumeConfig(video_path="default.mp4").model_dump(mode="json"),
        "simulation": SimulationConfig().model_dump(mode="json"),
    }
...
def resolve_env_value(value: str, default: str = "") -> str:
    """Resolve environment variable references in a string without type conversion."""
    if not isinstance(value, str):
        return str(value)
