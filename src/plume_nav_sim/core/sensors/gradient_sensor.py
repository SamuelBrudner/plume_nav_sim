...
class GradientSensorConfig:
    ...
    def __post_init__(self):
        ...
        if any(r <= 0 for r in self.spatial_resolution):
            raise ValueError(f"spatial_resolution components must be positive, got {self.spatial_resolution}")
...
class GradientResult:
    ...
    def __post_init__(self):
        ...
        shapes = [self.gradient.shape[0], self.magnitude.shape[0], 
                 self.direction.shape[0], self.confidence.shape[0]]
        if len(set(shapes)) > 1:
...
class GradientSensor(BaseSensor):
    ...
    def _precompute_fd_coefficients(self) -> Dict[int, Dict[str, np.ndarray]]:
        ...
        central_coeffs = {
            ...
            6: np.array([-1/60, 3/20, -3/4, 0, 3/4, -3/20, 1/60]),  # 6th order
            8: np.array([1/280, -4/105, 1/5, -4/5, 0, 4/5, -1/5, 4/105, -1/280])  # 8th order
        }
        ...
        forward_coeffs = {
            ...
            6: np.array([-147/60, 6, -15/2, 20/3, -15/4, 6/5, -1/6]),  # 6th order
            8: np.array([-761/280, 8, -14, 56/3, -35/2, 56/5, -14/3, 8/7, -1/8])  # 8th order
        }
    ...
    def _sample_concentration_at_offset(
        ...
        if hasattr(plume_state, 'concentration_at'):
            ...
            try:
                return np.array([plume_state[int(pos[1]), int(pos[0])] 
    ...
    def _compute_adaptive_step_size(
        ...
        test_offsets = [
            (base_dx, 0), (-base_dx, 0), (0, base_dy), (0, -base_dy),
            (base_dx, base_dy), (-base_dx, -base_dy)
        ]
        
        concentration_samples = []
        for offset in test_offsets:
            samples = self._sample_concentration_at_offset(plume_state, positions, offset)
            concentration_samples.append(samples)
        
        concentration_samples = np.array(concentration_samples)  # Shape: (n_offsets, n_agents)
        
        # Compute local variation metrics
        center_concentration = self._sample_concentration_at_offset(plume_state, positions, (0, 0))
        variation = np.std(concentration_samples - center_concentration[np.newaxis, :], axis=0)
    ...
    def _compute_finite_difference_gradient(
        ...
        if self.config.adaptive_step_size:
            dx_steps, dy_steps = self._compute_adaptive_step_size(plume_state, positions)
        else:
        ...
        methods_used = self._select_fd_methods(plume_state, positions, dx_steps, dy_steps)
        ...
        for method in set(methods_used):
            ...
            # Store metadata
            for i, (pos_idx, _) in enumerate(zip(np.where(method_mask)[0], method_positions)):
                ...
                edge_correction = self._detect_edge_effects(plume_state, method_positions[i:i+1])
    ...
    def _compute_directional_derivative(
        ...
        for i, coeff in enumerate(coefficients):
            ...
            concentrations = np.array([
                self._sample_concentration_at_offset(plume_state, pos.reshape(1, -1), offset[:2])[0]
    ...
    def _compute_gradient_metadata(
        ...
        confidence[magnitudes < self.config.noise_threshold] *= 0.3  # Low confidence for weak gradients
    ...
    def detect(self, plume_state: Any, positions: np.ndarray) -> np.ndarray:
        ...
        detection_threshold = self.config.noise_threshold * 10  # Scale for detection sensitivity
    ...
    def _compute_gradient_raw(self, plume_state: Any, positions: np.ndarray) -> np.ndarray:
        ...
        if positions.ndim != 2 or positions.shape[1] != 2:
            raise ValueError(f"positions must have shape (n_agents, 2), got {positions.shape}")
        ...
        if self.config.enable_caching:
            ...
            if (cache_key in self._gradient_cache and 
                time.time() - self._cache_timestamps[cache_key] < self.config.cache_ttl_seconds):
        ...
        try:
            ...
            if self._logger and self._performance_metrics['total_computations'] % 100 == 0:
                ...
                self._logger.debug(
                    ...
                    avg_computation_time_ms=np.mean(recent_times) if recent_times else 0,
                    cache_hit_rate=(self._performance_metrics['cache_hits'] / 
                                  max(1, self._performance_metrics['cache_hits'] + 
    ...
    def compute_gradient_with_metadata(
        ...
        gradients, computation_metadata = self._compute_finite_difference_gradient(plume_state, positions)
            ...
            if hasattr(self.config, key):
                ...
                if key == 'method':
                    if isinstance(value, str):
                        ...
                        raise ValueError(f"method must be FiniteDifferenceMethod or string, got {type(value)}")
                ...
                if key == 'spatial_resolution':
                    if not (isinstance(value, (tuple, list)) and len(value) == 2):
                        raise ValueError(f"spatial_resolution must be 2-element tuple, got {value}")
                    if any(r <= 0 for r in value):
                        raise ValueError(f"spatial_resolution components must be positive, got {value}")
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        if self._performance_metrics['computation_times']:
            ...
            metrics.update({
                ...
                'performance_violations': int(np.sum(times > 0.2))  # >0.2ms per agent
            })
        ...
        if self.config.enable_caching:
            ...
            if total_requests > 0:
                metrics.update({
                    ...
                    'cache_size': len(self._gradient_cache) if self._gradient_cache else 0,
                    'cache_efficiency': self._performance_metrics['cache_hits'] / max(1, total_requests)
                })
        ...
        metrics.update({
            ...
            'error_rate': (self._performance_metrics['numerical_warnings'] / 
