...
try:
    ...
    warnings.warn("Simulation infrastructure not available. CLI functionality limited.", ImportWarning)
...
def run_agent_comparison(
    ...
    **kwargs: Any
    ...
    if not SIMULATION_AVAILABLE:
        raise RuntimeError("Simulation infrastructure not available for agent comparison")
    ...
    unavailable_agents = [name for name in agents if not AGENT_REGISTRY[name]["available"]]
        ...
        try:
            ...
            if SIMULATION_AVAILABLE:
                ...
                agent_metrics = {
                    ...
                    "final_reward": results.rewards_history[-1] if results.rewards_history else 0.0,
                ...
                memory_category = "memory_enabled" if memory_enabled else "memory_disabled"
                comparison_results["memory_vs_memoryless"][memory_category][agent_type] = agent_metrics
    ...
    successful_results = {
        name: metrics for name, metrics in comparison_results["agent_results"].items()
    ...
    if successful_results:
        ...
        memory_enabled_agents = [
            metrics for metrics in successful_results.values()
    ...
    logger.info(
        ...
        best_agent=comparison_results["comparative_analysis"].get("best_performance", {}).get("agent_type", "N/A")
    )
...
def run_single_agent(
    ...
    **kwargs: Any
    ...
    try:
        ...
        start_time = time.perf_counter()
        agent = create_agent(agent_type, config=final_config, memory_enabled=memory_enabled)
        ...
        if SIMULATION_AVAILABLE:
            ...
            simulation_results = {
                ...
                "final_reward": sim_results.rewards_history[-1] if sim_results.rewards_history else 0.0,
                "average_reward": np.mean(sim_results.rewards_history) if sim_results.rewards_history else 0.0,
        ...
        results = {
            "agent_info": {
                "agent_type": agent_type,
                "memory_enabled": getattr(agent, '_memory_enabled', final_config.get('memory_enabled', False)),
                "configuration": final_config,
                "creation_time_s": agent_creation_time,
                "protocol_compliant": PROTOCOLS_AVAILABLE and isinstance(agent, NavigatorProtocol)
            },
            "simulation_results": simulation_results,
            "performance_analysis": {
                "step_time_requirement_met": (
                    simulation_results.get("performance_metrics", {}).get("average_step_time_ms", 0) < 10.0
                    if simulation_results else True
                ),
                "memory_overhead_acceptable": True,  # Would be calculated by agent
                "real_time_capable": (
                    simulation_results.get("performance_metrics", {}).get("average_fps", 0) >= 30.0
                    if simulation_results else True
                )
            },
            "metadata": {
        ...
        logger.info(
            ...
            execution_time_s=simulation_results.get("execution_time_s", agent_creation_time)
        )
...
def create_agent_from_cli(args: argparse.Namespace) -> Tuple[NavigatorProtocol, Dict[str, Any]]:
    """
    ...
    if hasattr(args, 'config_file') and args.config_file:
        if YAML_AVAILABLE:
            ...
            raise RuntimeError("YAML support not available for configuration file loading")
...
def validate_agent_protocol(agent: Any) -> Dict[str, bool]:
    ...
    validation_results = {
        ...
        "has_step_method": hasattr(agent, 'step') and callable(getattr(agent, 'step', None)),
        "has_reset_method": hasattr(agent, 'reset') and callable(getattr(agent, 'reset', None)),
        "has_sample_odor": hasattr(agent, 'sample_odor') and callable(getattr(agent, 'sample_odor', None)),
        "memory_interface_valid": True,  # Default assumption
        "performance_compliant": True,   # Would need actual performance testing
        "protocol_instance": PROTOCOLS_AVAILABLE and isinstance(agent, NavigatorProtocol)
    }
    ...
    if hasattr(agent, '_memory_enabled') and getattr(agent, '_memory_enabled', False):
        validation_results["memory_interface_valid"] = (
            hasattr(agent, 'load_memory') and callable(getattr(agent, 'load_memory', None)) and
            hasattr(agent, 'save_memory') and callable(getattr(agent, 'save_memory', None))
        )
    ...
    try:
        ...
        if validation_results["has_num_agents"]:
            ...
            validation_results["num_agents_type_valid"] = isinstance(num_agents, int) and num_agents > 0
    except Exception as e:
...
def test_memory_compatibility(agent_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        test_results["performance_impact"] = {
            ...
            "creation_overhead_ratio": memory_creation_time / no_memory_creation_time if no_memory_creation_time > 0 else 1.0
        }
        ...
        test_results["compatible"] = (
            test_results["memory_enabled_creation"] and
            test_results["memory_disabled_creation"] and
            test_results["protocol_compliance"]["memory_enabled_compliant"] and
            test_results["protocol_compliance"]["memory_disabled_compliant"]
        )
...
def benchmark_agent_performance(
    ...
    if agent_types is None:
        agent_types = [name for name, info in AGENT_REGISTRY.items() if info["available"]]
    ...
    for agent_type in agent_types:
            ...
            for trial in range(num_trials):
                try:
                    ...
                    trial_result = {
                        ...
                        "estimated_fps": 1000.0 / np.mean(step_times) if step_times else 0
                    }
                    
                    scenario_metrics["trial_results"].append(trial_result)
                    
                except Exception as e:
                    logger.warning(f"Trial {trial} failed for {agent_type} in {scenario}: {e}")
            
            # Calculate scenario statistics
            if scenario_metrics["trial_results"]:
                trials = scenario_metrics["trial_results"]
                scenario_metrics["statistics"] = {
                    "mean_step_time_ms": np.mean([t["avg_step_time_ms"] for t in trials]),
                    "std_step_time_ms": np.std([t["avg_step_time_ms"] for t in trials]),
                    "mean_fps": np.mean([t["estimated_fps"] for t in trials]),
                    "violation_rate": np.mean([t["step_time_violations"] for t in trials]) / min(100, num_steps),
        ...
        if all_step_times:
            agent_metrics["aggregate_metrics"] = {
                ...
                "consistency_score": 1.0 / (1.0 + np.std(all_step_times))  # Higher = more consistent
            }
    ...
    benchmark_results["requirement_compliance"] = {
        "agents_meeting_step_time": [
            agent for agent, metrics in benchmark_results["agent_performance"].items()
    ...
    logger.info(
        ...
        agents_meeting_requirements=len(benchmark_results["requirement_compliance"]["agents_meeting_step_time"])
    )
...
Examples:
  ...
  python -m plume_nav_sim.examples.agents --agent infotaxis --config configs/infotaxis.yaml
    ...
    mode_group.add_argument(
        '--validate', 
        action='store_true',
        help='Validate agent protocol compliance'
    )
    ...
    agent_group.add_argument('--gradient-threshold', type=float, help='Reactive agent gradient threshold')
    agent_group.add_argument('--turning-rate', type=float, help='Agent turning rate (degrees/s)')
    agent_group.add_argument('--exploration-bonus', type=float, help='Infotaxis exploration bonus')
    agent_group.add_argument('--belief-decay-rate', type=float, help='Infotaxis belief decay rate')
    agent_group.add_argument('--casting-angle', type=float, help='Casting agent zigzag angle')
    agent_group.add_argument('--casting-distance', type=float, help='Casting agent segment distance')
    
    # Plume configuration
    plume_group = parser.add_argument_group('Plume configuration')
    plume_group.add_argument('--plume-type', choices=['gaussian', 'turbulent', 'video'], default='gaussian')
    plume_group.add_argument('--source-strength', type=float, default=1000.0)
    plume_group.add_argument('--source-position', nargs=2, type=float, default=[50.0, 50.0])
    ...
    try:
        ...
        if parsed_args.agent:
            ...
            if results['simulation_results']:
                print(f"  Total reward: {results['simulation_results']['total_reward']:.2f}")
                print(f"  Execution time: {results['simulation_results']['execution_time_s']:.2f}s")
            ...
            if 'best_performance' in results['comparative_analysis']:
                ...
                print(f"  Best performer: {best['agent_type']} (reward: {best['total_reward']:.2f})")
            ...
            compliant_agents = results['requirement_compliance']['agents_meeting_step_time']
...
def _generate_comparison_plots(results: Dict[str, Any], output_dir: Path) -> None:
    ...
    for agent_name, metrics in results["agent_results"].items():
        if "error" not in metrics:
            ...
            memory_status.append("Memory" if metrics.get("memory_enabled", False) else "No Memory")
