...
class CastingAgentConfig:
    ...
    sensor_configs: List[Dict[str, Any]] = field(default_factory=lambda: [
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 < self.casting_angle <= 90:
            raise ValueError(f"casting_angle must be in (0, 90], got {self.casting_angle}")
        
        # Validate duration parameters
        if self.surge_duration <= 0:
            raise ValueError(f"surge_duration must be positive, got {self.surge_duration}")
        if self.cast_duration <= 0:
            raise ValueError(f"cast_duration must be positive, got {self.cast_duration}")
        if self.loop_duration <= 0:
            raise ValueError(f"loop_duration must be positive, got {self.loop_duration}")
        ...
        if self.detection_memory_length < 1:
            raise ValueError(f"detection_memory_length must be >= 1, got {self.detection_memory_length}")
        if not 0 < self.memory_fade_rate <= 1:
            raise ValueError(f"memory_fade_rate must be in (0, 1], got {self.memory_fade_rate}")
        
        # Validate noise parameters
        if self.noise_magnitude < 0:
            raise ValueError(f"noise_magnitude must be non-negative, got {self.noise_magnitude}")
...
class BinarySensor:
    ...
    def detect(self, plume_state: Any, positions: np.ndarray) -> np.ndarray:
        ...
        concentrations = np.random.rand(len(positions)) if positions.ndim > 1 else np.random.rand(1)
...
class CastingAgent(SingleAgentController):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.detection_history: List[Tuple[float, np.ndarray, bool]] = []  # (time, position, detected)
    ...
    @classmethod
    def from_config(cls, config: Union[CastingAgentConfig, Dict[str, Any], DictConfig]) -> 'CastingAgent':
        """
        ...
        non_constructor_params = {
            'detection_memory_length', 'wind_estimation_method', 'memory_fade_rate',
            'adaptive_casting', 'biological_noise', 'noise_magnitude',
            'performance_monitoring', 'log_state_transitions'
        }
        
        constructor_params = {k: v for k, v in config_dict.items() 
                            if k not in non_constructor_params}
    ...
    def step(self, env_array: np.ndarray, dt: float = 1.0) -> None:
        ...
        try:
            ...
            if old_state != self.state and self._logger:
                ...
                if hasattr(self, '_logger') and getattr(self, 'log_state_transitions', False):
                    self._logger.info(
    ...
    def _update_detection_history(self, detection_event: bool) -> None:
        ...
        self.detection_history.append((self.total_time, current_position, detection_event))
    ...
    def _set_movement_target(self, target_orientation: float, target_speed: float) -> None:
        """
    ...
    def load_memory(self, memory_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            ...
            if 'detection_history' in memory_data:
                self.detection_history = memory_data['detection_history'][-100:]  # Limit size
        ...
        try:
            memory_data = {
                ...
                'last_detection_position': (
                    ...
                    if self.last_detection_position is not None else None
                ),
    ...
    def get_casting_metrics(self) -> Dict[str, Any]:
        ...
        casting_metrics = {
            ...
            'successful_surgess': self.successful_surgess,
            'detection_rate': (
                self.total_detections / max(1, self.total_time) if self.total_time > 0 else 0.0
            ),
            'estimated_wind_direction': self.estimated_wind_direction,
            'wind_confidence': self.wind_confidence,
            'memory_enabled': self.memory_enabled,
        ...
        if len(self.detection_history) > 1:
            ...
            if len(positions) >= 2:
                ...
                casting_metrics['path_efficiency'] = (
                    straight_distance / max(total_distance, 0.001)
                )
