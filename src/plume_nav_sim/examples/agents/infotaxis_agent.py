...
try:
    from ...core.sensors import ConcentrationSensor, BinarySensor, create_sensor_from_config
...
class InfotaxisAgent:
    ...
    def _process_memory_config(self, config: Optional[Union[MemoryConfig, Dict[str, Any]]]) -> MemoryConfig:
        ...
        if config is None:
            ...
            raise TypeError(f"memory_config must be MemoryConfig or dict, got {type(config)}")
    
    def _process_inference_config(self, config: Optional[Union[InferenceConfig, Dict[str, Any]]]) -> InferenceConfig:
        ...
        if config is None:
            ...
            raise TypeError(f"inference_config must be InferenceConfig or dict, got {type(config)}")
    
    def _process_action_config(self, config: Optional[Union[ActionConfig, Dict[str, Any]]]) -> ActionConfig:
        ...
        if config is None:
            ...
            raise TypeError(f"action_config must be ActionConfig or dict, got {type(config)}")
    
    def _initialize_sensors(self, sensor_configs: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """Initialize sensors for multi-modal observation."""
        sensors = {}
        ...
        try:
            ...
            if 'concentration' in self.sensors:
                ...
                if hasattr(sensor, 'measure'):
                    ...
                    observations['concentration'] = float(conc[0] if isinstance(conc, np.ndarray) else conc)
                else:
            ...
            if 'binary' in self.sensors:
                ...
                if hasattr(sensor, 'detect'):
                    ...
                    observations['detection'] = bool(detection[0] if isinstance(detection, np.ndarray) else detection)
                else:
                    # Fallback threshold detection
                    threshold = getattr(sensor, 'config', {}).get('threshold', 0.1)
                    observations['detection'] = observations.get('concentration', 0.0) > threshold
    ...
    def _calculate_likelihood(self, observations: Dict[str, Any]) -> np.ndarray:
        ...
        try:
                    ...
                    if 'concentration' in observations:
                        ...
                        if SCIPY_AVAILABLE and stats:
                            ...
                            likelihood[i, j] *= stats.norm.pdf(
                                obs_conc,
                                loc=expected_conc,
                                scale=np.sqrt(self.inference_config.likelihood_variance)
                            )
                        else:
                            # Simplified likelihood
                            diff = abs(obs_conc - expected_conc)
                            likelihood[i, j] *= np.exp(-diff**2 / (2 * self.inference_config.likelihood_variance))
            ...
            likelihood = np.maximum(likelihood, self.inference_config.numerical_stability_threshold)
    ...
    def _expected_concentration(self, distance: float) -> float:
        ...
        base_concentration = 1.0 / (1.0 + distance**2 / self.inference_config.diffusion_coefficient)
    ...
    def _detection_probability(self, concentration: float) -> float:
        ...
        if concentration > threshold:
            ...
            return (1.0 - false_negative) * (1.0 / (1.0 + np.exp(-10 * (concentration - threshold))))
        else:
    ...
    def _calculate_entropy(self) -> float:
        ...
        try:
            ...
            if self.inference_config.entropy_calculation_method == 'shannon':
                return -np.sum(p * np.log2(p + self.inference_config.numerical_stability_threshold))
    ...
    def _select_action(self, dt: float) -> Dict[str, Any]:
        ...
        try:
            ...
            for action in actions:
                ...
                boundary_penalty = self._calculate_boundary_penalty(action['target_position'])
    ...
    def _generate_candidate_actions(self, current_pos: np.ndarray, current_orientation: float) -> List[Dict[str, Any]]:
    ...
    def _calculate_expected_information_gain(self, action: Dict[str, Any]) -> float:
        ...
        try:
            ...
            max_prob_idx = np.unravel_index(np.argmax(self.source_probability), self.source_probability.shape)
            max_prob_pos = np.array([self.grid_x[max_prob_idx[1]], self.grid_y[max_prob_idx[0]]])
            ...
            uncertainty_bonus = current_entropy / np.log2(self.grid_width * self.grid_height)
    ...
    def _calculate_movement_cost(self, action: Dict[str, Any], current_pos: np.ndarray) -> float:
    ...
    def _execute_action(self, action: Dict[str, Any], dt: float) -> None:
        ...
        try:
            ...
            self._position[0, 0] = np.clip(self._position[0, 0], 0, self.environment_bounds[0])
            self._position[0, 1] = np.clip(self._position[0, 1], 0, self.environment_bounds[1])
    ...
    def _update_memory(self, observations: Dict[str, Any], action: Dict[str, Any], dt: float) -> None:
        ...
        try:
            if 'concentration' in self.sensors:
                ...
                if hasattr(sensor, 'measure'):
                    ...
                    return float(result[0] if isinstance(result, np.ndarray) else result)
    ...
    def sample_multiple_sensors(
        ...
        try:
            ...
            for i in range(num_sensors):
                ...
                sensor_x = agent_pos[0] + sensor_distance * np.cos(np.radians(global_angle))
                sensor_y = agent_pos[1] + sensor_distance * np.sin(np.radians(global_angle))
    ...
    def load_memory(self, memory_state: Dict[str, Any]) -> None:
        ...
        try:
            if 'error' in memory_state:
                if self.logger:
                    self.logger.warning(f"Cannot load memory state with error: {memory_state['error']}")
                return
            ...
            if 'belief_history' in memory_state:
                self.belief_history = [np.array(belief) for belief in memory_state['belief_history']]
    ...
    def _get_max_probability_location(self) -> List[float]:
        ...
        try:
            max_idx = np.unravel_index(np.argmax(self.source_probability), self.source_probability.shape)
        ...
        try:
            ...
            stats = {
                'memory_usage': {
                    ...
                    'memory_utilization': len(self.observation_history) / self.memory_config.max_history_length
                },
                'belief_state': {
                    ...
                    'grid_coverage': {
                        'non_zero_cells': int(np.sum(self.source_probability > self.inference_config.numerical_stability_threshold)),
                        'total_cells': self.grid_width * self.grid_height,
                        'coverage_percentage': float(
                            np.sum(self.source_probability > self.inference_config.numerical_stability_threshold) / 
                ...
                'performance_metrics': {
                    **self.performance_metrics,
                    'average_inference_time_ms': (
                        ...
                        if self.performance_metrics['inference_times'] else 0.0
                    ),
                    'average_action_time_ms': (
                        np.mean(self.performance_metrics['action_selection_times'])
                        if self.performance_metrics['action_selection_times'] else 0.0
                    ),
                    'update_success_rate': (
                        self.performance_metrics['successful_updates'] / 
                        max(1, self.performance_metrics['successful_updates'] + self.performance_metrics['failed_updates'])
                    )
                },
                'sensor_statistics': {
                    ...
                    'average_concentration': (
                        np.mean([obs['concentration'] for obs in self.observation_history if 'concentration' in obs])
                        if self.observation_history else 0.0
                    )
                },
                'trajectory_analysis': {
    ...
    def _calculate_exploration_efficiency(self) -> float:
        ...
        try:
            ...
            visited_positions = np.array([obs['position'] for obs in self.observation_history])
            
            # Discretize visited positions to grid
            grid_visits = np.zeros((self.grid_height, self.grid_width))
            
            for pos in visited_positions:
                grid_x = int(np.clip(pos[0] / self.grid_resolution, 0, self.grid_width - 1))
                grid_y = int(np.clip(pos[1] / self.grid_resolution, 0, self.grid_height - 1))
    ...
    def compute_additional_obs(self, base_obs: dict) -> dict:
        ...
        try:
            additional_obs = {
                ...
                'memory_utilization': len(self.observation_history) / self.memory_config.max_history_length,
                'total_information_gain': self.performance_metrics['total_information_gain'],
            ...
            if self.observation_history:
                ...
                additional_obs.update({
                    ...
                    'recent_gradient_magnitude': np.mean([
                        np.linalg.norm(obs.get('gradient', [0, 0])) for obs in recent_obs
                    ])
                })
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    ...
                    exploration_coverage=final_stats['trajectory_analysis']['exploration_efficiency'],
                    max_posterior_probability=final_stats['belief_state']['max_posterior_probability'],
                    estimated_source_location=final_stats['belief_state']['max_probability_location'],
                    update_success_rate=final_stats['performance_metrics']['update_success_rate']
                )
