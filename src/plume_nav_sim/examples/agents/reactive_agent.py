...
try:
    ...
    logger.debug("NavigatorProtocol and SingleAgentController imported successfully")
except ImportError as e:
    logger.exception("Missing navigation protocols for ReactiveAgent: %s", e)
    raise ImportError(
        "ReactiveAgent requires NavigatorProtocol and SingleAgentController."
    ) from e
...
class ReactiveAgentConfig:
    ...
    def __post_init__(self):
        ...
        if self.gradient_threshold < 0:
            raise ValueError(
                f"gradient_threshold must be non-negative, got {self.gradient_threshold}"
            )
...
class ReactiveAgent:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self._gradient_threshold < 0:
            raise ValueError(
                f"gradient_threshold must be non-negative, got {gradient_threshold}"
            )
    ...
    def step_size(self, value: float) -> None:
        ...
        if self._logger:
            self._logger.debug(
                "Updated step_size parameter", old_value=old_value, new_value=value
            )
    ...
    def turning_rate(self, value: float) -> None:
        ...
        if self._logger:
            self._logger.debug(
                "Updated turning_rate parameter", old_value=old_value, new_value=value
            )
    ...
    def step(self, env_array: np.ndarray, dt: float = 1.0) -> None:
        ...
        try:
            ...
            gradient_start_time = (
                time.perf_counter() if self._performance_monitoring else None
            )

            try:
                # Ensure position is in correct format for gradient sensor
                position_for_sensor = current_position.reshape(1, -1)  # Shape: (1, 2)
                gradient = self._gradient_sensor.compute_gradient(
                    env_array, position_for_sensor
                )
            ...
            if self._performance_monitoring and gradient_start_time:
                gradient_computation_time = (
                    time.perf_counter() - gradient_start_time
                ) * 1000
                self._performance_metrics["gradient_computation_times"].append(
                    gradient_computation_time
                )
            ...
            previous_position = (
                current_position.copy() if self._performance_monitoring else None
            )
            ...
            if gradient_magnitude > self._gradient_threshold:
                ...
                new_speed = min(
                    gradient_magnitude * self._step_size, float(self.max_speeds[0])
                )

                # Update controller state for movement
                self._controller.reset(
                    speed=new_speed, angular_velocity=new_angular_velocity
                )
            ...
            if self._performance_monitoring:
                ...
                if previous_position is not None:
                    distance_moved = np.linalg.norm(
                        self.positions[0] - previous_position
                    )
                    self._performance_metrics[
            ...
            if (
                self._logger
                and self._performance_monitoring
                and self._performance_metrics["total_steps"] % 100 == 0
            ):
    ...
    def compute_additional_obs(self, base_obs: dict) -> dict:
        ...
        if (
            ...
            and self._performance_metrics["total_steps"] > 0
        ):
            additional_obs.update(
                {
                    "total_steps": self._performance_metrics["total_steps"],
                    "average_gradient_magnitude": self._performance_metrics[
                        "average_gradient_magnitude"
                    ],
                    "total_distance_traveled": self._performance_metrics[
                        "total_distance_traveled"
                    ],
                }
            )
    ...
    def compute_extra_reward(self, base_reward: float, info: dict) -> float:
        ...
        if (
            self._performance_monitoring
            and self._performance_metrics["total_steps"] > 0
            and self._performance_metrics["movement_decisions"] > 0
        ):
    ...
    def on_episode_end(self, final_info: dict) -> None:
        ...
        if (
            ...
            agent_metrics.update(
                {
                    "movement_ratio": self._performance_metrics["movement_decisions"]
                    / total_steps,
                    "avg_gradient_magnitude": self._performance_metrics[
                        "average_gradient_magnitude"
                    ],
                    "total_distance_traveled": self._performance_metrics[
                        "total_distance_traveled"
                    ],
                    "avg_distance_per_step": self._performance_metrics[
                        "total_distance_traveled"
                    ]
                    / total_steps,
                }
            ...
            if self._performance_metrics["gradient_computation_times"]:
                grad_times = np.array(
                    self._performance_metrics["gradient_computation_times"]
                )
                agent_metrics.update(
                    {
    ...
    @classmethod
    def from_config(
        cls, config: Union[ReactiveAgentConfig, Dict[str, Any], DictConfig]
    ) -> "ReactiveAgent":
        """
...
def create_reactive_agent_from_config(
...
def create_reactive_agent_with_sensor(
    sensor_config: Dict[str, Any], agent_config: Optional[Dict[str, Any]] = None
) -> ReactiveAgent:
    """
...
def demonstrate_reactive_navigation(
    ...
    results = {
        ...
        "final_position": (
            trajectory[-1] if len(trajectory) > 0 else np.array([0.0, 0.0])
        ),
        ...
        results.update(
            {
                ...
                "concentration_improvement": float(
...
def main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        if metrics.get("performance_monitoring_enabled", False):
            ...
            print(
                f"  Average gradient magnitude: {metrics.get('average_gradient_magnitude', 0):.6f}"
            )
            print(
                f"  Performance violations: {metrics.get('performance_violations', 0)}"
            )
