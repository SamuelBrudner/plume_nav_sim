...
class TurbulentWindField:
    ...
    def __init__(
        self,
        config: Optional[TurbulentWindFieldConfig] = None,
        **kwargs: Any
    ):
        """
        ...
        self._velocity_field = np.zeros((self._grid_ny, self._grid_nx, 2), dtype=np.float64)
        self._turbulent_component = np.zeros((self._grid_ny, self._grid_nx, 2), dtype=np.float64)
    ...
    def _initialize_spatial_grid(self) -> None:
        ...
        logger.debug(f"Initialized wind field grid: {self._grid_nx}x{self._grid_ny} points, "
    ...
    def _initialize_atmospheric_parameters(self) -> None:
        ...
        self._friction_velocity = np.linalg.norm(self.config.mean_velocity) * roughness * 0.1
        ...
        logger.debug(f"Atmospheric parameters: Richardson={self._richardson_number:.3f}, "
    ...
    def _initialize_spatial_correlation(self) -> None:
        ...
        if SCIPY_AVAILABLE and n_points < 10000:  # Memory constraint
            ...
            logger.debug(f"Initialized full spatial correlation matrix: {n_points}x{n_points}")
        else:
            # Sparse correlation for large grids or when SciPy unavailable
            self._distance_matrix = None
            self._spatial_correlation = None
            logger.debug("Using local correlation approximation for large grid or missing SciPy")
    ...
    def velocity_at(self, positions: np.ndarray) -> np.ndarray:
        ...
        if execution_time > 0.0005 and len(positions) == 1:  # 0.5ms threshold for single query
            logger.warning(
        ...
        return velocities.item() if velocities.size == 2 and len(positions) == 1 else velocities
    ...
    if NUMBA_AVAILABLE:
        ...
        def _velocity_interpolation_numba(
            ...
            Args:
                ...
                velocity_field: Grid velocity field (grid_ny, grid_nx, 2)
                grid_nx: Number of grid points in x direction
                grid_ny: Number of grid points in y direction  
                grid_resolution: Grid spacing
                domain_width: Domain width
                domain_height: Domain height
                boundary_periodic: Use periodic boundary conditions
                
            Returns:
                np.ndarray: Interpolated velocity vectors (n_positions, 2)
            """
            n_positions = positions.shape[0]
            velocities = np.zeros((n_positions, 2), dtype=np.float64)
    ...
    def _generate_turbulent_fluctuations(self, dt: float) -> None:
        ...
        random_fluctuations = np.random.normal(0, velocity_std * np.sqrt(dt), random_shape)
    ...
    def _apply_local_spatial_correlation(self) -> None:
        ...
        kernel_size = max(3, int(2 * self.config.correlation_length / self.config.grid_resolution))
        if kernel_size % 2 == 0:
        ...
        if SCIPY_AVAILABLE:
            for component in range(2):
                self._raw_fluctuations[:, :, component] = ndimage.convolve(
                    ...
                    correlation_kernel,
                    mode='wrap' if self.config.boundary_conditions == 'periodic' else 'nearest'
                )
        else:
                ...
                for i in range(1, self._grid_ny - 1):
                    for j in range(1, self._grid_nx - 1):
                        ...
                        smoothed[i, j] = np.mean(
                            self._raw_fluctuations[i-1:i+2, j-1:j+2, component]
                        )
    ...
    def _integrate_temporal_correlation(self, dt: float) -> None:
        ...
        self._turbulent_component = (
            (1 - alpha) * self._previous_turbulence + 
            alpha * self._raw_fluctuations
        )
    ...
    def _apply_atmospheric_corrections(self, dt: float) -> None:
        ...
        if self.config.surface_roughness > 0:
            height_factor = np.linspace(0.1, 1.0, self._grid_ny)  # Approximate height effect
    ...
    def _compute_total_velocity_field(self) -> None:
        ...
        if np.any(exceed_mask):
            ...
            logger.debug(f"Velocity limiting applied to {np.sum(exceed_mask)} grid points")
    ...
    def reset(self, **kwargs: Any) -> None:
        ...
        if any(key in kwargs for key in ['atmospheric_stability', 'surface_roughness', 
                                        'anisotropy_ratio', 'thermal_effects']):
            self._initialize_atmospheric_parameters()
        
        # Reinitialize spatial correlation if correlation parameters changed
        if any(key in kwargs for key in ['correlation_length', 'atmospheric_stability']):
            self._initialize_spatial_correlation()
