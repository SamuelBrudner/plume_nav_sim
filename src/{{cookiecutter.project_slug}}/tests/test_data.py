...
extraction, resource management, and configuration system integration.
...
class TestVideoPlumeInitialization:
    ...
    def test_initialization_with_zero_frame_count(self, temp_video_file):
        ...
        with patch('cv2.VideoCapture', return_value=mock_cap):
            with pytest.raises(IOError, match="Video file contains no frames"):
                VideoPlume(video_path=temp_video_file)
...
class TestVideoPlumeFrameProcessing:
    ...
    def test_get_frame_basic_functionality(self, mock_video_capture, temp_video_file):
        ...
        with patch('cv2.VideoCapture', return_value=mock_cap):
            plume = VideoPlume(
                video_path=temp_video_file,
                flip=False,
                kernel_size=0,  # Disable Gaussian smoothing
                grayscale=False,
                normalize=False
            )
    ...
    def test_get_frame_with_preprocessing(self, mock_video_capture, temp_video_file):
        ...
        with patch('cv2.VideoCapture', return_value=mock_cap), \
             ...
             patch('cv2.GaussianBlur') as mock_gaussianBlur, \
             patch('cv2.threshold') as mock_threshold:
            
            # Configure mocks for preprocessing pipeline
            mock_flip.return_value = test_frame
            mock_cvtColor.return_value = test_frame[:, :, 0]  # Simulate grayscale
            mock_gaussianBlur.return_value = test_frame[:, :, 0]
            mock_threshold.return_value = (127, test_frame[:, :, 0])
            ...
            # Validate preprocessing pipeline execution
            mock_flip.assert_called_once_with(test_frame, 1)
            mock_cvtColor.assert_called_once()
            mock_gaussianBlur.assert_called_once_with(
                mock_cvtColor.return_value, (5, 5), 1.5
            )
            mock_threshold.assert_called_once_with(
                mock_gaussianBlur.return_value, 127.0, 255, cv2.THRESH_BINARY
            )
...
class TestVideoPlumeMetadata:
    ...
    def test_get_metadata_comprehensive(self, mock_video_capture, temp_video_file):
        ...
        with patch('cv2.VideoCapture', return_value=mock_cap):
            ...
            assert metadata["filename"] == Path(temp_video_file).name
            ...
            # Validate workflow integration metadata
            assert metadata["dvc_compatible"] == True
            assert metadata["snakemake_ready"] == True
    ...
    def test_get_metadata_string_formatting(self, mock_video_capture, temp_video_file):
        ...
        with patch('cv2.VideoCapture', return_value=mock_cap):
            ...
            assert "horizontal_flip=True" in metadata_str
            assert "gaussian_blur=5x5 (σ=1.5)" in metadata_str
            assert "threshold=0.5" in metadata_str
            assert "normalize=True" in metadata_str
    ...
    def test_get_metadata_string_no_processing(self, mock_video_capture, temp_video_file):
        ...
        with patch('cv2.VideoCapture', return_value=mock_cap):
            plume = VideoPlume(
                video_path=temp_video_file,
                flip=False,
                kernel_size=0,  # No Gaussian smoothing
                normalize=False
            )
            ...
            assert "Processing: none" in metadata_str
...
class TestVideoPlumeConfigurationIntegration:
    ...
    def test_pydantic_schema_validation_integration(self, temp_video_file):
        ...
        assert valid_config.video_path == Path(temp_video_file)
        ...
        # Test invalid configuration - non-positive sigma
        with pytest.raises(ValueError, match="kernel_sigma must be positive"):
            VideoPlumeConfig(
                video_path=temp_video_file,
                kernel_size=3,
    ...
    def test_environment_variable_interpolation(self, mock_video_capture, temp_video_file):
        ...
        try:
            with patch('cv2.VideoCapture', return_value=mock_cap):
                ...
                plume = VideoPlume(video_path="$TEST_VIDEO_PATH")
                assert plume.video_path == Path(temp_video_file)
                plume.close()
    ...
    def test_hydra_interpolation_fallback(self, mock_video_capture, temp_video_file):
        ...
        try:
            with patch('cv2.VideoCapture', return_value=mock_cap):
                # Test Hydra-style interpolation
                plume = VideoPlume(video_path="${oc.env:HYDRA_TEST_VIDEO}")
                assert plume.video_path == Path(temp_video_file)
                plume.close()
        finally:
            # Clean up environment variable
...
class TestVideoPlumeWorkflowIntegration:
    ...
    def test_dvc_path_integration(self, mock_video_capture, temp_video_file):
        """Test VideoPlume integration with DVC data versioning paths."""
        mock_cap = mock_video_capture
        mock_cap.isOpened.return_value = True
        mock_cap.get.side_effect = lambda prop: {
            cv2.CAP_PROP_FRAME_COUNT: 200,
        ...
        with patch('cv2.VideoCapture', return_value=mock_cap):
            # Test DVC path utility function
            ...
            # Verify metadata includes workflow compatibility flags
            metadata = plume.get_metadata()
            assert metadata["dvc_compatible"] == True
            assert metadata["snakemake_ready"] == True
    ...
    def test_snakemake_rule_configuration_generation(self):
        """Test Snakemake workflow rule configuration generation."""
        input_video = "data/raw/experiment_001.mp4"
        output_metadata = "data/processed/experiment_001_meta.json"
        processing_params = {
            "flip": True,
            "kernel_size": 5,
            "kernel_sigma": 1.5,
        ...
        config = create_snakemake_rule_config(
        ...
        assert config["input"]["video"] == input_video
        assert config["output"]["metadata"] == output_metadata
        assert config["params"] == processing_params
        
        # Validate resource allocation
        assert "resources" in config
        assert config["resources"]["mem_mb"] == 2048
        assert config["resources"]["runtime"] == 300
...
class TestVideoPlumeErrorHandling:
    ...
    def test_string_representation_methods(self, mock_video_capture, temp_video_file):
        ...
        with patch('cv2.VideoCapture', return_value=mock_cap):
            ...
            assert str(temp_video_file) in repr_str
            assert "frames=100" in repr_str
            assert "shape=(480, 640)" in repr_str
            assert "fps=30.0" in repr_str
            assert "status=open" in repr_str
            
            # Test __str__ method
            str_str = str(plume)
            assert "VideoPlume" in str_str
            assert Path(temp_video_file).name in str_str
            assert "100 frames" in str_str
            
            # Test representation after close
            plume.close()
            repr_str_closed = repr(plume)
            assert "status=closed" in repr_str_closed
...
class TestVideoPlumeVideoFormatCompatibility:
    ...
    def test_video_format_support(self, video_format, extension, mock_video_capture, tmp_path):
        ...
        with patch('cv2.VideoCapture', return_value=mock_cap):
            ...
            metadata = plume.get_metadata()
            assert metadata["filename"].endswith(extension)
