...
def test_sample_odor_at_sensors_single_agent():
    ...
    for y, x in itertools.product(range(env_size), range(env_size)):
...
def test_calculate_sensor_positions_multi_agent():
    ...
    navigator = VectorizedNavigator(positions=positions, orientations=orientations)
...
def test_navigator_sample_multiple_sensors():
    ...
    if isinstance(odor_values, list):
    ...
    positions = np.array([[20, 20], [50, 50], [80, 80]])
    orientations = np.array([0, 90, 180])
    multi_nav = VectorizedNavigator(positions=positions, orientations=orientations)
    ...
    # Convert to numpy array if it's a list
    if isinstance(odor_values_multi, list):
        odor_values_multi = np.array(odor_values_multi)
...
def test_out_of_bounds_sensors():
    ...
    # Create a navigator near the edge of the plume
    navigator = Navigator(position=(95, 95), orientation=0)
    plume = MockPlume(shape=(100, 100))
    
    # Sample odor - some sensors will be out of bounds
    odor_values = sample_odor_at_sensors(
        navigator, plume.current_frame, sensor_distance=10.0, sensor_angle=45.0, num_sensors=3
    )
    
    # Check that values are low or zero for out-of-bounds sensors
    assert np.all(odor_values <= 0.5)
